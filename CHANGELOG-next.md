...
- Move TodoWrite tracker display configuration out of the daemon schema into ZeroCode's own `zerocode-config.toml`, with a Config-pane editor, per-session-boundary resolution so edits apply without a restart, and a deprecation bridge that keeps recognized legacy environment variables from blocking daemon startup (#9013).
...
- **Todo tracker display configuration moved from the daemon to ZeroCode.** The daemon-owned `config.toml [todotracker]` section (shipped in `v0.8.3` only) has been removed; `<config-dir>/zerocode-config.toml [todotracker]` is now the sole durable owner, editable from ZeroCode's Config pane. Existing on-disk values are **not** migrated automatically — copy `enabled`, `enabled_at_start`, `location`, `width`, and `max_height` into the new file, then delete the old section. The legacy `ZEROCLAW_todotracker__{enabled,enabled_at_start,location,width,max_height}` environment variables are **deprecated**: they are accepted and ignored with a migration warning so an upgrade cannot block daemon startup, but they no longer take effect. Use the canonical `ZEROCODE_todotracker__*` spelling instead. Any other `ZEROCLAW_todotracker__*` name remains a hard startup error. The deprecated allowlist is retained for at least one full release cycle; removal requires a separate breaking-change decision. Explicit zero `width`/`max_height` values are now rejected with a visible error instead of being silently normalized to `1`. See `docs/book/src/zerocode/config.md` for full upgrade steps (#9013, RFC #9246).
