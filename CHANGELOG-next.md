...
| MCP server config persistence | Per-field edits to `[[mcp.servers]]` from the dashboard / TUI (added in the per-field MCP server editor above) now actually write to disk. The incremental `save_dirty` writer was walking array-of-tables nodes as if they were plain tables, so `config/set` on `mcp.servers.<name>.<field>` paths returned success and the UI showed the new value, but the on-disk `config.toml` kept its stale contents until the next process restart wiped the in-memory change. The writer now takes a natural-key-aware branch for `#[natural_key]` Vec sections, locating the matching `[[mcp.servers]]` entry by alias and updating its inner fields in place. Adds, renames, and deletes route through the same code path. Hand-edited `config.toml` is unchanged on disk apart from the targeted entry. |
