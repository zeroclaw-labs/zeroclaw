...
- **Mutually authenticated WSS transport + frictionless enrollment**: the remote WSS plane is now always mutual-TLS (TLS 1.3 only, no server-only fallback). A certless `zerocode --connect` prompts for a one-time pairing code, confirms the daemon CA out of band via a short-auth-string, fetches a client certificate over a runtime-owned enrollment endpoint, and caches it so later runs are zero-config; the private key never leaves the device. Certificates auto-renew at ~50% of their TTL over the authenticated session. **Migration:** existing fleets cut over by opening a time-boxed code-less window with `[enroll].allow_unpaired_enrollment = <RFC3339 deadline>` (it self-closes by wall-clock; an open window logs a loud operator warning). A revoked client certificate is refused at the handshake (driven by the daemon's issued-cert ledger), not just at renewal. An un-migrated (certless) client gets an actionable "enroll first" message on both ends rather than a bare TLS error.
- **Nominated relay (`zerorelay`)**: a standalone blind-forwarder relay reaches NAT'd daemons without exposing plaintext. Daemons register a rotatable, unguessable node-id over a signed Ed25519 handshake; clients try the direct path first and fall back to the relay, re-probing to migrate back. The relay enforces per-source-IP / per-node DoS caps and per-conn credit-window flow control, hot-reloads admission on SIGHUP, and pins (or TOFU-pins) its own outer certificate; the inner mTLS is never decrypted.
