...
- **Conversation attribution signatures changed**: Channel listeners and Channel-backed webhooks now share one persisted conversation record per routing key, containing a server-generated UUID v4 and its message history. Channel reset/delete removes the current record; the next ordinary message creates its replacement, and stale turns cannot persist or deliver results. Internal delegate turns inherit the parent conversation ID, which is exported only as the `gen_ai.conversation.id` OpenTelemetry span attribute. Generic WS/RPC durable identity remains available through `SessionBackend`; callers must pass an explicit server-resolved ID rather than a raw caller-supplied session identifier.
