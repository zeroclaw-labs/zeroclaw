...
- **`Plans/glimmering-mixing-moore-v2.md`** — re-scoped the conscience refactor against current code, splitting the remaining work into Slice A (call-site wiring) and Slice B (`LoopContext` reduction of the 29-arg loop signature). **Slice A has since shipped:** the gate is no longer inert — `evaluate_tool_call` (`crates/zeroclaw-conscience/src/gate.rs:160`) now runs on every LLM-issued tool call through the `ConscienceHook` extension point, gated by `config.conscience.gate_enabled` (default `false`). Covered end-to-end by `tests/conscience_gate_integration.rs` (6/6). **Slice B has also shipped:** the 28-arg positional `run_tool_call_loop` was retired in favour of the single `run_tool_call_loop_ctx(LoopContext)` entry point.
...
- _Resolved._ The conscience gate is now invoked on every LLM-issued tool call via the `ConscienceHook` extension point — `config.conscience.gate_enabled = true` takes effect at runtime (Slice A of the v2 plan; see `tests/conscience_gate_integration.rs`). The gate remains **off by default** pending a product-owned rollout policy.
