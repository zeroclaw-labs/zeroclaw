...
#[test]
fn release_set_is_exactly_the_root_closure_plus_zerocode() {
    let crates = workspace_crates();
    let by_name: BTreeMap<&str, &Crate> = crates
        .iter()
        .map(|krate| (krate.name.as_str(), krate))
    ...
    assert_eq!(
        actual, expected,
        "the coordinated release must contain the root dependency closure plus zerocode, and no \
         test fixture, desktop bundle, or maintainer tool"
    );
