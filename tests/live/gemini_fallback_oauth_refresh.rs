...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.daemonclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.daemonclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let daemonclaw_dir = PathBuf::from(home).join(".daemonclaw");
    let auth_profiles_path = daemonclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: daemonclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: daemonclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.daemonclaw/auth-profiles.json (with expired token)
    let provider = daemonclaw::providers::create_provider("gemini", None)?;
