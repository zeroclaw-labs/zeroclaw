...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.jhedaiclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.jhedaiclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let jhedaiclaw_dir = PathBuf::from(home).join(".jhedaiclaw");
    let auth_profiles_path = jhedaiclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: jhedaiclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: jhedaiclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.jhedaiclaw/auth-profiles.json (with expired token)
