...
fn gateway_path_prefix_rejects_missing_leading_slash() {
    ...
    config.gateway.path_prefix = Some("daemonclaw".into());
...
fn gateway_path_prefix_accepts_valid_prefixes() {
    for prefix in ["/daemonclaw", "/apps/daemonclaw", "/api/hassio_ingress/abc123"] {
        let mut config = Config::default();
...
fn gateway_path_prefix_rejects_unsafe_characters() {
    ...
    for prefix in [" /daemonclaw ", " /daemonclaw"] {
        let mut config = Config::default();
        config.gateway.path_prefix = Some(prefix.into());
        assert!(
...
[channels.whatsapp]
session_path = "~/.daemonclaw/state/whatsapp-web/session.db"
allowed_numbers = ["*"]
"#;
    let parsed: Config = toml::from_str(toml_str)
    ...
    assert_eq!(
        ...
        Some("~/.daemonclaw/state/whatsapp-web/session.db")
    );
