...
//! registered when a plugin manifest declares `context.user_identity = true`,
...
// ---------------------------------------------------------------------------
// context_user_identity is registered when user_identity = true
// ---------------------------------------------------------------------------

#[test]
fn context_user_identity_registered_when_enabled() {
...
// ---------------------------------------------------------------------------
// context_user_identity is NOT registered when user_identity = false
// ---------------------------------------------------------------------------

#[test]
fn context_user_identity_not_registered_when_disabled() {
    let registry = make_registry();
    let manifest = manifest_with_caps(PluginCapabilities {
        context: Some(ContextCapability {
            session: true,
    ...
    assert!(
        !names.contains(&"context_user_identity"),
        "context_user_identity should NOT be registered when user_identity=false, got: {names:?}"
    );
}

// ---------------------------------------------------------------------------
// context_user_identity is NOT registered when context capability is absent
// ---------------------------------------------------------------------------

#[test]
fn context_user_identity_absent_when_no_context_capability() {
    let registry = make_registry();
...
// ---------------------------------------------------------------------------
// context_user_identity coexists with other context sub-flags
// ---------------------------------------------------------------------------

#[test]
fn context_user_identity_coexists_with_other_context_flags() {
    let registry = make_registry();
    let manifest = manifest_with_caps(PluginCapabilities {
        context: Some(ContextCapability {
            session: true,
...
// ---------------------------------------------------------------------------
// context_user_identity is registered alongside memory functions
// ---------------------------------------------------------------------------

#[test]
fn context_user_identity_registered_alongside_memory() {
    ...
    assert_eq!(
        fns.len(),
        2,
        "memory(read) + context(user_identity) should yield 2 functions"
    );
