...
fn tool_capability_gates_tool_plugin_registration() {
    ...
    assert_eq!(
        tool_plugins.len(),
        1,
        "only one plugin declares 'tool' capability"
    );
...
fn channel_capability_gates_channel_plugin_registration() {
    ...
    assert_eq!(
        channel_plugins.len(),
        1,
        "only one plugin declares 'channel' capability"
    );
...
fn plugin_with_no_capabilities_excluded_from_all_registrations() {
    let dir = setup_plugins(&[("bare-plugin", &[]), ("tool-plugin", &["tool"])]);
    ...
    assert_eq!(
        ...
        1,
        "bare plugin excluded from tool registration"
    );
    assert_eq!(
        host.channel_plugins().len(),
        0,
        "bare plugin excluded from channel registration"
    );
}
...
fn multi_capability_plugin_appears_in_all_matching_registrations() {
    ...
    assert_eq!(
        host.tool_plugins().len(),
        2,
        "both plugins have tool capability"
    );
    assert_eq!(
        host.channel_plugins().len(),
        1,
        "only multi-plugin has channel capability"
    );
