...
fn read_single_char(fd: RawFd) -> io::Result<Option<char>> {
    read_single_char_timeout(fd, 0)
}

fn read_single_char_timeout(fd: RawFd, timeout_ms: i32) -> io::Result<Option<char>> {
    let is_ready = select_or_poll_term_fd(fd, timeout_ms)?;
    ...
    loop {
        match read_single_char(fd)? {
            Some('\x1b') => {
                ...
                // Use a 50ms timeout for escape sequence continuation bytes.
                // Arrow keys send \x1b[A etc. and the bytes may not all be
                // available in the same poll cycle, especially under sudo or SSH.
                const ESC_TIMEOUT_MS: i32 = 50;
                break if let Some(c1) = read_single_char_timeout(fd, ESC_TIMEOUT_MS)? {
                    if c1 == '[' {
                        if let Some(c2) = read_single_char_timeout(fd, ESC_TIMEOUT_MS)? {
                            match c2 {
                                'A' => Ok(Key::ArrowUp),
                                'B' => Ok(Key::ArrowDown),
                                'C' => Ok(Key::ArrowRight),
                                ...
                                'Z' => Ok(Key::BackTab),
                                _ => {
                                    let c3 = read_single_char_timeout(fd, ESC_TIMEOUT_MS)?;
                                    if let Some(c3) = c3 {
                                        if c3 == '~' {
                                            match c2 {
                                                '1' => Ok(Key::Home), // tmux
                        ...
                        // SS3 sequences — many terminals send \x1bOA for arrows
                        // in application cursor key mode (raw mode)
                        if let Some(c2) = read_single_char_timeout(fd, ESC_TIMEOUT_MS)? {
                            match c2 {
                                'A' => Ok(Key::ArrowUp),
                                'B' => Ok(Key::ArrowDown),
