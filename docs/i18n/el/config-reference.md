...
## 7. Ρυθμίσεις Runtime (`[runtime]`)

- `reasoning_enabled`: Ελέγχει τη λειτουργία σκέψης (thinking/reasoning) για providers που το υποστηρίζουν.
  - `false`: στέλνει `think: false` στο Ollama και `enable_thinking: false` στο Qwen (όλα τα aliases, DashScope API).
  - `true`: στέλνει `think: true` στο Ollama και `enable_thinking: true` στο Qwen.
  - Αν δεν οριστεί: χρησιμοποιείται η προεπιλογή του provider/μοντέλου.
