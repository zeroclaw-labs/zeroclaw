...
- Νέα ρύθμιση παρατηρησιμότητας: `observability.runtime_trace_record_http` (προεπιλογή `false`) για καταγραφή λεπτομερειών HTTP κλήσεων LLM (`llm_http_request` / `llm_http_response`). Ενεργό μόνο όταν `runtime_trace_mode` είναι `rolling` ή `full`.
