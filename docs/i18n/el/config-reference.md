# Οδηγός Ρυθμίσεων JhedaiClaw (config.toml)
...
Το JhedaiClaw ψάχνει για τις ρυθμίσεις με την εξής σειρά:

1. Στη διαδρομή που ορίζει η μεταβλητή `JHEDAICLAW_WORKSPACE`.
2. Στο αρχείο `~/.jhedaiclaw/config.toml` (αυτή είναι η συνηθισμένη θέση).
...
- Αν αλλάξετε το αρχείο `config.toml`, πρέπει να κάνετε επανεκκίνηση το JhedaiClaw για να δει τις αλλαγές.
- Χρησιμοποιήστε την εντολή `jhedaiclaw doctor` για να βεβαιωθείτε ότι οι ρυθμίσεις σας είναι σωστές.
