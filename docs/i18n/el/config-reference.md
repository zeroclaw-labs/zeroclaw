...
- `allow_repeated_tool_calls`: Αν επιτρέπονται επαναλαμβανόμενα ίδια tool calls (ίδιο tool + ίδια args) στο ίδιο turn (προεπιλογή: `false`).

## 1.1 Πάροχος (Provider)

- `compatible_timeout_secs`: Timeout HTTP (σε δευτερόλεπτα) για OpenAI-compatible providers. Προεπιλογή όταν λείπει: `120`.
