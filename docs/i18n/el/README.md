...
<h1 align="center">🦀 DaemonClaw — Προσωπικός Βοηθός Τεχνητής Νοημοσύνης</h1>
...
Το `daemonclaw onboard` εκτελείται αυτόματα μετά την εγκατάσταση για τη ρύθμιση του χώρου εργασίας και του παρόχου.
...
daemonclaw gateway                # προεπιλογή: 127.0.0.1:42617
daemonclaw gateway --port 0       # τυχαία θύρα (ενισχυμένη ασφάλεια)
...
Αναβάθμιση; Εκτελέστε `daemonclaw doctor` μετά την ενημέρωση.
...
Το DaemonClaw μπορεί να εισάγει τον χώρο εργασίας, τη μνήμη και τη ρύθμιση παραμέτρων του OpenClaw:
...
Αυτό μετεγκαθιστά τις εγγραφές μνήμης, τα αρχεία χώρου εργασίας και τη ρύθμιση παραμέτρων από `~/.openclaw/` σε `~/.daemonclaw/`. Η ρύθμιση μετατρέπεται αυτόματα από JSON σε TOML.

## Προεπιλογές ασφάλειας (πρόσβαση DM)

Το DaemonClaw συνδέεται σε πραγματικές επιφάνειες μηνυμάτων. Αντιμετωπίστε τα εισερχόμενα DM ως μη αξιόπιστη είσοδο.
...
- Εγκρίνετε με: `daemonclaw pairing approve <channel> <code>` (τότε ο αποστολέας προστίθεται σε τοπική λίστα επιτρεπόμενων).
- Τα δημόσια εισερχόμενα DM απαιτούν ρητή ενεργοποίηση στο `config.toml`.
- Εκτελέστε `daemonclaw doctor` για να εντοπίσετε επικίνδυνες ή εσφαλμένες πολιτικές DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Κρίσιμο_ | **Δεν** είμαστε συνδεδεμένοι με `openagen/daemonclaw`, `daemonclaw.org` ή `daemonclaw.net`. Οι τομείς `daemonclaw.org` και `daemonclaw.net` δείχνουν αυτή τη στιγμή στο fork `openagen/daemonclaw`, και αυτός ο τομέας/αποθετήριο υποδύονται τον επίσημο ιστότοπο/έργο μας. | Μην εμπιστεύεστε πληροφορίες, δυαδικά αρχεία, εκστρατείες χρηματοδότησης ή ανακοινώσεις από αυτές τις πηγές. Χρησιμοποιήστε μόνο [αυτό το αποθετήριο](https://github.com/DeliveryBoyTech/daemonclaw) και τους επαληθευμένους λογαριασμούς μας στα μέσα κοινωνικής δικτύωσης. |
...
### Γιατί οι ομάδες επιλέγουν το DaemonClaw
...
## Στιγμιότυπο Benchmark (DaemonClaw vs OpenClaw, Αναπαραγώγιμο)
...
> Σημειώσεις: Τα αποτελέσματα του DaemonClaw μετρήθηκαν σε release builds χρησιμοποιώντας `/usr/bin/time -l`. Το OpenClaw απαιτεί Node.js runtime (τυπικά ~390MB επιπλέον επιβάρυνση μνήμης), ενώ το NanoBot απαιτεί Python runtime. Τα PicoClaw και DaemonClaw είναι στατικά δυαδικά. Τα στοιχεία RAM παραπάνω αφορούν μνήμη χρόνου εκτέλεσης· οι απαιτήσεις μεταγλώττισης κατά τον χρόνο κατασκευής είναι υψηλότερες.
...
Ελάχιστο `~/.daemonclaw/config.toml`:
...
Το DaemonClaw υποστηρίζει native προφίλ πιστοποίησης συνδρομής (πολλαπλοί λογαριασμοί, κρυπτογραφημένα σε αδράνεια).

- Αρχείο αποθήκευσης: `~/.daemonclaw/auth-profiles.json`
- Κλειδί κρυπτογράφησης: `~/.daemonclaw/.secret_key`
...
Ρίζα χώρου εργασίας: `~/.daemonclaw/workspace/` (ρυθμιζόμενο μέσω config).
...
Δεξιότητες: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` ή `SKILL.toml`.
...
daemonclaw onboard              # Οδηγός καθοδηγούμενης ρύθμισης
daemonclaw status               # Εμφάνιση κατάστασης daemon/agent
daemonclaw doctor               # Εκτέλεση διαγνωστικών συστήματος

# Gateway + daemon
daemonclaw gateway              # Εκκίνηση gateway server (127.0.0.1:42617)
daemonclaw daemon               # Εκκίνηση πλήρους αυτόνομου χρόνου εκτέλεσης

# Πράκτορας
daemonclaw agent                # Διαδραστική λειτουργία συνομιλίας
daemonclaw agent -m "message"   # Λειτουργία μεμονωμένου μηνύματος

# Διαχείριση υπηρεσίας
daemonclaw service install      # Εγκατάσταση ως υπηρεσία OS (launchd/systemd)
daemonclaw service start|stop|restart|status

# Κανάλια
daemonclaw channel list         # Λίστα ρυθμισμένων καναλιών
daemonclaw channel doctor       # Έλεγχος υγείας καναλιών
daemonclaw channel bind-telegram 123456789

# Cron + προγραμματισμός
daemonclaw cron list            # Λίστα προγραμματισμένων εργασιών
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# Μνήμη
daemonclaw memory list          # Λίστα εγγραφών μνήμης
daemonclaw memory get <key>     # Ανάκτηση μνήμης
daemonclaw memory stats         # Στατιστικά μνήμης
...
# Περιφερειακά υλικού
daemonclaw hardware discover    # Σάρωση για συνδεδεμένες συσκευές
daemonclaw peripheral list      # Λίστα συνδεδεμένων περιφερειακών
daemonclaw peripheral flash     # Flash firmware σε συσκευή
...
Ή παραλείψτε τα παραπάνω βήματα και εγκαταστήστε τα πάντα (εξαρτήσεις συστήματος, Rust, DaemonClaw) με μία εντολή:
...
- Ακολουθήστε [DaemonClaw Onboard](#γρήγορη-εκκίνηση-tldr) για καθοδηγούμενη ρύθμιση.
...
Το DaemonClaw δημιουργήθηκε για τον smooth crab 🦀, έναν γρήγορο και αποδοτικό βοηθό AI. Δημιουργήθηκε από τον Argenis De La Rosa και την κοινότητα.

- [daemonclawlabs.ai](https://daemonclawlabs.ai)
- [@daemonclawlabs](https://x.com/daemonclawlabs)

## Υποστήριξη DaemonClaw
...
Νέος στο DaemonClaw; Ψάξτε για issues με ετικέτα [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — δείτε τον [Οδηγό Συνεισφοράς](CONTRIBUTING.md#first-time-contributors) για το πώς να ξεκινήσετε. PR με AI/vibe-coding καλοδεχούμενα! 🤖
...
- Νέο `Skill` → `~/.daemonclaw/workspace/skills/<name>/`
...
**Αυτό είναι το μόνο επίσημο αποθετήριο DaemonClaw:**

> https://github.com/DeliveryBoyTech/daemonclaw

Οποιοδήποτε άλλο αποθετήριο, οργανισμός, τομέας ή πακέτο που ισχυρίζεται ότι είναι "DaemonClaw" ή υπονοεί σχέση με τα DaemonClaw Labs είναι **μη εξουσιοδοτημένο και δεν σχετίζεται με αυτό το έργο**. Τα γνωστά μη εξουσιοδοτημένα forks θα αναφέρονται στο [TRADEMARK.md](docs/maintainers/trademark.md).

Αν αντιμετωπίσετε πλαστοπροσωπία ή κατάχρηση εμπορικού σήματος, παρακαλούμε [ανοίξτε ένα issue](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## Άδεια

Το DaemonClaw έχει διπλή άδεια για μέγιστη ανοιχτότητα και προστασία συνεισφερόντων:
...
Το όνομα **DaemonClaw** και το λογότυπο είναι εμπορικά σήματα της DaemonClaw Labs. Αυτή η άδεια δεν παρέχει δικαίωμα χρήσης τους για να υπονοήσετε υποστήριξη ή σχέση. Δείτε [TRADEMARK.md](docs/maintainers/trademark.md) για επιτρεπόμενες και απαγορευμένες χρήσεις.
...
**DaemonClaw** — Μηδενική επιβάρυνση. Μηδενικοί συμβιβασμοί. Ανάπτυξη οπουδήποτε. Εναλλαγή οτιδήποτε. 🦀
