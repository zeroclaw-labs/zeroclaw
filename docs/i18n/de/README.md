...
<h1 align="center">🦀 QuantClaw — Persönlicher KI-Assistent</h1>
...
> **Empfohlene Einrichtung:** Führe `quantclaw onboard` in deinem Terminal aus. QuantClaw Onboard führt dich Schritt für Schritt durch die Einrichtung von Gateway, Workspace, Kanälen und Provider. Es ist der empfohlene Einrichtungspfad und funktioniert auf macOS, Linux und Windows (über WSL2). Neue Installation? Starte hier: [Erste Schritte](#schnellstart)
...
`quantclaw onboard` wird nach der Installation automatisch ausgeführt, um deinen Workspace und Provider zu konfigurieren.
...
quantclaw gateway --port 0       # Zufälliger Port (gehärtete Sicherheit)
...
Aktualisierung? Führe `quantclaw doctor` nach dem Update aus.
...
QuantClaw kann deinen OpenClaw-Workspace, Speicher und Konfiguration importieren:
...
Dies migriert deine Speichereinträge, Workspace-Dateien und Konfiguration von `~/.openclaw/` nach `~/.quantclaw/`. Die Konfiguration wird automatisch von JSON nach TOML konvertiert.

## Sicherheitsstandards (DM-Zugriff)

QuantClaw verbindet sich mit echten Messaging-Oberflächen. Behandle eingehende DMs als nicht vertrauenswürdige Eingabe.
...
- Genehmige mit: `quantclaw pairing approve <channel> <code>` (der Absender wird dann zu einer lokalen Allowlist hinzugefügt).
- Öffentliche eingehende DMs erfordern eine explizite Aktivierung in `config.toml`.
- Führe `quantclaw doctor` aus, um riskante oder falsch konfigurierte DM-Richtlinien aufzudecken.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisch_  | Wir sind **nicht verbunden** mit `openagen/quantclaw`, `quantclaw.org` oder `quantclaw.net`. Die Domains `quantclaw.org` und `quantclaw.net` verweisen derzeit auf den Fork `openagen/quantclaw`, und diese Domain/dieses Repository geben sich als unsere offizielle Website/unser offizielles Projekt aus.                                                                                       | Vertraue keinen Informationen, Binaries, Spendenaktionen oder Ankündigungen aus diesen Quellen. Verwende nur [dieses Repository](https://github.com/quant-speed/quantclaw) und unsere verifizierten Social-Media-Konten.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## Benchmark-Übersicht (QuantClaw vs OpenClaw, reproduzierbar)
...
QuantClaw unterstützt native Abonnement-Authentifizierungsprofile (Multi-Account, verschlüsselt im Ruhezustand).

- Speicherdatei: `~/.quantclaw/auth-profiles.json`
- Verschlüsselungsschlüssel: `~/.quantclaw/.secret_key`
...
Workspace-Root: `~/.quantclaw/workspace/` (konfigurierbar über Config).
...
Skills: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` oder `SKILL.toml`.
...
quantclaw onboard              # Geführter Einrichtungsassistent
quantclaw status               # Daemon/Agenten-Status anzeigen
quantclaw doctor               # Systemdiagnose ausführen

# Gateway + Daemon
quantclaw gateway              # Gateway-Server starten (127.0.0.1:42617)
quantclaw daemon               # Vollständige autonome Laufzeit starten

# Agent
quantclaw agent                # Interaktiver Chat-Modus
quantclaw agent -m "message"   # Einzelnachrichten-Modus

# Service-Verwaltung
quantclaw service install      # Als OS-Dienst installieren (launchd/systemd)
quantclaw service start|stop|restart|status

# Kanäle
quantclaw channel list         # Konfigurierte Kanäle auflisten
quantclaw channel doctor       # Kanalgesundheit prüfen
quantclaw channel bind-telegram 123456789

# Cron + Planung
quantclaw cron list            # Geplante Aufgaben auflisten
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Speicher
quantclaw memory list          # Speichereinträge auflisten
quantclaw memory get <key>     # Speicher abrufen
quantclaw memory stats         # Speicherstatistiken
...
# Hardware-Peripherie
quantclaw hardware discover    # Angeschlossene Geräte scannen
quantclaw peripheral list      # Angeschlossene Peripherie auflisten
quantclaw peripheral flash     # Firmware auf Gerät flashen
...
Oder überspringe die obigen Schritte und installiere alles (Systemabhängigkeiten, Rust, QuantClaw) mit einem einzigen Befehl:
...
- Folge [QuantClaw Onboard](#schnellstart) für eine geführte Einrichtung.
...
QuantClaw wurde für den glatten Krebs 🦀 gebaut, einen schnellen und effizienten KI-Assistenten. Entwickelt von Argenis De La Rosa und der Community.

- [quantspeed.ai](https://quantspeed.ai)
- [@quantspeed](https://x.com/quantspeed)

## QuantClaw unterstützen

Wenn QuantClaw bei deiner Arbeit hilft und du die laufende Entwicklung unterstützen möchtest, kannst du hier spenden:
...
Neu bei QuantClaw? Suche nach Issues mit dem Label [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — siehe unseren [Beitragsleitfaden](CONTRIBUTING.md#first-time-contributors) für den Einstieg. KI-/Vibe-coded PRs willkommen! 🤖
...
- Neuer `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**Dies ist das einzige offizielle QuantClaw-Repository:**

> https://github.com/quant-speed/quantclaw

Jedes andere Repository, jede Organisation, Domain oder jedes Paket, das behauptet, "QuantClaw" zu sein oder eine Zugehörigkeit zu QuantClaw Labs impliziert, ist **nicht autorisiert und nicht mit diesem Projekt verbunden**. Bekannte nicht autorisierte Forks werden in [TRADEMARK.md](docs/maintainers/trademark.md) aufgelistet.

Wenn du auf Identitätsdiebstahl oder Markenrechtsmissbrauch stößt, [eröffne bitte ein Issue](https://github.com/quant-speed/quantclaw/issues).

---

## Lizenz

QuantClaw ist doppelt lizenziert für maximale Offenheit und Schutz der Mitwirkenden:
...
Der **QuantClaw**-Name und das Logo sind Marken von QuantClaw Labs. Diese Lizenz gewährt keine Erlaubnis, sie zu verwenden, um Unterstützung oder Zugehörigkeit zu implizieren. Siehe [TRADEMARK.md](docs/maintainers/trademark.md) für erlaubte und verbotene Verwendungen.
...
**QuantClaw** — Null Overhead. Null Kompromisse. Überall bereitstellen. Alles austauschen. 🦀
