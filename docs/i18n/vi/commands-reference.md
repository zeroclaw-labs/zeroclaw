...
`config reload` tải lại `config.toml` vào gateway đang chạy mà không cần khởi động lại daemon. Lệnh này gọi `POST /admin/reload-config` trên gateway cục bộ. Lịch sử hội thoại và trạng thái sub-agent được bảo toàn.

Lưu ý:

- Yêu cầu daemon hoặc gateway đang chạy (`zeroclaw daemon` / `zeroclaw gateway`).
- Chỉ cho phép địa chỉ loopback (`127.0.0.0/8`, `::1`, `localhost`) làm đích.
- Nếu `config.toml` bị lỗi cú pháp, reload thất bại với thông báo lỗi mô tả và cấu hình đang chạy không thay đổi.
- Một số trường (ví dụ `default_provider`, `default_model`) yêu cầu khởi động lại daemon hoàn toàn. Lệnh sẽ in cảnh báo khi phát hiện các thay đổi này.
