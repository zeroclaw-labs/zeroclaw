...
| `allow_repeated_tool_calls` | `false` | Cho phép lặp lại tool call trùng chữ ký (cùng tool + cùng args) trong cùng một lượt |
...
- Mặc định, các tool call trùng đối số sẽ bị bỏ qua trong cùng một lượt. Đặt `allow_repeated_tool_calls = true` nếu muốn cho phép lặp lại.
...
| `compatible_timeout_secs` | chưa đặt (`None`, hiệu lực `120`) | Timeout HTTP (giây) cho các provider OpenAI-compatible |
...
- `compatible_timeout_secs` phải lớn hơn `0` nếu được đặt.
