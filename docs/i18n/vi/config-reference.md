...
| `skills_directory` | chưa đặt | Đường dẫn thư mục skills tùy chỉnh (tương đối với workspace root) cho việc tải skills theo phạm vi |
...
- Agent phụ nhận system prompt được bổ sung đầy đủ bao gồm: phần tools (các tool được phép với tham số), phần skills (từ thư mục scoped hoặc mặc định), đường dẫn workspace, ngày giờ hiện tại, ràng buộc an toàn, và chính sách shell khi `shell` nằm trong danh sách tool có hiệu lực.
- Khi `skills_directory` chưa đặt hoặc rỗng, agent phụ tải skills từ thư mục `skills/` mặc định của workspace. Khi được đặt, skills được tải riêng từ thư mục đó (tương đối với workspace root), cho phép tập skills riêng cho từng agent.
