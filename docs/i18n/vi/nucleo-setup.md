# QuantClaw trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy QuantClaw trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

QuantClaw có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
QuantClaw bao gồm mọi thứ cần thiết cho Nucleo-F401RE:
...
| Flash command | `quantclaw peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
### 1.2 Nạp qua QuantClaw

Từ thư mục gốc của repo quantclaw:
...
Thêm vào `~/.quantclaw/config.toml`:
...
```bash
quantclaw agent --message "Turn on the LED on pin 13"
```
...
| 3 | `quantclaw peripheral flash-nucleo` |
| 4 | Thêm Nucleo vào config.toml (path = serial port của bạn) |
| 5 | `quantclaw daemon` hoặc `quantclaw agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `quantclaw peripheral list` để xác nhận.
