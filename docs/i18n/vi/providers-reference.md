...
### Timeout cho provider OpenAI-compatible

Bạn có thể ghi đè timeout HTTP cho các provider OpenAI-compatible trong `config.toml`:
...
- Áp dụng cho các provider dùng lớp OpenAI-compatible.
- Giá trị phải lớn hơn `0` nếu được đặt.
- Không đặt sẽ dùng mặc định `120` giây.
