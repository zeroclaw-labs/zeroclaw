...
- `cargo check` / `cargo build` dừng lâu ở `Checking quantclaw`
...
### Không tìm thấy lệnh `quantclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `quantclaw`
...
Xác minh `~/.quantclaw/config.toml`:
...
- Dừng các tiến trình `quantclaw daemon` / `quantclaw channel start` thừa
