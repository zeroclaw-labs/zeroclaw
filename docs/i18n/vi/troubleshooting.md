...
- `cargo check` / `cargo build` dừng lâu ở `Checking jhedaiclaw`
...
### Không tìm thấy lệnh `jhedaiclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `jhedaiclaw`
...
Xác minh `~/.jhedaiclaw/config.toml`:
...
- Dừng các tiến trình `jhedaiclaw daemon` / `jhedaiclaw channel start` thừa
