...
- `cargo check` / `cargo build` dừng lâu ở `Checking daemonclaw`
...
### Không tìm thấy lệnh `daemonclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `daemonclaw`
...
Xác minh `~/.daemonclaw/config.toml`:
...
- Dừng các tiến trình `daemonclaw daemon` / `daemonclaw channel start` thừa
