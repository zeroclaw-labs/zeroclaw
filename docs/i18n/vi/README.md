...
<h1 align="center">🦀 QuantClaw — Trợ lý AI Cá nhân</h1>
...
> **Cài đặt khuyến nghị:** chạy `quantclaw onboard` trong terminal. QuantClaw Onboard hướng dẫn bạn từng bước thiết lập gateway, workspace, kênh và provider. Đây là đường dẫn cài đặt được khuyến nghị và hoạt động trên macOS, Linux, và Windows (qua WSL2). Cài đặt mới? Bắt đầu tại đây: [Bắt đầu](#bắt-đầu-nhanh-tldr)
...
`quantclaw onboard` tự động chạy sau khi cài đặt để cấu hình workspace và provider.
...
quantclaw gateway                # mặc định: 127.0.0.1:42617
quantclaw gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
Đang nâng cấp? Chạy `quantclaw doctor` sau khi cập nhật.
...
QuantClaw có thể nhập workspace, bộ nhớ và cấu hình OpenClaw của bạn:
...
Thao tác này chuyển đổi các mục bộ nhớ, file workspace và cấu hình từ `~/.openclaw/` sang `~/.quantclaw/`. Cấu hình được tự động chuyển từ JSON sang TOML.

## Mặc định bảo mật (truy cập DM)

QuantClaw kết nối với các dịch vụ nhắn tin thực. Xem DM đến như đầu vào không đáng tin cậy.
...
- Phê duyệt bằng: `quantclaw pairing approve <channel> <code>` (người gửi được thêm vào danh sách cho phép cục bộ).
- DM đến công khai yêu cầu opt-in rõ ràng trong `config.toml`.
- Chạy `quantclaw doctor` để phát hiện chính sách DM nguy hiểm hoặc cấu hình sai.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không liên kết** với `openagen/quantclaw`, `quantclaw.org` hay `quantclaw.net`. Các tên miền `quantclaw.org` và `quantclaw.net` hiện đang trỏ đến fork `openagen/quantclaw`, và các tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng [repository này](https://github.com/quant-speed/quantclaw) và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
...
### Vì sao các team chọn QuantClaw
...
## So sánh hiệu năng (QuantClaw vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả QuantClaw được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), NanoBot yêu cầu runtime Python. PicoClaw và QuantClaw là static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build cao hơn.
...
Tối thiểu `~/.quantclaw/config.toml`:
...
QuantClaw hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.quantclaw/auth-profiles.json`
- Khóa mã hóa: `~/.quantclaw/.secret_key`
...
Thư mục gốc workspace: `~/.quantclaw/workspace/` (có thể cấu hình qua config).
...
Skill: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` hoặc `SKILL.toml`.
...
```bash
# Quản lý workspace
quantclaw onboard              # Trình hướng dẫn cài đặt
quantclaw status               # Hiển thị trạng thái daemon/agent
quantclaw doctor               # Chạy chẩn đoán hệ thống

# Gateway + daemon
quantclaw gateway              # Khởi động gateway server (127.0.0.1:42617)
quantclaw daemon               # Khởi động runtime tự trị đầy đủ

# Agent
quantclaw agent                # Chế độ chat tương tác
quantclaw agent -m "message"   # Chế độ tin nhắn đơn

# Quản lý dịch vụ
quantclaw service install      # Cài đặt làm dịch vụ OS (launchd/systemd)
quantclaw service start|stop|restart|status

# Kênh
quantclaw channel list         # Liệt kê kênh đã cấu hình
quantclaw channel doctor       # Kiểm tra sức khỏe kênh
quantclaw channel bind-telegram 123456789

# Cron + lên lịch
quantclaw cron list            # Liệt kê tác vụ đã lên lịch
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Bộ nhớ
quantclaw memory list          # Liệt kê mục bộ nhớ
quantclaw memory get <key>     # Truy xuất bộ nhớ
quantclaw memory stats         # Thống kê bộ nhớ
...
quantclaw hardware discover    # Quét thiết bị đã kết nối
quantclaw peripheral list      # Liệt kê thiết bị ngoại vi đã kết nối
quantclaw peripheral flash     # Flash firmware vào thiết bị
...
Hoặc bỏ qua các bước trên và cài hết mọi thứ (system deps, Rust, QuantClaw) bằng một lệnh:
...
- Theo [QuantClaw Onboard](#bắt-đầu-nhanh-tldr) để cài đặt có hướng dẫn.
...
QuantClaw được xây dựng cho smooth crab 🦀, một trợ lý AI nhanh và hiệu quả. Được xây dựng bởi Argenis De La Rosa và cộng đồng.
...
Nếu QuantClaw giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển, bạn có thể quyên góp tại đây:
...
Mới với QuantClaw? Tìm các issue có nhãn [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — xem [Hướng dẫn đóng góp](CONTRIBUTING.md#first-time-contributors) để bắt đầu. PR AI/vibe-coded đều được chào đón! 🤖
...
- `Skill` mới → `~/.quantclaw/workspace/skills/<name>/`
...
**Đây là repository QuantClaw chính thức duy nhất:**

> https://github.com/quant-speed/quantclaw

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "QuantClaw" hoặc ngụ ý liên kết với QuantClaw Labs đều **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](docs/maintainers/trademark.md).

Nếu bạn phát hiện mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/quant-speed/quantclaw/issues).

---

## Giấy phép

QuantClaw được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **QuantClaw** và logo là nhãn hiệu của QuantClaw Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](docs/maintainers/trademark.md) để biết các sử dụng được phép và bị cấm.
...
**QuantClaw** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
