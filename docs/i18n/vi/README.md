...
<h1 align="center">🦀 DaemonClaw — Trợ lý AI Cá nhân</h1>
...
> **Cài đặt khuyến nghị:** chạy `daemonclaw onboard` trong terminal. DaemonClaw Onboard hướng dẫn bạn từng bước thiết lập gateway, workspace, kênh và provider. Đây là đường dẫn cài đặt được khuyến nghị và hoạt động trên macOS, Linux, và Windows (qua WSL2). Cài đặt mới? Bắt đầu tại đây: [Bắt đầu](#bắt-đầu-nhanh-tldr)
...
`daemonclaw onboard` tự động chạy sau khi cài đặt để cấu hình workspace và provider.
...
daemonclaw gateway                # mặc định: 127.0.0.1:42617
daemonclaw gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
Đang nâng cấp? Chạy `daemonclaw doctor` sau khi cập nhật.
...
DaemonClaw có thể nhập workspace, bộ nhớ và cấu hình OpenClaw của bạn:
...
Thao tác này chuyển đổi các mục bộ nhớ, file workspace và cấu hình từ `~/.openclaw/` sang `~/.daemonclaw/`. Cấu hình được tự động chuyển từ JSON sang TOML.

## Mặc định bảo mật (truy cập DM)

DaemonClaw kết nối với các dịch vụ nhắn tin thực. Xem DM đến như đầu vào không đáng tin cậy.
...
- Phê duyệt bằng: `daemonclaw pairing approve <channel> <code>` (người gửi được thêm vào danh sách cho phép cục bộ).
- DM đến công khai yêu cầu opt-in rõ ràng trong `config.toml`.
- Chạy `daemonclaw doctor` để phát hiện chính sách DM nguy hiểm hoặc cấu hình sai.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không liên kết** với `openagen/daemonclaw`, `daemonclaw.org` hay `daemonclaw.net`. Các tên miền `daemonclaw.org` và `daemonclaw.net` hiện đang trỏ đến fork `openagen/daemonclaw`, và các tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng [repository này](https://github.com/DeliveryBoyTech/daemonclaw) và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
...
### Vì sao các team chọn DaemonClaw
...
## So sánh hiệu năng (DaemonClaw vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả DaemonClaw được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), NanoBot yêu cầu runtime Python. PicoClaw và DaemonClaw là static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build cao hơn.
...
Tối thiểu `~/.daemonclaw/config.toml`:
...
DaemonClaw hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.daemonclaw/auth-profiles.json`
- Khóa mã hóa: `~/.daemonclaw/.secret_key`
...
Thư mục gốc workspace: `~/.daemonclaw/workspace/` (có thể cấu hình qua config).
...
Skill: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` hoặc `SKILL.toml`.
...
```bash
# Quản lý workspace
daemonclaw onboard              # Trình hướng dẫn cài đặt
daemonclaw status               # Hiển thị trạng thái daemon/agent
daemonclaw doctor               # Chạy chẩn đoán hệ thống

# Gateway + daemon
daemonclaw gateway              # Khởi động gateway server (127.0.0.1:42617)
daemonclaw daemon               # Khởi động runtime tự trị đầy đủ

# Agent
daemonclaw agent                # Chế độ chat tương tác
daemonclaw agent -m "message"   # Chế độ tin nhắn đơn

# Quản lý dịch vụ
daemonclaw service install      # Cài đặt làm dịch vụ OS (launchd/systemd)
daemonclaw service start|stop|restart|status

# Kênh
daemonclaw channel list         # Liệt kê kênh đã cấu hình
daemonclaw channel doctor       # Kiểm tra sức khỏe kênh
daemonclaw channel bind-telegram 123456789

# Cron + lên lịch
daemonclaw cron list            # Liệt kê tác vụ đã lên lịch
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# Bộ nhớ
daemonclaw memory list          # Liệt kê mục bộ nhớ
daemonclaw memory get <key>     # Truy xuất bộ nhớ
daemonclaw memory stats         # Thống kê bộ nhớ
...
daemonclaw hardware discover    # Quét thiết bị đã kết nối
daemonclaw peripheral list      # Liệt kê thiết bị ngoại vi đã kết nối
daemonclaw peripheral flash     # Flash firmware vào thiết bị
...
Hoặc bỏ qua các bước trên và cài hết mọi thứ (system deps, Rust, DaemonClaw) bằng một lệnh:
...
- Theo [DaemonClaw Onboard](#bắt-đầu-nhanh-tldr) để cài đặt có hướng dẫn.
...
DaemonClaw được xây dựng cho smooth crab 🦀, một trợ lý AI nhanh và hiệu quả. Được xây dựng bởi Argenis De La Rosa và cộng đồng.
...
Mới với DaemonClaw? Tìm các issue có nhãn [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — xem [Hướng dẫn đóng góp](CONTRIBUTING.md#first-time-contributors) để bắt đầu. PR AI/vibe-coded đều được chào đón! 🤖
...
- `Skill` mới → `~/.daemonclaw/workspace/skills/<name>/`
...
**Đây là repository DaemonClaw chính thức duy nhất:**

> https://github.com/DeliveryBoyTech/daemonclaw

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "DaemonClaw" hoặc ngụ ý liên kết với DaemonClaw Labs đều **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](docs/maintainers/trademark.md).

Nếu bạn phát hiện mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## Giấy phép

DaemonClaw được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **DaemonClaw** và logo là nhãn hiệu của DaemonClaw Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](docs/maintainers/trademark.md) để biết các sử dụng được phép và bị cấm.
...
**DaemonClaw** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
