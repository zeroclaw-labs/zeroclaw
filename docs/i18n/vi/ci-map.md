...
- `.github/workflows/pub-docker-img.yml` (`Docker`)
    - Mục đích: kiểm tra Docker smoke trên PR và publish image khi push lên `master` (các đường dẫn build-input), push tag (`v*`) và khi dispatch thủ công
...
- `Docker`: push lên `master` khi Docker build input thay đổi, push tag (`v*`), PR tương ứng, dispatch thủ công
