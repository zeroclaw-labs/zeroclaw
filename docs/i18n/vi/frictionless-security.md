...
[INFO] ✓ Audit logging enabled (~/.config/quantclaw/audit.log)
...
$ quantclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/quantclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ quantclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ quantclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ quantclaw config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `quantclaw security --status`
