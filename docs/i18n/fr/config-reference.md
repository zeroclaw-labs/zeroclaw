...
- Ajout de `agent.allow_repeated_tool_calls` (par défaut `false`) pour contrôler les appels d'outils identiques répétés dans le même tour.
- Ajout de `provider.compatible_timeout_secs` (défaut effectif `120s`) pour le timeout HTTP des providers OpenAI-compatibles.
