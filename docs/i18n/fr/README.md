...
<h1 align="center">🦀 QuantClaw — Assistant Personnel IA</h1>
...
`quantclaw onboard` s'exécute automatiquement après l'installation pour configurer votre workspace et fournisseur.
...
quantclaw gateway                # par défaut : 127.0.0.1:42617
quantclaw gateway --port 0       # port aléatoire (sécurité renforcée)
...
Mise à jour ? Exécutez `quantclaw doctor` après la mise à jour.
...
QuantClaw peut importer votre workspace, mémoire et configuration OpenClaw :
...
Cela migre vos entrées de mémoire, fichiers du workspace et configuration de `~/.openclaw/` vers `~/.quantclaw/`. La configuration est convertie de JSON en TOML automatiquement.

## Paramètres de sécurité par défaut (accès DM)

QuantClaw se connecte à de vraies surfaces de messagerie. Traitez les DM entrants comme des entrées non fiables.
...
- Approuver avec : `quantclaw pairing approve <channel> <code>` (l'expéditeur est alors ajouté à une liste d'autorisation locale).
- Les DM publics entrants nécessitent une activation explicite dans `config.toml`.
- Exécutez `quantclaw doctor` pour détecter les politiques DM risquées ou mal configurées.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critique_  | Nous ne sommes **pas affiliés** à `openagen/quantclaw`, `quantclaw.org` ou `quantclaw.net`. Les domaines `quantclaw.org` et `quantclaw.net` pointent actuellement vers le fork `openagen/quantclaw`, et ce domaine/dépôt usurpent l'identité de notre site web/projet officiel.                                                                                       | Ne faites pas confiance aux informations, binaires, collectes de fonds ou annonces provenant de ces sources. Utilisez uniquement [ce dépôt](https://github.com/quant-speed/quantclaw) et nos comptes sociaux vérifiés.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Pourquoi les équipes choisissent QuantClaw
...
## Résumé des benchmarks (QuantClaw vs OpenClaw, reproductible)
...
> Notes : Les résultats de QuantClaw sont mesurés sur des builds release avec `/usr/bin/time -l`. OpenClaw nécessite le runtime Node.js (typiquement ~390Mo de surcharge mémoire supplémentaire), tandis que NanoBot nécessite le runtime Python. PicoClaw et QuantClaw sont des binaires statiques. Les chiffres de RAM ci-dessus sont la mémoire à l'exécution ; les besoins de compilation sont plus élevés.
...
QuantClaw supporte les profils d'authentification natifs par abonnement (multi-compte, chiffrés au repos).

- Fichier de stockage : `~/.quantclaw/auth-profiles.json`
- Clé de chiffrement : `~/.quantclaw/.secret_key`
...
Racine du workspace : `~/.quantclaw/workspace/` (configurable via config).
...
quantclaw onboard              # Assistant de configuration guidée
quantclaw status               # Afficher le statut du daemon/agent
quantclaw doctor               # Exécuter les diagnostics système

# Gateway + daemon
quantclaw gateway              # Démarrer le serveur gateway (127.0.0.1:42617)
quantclaw daemon               # Démarrer le runtime autonome complet

# Agent
quantclaw agent                # Mode chat interactif
quantclaw agent -m "message"   # Mode message unique

# Gestion des services
quantclaw service install      # Installer comme service OS (launchd/systemd)
quantclaw service start|stop|restart|status

# Canaux
quantclaw channel list         # Lister les canaux configurés
quantclaw channel doctor       # Vérifier la santé des canaux
quantclaw channel bind-telegram 123456789

# Cron + planification
quantclaw cron list            # Lister les tâches planifiées
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Mémoire
quantclaw memory list          # Lister les entrées de mémoire
quantclaw memory get <key>     # Récupérer une mémoire
quantclaw memory stats         # Statistiques de la mémoire
...
quantclaw hardware discover    # Scanner les appareils connectés
quantclaw peripheral list      # Lister les périphériques connectés
quantclaw peripheral flash     # Flasher le firmware sur l'appareil
...
Ou passez les étapes ci-dessus et installez tout (dépendances système, Rust, QuantClaw) en une seule commande :
...
- Suivez [QuantClaw Onboard](#démarrage-rapide) pour une configuration guidée.
...
QuantClaw a été construit pour le crabe lisse 🦀, un assistant IA rapide et efficace. Construit par Argenis De La Rosa et la communauté.
...
Si QuantClaw vous aide dans votre travail et que vous souhaitez soutenir le développement continu, vous pouvez faire un don ici :
...
Nouveau sur QuantClaw ? Recherchez les issues étiquetées [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — consultez notre [Guide de contribution](CONTRIBUTING.md#first-time-contributors) pour savoir comment commencer. Les PRs IA/vibe-coded sont les bienvenus ! 🤖
...
- Nouveau `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**Ceci est le seul dépôt officiel de QuantClaw :**

> https://github.com/quant-speed/quantclaw

Tout autre dépôt, organisation, domaine ou package prétendant être "QuantClaw" ou impliquant une affiliation avec QuantClaw Labs est **non autorisé et non affilié à ce projet**. Les forks non autorisés connus seront listés dans [TRADEMARK.md](docs/maintainers/trademark.md).

Si vous rencontrez une usurpation d'identité ou un usage abusif de la marque, veuillez [ouvrir une issue](https://github.com/quant-speed/quantclaw/issues).

---

## Licence

QuantClaw est sous double licence pour une ouverture maximale et la protection des contributeurs :
...
Le nom et le logo **QuantClaw** sont des marques de QuantClaw Labs. Cette licence n'accorde pas la permission de les utiliser pour impliquer un soutien ou une affiliation. Voir [TRADEMARK.md](docs/maintainers/trademark.md) pour les usages autorisés et interdits.
...
**QuantClaw** — Zéro overhead. Zéro compromis. Déployez partout. Échangez n'importe quoi. 🦀
