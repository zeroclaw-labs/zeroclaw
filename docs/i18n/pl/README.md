...
<h1 align="center">🦀 QuantClaw — Osobisty Asystent AI</h1>
...
> **Zalecana konfiguracja:** uruchom `quantclaw onboard` w terminalu. QuantClaw Onboard prowadzi Cię krok po kroku przez konfigurację gateway, workspace, kanałów i dostawcy. Jest to zalecana ścieżka konfiguracji i działa na macOS, Linux i Windows (przez WSL2). Nowa instalacja? Zacznij tutaj: [Rozpocznij](#szybki-start)
...
`quantclaw onboard` uruchamia się automatycznie po instalacji, aby skonfigurować workspace i dostawcę.
...
quantclaw gateway                # domyślnie: 127.0.0.1:42617
quantclaw gateway --port 0       # losowy port (wzmocnione bezpieczeństwo)
...
Aktualizujesz? Uruchom `quantclaw doctor` po aktualizacji.
...
QuantClaw może zaimportować Twój workspace, pamięć i konfigurację OpenClaw:
...
Migruje wpisy pamięci, pliki workspace i konfigurację z `~/.openclaw/` do `~/.quantclaw/`. Konfiguracja jest automatycznie konwertowana z JSON do TOML.

## Domyślne ustawienia bezpieczeństwa (dostęp DM)

QuantClaw łączy się z prawdziwymi platformami komunikacyjnymi. Traktuj przychodzące DM jako niezaufane dane wejściowe.
...
- Zatwierdź za pomocą: `quantclaw pairing approve <channel> <code>` (wtedy nadawca jest dodawany do lokalnej listy dozwolonych).
- Publiczne przychodzące DM wymagają jawnej zgody w `config.toml`.
- Uruchom `quantclaw doctor`, aby wykryć ryzykowne lub błędnie skonfigurowane polityki DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Krytyczny_  | **Nie jesteśmy powiązani** z `openagen/quantclaw`, `quantclaw.org` ani `quantclaw.net`. Domeny `quantclaw.org` i `quantclaw.net` obecnie kierują do forka `openagen/quantclaw`, a ta domena/repozytorium podszywają się pod naszą oficjalną stronę/projekt.                                                                                       | Nie ufaj informacjom, plikom binarnym, zbiórkom funduszy ani ogłoszeniom z tych źródeł. Używaj wyłącznie [tego repozytorium](https://github.com/quant-speed/quantclaw) i naszych zweryfikowanych kont społecznościowych.                                                                                                                                                                                                                                                                                                                                                                       |
...
### Dlaczego zespoły wybierają QuantClaw
...
## Porównanie wydajności (QuantClaw vs OpenClaw, odtwarzalne)
...
> Uwagi: Wyniki QuantClaw są mierzone na buildach release przy użyciu `/usr/bin/time -l`. OpenClaw wymaga środowiska Node.js (typowo ~390MB dodatkowego narzutu pamięci), natomiast NanoBot wymaga środowiska Python. PicoClaw i QuantClaw to statyczne pliki binarne. Powyższe wartości RAM dotyczą pamięci runtime; wymagania kompilacji są wyższe.
...
Minimalna `~/.quantclaw/config.toml`:
...
QuantClaw obsługuje natywne profile autoryzacyjne subskrypcji (wiele kont, szyfrowanie w spoczynku).

- Plik przechowywania: `~/.quantclaw/auth-profiles.json`
- Klucz szyfrowania: `~/.quantclaw/.secret_key`
...
Katalog główny workspace: `~/.quantclaw/workspace/` (konfigurowalne przez config).
...
Umiejętności: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` lub `SKILL.toml`.
...
quantclaw onboard              # Kreator konfiguracji z przewodnikiem
quantclaw status               # Pokaż status demona/agenta
quantclaw doctor               # Uruchom diagnostykę systemu

# Gateway + demon
quantclaw gateway              # Uruchom serwer gateway (127.0.0.1:42617)
quantclaw daemon               # Uruchom pełne autonomiczne środowisko

# Agent
quantclaw agent                # Tryb interaktywnego czatu
quantclaw agent -m "message"   # Tryb pojedynczej wiadomości

# Zarządzanie usługami
quantclaw service install      # Zainstaluj jako usługę OS (launchd/systemd)
quantclaw service start|stop|restart|status

# Kanały
quantclaw channel list         # Lista skonfigurowanych kanałów
quantclaw channel doctor       # Sprawdź zdrowie kanałów
quantclaw channel bind-telegram 123456789

# Cron + planowanie
quantclaw cron list            # Lista zaplanowanych zadań
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Pamięć
quantclaw memory list          # Lista wpisów pamięci
quantclaw memory get <key>     # Pobierz wspomnienie
quantclaw memory stats         # Statystyki pamięci
...
# Peryferia sprzętowe
quantclaw hardware discover    # Skanuj podłączone urządzenia
quantclaw peripheral list      # Lista podłączonych peryferiów
quantclaw peripheral flash     # Flash firmware na urządzenie
...
Lub pomiń powyższe kroki i zainstaluj wszystko (zależności systemowe, Rust, QuantClaw) jednym poleceniem:
...
- Postępuj zgodnie z [QuantClaw Onboard](#szybki-start) dla konfiguracji z przewodnikiem.
...
QuantClaw został zbudowany dla smooth crab 🦀, szybkiego i wydajnego asystenta AI. Stworzony przez Argenisa De La Rosę i społeczność.
...
Jeśli QuantClaw pomaga w Twojej pracy i chcesz wesprzeć dalszy rozwój, możesz przekazać darowiznę tutaj:
...
Nowy w QuantClaw? Szukaj issues oznaczonych [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — zobacz nasz [Przewodnik kontrybutora](CONTRIBUTING.md#first-time-contributors), aby dowiedzieć się jak zacząć. PR-y z AI/vibe-coded mile widziane! 🤖
...
- Nowy `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**To jest jedyne oficjalne repozytorium QuantClaw:**

> https://github.com/quant-speed/quantclaw

Każde inne repozytorium, organizacja, domena lub pakiet twierdzący, że jest "QuantClaw" lub sugerujący powiązanie z QuantClaw Labs jest **nieautoryzowany i niepowiązany z tym projektem**. Znane nieautoryzowane forki będą wymienione w [TRADEMARK.md](docs/maintainers/trademark.md).

Jeśli napotkasz podszywanie się lub nadużycie znaku towarowego, proszę [otwórz zgłoszenie](https://github.com/quant-speed/quantclaw/issues).

---

## Licencja

QuantClaw jest podwójnie licencjonowany dla maksymalnej otwartości i ochrony kontrybutorów:
...
Nazwa **QuantClaw** i logo są znakami towarowymi QuantClaw Labs. Ta licencja nie udziela pozwolenia na ich używanie w celu sugerowania poparcia lub powiązania. Zobacz [TRADEMARK.md](docs/maintainers/trademark.md) po dozwolone i zabronione użycia.
...
**QuantClaw** — Zero narzutu. Zero kompromisów. Wdrażaj wszędzie. Wymieniaj wszystko. 🦀
