...
<h1 align="center">🦀 QuantClaw — Osobní AI Asistent</h1>
...
> **Doporučené nastavení:** spusťte `quantclaw onboard` ve vašem terminálu. QuantClaw Onboard vás krok za krokem provede nastavením gateway, workspace, kanálů a poskytovatele. Je to doporučená cesta nastavení a funguje na macOS, Linux a Windows (přes WSL2). Nová instalace? Začněte zde: [Začínáme](#rychlý-start)
...
`quantclaw onboard` se automaticky spustí po instalaci pro konfiguraci vašeho workspace a poskytovatele.
...
quantclaw gateway                # výchozí: 127.0.0.1:42617
quantclaw gateway --port 0       # náhodný port (posílené zabezpečení)
...
Aktualizujete? Spusťte `quantclaw doctor` po aktualizaci.
...
QuantClaw může importovat váš workspace, paměť a konfiguraci OpenClaw:
...
Migruje záznamy paměti, soubory workspace a konfiguraci z `~/.openclaw/` do `~/.quantclaw/`. Konfigurace je automaticky převedena z JSON do TOML.

## Výchozí nastavení zabezpečení (přístup DM)

QuantClaw se připojuje k reálným komunikačním platformám. Zacházejte s příchozími DM jako s nedůvěryhodným vstupem.
...
- Schvalte pomocí: `quantclaw pairing approve <channel> <code>` (poté je odesílatel přidán na lokální allowlist).
- Veřejné příchozí DM vyžadují explicitní opt-in v `config.toml`.
- Spusťte `quantclaw doctor` pro odhalení rizikových nebo špatně nakonfigurovaných DM politik.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritické_  | **Nejsme spojeni** s `openagen/quantclaw`, `quantclaw.org` ani `quantclaw.net`. Domény `quantclaw.org` a `quantclaw.net` aktuálně směřují na fork `openagen/quantclaw` a tato doména/repozitář se vydávají za naši oficiální stránku/projekt.                                                                                       | Nedůvěřujte informacím, binárním souborům, sbírkám ani oznámením z těchto zdrojů. Používejte pouze [toto repozitárium](https://github.com/quant-speed/quantclaw) a naše ověřené sociální účty.                                                                                                                                                                                                                                                                                                                                                                       |
...
## Srovnání výkonu (QuantClaw vs OpenClaw, reprodukovatelné)
...
> Poznámky: Výsledky QuantClaw jsou měřeny na release buildech pomocí `/usr/bin/time -l`. OpenClaw vyžaduje běhové prostředí Node.js (typicky ~390MB dodatečné paměťové režie), zatímco NanoBot vyžaduje běhové prostředí Python. PicoClaw a QuantClaw jsou statické binárky. Výše uvedené hodnoty RAM jsou runtime paměť; požadavky kompilace jsou vyšší.
...
Minimální `~/.quantclaw/config.toml`:
...
QuantClaw podporuje nativní autorizační profily předplatného (více účtů, šifrování v klidu).

- Soubor úložiště: `~/.quantclaw/auth-profiles.json`
- Šifrovací klíč: `~/.quantclaw/.secret_key`
...
Kořenový adresář workspace: `~/.quantclaw/workspace/` (konfigurovatelné přes config).
...
Dovednosti: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` nebo `SKILL.toml`.
...
```bash
# Správa workspace
quantclaw onboard              # Průvodce nastavením
quantclaw status               # Zobrazení stavu démona/agenta
quantclaw doctor               # Spuštění diagnostiky systému

# Gateway + démon
quantclaw gateway              # Spuštění gateway serveru (127.0.0.1:42617)
quantclaw daemon               # Spuštění plného autonomního runtime

# Agent
quantclaw agent                # Interaktivní režim chatu
quantclaw agent -m "message"   # Režim jedné zprávy

# Správa služeb
quantclaw service install      # Instalace jako služba OS (launchd/systemd)
quantclaw service start|stop|restart|status

# Kanály
quantclaw channel list         # Seznam konfigurovaných kanálů
quantclaw channel doctor       # Kontrola zdraví kanálů
quantclaw channel bind-telegram 123456789

# Cron + plánování
quantclaw cron list            # Seznam naplánovaných úloh
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Paměť
quantclaw memory list          # Seznam záznamů paměti
quantclaw memory get <key>     # Získání záznamu
quantclaw memory stats         # Statistiky paměti
...
quantclaw hardware discover    # Skenování připojených zařízení
quantclaw peripheral list      # Seznam připojených periferií
quantclaw peripheral flash     # Flash firmwaru na zařízení
...
Nebo přeskočte výše uvedené kroky a nainstalujte vše (systémové závislosti, Rust, QuantClaw) jedním příkazem:
...
- Následujte [QuantClaw Onboard](#rychlý-start) pro průvodce nastavením.
...
QuantClaw byl vytvořen pro smooth crab 🦀, rychlého a efektivního AI asistenta. Vytvořil Argenis De La Rosa a komunita.
...
Pokud vám QuantClaw pomáhá v práci a chcete podpořit další vývoj, můžete přispět zde:
...
Jste v QuantClaw noví? Hledejte issues označené [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — podívejte se na náš [Průvodce přispíváním](CONTRIBUTING.md#first-time-contributors), jak začít. AI/vibe-coded PR vítány! 🤖
...
- Nový `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**Toto je jediný oficiální repozitář QuantClaw:**

> https://github.com/quant-speed/quantclaw

Jakýkoli jiný repozitář, organizace, doména nebo balíček tvrdící, že je „QuantClaw" nebo naznačující spojení se QuantClaw Labs je **neautorizovaný a není spojen s tímto projektem**. Známé neautorizované forky budou uvedeny v [TRADEMARK.md](docs/maintainers/trademark.md).

Pokud narazíte na podvržení identity nebo zneužití ochranné známky, prosím [otevřete issue](https://github.com/quant-speed/quantclaw/issues).

---

## Licence

QuantClaw je dvojitě licencován pro maximální otevřenost a ochranu přispěvatelů:
...
Název **QuantClaw** a logo jsou ochranné známky QuantClaw Labs. Tato licence neuděluje povolení k jejich použití pro naznačení podpory nebo spojení. Viz [TRADEMARK.md](docs/maintainers/trademark.md) pro povolená a zakázaná použití.
...
**QuantClaw** — Nulová režie. Nulový kompromis. Nasaďte kdekoli. Vyměňte cokoli. 🦀
