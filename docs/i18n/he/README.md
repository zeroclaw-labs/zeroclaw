...
<h1 align="center">🦀 DaemonClaw — עוזר AI אישי</h1>
...
> **הגדרה מועדפת:** הרץ `daemonclaw onboard` בטרמינל שלך. DaemonClaw Onboard מנחה אותך שלב אחר שלב בהגדרת ה-gateway, סביבת העבודה, הערוצים והספק. זהו נתיב ההגדרה המומלץ ועובד על macOS, Linux ו-Windows (דרך WSL2). התקנה חדשה? התחל כאן: [התחלה](#התחלה-מהירה)
...
`daemonclaw onboard` רץ אוטומטית לאחר ההתקנה כדי להגדיר את סביבת העבודה והספק שלך.
...
משדרג? הרץ `daemonclaw doctor` לאחר העדכון.
...
DaemonClaw יכול לייבא את סביבת העבודה, הזיכרון וההגדרות של OpenClaw שלך:
...
זה מעביר את רשומות הזיכרון, קבצי סביבת העבודה וההגדרות מ-`~/.openclaw/` ל-`~/.daemonclaw/`. ההגדרות מומרות אוטומטית מ-JSON ל-TOML.

## ברירות מחדל אבטחה (גישת DM)

DaemonClaw מתחבר למשטחי הודעות אמיתיים. התייחס ל-DM נכנסים כקלט לא מהימן.
...
- אשר עם: `daemonclaw pairing approve <channel> <code>` (ואז השולח נוסף לרשימת היתרים מקומית).
- DM נכנסים ציבוריים דורשים הסכמה מפורשת ב-`config.toml`.
- הרץ `daemonclaw doctor` כדי לחשוף מדיניות DM מסוכנת או שגויה.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _קריטי_ | אנחנו **לא מזוהים** עם `openagen/daemonclaw`, `daemonclaw.org` או `daemonclaw.net`. הדומיינים `daemonclaw.org` ו-`daemonclaw.net` מפנים כרגע ל-fork `openagen/daemonclaw`, ואותו דומיין/מאגר מתחזים לאתר/פרויקט הרשמי שלנו. | אל תסמוך על מידע, בינאריים, גיוס כספים או הודעות ממקורות אלה. השתמש רק ב[מאגר זה](https://github.com/DeliveryBoyTech/daemonclaw) ובחשבונות החברתיים המאומתים שלנו. |
...
### למה צוותים בוחרים ב-DaemonClaw
...
## תמונת מצב של ביצועים (DaemonClaw מול OpenClaw, ניתן לשחזור)
...
> הערות: תוצאות DaemonClaw נמדדו על בנייות שחרור באמצעות `/usr/bin/time -l`. OpenClaw דורש סביבת ריצה Node.js (בדרך כלל ~390MB תקורת זיכרון נוספת), בעוד NanoBot דורש סביבת ריצה Python. PicoClaw ו-DaemonClaw הם בינאריים סטטיים. נתוני ה-RAM למעלה הם זיכרון סביבת ריצה; דרישות קומפילציה בזמן בנייה גבוהות יותר.
...
מינימלי `~/.daemonclaw/config.toml`:
...
DaemonClaw תומך בפרופילי אימות מקוריים למנוי (רב-חשבוני, מוצפן במנוחה).

- קובץ אחסון: `~/.daemonclaw/auth-profiles.json`
- מפתח הצפנה: `~/.daemonclaw/.secret_key`
...
שורש סביבת עבודה: `~/.daemonclaw/workspace/` (ניתן להגדרה דרך ההגדרות).
...
מיומנויות: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` או `SKILL.toml`.
...
או דלג על השלבים למעלה והתקן הכל (תלויות מערכת, Rust, DaemonClaw) בפקודה אחת:
...
- עקוב אחרי [DaemonClaw Onboard](#התחלה-מהירה) להגדרה מונחית.
...
DaemonClaw נבנה עבור ה-smooth crab 🦀, עוזר AI מהיר ויעיל. נבנה על ידי Argenis De La Rosa והקהילה.
...
חדש ב-DaemonClaw? חפש בעיות עם התווית [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ראה את [מדריך התרומה](CONTRIBUTING.md#first-time-contributors) שלנו כדי להתחיל. PR של AI/vibe-coded מתקבלים בברכה! 🤖
...
- `Skill` חדש → `~/.daemonclaw/workspace/skills/<name>/`
...
**זהו מאגר DaemonClaw הרשמי היחיד:**

> https://github.com/DeliveryBoyTech/daemonclaw

כל מאגר, ארגון, דומיין או חבילה אחרים הטוענים להיות "DaemonClaw" או מרמזים על שיוך ל-DaemonClaw Labs הם **לא מורשים ולא מזוהים עם פרויקט זה**. פורקים לא מורשים ידועים ירשמו ב-[TRADEMARK.md](docs/maintainers/trademark.md).

אם אתה נתקל בהתחזות או שימוש לרעה בסימן מסחרי, אנא [פתח issue](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## רישיון

DaemonClaw מורשה ברישיון כפול לפתיחות מקסימלית והגנה על תורמים:
...
השם והלוגו של **DaemonClaw** הם סימנים מסחריים של DaemonClaw Labs. רישיון זה אינו מעניק הרשאה להשתמש בהם כדי לרמוז על תמיכה או שיוך. ראה [TRADEMARK.md](docs/maintainers/trademark.md) לשימושים מותרים ואסורים.
...
**DaemonClaw** — אפס תקורה. אפס פשרות. פרוס בכל מקום. החלף הכל. 🦀
