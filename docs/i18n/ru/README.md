...
<h1 align="center">🦀 DaemonClaw — Персональный ИИ-ассистент</h1>
...
<p align="center">
  <a href="https://daemonclawlabs.ai">Веб-сайт</a> ·
...
`daemonclaw onboard` запускается автоматически после установки для настройки рабочего пространства и провайдера.
...
Обновляетесь? Выполните `daemonclaw doctor` после обновления.
...
DaemonClaw может импортировать ваше рабочее пространство, память и конфигурацию OpenClaw:
...
Это переносит ваши записи памяти, файлы рабочего пространства и конфигурацию из `~/.openclaw/` в `~/.daemonclaw/`. Конфигурация автоматически конвертируется из JSON в TOML.

## Настройки безопасности по умолчанию (доступ через ЛС)

DaemonClaw подключается к реальным поверхностям обмена сообщениями. Относитесь к входящим ЛС как к ненадёжному вводу.
...
- Одобрение через: `daemonclaw pairing approve <channel> <code>` (затем отправитель добавляется в локальный список разрешённых).
- Публичные входящие ЛС требуют явного включения в `config.toml`.
- Выполните `daemonclaw doctor` для выявления рискованных или неправильно настроенных политик ЛС.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критический_ | Мы **не связаны** с `openagen/daemonclaw`, `daemonclaw.org` или `daemonclaw.net`. Домены `daemonclaw.org` и `daemonclaw.net` в настоящее время указывают на форк `openagen/daemonclaw`, и этот домен/репозиторий выдают себя за наш официальный сайт/проект. | Не доверяйте информации, бинарным файлам, сбору средств или объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/DeliveryBoyTech/daemonclaw) и наши верифицированные аккаунты в социальных сетях. |
...
### Почему команды выбирают DaemonClaw
...
## Снимок бенчмарков (DaemonClaw vs OpenClaw, воспроизводимый)
...
> Примечания: результаты DaemonClaw измерены на релизных сборках с использованием `/usr/bin/time -l`. OpenClaw требует среду выполнения Node.js (обычно ~390 МБ дополнительных накладных расходов памяти), а NanoBot требует среду выполнения Python. PicoClaw и DaemonClaw — статические бинарные файлы. Показатели ОЗУ выше — это память времени выполнения; требования к компиляции при сборке выше.
...
Минимальный `~/.daemonclaw/config.toml`:
...
DaemonClaw поддерживает нативные профили аутентификации по подписке (мультиаккаунт, шифрование в состоянии покоя).

- Файл хранилища: `~/.daemonclaw/auth-profiles.json`
- Ключ шифрования: `~/.daemonclaw/.secret_key`
...
Корень рабочего пространства: `~/.daemonclaw/workspace/` (настраивается через конфигурацию).
...
Навыки: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` или `SKILL.toml`.
...
Или пропустите шаги выше и установите всё (системные зависимости, Rust, DaemonClaw) одной командой:
...
- Следуйте [DaemonClaw Onboard](#быстрый-старт) для управляемой настройки.
...
DaemonClaw был создан для smooth crab 🦀 — быстрого и эффективного ИИ-ассистента. Создан Argenis De La Rosa и сообществом.

- [daemonclawlabs.ai](https://daemonclawlabs.ai)
- [@daemonclawlabs](https://x.com/daemonclawlabs)

## Поддержите DaemonClaw
...
Новичок в DaemonClaw? Ищите задачи с меткой [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — см. наше [Руководство по участию](CONTRIBUTING.md#first-time-contributors) для начала. AI/vibe-coded PR приветствуются! 🤖
...
- Новый `Skill` → `~/.daemonclaw/workspace/skills/<name>/`
...
**Это единственный официальный репозиторий DaemonClaw:**

> https://github.com/DeliveryBoyTech/daemonclaw

Любой другой репозиторий, организация, домен или пакет, претендующий на звание «DaemonClaw» или подразумевающий связь с DaemonClaw Labs, является **неавторизованным и не связанным с этим проектом**. Известные неавторизованные форки будут перечислены в [TRADEMARK.md](docs/maintainers/trademark.md).

Если вы столкнётесь с имитацией или неправомерным использованием товарного знака, пожалуйста, [откройте issue](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## Лицензия

DaemonClaw распространяется под двойной лицензией для максимальной открытости и защиты участников:
...
Название и логотип **DaemonClaw** являются товарными знаками DaemonClaw Labs. Эта лицензия не предоставляет разрешения на их использование для подразумевания одобрения или принадлежности. См. [TRADEMARK.md](docs/maintainers/trademark.md) для разрешённых и запрещённых использований.
...
**DaemonClaw** — Нулевые накладные расходы. Нулевые компромиссы. Развёртывайте где угодно. Заменяйте что угодно. 🦀
