...
<h1 align="center">🦀 QuantClaw — Персональный ИИ-ассистент</h1>
...
<p align="center">
  <a href="https://quantspeed.ai">Веб-сайт</a> ·
...
`quantclaw onboard` запускается автоматически после установки для настройки рабочего пространства и провайдера.
...
Обновляетесь? Выполните `quantclaw doctor` после обновления.
...
QuantClaw может импортировать ваше рабочее пространство, память и конфигурацию OpenClaw:
...
Это переносит ваши записи памяти, файлы рабочего пространства и конфигурацию из `~/.openclaw/` в `~/.quantclaw/`. Конфигурация автоматически конвертируется из JSON в TOML.

## Настройки безопасности по умолчанию (доступ через ЛС)

QuantClaw подключается к реальным поверхностям обмена сообщениями. Относитесь к входящим ЛС как к ненадёжному вводу.
...
- Одобрение через: `quantclaw pairing approve <channel> <code>` (затем отправитель добавляется в локальный список разрешённых).
- Публичные входящие ЛС требуют явного включения в `config.toml`.
- Выполните `quantclaw doctor` для выявления рискованных или неправильно настроенных политик ЛС.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критический_ | Мы **не связаны** с `openagen/quantclaw`, `quantclaw.org` или `quantclaw.net`. Домены `quantclaw.org` и `quantclaw.net` в настоящее время указывают на форк `openagen/quantclaw`, и этот домен/репозиторий выдают себя за наш официальный сайт/проект. | Не доверяйте информации, бинарным файлам, сбору средств или объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/quant-speed/quantclaw) и наши верифицированные аккаунты в социальных сетях. |
...
### Почему команды выбирают QuantClaw
...
## Снимок бенчмарков (QuantClaw vs OpenClaw, воспроизводимый)
...
> Примечания: результаты QuantClaw измерены на релизных сборках с использованием `/usr/bin/time -l`. OpenClaw требует среду выполнения Node.js (обычно ~390 МБ дополнительных накладных расходов памяти), а NanoBot требует среду выполнения Python. PicoClaw и QuantClaw — статические бинарные файлы. Показатели ОЗУ выше — это память времени выполнения; требования к компиляции при сборке выше.
...
Минимальный `~/.quantclaw/config.toml`:
...
QuantClaw поддерживает нативные профили аутентификации по подписке (мультиаккаунт, шифрование в состоянии покоя).

- Файл хранилища: `~/.quantclaw/auth-profiles.json`
- Ключ шифрования: `~/.quantclaw/.secret_key`
...
Корень рабочего пространства: `~/.quantclaw/workspace/` (настраивается через конфигурацию).
...
Навыки: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` или `SKILL.toml`.
...
Или пропустите шаги выше и установите всё (системные зависимости, Rust, QuantClaw) одной командой:
...
- Следуйте [QuantClaw Onboard](#быстрый-старт) для управляемой настройки.
...
QuantClaw был создан для smooth crab 🦀 — быстрого и эффективного ИИ-ассистента. Создан Argenis De La Rosa и сообществом.

- [quantspeed.ai](https://quantspeed.ai)
- [@quantspeed](https://x.com/quantspeed)

## Поддержите QuantClaw

Если QuantClaw помогает вашей работе и вы хотите поддержать дальнейшую разработку, вы можете пожертвовать здесь:
...
Новичок в QuantClaw? Ищите задачи с меткой [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — см. наше [Руководство по участию](CONTRIBUTING.md#first-time-contributors) для начала. AI/vibe-coded PR приветствуются! 🤖
...
- Новый `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**Это единственный официальный репозиторий QuantClaw:**

> https://github.com/quant-speed/quantclaw

Любой другой репозиторий, организация, домен или пакет, претендующий на звание «QuantClaw» или подразумевающий связь с QuantClaw Labs, является **неавторизованным и не связанным с этим проектом**. Известные неавторизованные форки будут перечислены в [TRADEMARK.md](docs/maintainers/trademark.md).

Если вы столкнётесь с имитацией или неправомерным использованием товарного знака, пожалуйста, [откройте issue](https://github.com/quant-speed/quantclaw/issues).

---

## Лицензия

QuantClaw распространяется под двойной лицензией для максимальной открытости и защиты участников:
...
Название и логотип **QuantClaw** являются товарными знаками QuantClaw Labs. Эта лицензия не предоставляет разрешения на их использование для подразумевания одобрения или принадлежности. См. [TRADEMARK.md](docs/maintainers/trademark.md) для разрешённых и запрещённых использований.
...
**QuantClaw** — Нулевые накладные расходы. Нулевые компромиссы. Развёртывайте где угодно. Заменяйте что угодно. 🦀
