...
<h1 align="center">🦀 DaemonClaw — パーソナルAIアシスタント</h1>
...
> **推奨セットアップ：** ターミナルで `daemonclaw onboard` を実行してください。DaemonClaw Onboardがゲートウェイ、ワークスペース、チャンネル、プロバイダーのセットアップをステップバイステップでガイドします。これは推奨されるセットアップパスで、macOS、Linux、Windows（WSL2経由）で動作します。新規インストール？ここから開始：[はじめに](#クイックスタートtldr)
...
`daemonclaw onboard` はインストール後に自動的に実行され、ワークスペースとプロバイダーを設定します。
...
daemonclaw gateway                # デフォルト：127.0.0.1:42617
daemonclaw gateway --port 0       # ランダムポート（セキュリティ強化）
...
アップグレード？更新後に `daemonclaw doctor` を実行してください。
...
DaemonClawはOpenClawのワークスペース、メモリ、設定をインポートできます：
...
これにより、メモリエントリ、ワークスペースファイル、設定が `~/.openclaw/` から `~/.daemonclaw/` に移行されます。設定はJSONからTOMLに自動変換されます。

## セキュリティデフォルト（DMアクセス）

DaemonClawは実際のメッセージングサービスに接続します。着信DMを信頼できない入力として扱ってください。
...
- 承認方法：`daemonclaw pairing approve <channel> <code>`（送信者がローカル許可リストに追加されます）。
- パブリック着信DMには `config.toml` での明示的なオプトインが必要です。
- `daemonclaw doctor` を実行してリスクのある、または設定ミスのあるDMポリシーを検出します。
...
| 日付 (UTC) | レベル | 通知 | 対応 |
| ---------- | ------ | ---- | ---- |
| 2026-02-19 | _重大_ | 当プロジェクトは `openagen/daemonclaw`、`daemonclaw.org`、`daemonclaw.net` とは**一切関係ありません**。`daemonclaw.org` と `daemonclaw.net` ドメインは現在 `openagen/daemonclaw` フォークを指しており、そのドメイン/リポジトリは当プロジェクトの公式ウェブサイト/プロジェクトを偽装しています。 | それらのソースからの情報、バイナリ、資金調達、告知を信頼しないでください。[このリポジトリ](https://github.com/DeliveryBoyTech/daemonclaw)と認証済みのソーシャルアカウントのみを使用してください。 |
...
## ベンチマークスナップショット（DaemonClaw vs OpenClaw、再現可能）
...
> 注意：DaemonClawの結果はリリースビルドで `/usr/bin/time -l` を使用して測定されています。OpenClawにはNode.jsランタイム（通常約390MBの追加メモリオーバーヘッド）が必要で、NanoBotにはPythonランタイムが必要です。PicoClawとDaemonClawは静的バイナリです。上記のRAM数値はランタイムメモリです。ビルド時のコンパイル要件はより高くなります。
...
DaemonClawはサブスクリプションネイティブ認証プロファイル（マルチアカウント、保存時暗号化）をサポートしています。

- ストアファイル：`~/.daemonclaw/auth-profiles.json`
- 暗号化キー：`~/.daemonclaw/.secret_key`
...
ワークスペースルート：`~/.daemonclaw/workspace/`（設定で変更可能）。
...
スキル：`~/.daemonclaw/workspace/skills/<skill>/SKILL.md` または `SKILL.toml`。
...
```bash
# ワークスペース管理
daemonclaw onboard              # ガイド付きセットアップウィザード
daemonclaw status               # デーモン/エージェントのステータス表示
daemonclaw doctor               # システム診断を実行

# ゲートウェイ + デーモン
daemonclaw gateway              # ゲートウェイサーバーを起動（127.0.0.1:42617）
daemonclaw daemon               # フル自律ランタイムを起動

# エージェント
daemonclaw agent                # インタラクティブチャットモード
daemonclaw agent -m "message"   # 単一メッセージモード

# サービス管理
daemonclaw service install      # OSサービスとしてインストール（launchd/systemd）
daemonclaw service start|stop|restart|status

# チャンネル
daemonclaw channel list         # 設定済みチャンネルの一覧
daemonclaw channel doctor       # チャンネルヘルスの確認
daemonclaw channel bind-telegram 123456789

# Cron + スケジューリング
daemonclaw cron list            # スケジュールタスクの一覧
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# メモリ
daemonclaw memory list          # メモリエントリの一覧
daemonclaw memory get <key>     # メモリの取得
daemonclaw memory stats         # メモリ統計
...
# ハードウェア周辺機器
daemonclaw hardware discover    # 接続デバイスのスキャン
daemonclaw peripheral list      # 接続周辺機器の一覧
daemonclaw peripheral flash     # デバイスへのファームウェア書き込み
...
または、上記のステップをスキップして、単一コマンドですべてをインストール（システム依存、Rust、DaemonClaw）：
...
- [DaemonClaw Onboard](#クイックスタートtldr)でガイド付きセットアップ。
...
DaemonClawはsmooth crab 🦀のために構築されました。高速で効率的なAIアシスタント。Argenis De La Rosaとコミュニティによって構築されました。
...
DaemonClaw初心者ですか？[`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) ラベルの付いた課題を探してください — 始め方は[貢献ガイド](CONTRIBUTING.md#first-time-contributors)を参照。AI/vibe-coded PRも歓迎します！🤖
...
**これがDaemonClawの唯一の公式リポジトリです：**

> https://github.com/DeliveryBoyTech/daemonclaw

「DaemonClaw」を名乗る、またはDaemonClaw Labsとの提携を示唆する他のリポジトリ、組織、ドメイン、パッケージは**無許可であり、本プロジェクトとは無関係です**。既知の無許可フォークは [TRADEMARK.md](docs/maintainers/trademark.md) に記載されます。

偽装や商標の悪用を見つけた場合は、[issueを作成](https://github.com/DeliveryBoyTech/daemonclaw/issues)してください。

---

## ライセンス

DaemonClawは最大限のオープン性と貢献者保護のためにデュアルライセンスです：
...
**DaemonClaw** の名称とロゴはDaemonClaw Labsの商標です。このライセンスは、推薦や提携を暗示するための使用許可を付与しません。許可された使用と禁止された使用については [TRADEMARK.md](docs/maintainers/trademark.md) を参照してください。
...
**DaemonClaw** — ゼロオーバーヘッド。ゼロ妥協。どこでもデプロイ。何でも交換。🦀
