...
- `cargo check` / `cargo build` 似乎长时间卡在 `Checking daemonclaw`
- 重复出现 `Blocking waiting for file lock on package cache` 或 `build directory`

DaemonClaw 中出现此问题的原因：
...
### 安装后找不到 `daemonclaw` 命令

症状：

- 安装成功，但 shell 找不到 `daemonclaw`
...
- 为该令牌仅保留一个活动运行时
- 停止额外的 `daemonclaw daemon` / `daemonclaw channel start` 进程
