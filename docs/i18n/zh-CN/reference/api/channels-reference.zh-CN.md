...
- 内置的 `read_image` 工具处理视觉回退：当活动提供商不支持视觉时，图像标记会被替换为提示，LLM 可调用 `read_image` 通过视觉提供商级联或 MCP 服务器来描述图像。
- 如果完全没有可用的视觉能力（无视觉提供商、无视觉模型路由、无 MCP 回退），请求将失败并返回结构化能力错误（`capability=vision`）。
