...
6. **运行时状态陈旧**：配置已更改但 `daemonclaw daemon` 未重启。

---

## 1. 配置命名空间

所有渠道设置都位于 `~/.daemonclaw/config.toml` 的 `channels_config` 下。
...
运行 `daemonclaw channel start`（或守护进程模式）时，Telegram 和 Discord 现在支持发送者范围的运行时切换：
...
- 模型缓存预览来自 `daemonclaw models refresh --provider <ID>`。
- 这些是运行时聊天命令，不是 CLI 子命令。

## 入站图像标记协议

DaemonClaw 通过内联消息标记支持多模态输入：
...
如果存在 `[channels_config.matrix]`、`[channels_config.lark]` 或 `[channels_config.feishu]`，但对应的功能未编译进去，`daemonclaw channel list`、`daemonclaw channel doctor` 和 `daemonclaw channel start` 会报告该渠道在此构建中被故意跳过。
...
access_token = \"syt_...\"
user_id = \"@daemonclaw:matrix.example.com\"   # 可选，推荐用于 E2EE
...
- `dm_mention_patterns` 和 `group_mention_patterns` 分别为私聊和群聊提供基于正则表达式的提及门控。当非空时，只有匹配至少一个模式的消息才会被处理；匹配的片段将从转发内容中去除。模式不区分大小写。示例：`["@?DaemonClaw", "\\+?15555550123"]`。无效或过大的模式将被记录日志并跳过。
...
- `DAEMONCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` 会覆盖配置中的密钥。
...
- `DAEMONCLAW_LINQ_SIGNING_SECRET` 会覆盖配置中的密钥。
