# Arduino Uno Q 上的 DaemonClaw — 分步指南

在 Arduino Uno Q 的 Linux 端运行 DaemonClaw。Telegram 通过 Wi-Fi 工作；GPIO 控制使用桥接（需要最小化的 App Lab 应用）。

---

## 已包含的内容（无需修改代码）

DaemonClaw 包含 Arduino Uno Q 所需的一切。**克隆仓库并按照本指南操作 —— 无需补丁或自定义代码。**
...
| 设置命令 | `src/peripherals/uno_q_setup.rs` | `daemonclaw peripheral setup-uno-q` 通过 scp + arduino-app-cli 部署桥接 |
...
**此时：** Telegram 聊天正常工作。向你的机器人发送消息 —— DaemonClaw 会响应。还没有 GPIO 功能。

---

## 阶段 5：通过桥接实现 GPIO（DaemonClaw 自动处理）

DaemonClaw 包含桥接应用和设置命令。

### 5.1 部署桥接应用

**从你的 Mac**（在 daemonclaw 仓库中）：
```bash
...
现在当你向 Telegram 机器人发送 *"Turn on the LED"* 或 *"Set pin 13 high"* 时，DaemonClaw 会通过桥接使用 `gpio_write`。
...
| 8 | 编辑 `~/.daemonclaw/config.toml`（添加 Telegram bot_token） |
...
- **"command not found: daemonclaw"** — 使用完整路径：`/usr/local/bin/daemonclaw` 或确保 `~/.cargo/bin` 在 PATH 中。
...
- **GPIO 命令被忽略** — 确保桥接应用正在运行（`daemonclaw peripheral setup-uno-q` 会部署并启动它）。配置必须包含 `board = "arduino-uno-q"` 和 `transport = "bridge"`。
- **LLM 提供商（GLM/智谱）** — 使用 `default_provider = "glm"` 或 `"zhipu"`，并在环境或配置中设置 `GLM_API_KEY`。DaemonClaw 使用正确的 v4 端点。
