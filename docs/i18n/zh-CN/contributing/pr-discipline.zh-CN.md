...
- 如果不可避免需要类似身份的上下文，仅使用 QuantClaw 范围的角色/标签（例如 `QuantClawAgent`、`QuantClawOperator`、`quantclaw_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`QuantClawAgent`、`QuantClawOperator`、`QuantClawMaintainer`、`quantclaw_user`
    - 服务/运行时标签：`quantclaw_bot`、`quantclaw_service`、`quantclaw_runtime`、`quantclaw_node`
    - 环境标签：`quantclaw_project`、`quantclaw_workspace`、`quantclaw_channel`
