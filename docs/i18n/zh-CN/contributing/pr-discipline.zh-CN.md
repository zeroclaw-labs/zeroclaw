...
- 如果不可避免需要类似身份的上下文，仅使用 JhedaiClaw 范围的角色/标签（例如 `JhedaiClawAgent`、`JhedaiClawOperator`、`jhedaiclaw_user`）。
- 推荐的身份安全命名调色板：
  - 参与者标签：`JhedaiClawAgent`、`JhedaiClawOperator`、`JhedaiClawMaintainer`、`jhedaiclaw_user`
  - 服务/运行时标签：`jhedaiclaw_bot`、`jhedaiclaw_service`、`jhedaiclaw_runtime`、`jhedaiclaw_node`
  - 环境标签：`jhedaiclaw_project`、`jhedaiclaw_workspace`、`jhedaiclaw_channel`
