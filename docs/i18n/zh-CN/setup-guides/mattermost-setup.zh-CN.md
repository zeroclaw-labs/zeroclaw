...
channel_id = \"your-channel-id\"              # 可选：单频道回退配置
channel_ids = [\"your-channel-id\", \"dm-id\"] # 可选：显式监听列表，优先于 channel_id
...
- 设置 `channel_ids` 可同时监听多个指定频道和私信。
- `channel_ids` 为空且设置 `channel_id` 时，只监听一个频道。
- 设置 `channel_id = "*"`，或两者都为空时，自动发现所有可访问的 Mattermost 频道和私信。
...
| `channel_id` | （可选）单频道回退 ID。仅在 `channel_ids` 为空时使用。 |
| `channel_ids` | （可选）要同时监听的频道或私信 ID 列表。设置后优先于 `channel_id`。 |
