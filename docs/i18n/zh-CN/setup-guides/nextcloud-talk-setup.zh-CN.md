...
本指南介绍 QuantClaw 的原生 Nextcloud Talk 集成。
...
在 `~/.quantclaw/config.toml` 中添加以下部分：
...
- 设置 `QUANTCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` 时会覆盖 `webhook_secret`。
...
配置 `webhook_secret` 时，QuantClaw 会验证：
...
- QuantClaw 忽略来自机器人的 webhook 事件（`actorType = bots`）。
- QuantClaw 忽略非消息/系统事件。
