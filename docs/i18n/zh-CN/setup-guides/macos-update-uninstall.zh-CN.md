...
本页面记录了 macOS（OS X）上 QuantClaw 支持的更新和卸载流程。
...
- Homebrew：`/opt/homebrew/bin/quantclaw`（Apple Silicon）或 `/usr/local/bin/quantclaw`（Intel）
- Cargo/引导安装/手动安装：`~/.cargo/bin/quantclaw`
...
```bash
command -v quantclaw || echo \"quantclaw 二进制文件未找到\"
pgrep -fl quantclaw || echo \"没有运行中的 quantclaw 进程\"
```
