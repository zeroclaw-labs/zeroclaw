...
`daemonclaw onboard` koerer automatisk efter installation for at konfigurere dit arbejdsomrade og din udbyder.
...
daemonclaw gateway --port 0       # tilfaeldig port (sikkerhedshaerdet)
...
Opgradering? Koer `daemonclaw doctor` efter opdatering.
...
DaemonClaw kan importere dit OpenClaw-arbejdsomrade, hukommelse og konfiguration:
...
Dette migrerer dine hukommelsesposter, arbejdsomradefiler og konfiguration fra `~/.openclaw/` til `~/.daemonclaw/`. Konfiguration konverteres automatisk fra JSON til TOML.

## Sikkerhedsstandarder (DM-adgang)

DaemonClaw forbinder til rigtige beskedplatforme. Behandl indgaaende DM'er som utrovaerdigt input.
...
- Godkend med: `daemonclaw pairing approve <channel> <code>` (derefter tilfojes afsenderen til en lokal godkendelsesliste).
- Offentlige indgaaende DM'er kraever et eksplicit opt-in i `config.toml`.
- Koer `daemonclaw doctor` for at afsloere risikable eller forkert konfigurerede DM-politikker.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisk_ | Vi er **ikke tilknyttet** `openagen/daemonclaw`, `daemonclaw.org` eller `daemonclaw.net`. Domaenerne `daemonclaw.org` og `daemonclaw.net` peger i oejeblikket pa `openagen/daemonclaw`-forken, og det domaene/repository udgiver sig for at vaere vores officielle hjemmeside/projekt. | Stol ikke pa information, binaerfiler, fundraising eller meddelelser fra disse kilder. Brug kun [dette repository](https://github.com/DeliveryBoyTech/daemonclaw) og vores verificerede sociale konti. |
...
### Hvorfor hold vaelger DaemonClaw
...
## Benchmark-overblik (DaemonClaw vs OpenClaw, Reproducerbart)
...
> Notat: DaemonClaw-resultater er maalt pa release-builds ved brug af `/usr/bin/time -l`. OpenClaw kraever Node.js-koerselsmiljoe (typisk ~390MB ekstra hukommelsesoverhead), mens NanoBot kraever Python-koerselsmiljoe. PicoClaw og DaemonClaw er statiske binaerer. RAM-tallene ovenfor er koerselstidshukommelse; kompileringstidskrav er hoejere.
...
DaemonClaw understoetter abonnements-native godkendelsesprofiler (flere konti, krypteret i hvile).

- Lagerfil: `~/.daemonclaw/auth-profiles.json`
- Krypteringsnoegle: `~/.daemonclaw/.secret_key`
...
Arbejdsomraderod: `~/.daemonclaw/workspace/` (konfigurerbart via config).
...
Faerdigheder: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
daemonclaw onboard              # Guidet opsaetningsguide
daemonclaw status               # Vis daemon/agent-status
daemonclaw doctor               # Koer systemdiagnostik

# Gateway + daemon
daemonclaw gateway              # Start gateway-server (127.0.0.1:42617)
daemonclaw daemon               # Start fuld autonom koersel

# Agent
daemonclaw agent                # Interaktiv chattilstand
daemonclaw agent -m "message"   # Enkeltbeskedtilstand

# Servicestyring
daemonclaw service install      # Installer som OS-service (launchd/systemd)
daemonclaw service start|stop|restart|status

# Kanaler
daemonclaw channel list         # List konfigurerede kanaler
daemonclaw channel doctor       # Tjek kanalsundhed
daemonclaw channel bind-telegram 123456789

# Cron + planlaegning
daemonclaw cron list            # List planlagte opgaver
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# Hukommelse
daemonclaw memory list          # List hukommelsesposter
daemonclaw memory get <key>     # Hent en hukommelse
daemonclaw memory stats         # Hukommelsesstatistik
...
# Hardware-periferier
daemonclaw hardware discover    # Skan efter tilsluttede enheder
daemonclaw peripheral list      # List tilsluttede periferier
daemonclaw peripheral flash     # Flash firmware til enhed
...
Eller spring trinnene ovenfor over og installer alt (systemafhaengigheder, Rust, DaemonClaw) med en enkelt kommando:
...
- Foelg [DaemonClaw Onboard](#hurtig-start-tldr) for en guidet opsaetning.
...
DaemonClaw blev bygget til smooth crab 🦀, en hurtig og effektiv AI-assistent. Bygget af Argenis De La Rosa og faellesskabet.
...
Ny til DaemonClaw? Kig efter issues maerket [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se vores [Bidragsguide](CONTRIBUTING.md#first-time-contributors) for at komme i gang. AI/vibe-kodede PR'er velkomne! 🤖
...
**Dette er det eneste officielle DaemonClaw-repository:**

> https://github.com/DeliveryBoyTech/daemonclaw

Ethvert andet repository, organisation, domaene eller pakke, der haevder at vaere "DaemonClaw" eller antyder tilknytning til DaemonClaw Labs, er **uautoriseret og ikke tilknyttet dette projekt**. Kendte uautoriserede forks vil blive opfoert i [TRADEMARK.md](docs/maintainers/trademark.md).

Hvis du stoeder pa identitetstyveri eller varemaerkemisbrug, bedes du [aabne et issue](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## Licens

DaemonClaw er dobbeltlicenseret for maksimal aabenhed og bidragyderbeskyttelse:
...
Navnet **DaemonClaw** og logoet er varemaerker tilhoerende DaemonClaw Labs. Denne licens giver ikke tilladelse til at bruge dem til at antyde stoette eller tilknytning. Se [TRADEMARK.md](docs/maintainers/trademark.md) for tilladte og forbudte anvendelser.
...
**DaemonClaw** — Nul overhead. Nul kompromis. Udrulning overalt. Udskift hvad som helst. 🦀
