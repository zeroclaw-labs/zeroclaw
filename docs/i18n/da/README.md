...
> **Anbefalet opsaetning:** kor `quantclaw onboard` i din terminal. QuantClaw Onboard guider dig trin for trin gennem opsaetning af gateway, arbejdsomrade, kanaler og udbyder. Det er den anbefalede opsaetningssti og virker pa macOS, Linux og Windows (via WSL2). Ny installation? Start her: [Kom i gang](#hurtig-start-tldr)
...
`quantclaw onboard` koerer automatisk efter installation for at konfigurere dit arbejdsomrade og din udbyder.
...
quantclaw gateway --port 0       # tilfaeldig port (sikkerhedshaerdet)
...
Opgradering? Koer `quantclaw doctor` efter opdatering.
...
QuantClaw kan importere dit OpenClaw-arbejdsomrade, hukommelse og konfiguration:
...
Dette migrerer dine hukommelsesposter, arbejdsomradefiler og konfiguration fra `~/.openclaw/` til `~/.quantclaw/`. Konfiguration konverteres automatisk fra JSON til TOML.

## Sikkerhedsstandarder (DM-adgang)

QuantClaw forbinder til rigtige beskedplatforme. Behandl indgaaende DM'er som utrovaerdigt input.
...
- Godkend med: `quantclaw pairing approve <channel> <code>` (derefter tilfojes afsenderen til en lokal godkendelsesliste).
- Offentlige indgaaende DM'er kraever et eksplicit opt-in i `config.toml`.
- Koer `quantclaw doctor` for at afsloere risikable eller forkert konfigurerede DM-politikker.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisk_ | Vi er **ikke tilknyttet** `openagen/quantclaw`, `quantclaw.org` eller `quantclaw.net`. Domaenerne `quantclaw.org` og `quantclaw.net` peger i oejeblikket pa `openagen/quantclaw`-forken, og det domaene/repository udgiver sig for at vaere vores officielle hjemmeside/projekt. | Stol ikke pa information, binaerfiler, fundraising eller meddelelser fra disse kilder. Brug kun [dette repository](https://github.com/quant-speed/quantclaw) og vores verificerede sociale konti. |
...
### Hvorfor hold vaelger QuantClaw
...
## Benchmark-overblik (QuantClaw vs OpenClaw, Reproducerbart)
...
> Notat: QuantClaw-resultater er maalt pa release-builds ved brug af `/usr/bin/time -l`. OpenClaw kraever Node.js-koerselsmiljoe (typisk ~390MB ekstra hukommelsesoverhead), mens NanoBot kraever Python-koerselsmiljoe. PicoClaw og QuantClaw er statiske binaerer. RAM-tallene ovenfor er koerselstidshukommelse; kompileringstidskrav er hoejere.
...
QuantClaw understoetter abonnements-native godkendelsesprofiler (flere konti, krypteret i hvile).

- Lagerfil: `~/.quantclaw/auth-profiles.json`
- Krypteringsnoegle: `~/.quantclaw/.secret_key`
...
Arbejdsomraderod: `~/.quantclaw/workspace/` (konfigurerbart via config).
...
Faerdigheder: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
quantclaw onboard              # Guidet opsaetningsguide
quantclaw status               # Vis daemon/agent-status
quantclaw doctor               # Koer systemdiagnostik

# Gateway + daemon
quantclaw gateway              # Start gateway-server (127.0.0.1:42617)
quantclaw daemon               # Start fuld autonom koersel

# Agent
quantclaw agent                # Interaktiv chattilstand
quantclaw agent -m "message"   # Enkeltbeskedtilstand

# Servicestyring
quantclaw service install      # Installer som OS-service (launchd/systemd)
quantclaw service start|stop|restart|status

# Kanaler
quantclaw channel list         # List konfigurerede kanaler
quantclaw channel doctor       # Tjek kanalsundhed
quantclaw channel bind-telegram 123456789

# Cron + planlaegning
quantclaw cron list            # List planlagte opgaver
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Hukommelse
quantclaw memory list          # List hukommelsesposter
quantclaw memory get <key>     # Hent en hukommelse
quantclaw memory stats         # Hukommelsesstatistik
...
# Hardware-periferier
quantclaw hardware discover    # Skan efter tilsluttede enheder
quantclaw peripheral list      # List tilsluttede periferier
quantclaw peripheral flash     # Flash firmware til enhed
...
Eller spring trinnene ovenfor over og installer alt (systemafhaengigheder, Rust, QuantClaw) med en enkelt kommando:
...
- Foelg [QuantClaw Onboard](#hurtig-start-tldr) for en guidet opsaetning.
...
QuantClaw blev bygget til smooth crab 🦀, en hurtig og effektiv AI-assistent. Bygget af Argenis De La Rosa og faellesskabet.
...
Hvis QuantClaw hjaelper dit arbejde, og du vil stoette den igangvaerende udvikling, kan du donere her:
...
Ny til QuantClaw? Kig efter issues maerket [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se vores [Bidragsguide](CONTRIBUTING.md#first-time-contributors) for at komme i gang. AI/vibe-kodede PR'er velkomne! 🤖
...
**Dette er det eneste officielle QuantClaw-repository:**

> https://github.com/quant-speed/quantclaw

Ethvert andet repository, organisation, domaene eller pakke, der haevder at vaere "QuantClaw" eller antyder tilknytning til QuantClaw Labs, er **uautoriseret og ikke tilknyttet dette projekt**. Kendte uautoriserede forks vil blive opfoert i [TRADEMARK.md](docs/maintainers/trademark.md).

Hvis du stoeder pa identitetstyveri eller varemaerkemisbrug, bedes du [aabne et issue](https://github.com/quant-speed/quantclaw/issues).

---

## Licens

QuantClaw er dobbeltlicenseret for maksimal aabenhed og bidragyderbeskyttelse:
...
Navnet **QuantClaw** og logoet er varemaerker tilhoerende QuantClaw Labs. Denne licens giver ikke tilladelse til at bruge dem til at antyde stoette eller tilknytning. Se [TRADEMARK.md](docs/maintainers/trademark.md) for tilladte og forbudte anvendelser.
...
**QuantClaw** — Nul overhead. Nul kompromis. Udrulning overalt. Udskift hvad som helst. 🦀
