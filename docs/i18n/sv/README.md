...
> **Rekommenderad konfiguration:** kör `daemonclaw onboard` i din terminal. DaemonClaw Onboard guidar dig steg för steg genom att konfigurera gateway, arbetsyta, kanaler och leverantör. Det är den rekommenderade installationsvägen och fungerar på macOS, Linux och Windows (via WSL2). Ny installation? Börja här: [Kom igång](#snabbstart)
...
`daemonclaw onboard` körs automatiskt efter installationen för att konfigurera din arbetsyta och leverantör.
...
daemonclaw gateway --port 0       # slumpmässig port (säkerhetshärdad)
...
Uppgraderar du? Kör `daemonclaw doctor` efter uppdatering.
...
DaemonClaw kan importera din OpenClaw-arbetsyta, minne och konfiguration:
...
Detta migrerar dina minnesposter, arbetsytefiler och konfiguration från `~/.openclaw/` till `~/.daemonclaw/`. Konfiguration konverteras automatiskt från JSON till TOML.

## Säkerhetsstandarder (DM-åtkomst)

DaemonClaw ansluter till riktiga meddelandeytor. Behandla inkommande DM som opålitlig indata.
...
- Godkänn med: `daemonclaw pairing approve <channel> <code>` (sedan läggs avsändaren till i en lokal tillåtlista).
- Offentliga inkommande DM kräver ett explicit opt-in i `config.toml`.
- Kör `daemonclaw doctor` för att hitta riskfyllda eller felkonfigurerade DM-policyer.
...
| ----------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19  | _Kritisk_   | Vi är **inte affilierade** med `openagen/daemonclaw`, `daemonclaw.org` eller `daemonclaw.net`. Domänerna `daemonclaw.org` och `daemonclaw.net` pekar för närvarande till `openagen/daemonclaw`-forken, och den domänen/repositoryt utger sig för att vara vår officiella webbplats/projekt.                                                                         | Lita inte på information, binärer, insamlingar eller meddelanden från dessa källor. Använd bara [detta repository](https://github.com/DeliveryBoyTech/daemonclaw) och våra verifierade sociala konton.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
## Benchmarkögonblicksbild (DaemonClaw vs OpenClaw, Reproducerbar)
...
> Noteringar: DaemonClaw-resultat mäts på release-byggen med `/usr/bin/time -l`. OpenClaw kräver Node.js-körmiljö (typiskt ~390MB extra minnesoverhead), medan NanoBot kräver Python-körmiljö. PicoClaw och DaemonClaw är statiska binärer. RAM-siffrorna ovan är körtidsminne; kompileringskrav vid byggtid är högre.

<p align="center">
  <img src="docs/assets/daemonclaw-comparison.jpeg" alt="DaemonClaw vs OpenClaw jämförelse" width="800" />
</p>
...
DaemonClaw stöder prenumerationsnativa autentiseringsprofiler (multikonto, krypterat i vila).

- Lagringsfil: `~/.daemonclaw/auth-profiles.json`
- Krypteringsnyckel: `~/.daemonclaw/.secret_key`
...
Arbetsyterot: `~/.daemonclaw/workspace/` (konfigurerbart via config).
...
Färdigheter: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
```bash
# Arbetsytehantering
daemonclaw onboard              # Guidad installationsguide
daemonclaw status               # Visa daemon-/agentstatus
daemonclaw doctor               # Kör systemdiagnostik

# Gateway + daemon
daemonclaw gateway              # Starta gateway-server (127.0.0.1:42617)
daemonclaw daemon               # Starta full autonom körmiljö

# Agent
daemonclaw agent                # Interaktivt chattläge
daemonclaw agent -m "message"   # Enstaka meddelandeläge

# Tjänstehantering
daemonclaw service install      # Installera som OS-tjänst (launchd/systemd)
daemonclaw service start|stop|restart|status

# Kanaler
daemonclaw channel list         # Lista konfigurerade kanaler
daemonclaw channel doctor       # Kontrollera kanalhälsa
daemonclaw channel bind-telegram 123456789

# Cron + schemaläggning
daemonclaw cron list            # Lista schemalagda jobb
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# Minne
daemonclaw memory list          # Lista minnesposter
daemonclaw memory get <key>     # Hämta ett minne
daemonclaw memory stats         # Minnesstatistik
...
daemonclaw hardware discover    # Sök efter anslutna enheter
daemonclaw peripheral list      # Lista anslutna periferienheter
daemonclaw peripheral flash     # Flasha firmware till enhet
...
Eller hoppa över stegen ovan och installera allt (systemberoenden, Rust, DaemonClaw) med ett enda kommando:
...
- Följ [DaemonClaw Onboard](#snabbstart) för en guidad installation.
...
DaemonClaw byggdes för smooth crab 🦀, en snabb och effektiv AI-assistent. Byggd av Argenis De La Rosa och gemenskapen.
...
Ny till DaemonClaw? Leta efter ärenden märkta [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se vår [Bidragsguide](CONTRIBUTING.md#first-time-contributors) för hur du kommer igång. AI/vibe-kodade PR:er är välkomna! 🤖
...
**Detta är det enda officiella DaemonClaw-repositoryt:**

> https://github.com/DeliveryBoyTech/daemonclaw

Alla andra repositorier, organisationer, domäner eller paket som hävdar att vara "DaemonClaw" eller antyder anslutning till DaemonClaw Labs är **obehöriga och inte affilierade med detta projekt**. Kända obehöriga forkar listas i [TRADEMARK.md](docs/maintainers/trademark.md).

Om du stöter på imitation eller varumärkesmissbruk, vänligen [öppna ett ärende](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## Licens

DaemonClaw är dubbellicensierat för maximal öppenhet och bidragsgivarskydd:
...
**DaemonClaw**-namnet och logotypen är varumärken som tillhör DaemonClaw Labs. Denna licens beviljar inte tillstånd att använda dem för att antyda stöd eller anslutning. Se [TRADEMARK.md](docs/maintainers/trademark.md) för tillåtna och förbjudna användningar.
...
**DaemonClaw** — Noll overhead. Noll kompromiss. Distribuera var som helst. Byt ut vad som helst. 🦀

## Bidragsgivare

<a href="https://github.com/DeliveryBoyTech/daemonclaw/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=DeliveryBoyTech/daemonclaw" alt="DaemonClaw-bidragsgivare" />
</a>
