...
<h1 align="center">🦀 DaemonClaw — مساعد الذكاء الاصطناعي الشخصي</h1>
...
> **الإعداد المفضل:** شغّل `daemonclaw onboard` في طرفيتك. DaemonClaw Onboard يرشدك خطوة بخطوة لإعداد البوابة ومساحة العمل والقنوات والمزود. إنه مسار الإعداد الموصى به ويعمل على macOS وLinux وWindows (عبر WSL2). تثبيت جديد؟ ابدأ هنا: [البدء](#البداية-السريعة)
...
`daemonclaw onboard` يعمل تلقائيًا بعد التثبيت لتكوين مساحة العمل والمزود.
...
هل تقوم بالترقية؟ شغّل `daemonclaw doctor` بعد التحديث.
...
يمكن لـ DaemonClaw استيراد مساحة عمل OpenClaw والذاكرة والتكوين الخاص بك:
...
يقوم هذا بترحيل إدخالات الذاكرة وملفات مساحة العمل والتكوين من `~/.openclaw/` إلى `~/.daemonclaw/`. يتم تحويل التكوين من JSON إلى TOML تلقائيًا.

## إعدادات الأمان الافتراضية (الوصول عبر الرسائل المباشرة)

يتصل DaemonClaw بأسطح المراسلة الحقيقية. تعامل مع الرسائل المباشرة الواردة كمدخلات غير موثوقة.
...
- الموافقة باستخدام: `daemonclaw pairing approve <channel> <code>` (ثم يُضاف المرسل إلى قائمة السماح المحلية).
- تتطلب الرسائل المباشرة العامة الواردة اشتراكًا صريحًا في `config.toml`.
- شغّل `daemonclaw doctor` لكشف سياسات الرسائل المباشرة الخطرة أو المُعدة خطأ.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _حرج_ | نحن **غير مرتبطين** بـ `openagen/daemonclaw` أو `daemonclaw.org` أو `daemonclaw.net`. نطاقا `daemonclaw.org` و`daemonclaw.net` يشيران حاليًا إلى نسخة `openagen/daemonclaw` المتفرعة، وهذا النطاق/المستودع ينتحل صفة موقعنا/مشروعنا الرسمي. | لا تثق بالمعلومات أو الملفات الثنائية أو جمع التبرعات أو الإعلانات من تلك المصادر. استخدم فقط [هذا المستودع](https://github.com/DeliveryBoyTech/daemonclaw) وحساباتنا الاجتماعية الموثقة. |
...
## لقطة المقارنة المرجعية (DaemonClaw مقابل OpenClaw، قابلة للتكرار)
...
> ملاحظات: نتائج DaemonClaw تم قياسها على إصدارات الإنتاج باستخدام `/usr/bin/time -l`. يتطلب OpenClaw بيئة تشغيل Node.js (عادةً ~390 ميجابايت حمل ذاكرة إضافي)، بينما يتطلب NanoBot بيئة تشغيل Python. PicoClaw وDaemonClaw ملفات ثنائية ثابتة. أرقام الرام أعلاه هي ذاكرة وقت التشغيل؛ متطلبات التجميع في وقت البناء أعلى.
...
الحد الأدنى `~/.daemonclaw/config.toml`:
...
يدعم DaemonClaw ملفات تعريف مصادقة أصلية للاشتراك (متعددة الحسابات، مشفرة عند الراحة).

- ملف التخزين: `~/.daemonclaw/auth-profiles.json`
- مفتاح التشفير: `~/.daemonclaw/.secret_key`
...
جذر مساحة العمل: `~/.daemonclaw/workspace/` (قابل للتكوين عبر التكوين).
...
المهارات: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` أو `SKILL.toml`.
...
أو تخطى الخطوات أعلاه وثبّت كل شيء (تبعيات النظام، Rust، DaemonClaw) بأمر واحد:
...
- اتبع [DaemonClaw Onboard](#البداية-السريعة) للإعداد الموجه.
...
تم بناء DaemonClaw للسلطعون الناعم 🦀، مساعد ذكاء اصطناعي سريع وفعال. بناه Argenis De La Rosa والمجتمع.
...
جديد على DaemonClaw؟ ابحث عن المشكلات المصنفة [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — انظر [دليل المساهمة](CONTRIBUTING.md#first-time-contributors) لمعرفة كيفية البدء. مرحبًا بمساهمات AI/vibe-coded! 🤖
...
- `Skill` جديد → `~/.daemonclaw/workspace/skills/<name>/`
...
**هذا هو مستودع DaemonClaw الرسمي الوحيد:**

> https://github.com/DeliveryBoyTech/daemonclaw

أي مستودع أو منظمة أو نطاق أو حزمة أخرى تدعي أنها "DaemonClaw" أو تشير إلى انتمائها لـ DaemonClaw Labs هي **غير مصرح بها وغير مرتبطة بهذا المشروع**. سيتم سرد النسخ المتفرعة غير المصرح بها المعروفة في [TRADEMARK.md](docs/maintainers/trademark.md).

إذا واجهت انتحالًا أو إساءة استخدام للعلامة التجارية، يرجى [فتح مشكلة](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## الترخيص

DaemonClaw مرخص بترخيص مزدوج لأقصى انفتاح وحماية للمساهمين:
...
اسم وشعار **DaemonClaw** هما علامتان تجاريتان لـ DaemonClaw Labs. لا يمنح هذا الترخيص إذنًا لاستخدامهما للإشارة إلى التأييد أو الانتماء. انظر [TRADEMARK.md](docs/maintainers/trademark.md) للاستخدامات المسموحة والمحظورة.
...
**DaemonClaw** — صفر حمل زائد. صفر تنازلات. انشر في أي مكان. بدّل أي شيء. 🦀
