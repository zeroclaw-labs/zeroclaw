...
<h1 align="center">🦀 QuantClaw — مساعد الذكاء الاصطناعي الشخصي</h1>
...
> **الإعداد المفضل:** شغّل `quantclaw onboard` في طرفيتك. QuantClaw Onboard يرشدك خطوة بخطوة لإعداد البوابة ومساحة العمل والقنوات والمزود. إنه مسار الإعداد الموصى به ويعمل على macOS وLinux وWindows (عبر WSL2). تثبيت جديد؟ ابدأ هنا: [البدء](#البداية-السريعة)
...
`quantclaw onboard` يعمل تلقائيًا بعد التثبيت لتكوين مساحة العمل والمزود.
...
هل تقوم بالترقية؟ شغّل `quantclaw doctor` بعد التحديث.
...
يمكن لـ QuantClaw استيراد مساحة عمل OpenClaw والذاكرة والتكوين الخاص بك:
...
يقوم هذا بترحيل إدخالات الذاكرة وملفات مساحة العمل والتكوين من `~/.openclaw/` إلى `~/.quantclaw/`. يتم تحويل التكوين من JSON إلى TOML تلقائيًا.

## إعدادات الأمان الافتراضية (الوصول عبر الرسائل المباشرة)

يتصل QuantClaw بأسطح المراسلة الحقيقية. تعامل مع الرسائل المباشرة الواردة كمدخلات غير موثوقة.
...
- الموافقة باستخدام: `quantclaw pairing approve <channel> <code>` (ثم يُضاف المرسل إلى قائمة السماح المحلية).
- تتطلب الرسائل المباشرة العامة الواردة اشتراكًا صريحًا في `config.toml`.
- شغّل `quantclaw doctor` لكشف سياسات الرسائل المباشرة الخطرة أو المُعدة خطأ.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _حرج_ | نحن **غير مرتبطين** بـ `openagen/quantclaw` أو `quantclaw.org` أو `quantclaw.net`. نطاقا `quantclaw.org` و`quantclaw.net` يشيران حاليًا إلى نسخة `openagen/quantclaw` المتفرعة، وهذا النطاق/المستودع ينتحل صفة موقعنا/مشروعنا الرسمي. | لا تثق بالمعلومات أو الملفات الثنائية أو جمع التبرعات أو الإعلانات من تلك المصادر. استخدم فقط [هذا المستودع](https://github.com/quant-speed/quantclaw) وحساباتنا الاجتماعية الموثقة. |
...
## لقطة المقارنة المرجعية (QuantClaw مقابل OpenClaw، قابلة للتكرار)
...
> ملاحظات: نتائج QuantClaw تم قياسها على إصدارات الإنتاج باستخدام `/usr/bin/time -l`. يتطلب OpenClaw بيئة تشغيل Node.js (عادةً ~390 ميجابايت حمل ذاكرة إضافي)، بينما يتطلب NanoBot بيئة تشغيل Python. PicoClaw وQuantClaw ملفات ثنائية ثابتة. أرقام الرام أعلاه هي ذاكرة وقت التشغيل؛ متطلبات التجميع في وقت البناء أعلى.
...
الحد الأدنى `~/.quantclaw/config.toml`:
...
يدعم QuantClaw ملفات تعريف مصادقة أصلية للاشتراك (متعددة الحسابات، مشفرة عند الراحة).

- ملف التخزين: `~/.quantclaw/auth-profiles.json`
- مفتاح التشفير: `~/.quantclaw/.secret_key`
...
جذر مساحة العمل: `~/.quantclaw/workspace/` (قابل للتكوين عبر التكوين).
...
المهارات: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` أو `SKILL.toml`.
...
أو تخطى الخطوات أعلاه وثبّت كل شيء (تبعيات النظام، Rust، QuantClaw) بأمر واحد:
...
- اتبع [QuantClaw Onboard](#البداية-السريعة) للإعداد الموجه.
...
تم بناء QuantClaw للسلطعون الناعم 🦀، مساعد ذكاء اصطناعي سريع وفعال. بناه Argenis De La Rosa والمجتمع.
...
إذا ساعدك QuantClaw في عملك وتريد دعم التطوير المستمر، يمكنك التبرع هنا:
...
جديد على QuantClaw؟ ابحث عن المشكلات المصنفة [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — انظر [دليل المساهمة](CONTRIBUTING.md#first-time-contributors) لمعرفة كيفية البدء. مرحبًا بمساهمات AI/vibe-coded! 🤖
...
- `Skill` جديد → `~/.quantclaw/workspace/skills/<name>/`
...
**هذا هو مستودع QuantClaw الرسمي الوحيد:**

> https://github.com/quant-speed/quantclaw

أي مستودع أو منظمة أو نطاق أو حزمة أخرى تدعي أنها "QuantClaw" أو تشير إلى انتمائها لـ QuantClaw Labs هي **غير مصرح بها وغير مرتبطة بهذا المشروع**. سيتم سرد النسخ المتفرعة غير المصرح بها المعروفة في [TRADEMARK.md](docs/maintainers/trademark.md).

إذا واجهت انتحالًا أو إساءة استخدام للعلامة التجارية، يرجى [فتح مشكلة](https://github.com/quant-speed/quantclaw/issues).

---

## الترخيص

QuantClaw مرخص بترخيص مزدوج لأقصى انفتاح وحماية للمساهمين:
...
اسم وشعار **QuantClaw** هما علامتان تجاريتان لـ QuantClaw Labs. لا يمنح هذا الترخيص إذنًا لاستخدامهما للإشارة إلى التأييد أو الانتماء. انظر [TRADEMARK.md](docs/maintainers/trademark.md) للاستخدامات المسموحة والمحظورة.
...
**QuantClaw** — صفر حمل زائد. صفر تنازلات. انشر في أي مكان. بدّل أي شيء. 🦀
