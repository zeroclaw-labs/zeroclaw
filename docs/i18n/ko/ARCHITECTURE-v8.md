...
> - 한국어로 작성된 섹션: 헤더 · 목차 · §1~§3 개요 · §8 도구 카탈로그 · §5 (Core Workflow) · §11 일부 (Dual-Brain, Vault) · §12 (Sync Journal v3).
> - 원본 v8 에서 영문 산문으로 기술된 섹션(§4 Deployment, §7 Browser Daemon, §9 Web Chat, §10 First Brain, §13 부록 등)은 **점진적 번역 대상**이며 현재는 원문 영문이 그대로 포함되어 있습니다. 코드 블록 · 파일 경로 · 트레이트명 · 함수명 · CLI 커맨드는 **원문 영문 그대로** 유지합니다 (사실성 보존).
>
