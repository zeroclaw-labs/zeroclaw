...
<h1 align="center">🦀 QuantClaw — ذاتی AI اسسٹنٹ</h1>
...
> **تجویز کردہ سیٹ اپ:** اپنے ٹرمینل میں `quantclaw onboard` چلائیں۔ QuantClaw Onboard آپ کو gateway، workspace، چینلز، اور provider ترتیب دینے میں مرحلہ وار رہنمائی کرتا ہے۔ یہ تجویز کردہ سیٹ اپ راستہ ہے اور macOS، Linux، اور Windows (WSL2 کے ذریعے) پر کام کرتا ہے۔ نئی تنصیب؟ یہاں سے شروع کریں: [شروع کریں](#فوری-آغاز)
...
`quantclaw onboard` انسٹال کے بعد خود بخود چلتا ہے تاکہ آپ کا workspace اور provider ترتیب دیا جا سکے۔
...
quantclaw gateway                # ڈیفالٹ: 127.0.0.1:42617
quantclaw gateway --port 0       # بے ترتیب پورٹ (سیکیورٹی مضبوط)
...
اپ گریڈ کر رہے ہیں؟ اپ ڈیٹ کے بعد `quantclaw doctor` چلائیں۔
...
QuantClaw آپ کا OpenClaw workspace، میموری، اور کنفیگریشن درآمد کر سکتا ہے:
...
یہ آپ کے میموری اندراجات، workspace فائلیں، اور کنفیگریشن `~/.openclaw/` سے `~/.quantclaw/` میں منتقل کرتا ہے۔ کنفیگ خود بخود JSON سے TOML میں تبدیل ہو جاتی ہے۔

## سیکیورٹی ڈیفالٹس (DM رسائی)

QuantClaw حقیقی پیغام رسانی سطحوں سے جڑتا ہے۔ آنے والے DMs کو غیر بھروسہ مند ان پٹ سمجھیں۔
...
- منظوری دیں: `quantclaw pairing approve <channel> <code>` (پھر بھیجنے والا مقامی اجازت نامہ میں شامل ہو جاتا ہے)۔
- عوامی آنے والے DMs کے لیے `config.toml` میں واضح opt-in ضروری ہے۔
- خطرناک یا غلط ترتیب شدہ DM پالیسیوں کا پتہ لگانے کے لیے `quantclaw doctor` چلائیں۔
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _اہم ترین_  | ہم `openagen/quantclaw`، `quantclaw.org` یا `quantclaw.net` سے **وابستہ نہیں** ہیں۔ `quantclaw.org` اور `quantclaw.net` ڈومینز فی الحال `openagen/quantclaw` فورک کی طرف اشارہ کرتے ہیں، اور وہ ڈومین/ریپوزٹری ہماری سرکاری ویب سائٹ/پروجیکٹ کی نقل کر رہے ہیں۔                                                                                       | ان ذرائع سے معلومات، بائنریز، فنڈ ریزنگ، یا اعلانات پر بھروسہ نہ کریں۔ صرف [یہ ریپوزٹری](https://github.com/quant-speed/quantclaw) اور ہمارے تصدیق شدہ سوشل اکاؤنٹس استعمال کریں۔                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## بینچ مارک سنیپ شاٹ (QuantClaw بمقابلہ OpenClaw، قابل تکرار)
...
کم از کم `~/.quantclaw/config.toml`:
...
QuantClaw سبسکرپشن نیٹو auth پروفائلز سپورٹ کرتا ہے (ملٹی اکاؤنٹ، آرام پر خفیہ)۔

- اسٹور فائل: `~/.quantclaw/auth-profiles.json`
- خفیہ کاری کلید: `~/.quantclaw/.secret_key`
...
Workspace روٹ: `~/.quantclaw/workspace/` (config کے ذریعے قابل ترتیب)۔
...
اسکلز: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` یا `SKILL.toml`۔
...
```bash
# Workspace مینجمنٹ
quantclaw onboard              # رہنمائی شدہ سیٹ اپ وزرڈ
quantclaw status               # daemon/ایجنٹ اسٹیٹس دکھائیں
quantclaw doctor               # سسٹم تشخیص چلائیں

# Gateway + daemon
quantclaw gateway              # Gateway سرور شروع کریں (127.0.0.1:42617)
quantclaw daemon               # مکمل خودمختار رن ٹائم شروع کریں

# ایجنٹ
quantclaw agent                # انٹرایکٹو چیٹ موڈ
quantclaw agent -m "message"   # واحد پیغام موڈ

# سروس مینجمنٹ
quantclaw service install      # OS سروس کے طور پر انسٹال کریں (launchd/systemd)
quantclaw service start|stop|restart|status

# چینلز
quantclaw channel list         # ترتیب شدہ چینلز کی فہرست
quantclaw channel doctor       # چینل صحت چیک کریں
quantclaw channel bind-telegram 123456789

# Cron + شیڈولنگ
quantclaw cron list            # شیڈولڈ جابز کی فہرست
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# میموری
quantclaw memory list          # میموری اندراجات کی فہرست
quantclaw memory get <key>     # میموری حاصل کریں
quantclaw memory stats         # میموری اعداد و شمار
...
# ہارڈویئر پیری فیرلز
quantclaw hardware discover    # منسلک آلات اسکین کریں
quantclaw peripheral list      # منسلک پیری فیرلز کی فہرست
quantclaw peripheral flash     # آلے پر فرم ویئر فلیش کریں
...
یا اوپر کے مراحل چھوڑیں اور سب کچھ (سسٹم dependencies، Rust، QuantClaw) ایک کمانڈ میں انسٹال کریں:
...
- رہنمائی شدہ سیٹ اپ کے لیے [QuantClaw Onboard](#فوری-آغاز) فالو کریں۔
...
QuantClaw smooth crab 🦀 کے لیے بنایا گیا تھا، ایک تیز اور مؤثر AI اسسٹنٹ۔ Argenis De La Rosa اور کمیونٹی نے بنایا۔

- [quantspeed.ai](https://quantspeed.ai)
- [@quantspeed](https://x.com/quantspeed)

## QuantClaw کی حمایت کریں

اگر QuantClaw آپ کے کام میں مدد کرتا ہے اور آپ جاری ترقی کی حمایت کرنا چاہتے ہیں، تو آپ یہاں عطیہ دے سکتے ہیں:
...
QuantClaw میں نئے ہیں؟ [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) لیبل والے issues تلاش کریں — شروع کرنے کے طریقے کے لیے [شراکت گائیڈ](CONTRIBUTING.md#first-time-contributors) دیکھیں۔ AI/vibe-coded PRs کا خیرمقدم ہے! 🤖
...
- نیا `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**یہ QuantClaw کی واحد سرکاری ریپوزٹری ہے:**

> https://github.com/quant-speed/quantclaw

کوئی بھی دوسری ریپوزٹری، تنظیم، ڈومین، یا پیکیج جو "QuantClaw" ہونے کا دعویٰ کرے یا QuantClaw Labs سے وابستگی کا اشارہ کرے **غیر مجاز ہے اور اس پروجیکٹ سے وابستہ نہیں ہے**۔ معلوم غیر مجاز فورکس [TRADEMARK.md](docs/maintainers/trademark.md) میں درج ہوں گے۔

اگر آپ کو نقل یا ٹریڈ مارک کا غلط استعمال ملے، براہ کرم [issue کھولیں](https://github.com/quant-speed/quantclaw/issues)۔

---

## لائسنس

QuantClaw زیادہ سے زیادہ کشادگی اور شراکت دار تحفظ کے لیے دوہری لائسنس یافتہ ہے:
...
**QuantClaw** نام اور لوگو QuantClaw Labs کے ٹریڈ مارکس ہیں۔ یہ لائسنس انہیں توثیق یا وابستگی کا اشارہ دینے کے لیے استعمال کرنے کی اجازت نہیں دیتا۔ مجاز اور ممنوع استعمال کے لیے [TRADEMARK.md](docs/maintainers/trademark.md) دیکھیں۔
...
**QuantClaw** — صفر اوور ہیڈ۔ صفر سمجھوتا۔ کہیں بھی تعینات کریں۔ کچھ بھی تبدیل کریں۔ 🦀
