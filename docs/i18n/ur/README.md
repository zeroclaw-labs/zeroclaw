...
<h1 align="center">🦀 DaemonClaw — ذاتی AI اسسٹنٹ</h1>
...
> **تجویز کردہ سیٹ اپ:** اپنے ٹرمینل میں `daemonclaw onboard` چلائیں۔ DaemonClaw Onboard آپ کو gateway، workspace، چینلز، اور provider ترتیب دینے میں مرحلہ وار رہنمائی کرتا ہے۔ یہ تجویز کردہ سیٹ اپ راستہ ہے اور macOS، Linux، اور Windows (WSL2 کے ذریعے) پر کام کرتا ہے۔ نئی تنصیب؟ یہاں سے شروع کریں: [شروع کریں](#فوری-آغاز)
...
`daemonclaw onboard` انسٹال کے بعد خود بخود چلتا ہے تاکہ آپ کا workspace اور provider ترتیب دیا جا سکے۔
...
daemonclaw gateway                # ڈیفالٹ: 127.0.0.1:42617
daemonclaw gateway --port 0       # بے ترتیب پورٹ (سیکیورٹی مضبوط)
...
اپ گریڈ کر رہے ہیں؟ اپ ڈیٹ کے بعد `daemonclaw doctor` چلائیں۔
...
DaemonClaw آپ کا OpenClaw workspace، میموری، اور کنفیگریشن درآمد کر سکتا ہے:
...
یہ آپ کے میموری اندراجات، workspace فائلیں، اور کنفیگریشن `~/.openclaw/` سے `~/.daemonclaw/` میں منتقل کرتا ہے۔ کنفیگ خود بخود JSON سے TOML میں تبدیل ہو جاتی ہے۔

## سیکیورٹی ڈیفالٹس (DM رسائی)

DaemonClaw حقیقی پیغام رسانی سطحوں سے جڑتا ہے۔ آنے والے DMs کو غیر بھروسہ مند ان پٹ سمجھیں۔
...
- منظوری دیں: `daemonclaw pairing approve <channel> <code>` (پھر بھیجنے والا مقامی اجازت نامہ میں شامل ہو جاتا ہے)۔
- عوامی آنے والے DMs کے لیے `config.toml` میں واضح opt-in ضروری ہے۔
- خطرناک یا غلط ترتیب شدہ DM پالیسیوں کا پتہ لگانے کے لیے `daemonclaw doctor` چلائیں۔
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _اہم ترین_  | ہم `openagen/daemonclaw`، `daemonclaw.org` یا `daemonclaw.net` سے **وابستہ نہیں** ہیں۔ `daemonclaw.org` اور `daemonclaw.net` ڈومینز فی الحال `openagen/daemonclaw` فورک کی طرف اشارہ کرتے ہیں، اور وہ ڈومین/ریپوزٹری ہماری سرکاری ویب سائٹ/پروجیکٹ کی نقل کر رہے ہیں۔                                                                                       | ان ذرائع سے معلومات، بائنریز، فنڈ ریزنگ، یا اعلانات پر بھروسہ نہ کریں۔ صرف [یہ ریپوزٹری](https://github.com/DeliveryBoyTech/daemonclaw) اور ہمارے تصدیق شدہ سوشل اکاؤنٹس استعمال کریں۔                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### ٹیمیں DaemonClaw کیوں چنتی ہیں
...
## بینچ مارک سنیپ شاٹ (DaemonClaw بمقابلہ OpenClaw، قابل تکرار)
...
کم از کم `~/.daemonclaw/config.toml`:
...
DaemonClaw سبسکرپشن نیٹو auth پروفائلز سپورٹ کرتا ہے (ملٹی اکاؤنٹ، آرام پر خفیہ)۔

- اسٹور فائل: `~/.daemonclaw/auth-profiles.json`
- خفیہ کاری کلید: `~/.daemonclaw/.secret_key`
...
Workspace روٹ: `~/.daemonclaw/workspace/` (config کے ذریعے قابل ترتیب)۔
...
اسکلز: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` یا `SKILL.toml`۔
...
```bash
# Workspace مینجمنٹ
daemonclaw onboard              # رہنمائی شدہ سیٹ اپ وزرڈ
daemonclaw status               # daemon/ایجنٹ اسٹیٹس دکھائیں
daemonclaw doctor               # سسٹم تشخیص چلائیں

# Gateway + daemon
daemonclaw gateway              # Gateway سرور شروع کریں (127.0.0.1:42617)
daemonclaw daemon               # مکمل خودمختار رن ٹائم شروع کریں

# ایجنٹ
daemonclaw agent                # انٹرایکٹو چیٹ موڈ
daemonclaw agent -m "message"   # واحد پیغام موڈ

# سروس مینجمنٹ
daemonclaw service install      # OS سروس کے طور پر انسٹال کریں (launchd/systemd)
daemonclaw service start|stop|restart|status

# چینلز
daemonclaw channel list         # ترتیب شدہ چینلز کی فہرست
daemonclaw channel doctor       # چینل صحت چیک کریں
daemonclaw channel bind-telegram 123456789

# Cron + شیڈولنگ
daemonclaw cron list            # شیڈولڈ جابز کی فہرست
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# میموری
daemonclaw memory list          # میموری اندراجات کی فہرست
daemonclaw memory get <key>     # میموری حاصل کریں
daemonclaw memory stats         # میموری اعداد و شمار
...
# ہارڈویئر پیری فیرلز
daemonclaw hardware discover    # منسلک آلات اسکین کریں
daemonclaw peripheral list      # منسلک پیری فیرلز کی فہرست
daemonclaw peripheral flash     # آلے پر فرم ویئر فلیش کریں
...
یا اوپر کے مراحل چھوڑیں اور سب کچھ (سسٹم dependencies، Rust، DaemonClaw) ایک کمانڈ میں انسٹال کریں:
...
- رہنمائی شدہ سیٹ اپ کے لیے [DaemonClaw Onboard](#فوری-آغاز) فالو کریں۔
...
DaemonClaw smooth crab 🦀 کے لیے بنایا گیا تھا، ایک تیز اور مؤثر AI اسسٹنٹ۔ Argenis De La Rosa اور کمیونٹی نے بنایا۔

- [daemonclawlabs.ai](https://daemonclawlabs.ai)
- [@daemonclawlabs](https://x.com/daemonclawlabs)

## DaemonClaw کی حمایت کریں
...
DaemonClaw میں نئے ہیں؟ [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) لیبل والے issues تلاش کریں — شروع کرنے کے طریقے کے لیے [شراکت گائیڈ](CONTRIBUTING.md#first-time-contributors) دیکھیں۔ AI/vibe-coded PRs کا خیرمقدم ہے! 🤖
...
- نیا `Skill` → `~/.daemonclaw/workspace/skills/<name>/`
...
**یہ DaemonClaw کی واحد سرکاری ریپوزٹری ہے:**

> https://github.com/DeliveryBoyTech/daemonclaw

کوئی بھی دوسری ریپوزٹری، تنظیم، ڈومین، یا پیکیج جو "DaemonClaw" ہونے کا دعویٰ کرے یا DaemonClaw Labs سے وابستگی کا اشارہ کرے **غیر مجاز ہے اور اس پروجیکٹ سے وابستہ نہیں ہے**۔ معلوم غیر مجاز فورکس [TRADEMARK.md](docs/maintainers/trademark.md) میں درج ہوں گے۔

اگر آپ کو نقل یا ٹریڈ مارک کا غلط استعمال ملے، براہ کرم [issue کھولیں](https://github.com/DeliveryBoyTech/daemonclaw/issues)۔

---

## لائسنس

DaemonClaw زیادہ سے زیادہ کشادگی اور شراکت دار تحفظ کے لیے دوہری لائسنس یافتہ ہے:
...
**DaemonClaw** نام اور لوگو DaemonClaw Labs کے ٹریڈ مارکس ہیں۔ یہ لائسنس انہیں توثیق یا وابستگی کا اشارہ دینے کے لیے استعمال کرنے کی اجازت نہیں دیتا۔ مجاز اور ممنوع استعمال کے لیے [TRADEMARK.md](docs/maintainers/trademark.md) دیکھیں۔
...
**DaemonClaw** — صفر اوور ہیڈ۔ صفر سمجھوتا۔ کہیں بھی تعینات کریں۔ کچھ بھی تبدیل کریں۔ 🦀
