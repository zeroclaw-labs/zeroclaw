...
<h1 align="center">🦀 QuantClaw — Персональний AI-Асистент</h1>
...
`quantclaw onboard` запускається автоматично після встановлення для налаштування вашого робочого простору та провайдера.
...
quantclaw gateway                # за замовчуванням: 127.0.0.1:42617
quantclaw gateway --port 0       # випадковий порт (посилена безпека)
...
Оновлюєтесь? Виконайте `quantclaw doctor` після оновлення.
...
QuantClaw може імпортувати ваш робочий простір, пам'ять та конфігурацію OpenClaw:
...
Це мігрує ваші записи пам'яті, файли робочого простору та конфігурацію з `~/.openclaw/` до `~/.quantclaw/`. Конфігурація автоматично конвертується з JSON у TOML.

## Стандартні налаштування безпеки (доступ через DM)

QuantClaw підключається до реальних платформ обміну повідомленнями. Розглядайте вхідні DM як ненадійний ввід.
...
- Підтвердіть за допомогою: `quantclaw pairing approve <channel> <code>` (після чого відправник додається до локального списку дозволених).
- Публічні вхідні DM вимагають явного увімкнення в `config.toml`.
- Виконайте `quantclaw doctor` для виявлення ризикованих або неправильно налаштованих політик DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критичний_ | Ми **не пов'язані** з `openagen/quantclaw`, `quantclaw.org` або `quantclaw.net`. Домени `quantclaw.org` та `quantclaw.net` наразі вказують на форк `openagen/quantclaw`, і цей домен/репозиторій видають себе за наш офіційний вебсайт/проєкт. | Не довіряйте інформації, бінарним файлам, збору коштів або оголошенням з цих джерел. Використовуйте лише [цей репозиторій](https://github.com/quant-speed/quantclaw) та наші верифіковані соціальні акаунти. |
...
### Чому команди обирають QuantClaw
...
## Порівняльний бенчмарк (QuantClaw проти OpenClaw, відтворюваний)
...
> Примітки: результати QuantClaw виміряні на релізних збірках за допомогою `/usr/bin/time -l`. OpenClaw вимагає середовище Node.js (зазвичай ~390MB додаткових накладних витрат пам'яті), тоді як NanoBot вимагає середовище Python. PicoClaw і QuantClaw — це статичні бінарні файли. Наведені цифри RAM — це пам'ять часу виконання; вимоги до компіляції вищі.
...
Мінімальний `~/.quantclaw/config.toml`:
...
QuantClaw підтримує профілі авторизації на основі підписки (мультиакаунт, шифрування в стані спокою).

- Файл сховища: `~/.quantclaw/auth-profiles.json`
- Ключ шифрування: `~/.quantclaw/.secret_key`
...
Корінь робочого простору: `~/.quantclaw/workspace/` (налаштовується через конфігурацію).
...
Навички: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` або `SKILL.toml`.
...
quantclaw onboard              # Покроковий майстер налаштування
quantclaw status               # Показати стан демона/агента
quantclaw doctor               # Запустити діагностику системи

# Gateway + демон
quantclaw gateway              # Запустити сервер gateway (127.0.0.1:42617)
quantclaw daemon               # Запустити повне автономне середовище

# Агент
quantclaw agent                # Інтерактивний режим чату
quantclaw agent -m "message"   # Режим одного повідомлення

# Керування сервісом
quantclaw service install      # Встановити як системний сервіс (launchd/systemd)
quantclaw service start|stop|restart|status

# Канали
quantclaw channel list         # Список налаштованих каналів
quantclaw channel doctor       # Перевірка стану каналів
quantclaw channel bind-telegram 123456789

# Cron + планування
quantclaw cron list            # Список запланованих завдань
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Пам'ять
quantclaw memory list          # Список записів пам'яті
quantclaw memory get <key>     # Отримати запис пам'яті
quantclaw memory stats         # Статистика пам'яті
...
quantclaw hardware discover    # Сканування підключених пристроїв
quantclaw peripheral list      # Список підключених периферійних пристроїв
quantclaw peripheral flash     # Прошивка пристрою
...
Або пропустіть кроки вище і встановіть все (системні залежності, Rust, QuantClaw) однією командою:
...
- Слідуйте [QuantClaw Onboard](#швидкий-старт-tldr) для покрокового налаштування.
...
QuantClaw створений для smooth crab 🦀, швидкого та ефективного AI-асистента. Створений Argenis De La Rosa та спільнотою.

- [quantspeed.ai](https://quantspeed.ai)
- [@quantspeed](https://x.com/quantspeed)

## Підтримайте QuantClaw

Якщо QuantClaw допомагає вашій роботі і ви хочете підтримати подальшу розробку, ви можете зробити пожертву тут:
...
Новачок у QuantClaw? Шукайте завдання з міткою [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — дивіться наш [Посібник з внеску](CONTRIBUTING.md#first-time-contributors) для початку. PR з AI-допомогою вітаються!
...
- Новий `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**Це єдиний офіційний репозиторій QuantClaw:**

> https://github.com/quant-speed/quantclaw

Будь-який інший репозиторій, організація, домен або пакет, що претендує на назву "QuantClaw" або натякає на зв'язок з QuantClaw Labs, є **неавторизованим і не пов'язаним з цим проєктом**. Відомі неавторизовані форки перелічені в [TRADEMARK.md](docs/maintainers/trademark.md).

Якщо ви зіткнулися з імітацією або зловживанням торговою маркою, будь ласка, [створіть issue](https://github.com/quant-speed/quantclaw/issues).

---

## Ліцензія

QuantClaw має подвійну ліцензію для максимальної відкритості та захисту учасників:
...
Назва та логотип **QuantClaw** є торговими марками QuantClaw Labs. Ця ліцензія не надає дозволу використовувати їх для підтвердження або зв'язку. Дивіться [TRADEMARK.md](docs/maintainers/trademark.md) для дозволених та заборонених використань.
...
**QuantClaw** — Нуль накладних витрат. Нуль компромісів. Розгортайте будь-де. Замінюйте будь-що. 🦀
