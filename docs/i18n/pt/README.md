...
<h1 align="center">🦀 QuantClaw — Assistente Pessoal de IA</h1>
...
`quantclaw onboard` executa automaticamente após a instalação para configurar seu workspace e provedor.
...
quantclaw gateway                # padrão: 127.0.0.1:42617
quantclaw gateway --port 0       # porta aleatória (segurança reforçada)
...
Atualizando? Execute `quantclaw doctor` após atualizar.
...
O QuantClaw pode importar seu workspace, memória e configuração do OpenClaw:
...
Isso migra suas entradas de memória, arquivos do workspace e configuração de `~/.openclaw/` para `~/.quantclaw/`. A configuração é convertida de JSON para TOML automaticamente.

## Padrões de segurança (acesso por DM)

O QuantClaw conecta-se a superfícies de mensagens reais. Trate DMs recebidas como entrada não confiável.
...
- Aprovar com: `quantclaw pairing approve <channel> <code>` (então o remetente é adicionado a uma lista de permitidos local).
- DMs públicas recebidas requerem uma ativação explícita em `config.toml`.
- Execute `quantclaw doctor` para detectar políticas de DM arriscadas ou mal configuradas.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **Não somos afiliados** a `openagen/quantclaw`, `quantclaw.org` ou `quantclaw.net`. Os domínios `quantclaw.org` e `quantclaw.net` atualmente apontam para o fork `openagen/quantclaw`, e esse domínio/repositório estão se passando pelo nosso site/projeto oficial.                                                                                       | Não confie em informações, binários, arrecadações de fundos ou anúncios dessas fontes. Use apenas [este repositório](https://github.com/quant-speed/quantclaw) e nossas contas sociais verificadas.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Por que equipes escolhem o QuantClaw
...
## Resumo de benchmarks (QuantClaw vs OpenClaw, reproduzível)
...
> Notas: Os resultados do QuantClaw são medidos em builds release usando `/usr/bin/time -l`. O OpenClaw requer o runtime Node.js (tipicamente ~390MB de overhead adicional de memória), enquanto o NanoBot requer o runtime Python. PicoClaw e QuantClaw são binários estáticos. Os valores de RAM acima são memória em runtime; os requisitos de compilação são maiores.
...
O QuantClaw suporta perfis de autenticação nativos de assinatura (multi-conta, criptografados em repouso).

- Arquivo de armazenamento: `~/.quantclaw/auth-profiles.json`
- Chave de criptografia: `~/.quantclaw/.secret_key`
...
Raiz do workspace: `~/.quantclaw/workspace/` (configurável via config).
...
Skills: `~/.quantclaw/workspace/skills/<skill>/SKILL.md` ou `SKILL.toml`.
...
quantclaw onboard              # Assistente de configuração guiada
quantclaw status               # Mostrar status do daemon/agente
quantclaw doctor               # Executar diagnósticos do sistema

# Gateway + daemon
quantclaw gateway              # Iniciar servidor gateway (127.0.0.1:42617)
quantclaw daemon               # Iniciar runtime autônomo completo

# Agente
quantclaw agent                # Modo de chat interativo
quantclaw agent -m "message"   # Modo de mensagem única

# Gerenciamento de serviços
quantclaw service install      # Instalar como serviço do SO (launchd/systemd)
quantclaw service start|stop|restart|status

# Canais
quantclaw channel list         # Listar canais configurados
quantclaw channel doctor       # Verificar saúde dos canais
quantclaw channel bind-telegram 123456789

# Cron + agendamento
quantclaw cron list            # Listar trabalhos agendados
quantclaw cron add "*/5 * * * *" --prompt "Check system health"
quantclaw cron remove <id>

# Memória
quantclaw memory list          # Listar entradas de memória
quantclaw memory get <key>     # Recuperar uma memória
quantclaw memory stats         # Estatísticas de memória
...
quantclaw hardware discover    # Escanear dispositivos conectados
quantclaw peripheral list      # Listar periféricos conectados
quantclaw peripheral flash     # Flashear firmware no dispositivo
...
Ou pule os passos acima e instale tudo (dependências do sistema, Rust, QuantClaw) em um único comando:
...
- Siga o [QuantClaw Onboard](#início-rápido) para uma configuração guiada.
...
O QuantClaw foi construído para o caranguejo suave 🦀, um assistente de IA rápido e eficiente. Construído por Argenis De La Rosa e a comunidade.
...
Se o QuantClaw ajuda no seu trabalho e você quer apoiar o desenvolvimento contínuo, pode doar aqui:
...
Novo no QuantClaw? Procure issues rotulados como [`good first issue`](https://github.com/quant-speed/quantclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — veja nosso [Guia de contribuição](CONTRIBUTING.md#first-time-contributors) para saber como começar. PRs com IA/vibe-coded são bem-vindos! 🤖
...
- Novo `Skill` → `~/.quantclaw/workspace/skills/<name>/`
...
**Este é o único repositório oficial do QuantClaw:**

> https://github.com/quant-speed/quantclaw

Qualquer outro repositório, organização, domínio ou pacote que afirme ser "QuantClaw" ou implique afiliação com QuantClaw Labs **não é autorizado e não é afiliado a este projeto**. Forks não autorizados conhecidos serão listados em [TRADEMARK.md](docs/maintainers/trademark.md).

Se encontrar falsificação ou uso indevido de marca, por favor [abra um issue](https://github.com/quant-speed/quantclaw/issues).

---

## Licença

O QuantClaw tem licença dupla para máxima abertura e proteção dos contribuidores:
...
O nome e logo do **QuantClaw** são marcas registradas da QuantClaw Labs. Esta licença não concede permissão para usá-los para implicar endosso ou afiliação. Veja [TRADEMARK.md](docs/maintainers/trademark.md) para usos permitidos e proibidos.
...
**QuantClaw** — Zero overhead. Zero compromisso. Implante em qualquer lugar. Troque qualquer coisa. 🦀
