...
<h1 align="center">🦀 DaemonClaw — Assistente Pessoal de IA</h1>
...
`daemonclaw onboard` executa automaticamente após a instalação para configurar seu workspace e provedor.
...
daemonclaw gateway                # padrão: 127.0.0.1:42617
daemonclaw gateway --port 0       # porta aleatória (segurança reforçada)
...
Atualizando? Execute `daemonclaw doctor` após atualizar.
...
O DaemonClaw pode importar seu workspace, memória e configuração do OpenClaw:
...
Isso migra suas entradas de memória, arquivos do workspace e configuração de `~/.openclaw/` para `~/.daemonclaw/`. A configuração é convertida de JSON para TOML automaticamente.

## Padrões de segurança (acesso por DM)

O DaemonClaw conecta-se a superfícies de mensagens reais. Trate DMs recebidas como entrada não confiável.
...
- Aprovar com: `daemonclaw pairing approve <channel> <code>` (então o remetente é adicionado a uma lista de permitidos local).
- DMs públicas recebidas requerem uma ativação explícita em `config.toml`.
- Execute `daemonclaw doctor` para detectar políticas de DM arriscadas ou mal configuradas.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **Não somos afiliados** a `openagen/daemonclaw`, `daemonclaw.org` ou `daemonclaw.net`. Os domínios `daemonclaw.org` e `daemonclaw.net` atualmente apontam para o fork `openagen/daemonclaw`, e esse domínio/repositório estão se passando pelo nosso site/projeto oficial.                                                                                       | Não confie em informações, binários, arrecadações de fundos ou anúncios dessas fontes. Use apenas [este repositório](https://github.com/DeliveryBoyTech/daemonclaw) e nossas contas sociais verificadas.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Por que equipes escolhem o DaemonClaw
...
## Resumo de benchmarks (DaemonClaw vs OpenClaw, reproduzível)
...
> Notas: Os resultados do DaemonClaw são medidos em builds release usando `/usr/bin/time -l`. O OpenClaw requer o runtime Node.js (tipicamente ~390MB de overhead adicional de memória), enquanto o NanoBot requer o runtime Python. PicoClaw e DaemonClaw são binários estáticos. Os valores de RAM acima são memória em runtime; os requisitos de compilação são maiores.
...
O DaemonClaw suporta perfis de autenticação nativos de assinatura (multi-conta, criptografados em repouso).

- Arquivo de armazenamento: `~/.daemonclaw/auth-profiles.json`
- Chave de criptografia: `~/.daemonclaw/.secret_key`
...
Raiz do workspace: `~/.daemonclaw/workspace/` (configurável via config).
...
Skills: `~/.daemonclaw/workspace/skills/<skill>/SKILL.md` ou `SKILL.toml`.
...
daemonclaw onboard              # Assistente de configuração guiada
daemonclaw status               # Mostrar status do daemon/agente
daemonclaw doctor               # Executar diagnósticos do sistema

# Gateway + daemon
daemonclaw gateway              # Iniciar servidor gateway (127.0.0.1:42617)
daemonclaw daemon               # Iniciar runtime autônomo completo

# Agente
daemonclaw agent                # Modo de chat interativo
daemonclaw agent -m "message"   # Modo de mensagem única

# Gerenciamento de serviços
daemonclaw service install      # Instalar como serviço do SO (launchd/systemd)
daemonclaw service start|stop|restart|status

# Canais
daemonclaw channel list         # Listar canais configurados
daemonclaw channel doctor       # Verificar saúde dos canais
daemonclaw channel bind-telegram 123456789

# Cron + agendamento
daemonclaw cron list            # Listar trabalhos agendados
daemonclaw cron add "*/5 * * * *" --prompt "Check system health"
daemonclaw cron remove <id>

# Memória
daemonclaw memory list          # Listar entradas de memória
daemonclaw memory get <key>     # Recuperar uma memória
daemonclaw memory stats         # Estatísticas de memória
...
daemonclaw hardware discover    # Escanear dispositivos conectados
daemonclaw peripheral list      # Listar periféricos conectados
daemonclaw peripheral flash     # Flashear firmware no dispositivo
...
Ou pule os passos acima e instale tudo (dependências do sistema, Rust, DaemonClaw) em um único comando:
...
- Siga o [DaemonClaw Onboard](#início-rápido) para uma configuração guiada.
...
O DaemonClaw foi construído para o caranguejo suave 🦀, um assistente de IA rápido e eficiente. Construído por Argenis De La Rosa e a comunidade.
...
Novo no DaemonClaw? Procure issues rotulados como [`good first issue`](https://github.com/DeliveryBoyTech/daemonclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — veja nosso [Guia de contribuição](CONTRIBUTING.md#first-time-contributors) para saber como começar. PRs com IA/vibe-coded são bem-vindos! 🤖
...
- Novo `Skill` → `~/.daemonclaw/workspace/skills/<name>/`
...
**Este é o único repositório oficial do DaemonClaw:**

> https://github.com/DeliveryBoyTech/daemonclaw

Qualquer outro repositório, organização, domínio ou pacote que afirme ser "DaemonClaw" ou implique afiliação com DaemonClaw Labs **não é autorizado e não é afiliado a este projeto**. Forks não autorizados conhecidos serão listados em [TRADEMARK.md](docs/maintainers/trademark.md).

Se encontrar falsificação ou uso indevido de marca, por favor [abra um issue](https://github.com/DeliveryBoyTech/daemonclaw/issues).

---

## Licença

O DaemonClaw tem licença dupla para máxima abertura e proteção dos contribuidores:
...
O nome e logo do **DaemonClaw** são marcas registradas da DaemonClaw Labs. Esta licença não concede permissão para usá-los para implicar endosso ou afiliação. Veja [TRADEMARK.md](docs/maintainers/trademark.md) para usos permitidos e proibidos.
...
**DaemonClaw** — Zero overhead. Zero compromisso. Implante em qualquer lugar. Troque qualquer coisa. 🦀
