...
> - **§6D MoA Vault (Second Brain)** (new): full implementation of v6 plan — 7-step wikilink extraction pipeline, 17 vault tables (including `vault_embeddings`), self-evolving vocabulary, `VaultDocUpsert` sync delta, unified first+second brain parallel search with mandatory `chat_retrieval_logs` audit, **4-way RAG** (FTS + vector + graph BFS + meta filter) with `QueryKind` adaptive weights, structure-mapped hub note engine with 4 entity-type skeletons + Evidence Gap warnings, vault health scoring (0–100), case-number focus briefings with archive, provider-backed `LlmAIEngine`, polling-based `FolderWatcher` with idempotent auto-ingest. Patent 4 claims 23–29 formalised here. 74 vault tests + 0 regressions.
...
| 상태 | 범위 |
|---|---|
| **[구현]** MVP | `src/vault/hub.rs` — `HubSubtype::{StatuteArticle, Person, Case, GeneralConcept}` 자동 분류, 4종 뼈대 템플릿, 📎 문서번호 매핑, Evidence Gap 경고, `refresh_backlink_counts`·`list_compile_candidates`·`compile_hub` API, `hub_notes.content_md` 영속화. 테스트 6개. |
| **[계획] Phase 2+** | 유휴 감지 + 중요도 큐 자동 디스패치, 상충 해소 3중 기준(문서권위/작성일/출처신뢰도), Light/Heavy/Full 영향도 갱신 분기, LLM 기반 섹션 할당(현재는 해시 기반 균등 분배) |
...
| 기능 | 구현 | 상태 |
|---|---|---|
| 헬스체크 | `src/vault/health.rs::run(vault)` — 5개 신호(고아·미생성링크·태그위생·허브 staleness·conflict pending) 가중 감점 → 0~100점, `health_reports` 테이블에 markdown 리포트 영속화. 결정적·AI 호출 없음. | **[구현]** 2 테스트 |
| 포커스 브리핑 | `src/vault/briefing.rs::generate(vault, case_number)` — `case_number` 프론트매터 매칭 + 1-depth 그래프 확장(inbound/outbound) → 시계열/관련 자료/체크리스트 markdown 생성 → `briefings` 테이블에 status='active' 기록. `archive()` 함수로 종결 처리. | **[구현]** 2 테스트 |

**[계획] Phase 4+**: LLM 기반 브리핑 서사 합성(현재는 결정적 구조적 요약), 모순 자동 탐지 LLM 파이프라인, 태그 위생의 의미적 유사도 기반 감지.

### 6D-8. 파일 감시 + 듀얼 포맷 변환

- **폴링 기반 watcher**: **[구현]** `src/vault/watcher.rs::FolderWatcher` — 외부 `notify` crate 의존 없이 `std::fs`로 2초 간격(기본) 폴링. `.md / .markdown / .txt`를 `VaultStore::ingest_markdown`으로 자동 전송. `.hwp / .hwpx / .docx / .pdf`는 변환 훅 지점을 준비하고 현재는 로그만 남김(Phase 5 후속에서 document_pipeline.rs 연결). 숨김 파일·`.moa-vault/` 재귀 방지. `tokio::sync::oneshot` 기반 안전 종료. 테스트 4개(ingest, idempotency, hidden skip, shutdown).
- **HWP/DOCX/PDF → MD + HTML 듀얼 변환**: 변환기는 **[구현]** (`src/tools/document_pipeline.rs` — Upstage/Hancom/Gemini 기반 변환기 기존 존재). `FolderWatcher`에서 이 변환기 호출 wiring은 **[계획]** 후속 phase.
...
| `LlmAIEngine` | 프로덕션. Step 2a/4를 provider(Haiku 등)로 호출, JSON 응답 파싱, 실패 시 Heuristic 폴백 | `providers::Provider::simple_chat` | **[구현]** `src/vault/llm_engine.rs` |
...
`cargo test --lib vault::` → **74 passed / 0 failed** (Phase 1~5 MVP, 2026-04-15 기준).
...
| `hub` | 6 | subtype 4종 분류, refresh+compile end-to-end, Evidence Gap |
| `health` | 2 | 빈 vault = 100점, 미생성 링크가 감점 |
| `briefing` | 2 | 빈 사건 경고, 매칭+1-depth 집계, archive |
| `llm_engine` | 3 | 유효 JSON 파싱, garbage 폴백, gatekeep 객체 파싱 |
| `unified_search` | 7 | 4-way RAG 병렬, FirstOnly scope, QueryKind 4종 분류, 가중치 합=1 |
| `watcher` | 4 | .md 자동 인제스트, mtime 기반 멱등성, 숨김 파일 skip, shutdown signal |

회귀 없음: memory 328 + sync 49 + phone 20 전부 green. **전체 합계 471 passed**.
...
| `src/vault/schema.rs` | 17개 테이블 (vault_embeddings 추가) + FTS5 + triggers | **[구현]** |
...
| `src/vault/hub.rs` | 허브노트 엔진 MVP (4 뼈대 + 백링크 refresh + 컴파일) | **[구현]** |
| `src/vault/health.rs` | 헬스 5신호 → 0~100점 리포트 | **[구현]** |
| `src/vault/briefing.rs` | case_number 기반 포커스 브리핑 + archive | **[구현]** |
| `src/vault/llm_engine.rs` | LlmAIEngine (provider 기반 프로덕션 AI 드라이버) | **[구현]** |
| `src/vault/watcher.rs` | 폴링 기반 FolderWatcher + 자동 인제스트 | **[구현]** |
