...
1. **항상 사용자의 로컬 디바이스를 먼저 확인** — 로컬 key가 유효하면 무료
2. **로컬 key가 없거나 디바이스가 오프라인일 때만 운영자 key 사용** — 크레딧 2.2× 차감
3. **운영자 key는 Railway에 항상 존재** — 정상 운영 상태에서 에러가 발생하지 않음
...
요약: 모든 경로에서 "로컬 디바이스 + 로컬 key"를 먼저 시도.
      실패 시에만 Railway 운영자 key를 사용 (유료).
```
...
mymoa.app 로그인 → Railway /ws/chat 연결
    │
    ▼
【최초 판단】 사용자의 로컬 디바이스가 온라인인가? (DeviceRouter)
    │
...
※ Railway에는 운영자의 ADMIN_*_API_KEY가 항상 설정되어 있으므로,
  로컬 key 확인이 실패해도 서비스가 중단되지 않습니다.
```
