...
1. **로컬 API key가 있으면 항상 로컬 key를 우선 사용** (무료 — 사용자가 직접 LLM 비용 부담)
2. **로컬 key가 없을 때만 운영자 key를 사용** (크레딧 2.2× 차감)
3. **운영자 key도 없으면 에러** (API key 입력 또는 크레딧 충전 안내)
...
│  │                                             │                             │
│  │  로컬 config에                              │  Railway 서버에 요청 도착    │
│  │  API key가 있는가?                          │         │                    │
│  │    │                                        │         ▼                    │
│  │    ├─ YES ──▶ 로컬 key로                    │  사용자의 로컬 디바이스가    │
│  │    │         직접 LLM 호출                   │  온라인인가? (DeviceRouter)  │
│  │    │         ✅ 무료                         │         │                    │
│  │    │         (Railway 미경유)                │         ├─ YES              │
│  │    │                                        │         │    │               │
│  │    └─ NO ───▶ Railway 서버로                │         │    ▼               │
...
사용자 (로컬 MoA 앱)
    │
    ▼
로컬 config에 API key 있는가?
    │
    ├─ YES → 디바이스에서 직접 LLM API 호출 → ✅ 무료
    │        (Railway 전혀 미경유, 인터넷만 있으면 됨)
    │
    └─ NO  → Railway 서버로 요청 전달
              → ADMIN_*_API_KEY 있는가?
                ├─ YES → 운영자 key로 LLM 호출 → 💰 크레딧 2.2× 차감
                └─ NO  → ❌ 에러 (key 입력 또는 충전 안내)
```

#### ② 채널채팅 (Channel Chat — 카카오톡/텔레그램/디스코드 등)

```
사용자 (카카오톡/텔레그램/디스코드 메시지)
    │
    ▼
채널 웹훅 → Railway 서버 수신
    │
    ▼
Railway 서버에서 처리 (채널은 항상 서버 경유)
    │
    ▼
ADMIN_*_API_KEY 환경변수로 LLM 호출 → 💰 크레딧 2.2× 차감
```

> **채널채팅의 특성**: 채널 메시지는 반드시 공개 HTTPS 엔드포인트를
> 경유해야 하므로 (카카오/텔레그램 웹훅 구조), Railway를 통한 처리가
> 기본입니다. 운영자가 채널 봇 토큰과 API key를 Railway 환경변수에
> 사전 설정하면, 이용자는 **세팅 없이 메시지만 보내면 됩니다.**

#### ③ 웹채팅 (Web Chat — mymoa.app 브라우저)

```
사용자 (공공 PC / 도서관 / PC방 — MoA 미설치)
    │
    ▼
mymoa.app 로그인 → Railway /ws/chat 연결
    │
    ▼
사용자의 로컬 디바이스가 온라인인가? (DeviceRouter 확인)
    │
    ├─ YES → "check_key" 프로브 전송 (5초 타임아웃)
    │         │
    │         ├─ 로컬에 유효한 API key 있음
    │         │   → 메시지를 로컬 디바이스로 릴레이
    │         │   → 로컬 디바이스가 자신의 API key로 LLM 호출
    │         │   → 응답을 Railway 경유하여 브라우저로 스트리밍
    │         │   → ✅ 무료 (로컬 key 사용)
    │         │
    │         └─ 로컬에 유효한 API key 없음 (또는 프로브 타임아웃)
    │             → Railway에서 ADMIN_*_API_KEY로 LLM 호출
    │             → 💰 크레딧 2.2× 차감
    │
    └─ NO → Railway에서 ADMIN_*_API_KEY로 LLM 호출
            → 💰 크레딧 2.2× 차감
```

### 비용 결정 요약표

| 채팅 방식 | 로컬 디바이스 | 로컬 API Key | LLM 호출 경로 | 비용 |
|-----------|-------------|-------------|--------------|------|
| **앱채팅** | — (로컬에서 실행) | ✅ 있음 | 디바이스 → LLM 직접 | **무료** |
| **앱채팅** | — (로컬에서 실행) | ❌ 없음 | 디바이스 → Railway → LLM | 크레딧 2.2× |
| **웹채팅** | ✅ 온라인 | ✅ 있음 | 브라우저 → Railway → 디바이스 → LLM | **무료** |
| **웹채팅** | ✅ 온라인 | ❌ 없음 | 브라우저 → Railway → LLM | 크레딧 2.2× |
| **웹채팅** | ❌ 오프라인 | — | 브라우저 → Railway → LLM | 크레딧 2.2× |
| **채널채팅** | — (해당 없음) | — (해당 없음) | 채널 → Railway → LLM | 크레딧 2.2× |
...
| API key 3단계 해석 | `src/gateway/ws.rs` | `handle_socket()` 내 "Resolve API key" 블록 |
| REST API key 해석 | `src/gateway/openclaw_compat.rs` | `handle_api_chat()` 내 key resolution |
| 디바이스 라우터 | `src/gateway/remote.rs` | `DeviceRouter`, `REMOTE_RESPONSE_CHANNELS` |
