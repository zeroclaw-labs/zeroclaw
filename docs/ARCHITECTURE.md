...
> **Version**: v7.0 (SLM-First Gatekeeper + Advisor Strategy + On-device Executor)
...
>
> **Change log**: For version-by-version history and per-session summaries see
> [`docs/architecture-changelog.md`](./architecture-changelog.md).
>
> **Restructured view**: A thematically reorganized companion document is
> available at [`docs/ARCHITECTURE-v8.md`](./ARCHITECTURE-v8.md). It preserves
> all content from this document but groups it by purpose, features,
> memory architecture, chat methods, sync, etc.
...
| **★ Phase 3 — Tool::safe_for_slm 큐레이션** | `src/tools/traits.rs` + 위험 도구들 | 기본 `true`; `shell` / `delegate` / `apply_patch` / `file_write` / `file_edit` / `cron_*` 는 `false` override. REST/WS 양쪽에서 SLM executor 에게 넘기는 도구 리스트를 `safe_for_slm()` 으로 필터링. cloud LLM 은 여전히 모든 도구 사용 가능. |
