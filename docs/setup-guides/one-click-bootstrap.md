...
This page defines the fastest supported path to install and initialize JhedaiClaw.
...
Default behavior is **app-only** (build/install JhedaiClaw) and expects existing Rust toolchain.
...
This builds a local JhedaiClaw image and launches onboarding inside a container while
persisting config/workspace to `./.jhedaiclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
the installer auto-falls back to `podman`. You can also set `JHEDAICLAW_CONTAINER_CLI`
explicitly (for example: `JHEDAICLAW_CONTAINER_CLI=podman ./install.sh --docker`).
...
Docker tag (`JHEDAICLAW_DOCKER_IMAGE`, default: `jhedaiclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/jhedai/jhedaiclaw:latest` and tags it locally before running.
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/jhedai/jhedaiclaw:latest`)
- `--skip-install`
- `--provider <id>`
