...
This page defines the fastest supported path to install and initialize QuantClaw.
...
4. Runs `quantclaw onboard` (interactive setup wizard)
...
/tmp/zc-test/.cargo/bin/quantclaw --version
...
Configure later with `quantclaw onboard`.
...
Removes the binary and optionally the config/data directory (`~/.quantclaw/`).
...
```bash
gh release download --repo quant-speed/quantclaw --pattern "quantclaw-$(uname -m)*"
```

Or download from [GitHub Releases](https://github.com/quant-speed/quantclaw/releases/latest).
