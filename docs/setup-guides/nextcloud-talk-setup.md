...
This guide covers native Nextcloud Talk integration for QuantClaw.
...
Add this section in `~/.quantclaw/config.toml`:
...
allowed_users = ["*"]
# bot_name is the Nextcloud Talk display name of the bot (e.g. "quantclaw").
...
- `QUANTCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
...
quantclaw daemon
# or
quantclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, QuantClaw verifies:
...
- QuantClaw ignores bot-originated webhook events (`actorType = bots`).
- QuantClaw ignores non-message/system events.
