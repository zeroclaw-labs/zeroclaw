...
> 日期：2026-04-18
> 分支：`one2x/custom-v7` (HEAD 6d08c788)
...
- **L2594-2638**：**Preemptive compaction（主路径）**
  ...
  estimate_tokens()
    → 如果进入 Hard zone
    → try_multi_stage_compress()  // 来自 crates/zeroclaw-runtime/src/one2x/compaction.rs
    → 失败则 warn，继续运行
  ```
...
2. `Memory::store()` 把关键事实存入当前配置的 memory backend（常见是 `SqliteMemory`，可通过 `RetrievalPipeline` 召回，但和 summary 无关联）

### 1.4 当前压缩算法（`crates/zeroclaw-runtime/src/one2x/compaction.rs` 400 行）
...
- 修改 `crates/zeroclaw-runtime/src/one2x/compaction.rs::try_multi_stage_compress()`：
...
**实施位置**：
- 多级降级：`crates/zeroclaw-runtime/src/one2x/compaction.rs` 加 `SummarizerStrategy` enum：
  ```rust
...
- [ ] `crates/zeroclaw-runtime/src/one2x/compaction.rs` 加 Aggressive retry 降级（1-2 天）
