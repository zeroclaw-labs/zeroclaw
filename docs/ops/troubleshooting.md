...
- `cargo check` / `cargo build` appears stuck at `Checking quantclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in QuantClaw:
...
### `quantclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `quantclaw`
...
- stop extra `quantclaw daemon` / `quantclaw channel start` processes
