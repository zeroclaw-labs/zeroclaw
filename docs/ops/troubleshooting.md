...
- `cargo check` / `cargo build` appears stuck at `Checking daemonclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in DaemonClaw:
...
### `daemonclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `daemonclaw`
...
- stop extra `daemonclaw daemon` / `daemonclaw channel start` processes
