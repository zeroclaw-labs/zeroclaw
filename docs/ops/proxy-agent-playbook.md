...
- **Audience:** operators and maintainers running QuantClaw in proxied networks.
- **Scope:** `proxy_config` actions, mode selection, verification flow, and troubleshooting.
- **Non-goals:** generic network debugging outside QuantClaw runtime behavior.
...
### 1.1 Proxy only QuantClaw internal traffic

1. Use scope `quantclaw`.
2. Set `http_proxy`/`https_proxy` or `all_proxy`.
3. Validate with `{"action":"get"}`.

Go to:

- [Section 4](#4-mode-a--proxy-only-for-quantclaw-internals)
...
|---|---|---|---|
| `quantclaw` | QuantClaw internal HTTP clients | No | Normal runtime proxying without process-level side effects |
...
## 4. Mode A — Proxy Only for QuantClaw Internals

Use when QuantClaw provider/channel/tool HTTP traffic should use proxy, without exporting process-level proxy env vars.
...
- Runtime proxy is active for QuantClaw HTTP clients.
