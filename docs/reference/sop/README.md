...
- CLI `daemonclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.daemonclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.daemonclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   daemonclaw sop show deploy-prod
   ```
