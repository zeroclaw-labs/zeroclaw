...
- CLI `quantclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.quantclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.quantclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   quantclaw sop list
   quantclaw sop validate
   quantclaw sop show deploy-prod
   ```
