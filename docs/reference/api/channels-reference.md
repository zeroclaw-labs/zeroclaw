...
This document is the canonical reference for channel configuration in JhedaiClaw.
...
6. **Stale runtime state**: config changed but `jhedaiclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.jhedaiclaw/config.toml`.
...
When running `jhedaiclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `jhedaiclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

JhedaiClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `jhedaiclaw channel list`, `jhedaiclaw channel doctor`, and `jhedaiclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@jhedaiclaw:matrix.example.com"   # optional, recommended for E2EE
...
JhedaiClaw supports two WhatsApp backends:
...
port = 6697
nickname = "jhedaiclaw-bot"
username = "jhedaiclaw"              # optional
channels = ["#jhedaiclaw"]
allowed_users = ["*"]
...
- `JHEDAICLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `JHEDAICLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `jhedaiclaw daemon` after config changes.
...
```bash
RUST_LOG=info jhedaiclaw daemon 2>&1 | tee /tmp/jhedaiclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/jhedaiclaw.log
```
