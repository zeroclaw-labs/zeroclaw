...
allowed_users = ["zeroclaw_user"]     # optional: WeCom userid allowlist, "*" = all users
allowed_groups = ["zeroclaw_group"]   # optional: WeCom group chatid allowlist, "*" = all groups
...
- `allowed_users` matches sender `userid`; `allowed_groups` matches group `chatid`. Empty allowlists deny inbound messages by default.
- Group deny replies include the `chatid` so operators can copy it into `allowed_groups`.
