...
3. Generic fallback env vars: `QUANTCLAW_API_KEY` then `API_KEY`
...
QuantClaw supports automatic failover to alternative providers when the primary encounters:
...
When a provider returns 429 (rate limit), QuantClaw:
...
- After multimodal normalization, QuantClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, QuantClaw returns a structured capability error instead of silently ignoring images.
...
- QuantClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `quantclaw models refresh --provider llamacpp`
...
- Model discovery: `quantclaw models refresh --provider sglang`
...
- Model discovery: `quantclaw models refresh --provider vllm`
...
- Model discovery: `quantclaw models refresh --provider osaurus`
...
- Model discovery: `quantclaw models refresh --provider nvidia`
