# JhedaiClaw Config Reference (Operator-Oriented)
...
1. `JHEDAICLAW_WORKSPACE` override (if set)
2. persisted `~/.jhedaiclaw/active_workspace.toml` marker (if present)
3. default `~/.jhedaiclaw/config.toml`

JhedaiClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `jhedaiclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name`         | `jhedaiclaw`                | Service name emitted to OTLP collector                                                         |
...
- Query runtime traces with:
  - `jhedaiclaw doctor traces --limit 20`
  - `jhedaiclaw doctor traces --event tool_call_result --contains \"error\"`
  - `jhedaiclaw doctor traces --id <trace-id>`
...
1. `JHEDAICLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `JHEDAICLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, JhedaiClaw generates one and prints an enrollment URI once.
...
| `state_file`            | `~/.jhedaiclaw/estop-state.json` | Persistent estop state path                     |
...
- Use CLI command `jhedaiclaw estop` to engage and `jhedaiclaw estop resume` to clear levels.
...
- Security-first default: JhedaiClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `JHEDAICLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `JHEDAICLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `JHEDAICLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `JHEDAICLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `jhedaiclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- JhedaiClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `jhedaiclaw doctor` before restart/rollout.
...
- While `jhedaiclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `JHEDAICLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
