# JhedaiClaw on Nucleo-F401RE — Step-by-Step Guide

Run JhedaiClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

JhedaiClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
JhedaiClaw includes everything for Nucleo-F401RE:
...
| Flash command     | `jhedaiclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs                 |
...
### 1.2 Flash via JhedaiClaw

From the jhedaiclaw repo root:
...
| 5    | `jhedaiclaw daemon` or `jhedaiclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `jhedaiclaw peripheral list` to verify.
