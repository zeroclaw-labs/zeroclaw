# DaemonClaw on Nucleo-F401RE — Step-by-Step Guide

Run DaemonClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

DaemonClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
DaemonClaw includes everything for Nucleo-F401RE:
...
| Flash command | `daemonclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via DaemonClaw

From the daemonclaw repo root:
...
| 5 | `daemonclaw daemon` or `daemonclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `daemonclaw peripheral list` to verify.
