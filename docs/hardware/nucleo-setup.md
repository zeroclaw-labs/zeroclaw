...
| Flash command | `quantclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
| 5 | `quantclaw daemon` or `quantclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `quantclaw peripheral list` to verify.
