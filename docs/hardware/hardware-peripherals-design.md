...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  QuantClaw on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  QuantClaw on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
quantclaw agent --mode edge

# Host-Mediated: connect to USB/J-Link target
quantclaw agent --peripheral nucleo-f401re:/dev/ttyACM0
quantclaw agent --probe jlink
...
board = "esp32"
transport = "wifi"
# Edge-Native: QuantClaw runs on ESP32
```
...
- **quantclaw-firmware** or **quantclaw-peripheral** — a separate crate/workspace.
...
- [x] Add `Peripheral` trait, config schema, CLI (`quantclaw peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `quantclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `quantclaw hardware introspect <path>`: memory map, peripheral list
