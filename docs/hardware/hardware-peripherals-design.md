# Hardware Peripherals Design — DaemonClaw

DaemonClaw enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** DaemonClaw acts as a hardware-aware AI agent that:
...
**Mental model:** DaemonClaw = brain that understands hardware. Peripherals = arms and legs it controls.
...
DaemonClaw runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  DaemonClaw on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. DaemonClaw fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
DaemonClaw runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  DaemonClaw on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. DaemonClaw identifies connected hardware (VID/PID, architecture)
...
2. DaemonClaw writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. DaemonClaw auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| DaemonClaw runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs DaemonClaw; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

DaemonClaw on Pi; GPIO via rppal or sysfs. No separate firmware.
...
board = "esp32"
transport = "wifi"
# Edge-Native: DaemonClaw runs on ESP32
```
...
1. **Startup:** DaemonClaw loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between DaemonClaw and peripherals:
...
- **daemonclaw-firmware** or **daemonclaw-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; DaemonClaw connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`daemonclaw peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `daemonclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `daemonclaw hardware introspect <path>`: memory map, peripheral list
...
- [x] DaemonClaw on Raspberry Pi (native GPIO via rppal)
...
- [ ] DaemonClaw *on* ESP32 (WiFi + LLM, edge-native) — future
...
- Running full DaemonClaw *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). DaemonClaw runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. DaemonClaw gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: DaemonClaw from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey DaemonClaw, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
