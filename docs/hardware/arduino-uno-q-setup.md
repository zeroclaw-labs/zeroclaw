...
| Setup command | `src/peripherals/uno_q_setup.rs` | `quantclaw peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
# Clone quantclaw (or scp your project)
...
**From your Mac** (with quantclaw repo):
```bash
...
| 8 | Edit `~/.quantclaw/config.toml` (add Telegram bot_token) |
...
- **"command not found: quantclaw"** — Use full path: `/usr/local/bin/quantclaw` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`quantclaw peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. QuantClaw uses the correct v4 endpoint.
