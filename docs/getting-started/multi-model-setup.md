...
This guide introduces multi-model concepts in QuantClaw, including fallback provider chains, model-level fallbacks, and API key rotation for resilience.
...
When a provider experiences a transient error (timeout, connection failure, auth issue), QuantClaw automatically attempts fallback providers in the order specified.

**Example**: If your primary provider is `openai` but it's temporarily unavailable, QuantClaw can automatically fall back to `anthropic`, then `groq`.
...
When the primary provider recovers, QuantClaw resumes using it (no sticky failover).
...
If `claude-opus-4-20250514` fails or is unavailable, QuantClaw tries the fallback models in order while staying within the same provider (unless a provider-level fallback is also configured).

### API Key Rotation

For providers that frequently encounter rate limits, you can supply additional API keys that QuantClaw will rotate through on `429` responses.
...
**Behavior**: If OpenAI times out or returns an error, QuantClaw will retry twice with exponential backoff, then attempt the same request using Anthropic.
...
quantclaw doctor traces --contains "fallback"

# Show provider retry details
quantclaw doctor traces --contains "provider"

# Show rate-limit rotation
quantclaw doctor traces --contains "429"
```
...
3. Generic fallback: `QUANTCLAW_API_KEY`, then `API_KEY`
