...
| `allow_repeated_tool_calls` | `false` | Allow repeating identical tool call signatures (same tool + same args) in one turn |
...
- By default, duplicate tool calls with identical arguments are skipped within the same turn. Set `allow_repeated_tool_calls = true` to permit repeated identical calls.
...
| `compatible_timeout_secs` | unset (`None`, effective `120`) | HTTP request timeout (seconds) for OpenAI-compatible providers |
...
- `compatible_timeout_secs` must be greater than `0` when set.
- `compatible_timeout_secs` applies to providers backed by the OpenAI-compatible provider implementation.
