...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong JhedaiClaw.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `jhedaiclaw daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.jhedaiclaw/config.toml`.
...
Khi chạy `jhedaiclaw channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `jhedaiclaw models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

JhedaiClaw hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]` có mặt nhưng binary được build mà không có `channel-matrix`, các lệnh `jhedaiclaw channel list`, `jhedaiclaw channel doctor`, và `jhedaiclaw channel start` sẽ ghi log rằng Matrix bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@jhedaiclaw:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
JhedaiClaw hỗ trợ hai backend WhatsApp:
...
5. Khởi động lại `jhedaiclaw daemon` sau khi thay đổi config.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/jhedaiclaw.log
```
