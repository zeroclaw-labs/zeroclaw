...
7. **Home Assistant**: `[homeassistant].url` and `[homeassistant].token` fall back to the `HASS_URL` / `HASS_TOKEN` env vars when left empty, so no secret is committed to `config.toml`. Generate `token` from the HA UI under *Profile → Security → Long-lived access tokens*. Prefer an HTTPS origin for `url` (or another trusted local transport, e.g. a WireGuard/Tailscale tunnel), since the token is sent as a bearer credential on every request. `[homeassistant].allowed_domains` is empty (deny-by-default) until you list the service domains `call_service` may invoke, e.g. `["light", "switch", "scene"]`.
