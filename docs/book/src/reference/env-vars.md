...
4. **KiloCLI / Gemini CLI / Grok Build CLI process settings**: `[providers.models.kilocli.<alias>] binary_path`, `[providers.models.gemini_cli.<alias>] binary_path`, and `[providers.models.grok_cli.<alias>] binary_path`. Grok Build aliases may also list tool environment variable names in `env_passthrough`; values are resolved only when the child is spawned and are not stored in config. Provider-owned `XAI_*` and `GROK_*` names are rejected, and Grok authentication remains in the CLI login cache.
