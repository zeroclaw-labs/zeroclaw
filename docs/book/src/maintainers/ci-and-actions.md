...
`fmt` checks formatting on a GitHub-hosted runner. Neither the GitHub-hosted jobs nor the compile-heavy jobs wait on it; path-detector and other job dependencies still apply. Compile jobs name their runner labels and cache-provider inputs directly (see Build cache behavior below), so resolving `runs-on` no longer puts a formatting queue wait in front of the compile fan-out. `tests/architecture/ci_runner_labels.rs` checks the reviewed runner labels and allowed cache-provider values; it does not check each job's runner/provider pairing. The deliberate trade-off is extra runner work on PRs that fail formatting. `CI Required Gate` still needs `fmt` directly and aggregates required results, so formatting errors still block merge. Advisory Windows jobs remain outside that gate. Branch protection pins the composite gate job. The `master` push run keeps the same quality signal while seeding trusted Rust caches for later PR runs.
