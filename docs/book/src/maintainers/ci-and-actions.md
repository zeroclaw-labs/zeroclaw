...
- **build**: matrix: `x86_64-unknown-linux-gnu`, `aarch64-apple-darwin`, `x86_64-pc-windows-msvc`. On pull requests the Windows leg runs only when `windows-check-changes` selects it through `scripts/ci/windows_check_change_filter.sh`; an unselected leg skips its steps but stays in the matrix so it keeps the rust-cache key that `master` seeds. `push` and `merge_group` runs always check Windows
...
`fmt` checks formatting and emits the runner label and cache provider that the Blacksmith compile jobs read (see Build cache behavior below), so only those jobs declare `needs: [fmt]`. Every other job, including the `Detect … changes` jobs, starts immediately, and `CI Required Gate` needs `fmt` directly, so a formatting error still blocks merge. The trade-off is deliberate: a PR with a formatting error still spends runner time on the GitHub-hosted jobs, but a slow `fmt` queue wait no longer holds all of them back during a runner shortage. `CI Required Gate` aggregates the required results only. The advisory Windows jobs are deliberately absent from that gate and cannot block a PR. Branch protection pins the composite gate job. A PR cannot merge until this is green. The `master` push run keeps the same quality signal while seeding trusted Rust caches for later PR runs.
