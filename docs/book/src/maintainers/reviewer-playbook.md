...
6. If the PR changes presentation-sensitive visual behavior, confirm the author exercised the actual supported interface and supplied privacy-safe screenshots at representative dimensions. If it changes only a rendered message, confirm actual-interface screenshot and action/result evidence; dimensions are not required when no presentation-sensitive property is claimed or at risk. For a qualifying plain-text change that makes no presentation-sensitive claim and has no concrete presentation concern identified by a reviewer, exact output from the supported interface on an identifiable revision is sufficient. Treat `N/A` as valid only when no user-visible interface state changed. A statement that the applicable interface smoke was not performed records the gap but does not satisfy it.
...
- Presentation-sensitive visual changes include actual-interface evidence from an identifiable revision plus screenshots with enough surrounding layout to assess the result. A message-only change in a rendered interface still needs actual-interface evidence on an identifiable revision, but dimensions are not required when no presentation-sensitive property is claimed or at risk. A noninteractive, unstyled, deterministic plain-text CLI, stdout, stderr, or log change may instead use exact output from the supported interface when it makes no presentation-sensitive claim and no reviewer has identified a concrete presentation concern. Record relevant output-shaping context such as locale and terminal width. Browser, TUI, prompt, and styled-output changes remain on the screenshot path. String assertions, component-only snapshots, and helper-level renderer tests cannot replace the applicable interface proof. Interaction and transition claims also name the user action and observed result.
