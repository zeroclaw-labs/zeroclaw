...
Pages` workflow for the release tag once `publish` succeeds, building and
publishing that version's documentation into `/vX.Y.Z/`. The bootstrap and
...
- **Ordering matters:** the tag deploy must land `/vX.Y.Z/` on `gh-pages` *before* a master deploy can flip the stable pointer to it. The `deploy-docs` job runs right after `publish`, so the tag dir normally exists before the version-bump merge's master deploy runs. If a master deploy runs first (dir not yet present), it deliberately retains the previous pointer and the flip publishes on the next master deploy; see the deferred-flip logic in `docs-deploy.yml`. To force the flip after the fact, re-run the master `docs-deploy` (`workflow_dispatch` with tag `master`) once `/vX.Y.Z/` is live.
