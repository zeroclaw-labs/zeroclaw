...
every app in the workspace is trusted. A `bot_message` that also carries a
`user` field is still attributed to the posting app's bot ID: the `user`
field never authorizes an app post. Admitted app posts also become
