...
| `permissions` | no | Host services the code may reach: `http_client`, `socket_client`, `config_read`, `state_read`, `state_write`, `file_read`, `file_write`, `memory_read`, `memory_write` (`PluginPermission`). HTTP, TCP/TLS/STARTTLS, config, and exact-instance state permissions are enforced for tool/channel adapters; filesystem and memory permissions remain inert. Declaring `config_read` requires `config_schema`. |
