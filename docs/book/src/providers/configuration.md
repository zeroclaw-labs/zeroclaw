...
- **Anthropic / Claude**: Console API keys go in `api_key` on `[providers.models.anthropic.<alias>]`. Quickstart's legacy `setup_token` choice writes that token to `api_key` and does not set persisted `auth_mode`. For a stored-profile OAuth alias, store a token with `zeroclaw auth setup-token --model-provider anthropic --profile <alias>` and set `auth_mode = "oauth"`; the runtime reads only the same-named stored Anthropic setup-token profile. It fails if that profile is missing and never falls back to an active or different profile. The setup command may update Anthropic's global active-profile selector; this does not affect an OAuth alias, which still selects its same-named profile. OAuth aliases use only `https://api.anthropic.com`; use an API-key alias for a custom Anthropic-compatible `uri`. Existing aliases whose setup token is in `api_key` retain their legacy behavior. Do not set both `auth_mode = "oauth"` and `api_key`. Anthropic's provider-local value is spelled `oauth`; this is distinct from the established `o_auth` spelling used by other provider families below.
