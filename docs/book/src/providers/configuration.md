...
- **Grok Build CLI**: `[providers.models.grok_cli.<alias>]` shells out to `grok --prompt-file` headless; use the CLI's own auth flow (`grok login` / `XAI_API_KEY`). Set `working_directory` to the agent workspace so project `.grok/config.toml` is loaded. ZeroClaw does not inject permission flags — see [Catalog → Grok Build CLI](./catalog.md#grok-build-cli-slot-grok_cli) for the recommended narrow Bash denies that block `zeroclaw channel send` dual delivery.
