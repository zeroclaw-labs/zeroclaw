...
- `tool_result_image_policy`: handling for image markers in native `role = "tool"` results sent to compatible chat-completions providers. Defaults to `"image_url"`; set to `"omit"` to remove image URI/base64 payloads and append a fixed notice. This does not change direct user images or OpenAI Responses providers. Independent of this policy, a data-URI marker is only lifted into a provider image when its base64 payload decodes as an image of the declared type; a truncated or mismatched payload stays as text.
