...
- **Grok Build CLI**: `[providers.models.grok_cli.<alias>]` shells out to `grok --prompt-file` headless; use the CLI's own auth flow (`grok login` / `XAI_API_KEY`). Set `working_directory` to the agent workspace so project `.grok/` is loaded. Optional `extra_args` appends CLI flags (for example `["--sandbox", "read-only"]` or `["--max-turns", "20"]`). Defaults inject no permission or sandbox flags — see [Catalog → Grok Build CLI](./catalog.md#grok-build-cli-slot-grok_cli) for the default-chatbot pattern, OS sandbox profiles, and narrow `.grok` denies that block `zeroclaw channel send` dual delivery.
