...
- **Grok Build CLI**: `[providers.models.grok_cli.<alias>]` shells out to documented headless mode (`--single` for short prompts; large prompts via `--prompt-file /dev/stdin` so the body never hits `ARG_MAX` or a shared temp file; `--output-format plain`). Auth is the CLI's own flow (`grok login` / `XAI_API_KEY`). Set `working_directory` to the agent workspace so project `.grok/` is loaded. Default argv injects `--sandbox strict` (override with `extra_args = ["--sandbox", "off"]`). Optional `extra_args` may add non-reserved flags (for example `["--max-turns", "20"]`); transport/model/session flags are reserved. See [Catalog → Grok Build CLI](./catalog.md#grok-build-cli-slot-grok_cli) for the default-chatbot pattern (**API `classifier_provider` for REPLY/NO_REPLY precheck**, OS sandbox profiles, and narrow `.grok` denies that block `zeroclaw channel send` dual delivery).
