...
Supports Console API keys and tokens generated by `claude setup-token` for Claude Max. API-key aliases use `api_key`, including existing setup tokens stored inline. Quickstart's `setup_token` choice retains that legacy behavior: it writes the token to the alias's `api_key` field and does not set persisted `auth_mode`. To opt into stored-profile OAuth, create an Anthropic setup-token profile with the same alias using `zeroclaw auth setup-token --model-provider anthropic --profile <alias>` and set `auth_mode = "oauth"`. A missing same-named profile fails instead of falling back to an active or different profile. OAuth setup-token aliases use only the official `https://api.anthropic.com` endpoint and accept only stored setup-token profiles, not generic refreshable OAuth token sets. Streaming, tool calls, vision, and reasoning all supported. API-key aliases may use custom Anthropic-compatible endpoints (for example, Z.AI's Anthropic API) by setting `uri`.
