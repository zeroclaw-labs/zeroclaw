...
Claude models on Amazon Bedrock. Model IDs carry the `anthropic.` prefix, for example `anthropic.claude-fable-5-1`. The same generation rules as the `anthropic` slot apply to the reasoning request shape; `thinking_display` is not available here.
