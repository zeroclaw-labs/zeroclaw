...
| [ADR-014](./ADR-014-execution-tree-iteration-budget-ownership.md) | proposed | The root runtime profile owns one aggregate iteration budget shared by joined agent loops. |
...
ADR-014 remains proposed until production roots own one configured execution-tree budget and participating child paths share it with tested exhaustion, cancellation, and finalization behavior.
