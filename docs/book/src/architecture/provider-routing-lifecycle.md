...
Rotation is factory-gated per selected credential. It applies only when the configured `api_key` is the credential the provider actually sends. OAuth refresh-token modes, Anthropic setup tokens, ambient credentials, CLI-backed authentication, `requires_openai_auth`, and provider families that ignore the key are excluded rather than being offered as ineffective alternates. Mixed pools retain only eligible outbound API keys.
