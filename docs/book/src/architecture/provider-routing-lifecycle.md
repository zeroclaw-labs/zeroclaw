...
| Retry, credential rotation, and failover | `ReliableModelProvider` plus the provider factory | The factory decides whether an `api_key` is an outbound credential and rebuilds an eligible provider for each selected key. Reliable classifies failures, owns bounded retry and cooldown state, and advances through materialized entries. |
...
2. When `model_routes` are configured, the factory builds a separate reliable provider for the primary route and for every route entry, then wraps them in `RouterModelProvider`. Each route entry retains its own route identity and optional route-level credential, even when several hints name the same provider profile.
...
- Route entries are not deduplicated by `model_provider`. A route-level `api_key` belongs only to that exact route. When a route omits `api_key`, provider construction falls back to the credential on the target provider profile. Several hints can therefore share one target profile without sharing a sibling route's credential.
...
`reliability.api_keys` supplies additional credentials for the primary provider profile. On a retryable rate limit, Reliable cools the selected credential and asks the provider factory to rebuild the same configured candidate with the next healthy key. The pool is resolved from the canonical live config before selection, so replacing or removing a profile key takes effect without reconstructing the long-lived agent provider. A route-level `model_routes[].api_key` remains scoped to that exact route, and fallback profiles resolve their own profile credentials; neither inherits the primary profile's additional-key pool.

Rotation is factory-gated per selected credential. It applies only when the configured `api_key` is the credential the provider actually sends. OAuth refresh-token modes, Anthropic setup tokens, ambient credentials, CLI-backed authentication, default `requires_openai_auth` subscription OAuth (custom gateways with outbound API keys remain eligible), and provider families that ignore the key are excluded rather than being offered as ineffective alternates. Mixed pools retain only eligible outbound API keys.

Credential cooldowns are keyed by a digest of the failed credential and owned per configured provider alias. Routes or fallback entries that resolve the same alias share that cooldown state. Removing a failed key does not erase its cooldown; if the same key is reintroduced before expiry, selection still skips it. This credential state is separate from Reliable's provider-profile cooldown, which can skip all entries sharing a profile after a rate limit when failover is available.
...
2. It opens that stream once. For an API-key-rotation-eligible entry, a retryable rate limit may select and bind another healthy key only before the stream emits any event. Once any event has been emitted, the request is not replayed with another credential.
