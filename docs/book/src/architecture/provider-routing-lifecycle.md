...
3. The concrete provider parser translates its protocol's completion semantics into `Final` or an error. Most guarded SSE parsers require their configured completion signal. Anthropic requires `message_stop` and one consistent recognized terminal reason; EOF or conflicting `message_delta.stop_reason` values return an error.
4. The runtime consumes and sanitizes stream events. If the stream fails before immutable event output is visible *and before provider-executed work*, the runtime retries the whole call through the non-streaming path, which re-enters the full reliability walk. A provider-side tool event is already an execution boundary and therefore forbids replay even when no user-visible text was produced.
