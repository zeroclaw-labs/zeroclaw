...
| Installation paths | Typed route contracts and install plan in `xtask/src/generate/spec.rs` | `cargo generate installers`, through `xtask/src/generate/docs.rs` | `docs/book/src/_snippets/install.md` | Tracked generated snippet | README-linked first-time setup, Quickstart, and platform setup pages |
...
Ignored files are reproducible materializations: CLI and config references, most generated snippets, rustdoc, rendered HTML, locale-switcher JavaScript, generated theme CSS, and the dashboard TypeScript API client. They may exist in a working tree after a docs build without belonging in a commit. The tracked `docs/book/src/_snippets/install.md` installation snippet is an explicit exception.
