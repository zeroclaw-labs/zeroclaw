...
The mid-stream protocol guard delivers protocol the model is quoting: shapes inside an inline code span that closes, a json fence with a real closing fence line and text after it, and tool-result shapes after a prose preamble. A tool-call envelope that follows a preamble is still withheld as a leak when the guard holds tool specs naming the tool, with the preamble itself delivered; tagged tool-call markup is withheld wherever it appears, except in the parser's explicit example context. The streaming and non-streaming paths deliberately differ here: the guard withholds a preamble-plus-call envelope that the complete-response detector does not flag.
