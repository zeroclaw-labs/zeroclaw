...
The mid-stream protocol guard withholds from the suppressed candidate onward and delivers the prose buffered ahead of it, recording the detector and the candidate's byte offset in a WARN trace event that never carries the withheld text. A tool-result shape after a prose preamble is delivered, bare or inside a json-labelled fence, once the object completes, or at the end of the stream when the quoted shape is not valid JSON but carries no call-shaped key, because a model cannot legitimately emit a tool result, while a result shape that leads the message is withheld, as is a quoted shape that carries a call-shaped key. A tool-call envelope after a preamble is withheld as before, including one quoted in a code span or a fence, so a reply that quotes call syntax in prose is delivered up to the quote and the turn ends with the withheld notice once a retry produces the same text.
