...
The scheduler polls for due, enabled, unclaimed rows. Claiming a row prevents duplicate selection while it is in flight. Completion records bounded output, then reschedules a recurring job, deletes a successful auto-delete one-shot, or disables another one-shot. Claims do not expire or get reclaimed while their process-local owner is active: only the current claim token can persist and release its row. A full process restart has no surviving worker and clears locks left by the dead process. An in-process daemon reload is different: dropping a supervisor requests worker cancellation, and the replacement scheduler waits for every cron-owned worker associated with the same canonical jobs database to exit before it performs startup stale-lock recovery. This keeps a non-yielding old-generation worker from overlapping a replacement run after the daemon's bounded shutdown grace expires. Agent job runs also request cancellation without a restart: each run is supervised against the owning agent's `agentic_timeout_secs` (or the scheduler's agent-job default when unset), and a yielding run that exceeds it is stopped and recorded as a non-retryable timeout. The supervisor observes the deadline on an independent runtime, but it releases the durable claim only after that runtime has dropped the run; a non-yielding in-process operation can delay release because Rust cannot safely preempt it, and retaining the claim prevents replacement work from overlapping it. Supervised cron runs reject background delegation (`background = true`) before spawn because background tasks would outlive the scheduler's private runtime and escape the durable claim boundary; cron jobs must use synchronous or parallel delegation instead. In-process delivery to a peer agent is the other detached route out of a run: it normally hands the recipient's turn to a spawned task and reports acceptance immediately, so under a supervised run the recipient is awaited inline instead and the reported outcome is the delivery that actually finished. The recipient's turn is built under the same run owner, so the tools it rebuilds inherit the run's cancellation and cannot spawn a child that outlives the claim. The supervised run token roots parallel child tokens, so timeout cancellation crosses those spawned paths before the scheduler releases the claim. Scheduled announcement delivery follows the same ownership rule, while manual delivery keeps its historical caller-owned behavior. Synchronous persistence runs on a bounded-await worker drawn from a bounded pool; a finished result waits for a worker slot under the same deadline rather than being dropped while the pool is busy, and if that deadline expires the public result is reported as persistence-pending. A completed result is never discarded: when admission times out the result is handed to a bounded retained owner that waits for capacity on its own task and then performs the same durable write, which releases the claim without a restart. If the write itself exceeds the deadline the claim stays owned until that worker commits or startup recovery runs. Best-effort isolated-session cleanup uses a separate short abandonment deadline and a run-unique session key. At most one detached cleanup worker is admitted process-wide; if it remains blocked, later cleanup attempts are skipped rather than accumulating threads.
