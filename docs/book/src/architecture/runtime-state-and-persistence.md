...
| Chat and channel sessions | `[channels].session_backend` plus `SessionBackend` | Default `data/sessions/sessions.db`; legacy/explicit JSONL uses `data/sessions/*.jsonl` | `zeroclaw-infra` backend handles are currently constructed independently by channels, gateway, RPC, and session tools | SQLite backend uses WAL and stores session prompt attachments with the owning session; `SessionActorQueue` serializes active turns and fences deleted session incarnations. A gateway WebSocket retains a lifecycle lease but remains a viewer, not the turn owner: disconnect detaches without cancelling the turn, while `POST /api/sessions/{id}/abort` cancels it. After deletion, a stale socket cannot join a newly created session with the same ID and receives `SESSION_NOT_FOUND`; an idle-queue reaper reclaims unused lifecycle state. JSONL mutations share a process-local sessions-directory lock. | Chat/Code sessions use the unified backend contract, but prompt attachments require SQLite. ACP protocol sessions use a separate store. Process-level backend ownership is not yet single-sourced. |
