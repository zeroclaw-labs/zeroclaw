...
| Conversation memory | `zeroclaw-memory` backend selected per agent | SQLite/Postgres/Qdrant/Markdown backend locations; SQLite shared store lives under `data/memory/` | `Arc<dyn Memory>` wrapped in agent-scoping adapters and, optionally, a Lucid enrichment connector | Authoritative backend choice is locked once an agent has written data; same-backend cross-agent recall is opt-in; enrichers may change independently | Memory rows are agent-scoped. Lucid is a best-effort connector over authoritative SQLite, not a durable backend. Do not replace memory ownership with copied prompt/session caches. |
...
External enrichment state is outside the install-root backup. The
`data/memory/` directory remains the authoritative ZeroClaw copy.
