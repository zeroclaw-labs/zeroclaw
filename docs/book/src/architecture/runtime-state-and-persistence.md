...
| Conversation memory | `zeroclaw-memory` backend selected per agent | SQLite/Postgres/Qdrant/Markdown backend locations; SQLite shared store lives under `data/memory/` | `Arc<dyn Memory>` wrapped in agent-scoping adapters and, optionally, one Lucid/Shodh enrichment connector | Authoritative backend choice is locked once an agent has written data; same-backend cross-agent recall is opt-in; enrichers may change independently | Memory rows are agent-scoped. Lucid and Shodh are best-effort connectors over authoritative SQLite, not durable backends. Do not replace memory ownership with copied prompt/session caches. |
...
External memory services are outside the install-root backup. With the Shodh
connector, `data/memory/` remains the authoritative ZeroClaw copy; back up
Shodh separately only when its derived semantic index must also be preserved.
