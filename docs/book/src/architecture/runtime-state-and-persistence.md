...
| Conversation memory | `zeroclaw-memory` backend selected per agent | SQLite/Postgres/Lucid/Qdrant/Markdown backend locations; SQLite shared store lives under `data/memory/`. The `hindsight` backend has no local store: persistence and vectorization are server-side, keyed by a per-agent bank (`[memory.hindsight]`). Hindsight also supports shared/family and system banks (`shared_bank` / `system_bank`), readable by every agent and writable only via the explicit `shared_memory_store` / `system_memory_store` tools | `Arc<dyn Memory>` wrapped in agent-scoping adapters (hindsight isolates per-agent by bank, so it needs no scoping wrapper) | Backend choice is locked once an agent has written data; same-backend cross-agent recall is opt-in. Shared/system WRITES are gated per agent by risk-profile `allowed_tools`/`excluded_tools` (the tiers are distinct tool names); automatic per-turn consolidation only ever writes the private bank | Memory rows are agent-scoped. Do not replace memory ownership with copied prompt/session caches. |
