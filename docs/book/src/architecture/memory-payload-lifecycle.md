...
| --- | --- | --- | --- |
| Long-term memory | `zeroclaw-memory` behind `Arc<dyn Memory>` | Backend-specific: shared SQLite/Postgres/Qdrant stores or per-agent Markdown files; SQLite may use optional Lucid/Shodh enrichment | Stores and recalls must stay agent-scoped. A tool result, log line, or session row is not long-term memory unless a memory write happened. |
...
and failures enter a short cooldown. Canonical-row references from Shodh must
map a remote candidate back to its SQLite key and agent UUID before returning
it, so stale or deleted remote data cannot become memory again. Derived-context
enrichers such as Lucid are not canonicalized as exact rows.
...
- Local-first external seam and connectors:
  ...
  `crates/zeroclaw-memory/src/lucid.rs`, and
