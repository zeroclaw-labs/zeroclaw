...
The same no-operator boundary applies to tool calls in a bounded agentic child. The child loop creates a fresh non-interactive approval manager from the target's risk profile at the dispatch seam. A prompt-required call is denied before the tool implementation runs, while a call explicitly listed in `auto_approve` (or allowed by full autonomy) may proceed. For inherited caller-owned command tools (`shell`, `schedule`, `cron_add`, `cron_update`, and `cron_run`), that target-profile decision approves only tool invocation: it does not supply the command-specific approval consumed by the caller's command policy. Those tools therefore continue to enforce the caller's approval requirements. The inherited caller-capped registry is unchanged: approval evaluates calls that survived capability filtering; it does not grant additional tools or forward the caller's approval session. This behavior follows the non-interactive manager pattern from merged #10188 and preserves the nested-delegate boundary from merged #10601. Open #10610 covers a separate shell-approval surface and is not a prerequisite for this bounded rule.
