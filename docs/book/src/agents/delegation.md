...
2. **Reachability**: the target agent must be in the caller's reachable set, resolved by `Config::reachable_delegate_target_configs`. The reachable set is the union of two per-agent sources on `[agents.<caller>]`, minus the caller itself:
   ...
   Cost tracking is enforced inside every delegated sub-loop, bounded and independent alike: the delegated sub-loop runs under a cost-tracking scope attributed to the target agent alias, so its token usage lands in the shared daily cost ledger under that alias. The target's effective per-hop ceiling from `[runtime_profiles.<target>].max_cost_per_day_cents` (a value of `0` inherits the global limit) is checked against the target agent's own daily spend on that ledger, while the process-global `[cost] daily_limit_usd` and monthly limits continue to apply to the shared total; the shared limit wins whenever it is the lower of the two. The scope kind (per-agent or shared-capped, from `[cost] track_per_agent`), whether a delegated run is scoped at all (`[cost] enabled`), and the per-agent attribution of its ledger rows are all decided once at the root delegation of a tree and inherited by every nested hop: a nested delegation derives its tracker from the delegating parent's, so the mode the tree started under binds the whole tree, and a mid-run reload of those flags can neither untrack a scoped run nor start reattributing an unattributed one halfway through. The limits and the pricing map stay live at every hop: the derived tracker shares the base's config handle and the pricing map is rebuilt from the resolved config, so an operator reloading the global `[cost]` limits binds running delegates at their next budget check. When the target's own spend already exceeds its ceiling, the delegation fails before the target's provider is called, and the refusal surfaces to the caller as the delegate tool's error naming the agent. An ancestor's ceiling also covers everything it delegated to: every bounded hop's budget scope carries the ancestor chain, so a bounded descendant's spend counts against each ancestor's per-hop ceiling even though the descendant's ledger rows stay attributed to the descendant's own alias. An independent target runs under its own per-hop ceiling and the shared global limits instead: it inherits the tree's enforcement mode but not the caller's ceiling or chain, so neither the caller's exhausted ceiling nor its descendant accumulator can gate the independent hop. Descendant accounting is per process lifetime and per UTC day: it rolls over with the ledger's daily totals at UTC midnight, and a daemon restart forgets descendant spend while every agent's own ledger totals persist. With `[cost] track_per_agent = false` no per-agent daily totals exist, so the per-hop ceiling degrades to a check against the shared daily total. Non-agentic delegate targets (single model call, no tool loop) run under the same scope, but their dispatch path reports no token usage, so those single calls are neither priced nor budget-checked today.
