...
Two approval and scoping rules apply to the granted instance. First, it is **delegate-only**: the task-management actions (`check_result`, `list_results`, `cancel_task`, `await_sessions`) are refused, because the child instance's identity is transient. The task row records the delegation chain of caller aliases, so every ancestor of the child is admitted by the owner check; only an ancestor whose own delegate tool carries task management (normally the root delegating agent, since bounded intermediate children are delegate-only too) can act on that admission, so a child passes the task id upward until it reaches one. Note the boundary precisely: this scopes the delegate API surface only. The underlying `delegate_results/*.json` artifact remains an ordinary workspace file holding task id and output only, and anything the owner check gates lives in the task row. Second, sub-agent loops have no operator approval route, so the tool fails closed on the target's own approval policy: if the target's risk profile would prompt for `delegate` (the supervised default, or the tool listed in `always_ask`), the nested call is refused with an error naming the profile. Nested delegation therefore requires the target profile to explicitly auto-approve `delegate` or run at full autonomy; a caller profile that would prompt keeps prompting in its own loop, unchanged.
