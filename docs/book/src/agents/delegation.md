...
Set `max_execution_tree_iterations` on the target's `[runtime_profiles.<alias>]` entry to cap aggregate tool-loop iterations across one foreground execution tree:

```toml
[runtime_profiles.default]
max_execution_tree_iterations = 12
```
...
Two approval and scoping rules apply to the granted instance. First, it is **delegate-only**: the task-management actions (`check_result`, `list_results`, `cancel_task`, `await_sessions`) are refused, because background task records live in a workspace-wide namespace without owner identity and a bounded child shares the delegating parent's workspace. Note the boundary precisely: this scopes the delegate API surface only. The underlying `delegate_results/*.json` records remain ordinary workspace files that any identity with workspace read access can read, and task records carry no owner binding; isolating them is a separate concern from the tool grant. Second, sub-agent loops have no operator approval route. If the target's risk profile would prompt for `delegate` (the supervised default, or the tool listed in `always_ask`), the child approval gate refuses the call before the delegate implementation runs, reporting that no operator decision was available. The delegate implementation retains its own target-policy check for calls that reach it. Nested delegation therefore requires the target profile to explicitly auto-approve `delegate` or run at full autonomy; a caller profile that would prompt keeps prompting in its own loop, unchanged.

The same no-operator boundary applies to tool calls in a bounded agentic child. The child loop creates a fresh non-interactive approval manager from the target's risk profile at the dispatch seam. A prompt-required call is denied before the tool implementation runs, while a call explicitly listed in `auto_approve` (or allowed by full autonomy) may proceed. For inherited caller-owned command tools (`shell`, `schedule`, `cron_add`, `cron_update`, and `cron_run`), that target-profile decision approves only tool invocation: it does not supply or override the command-specific approval consumed by the caller's command policy. The caller's command policy remains authoritative, so those tools continue to enforce its approval requirements. The inherited caller-capped registry is unchanged: approval evaluates calls that survived capability filtering; it does not grant additional tools or forward the caller's approval session.
