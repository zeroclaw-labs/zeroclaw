...
2. **Reachability**: the target agent must be in the caller's reachable set, resolved by `Config::reachable_delegate_target_configs`. The reachable set is the union of two per-agent sources on `[agents.<caller>]`, minus the caller itself:
   ...
   Cost tracking is enforced inside every delegated sub-loop, bounded and independent alike: the delegated sub-loop runs under a cost-tracking scope attributed to the target agent alias, so its token usage lands in the shared daily cost ledger under that alias. The target's effective per-hop ceiling from `[runtime_profiles.<target>].max_cost_per_day_cents` (a value of `0` inherits the global limit) is checked against the target agent's own daily spend on that ledger, while the process-global `[cost] daily_limit_usd` and monthly limits continue to apply to the shared total; the shared limit wins whenever it is the lower of the two. When the target's own spend already exceeds its ceiling, the delegation fails before the target's provider is called, and the refusal surfaces to the caller as the delegate tool's error naming the agent. With `[cost] track_per_agent = false` no per-agent daily totals exist, so the per-hop ceiling degrades to a check against the shared daily total. Non-agentic delegate targets (single model call, no tool loop) run under the same scope, but their dispatch path reports no token usage, so those single calls are neither priced nor budget-checked today.
