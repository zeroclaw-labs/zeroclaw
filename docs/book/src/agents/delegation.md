...
5. Background spawn success: output is the three-line literal
   ...
   ```
   Lifecycle state is stored in `data/control_plane.db`. The workspace artifact at `<workspace>/delegate_results/<uuid>.json` contains task identity and output only; it is not a second status record. A completed task that recovered through a configured provider fallback stores the same requested-versus-served attribution and generic recovery warning in its `output`; retrieve it with `check_result` or `await_sessions`. A failed task stores the same safe terminal summary as synchronous delegation, not provider response details. Supervised cron runs reject background delegation (`background=true`) before spawn with the localized error `delegate-background-cron-owned-rejected`, because background tasks would escape the supervisor's private runtime and durable claim boundary; cron jobs must use synchronous delegation or parallel fan-out instead. When a supervised cron run constructs the tool, its run token becomes the root for parallel child tokens, so cron timeout cancellation propagates across those concurrent paths before the scheduler releases the claim.
