...
1. **`SecurityPolicy`**: inherited by `Arc<SecurityPolicy>` cloning. Override path (`SubAgentOverrides::policy = Some(policy)`) runs `SecurityPolicy::ensure_no_escalation_beyond` (`crates/zeroclaw-config/src/policy.rs`) and rejects any field that adds privilege the parent doesn't have. Validated axes include autonomy level, allowed_roots (rw + ro + write-only), allowed_commands, workspace_only, forbidden_paths in the parent ⊆ child direction, shell_env_passthrough, `max_actions_per_hour`, `max_cost_per_day_cents`, `shell_timeout_secs`, `block_high_risk_commands`, `require_approval_for_medium_risk`, and the tool-name dimension, compared as effective access rather than raw field containment (via `SecurityPolicy::is_tool_allowed`): the child's effective tool set must sit within the parent's. A child `allowed_tools` entry the parent's effective policy denies is a violation (an unrestricted child under a restricted parent is itself a violation). The child may drop a parent `excluded_tools` entry only when its own allowlist already makes that tool unreachable; because a nonempty allowlist still auto-admits namespaced MCP `<server>__<tool>` names (see `ToolAccessPolicy::is_tool_allowed`), parent exclusions of those namespaced tools must always be retained. Rejections chain a precise `EscalationViolation` so diagnostics name the offending field.
