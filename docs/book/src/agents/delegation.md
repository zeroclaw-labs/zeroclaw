...
1. **`SecurityPolicy`**: inherited by `Arc<SecurityPolicy>` cloning. Override path (`SubAgentOverrides::policy = Some(policy)`) runs `SecurityPolicy::ensure_no_escalation_beyond` (`crates/zeroclaw-config/src/policy.rs`) and rejects any field that adds privilege the parent doesn't have. Validated axes include autonomy level, allowed_roots (rw + ro + write-only), allowed_commands, workspace_only, forbidden_paths in the parent ⊆ child direction, shell_env_passthrough, `max_actions_per_hour`, `max_cost_per_day_cents`, `shell_timeout_secs`, `block_high_risk_commands`, `require_approval_for_medium_risk`, and the tool-name dimension, compared as effective access rather than raw field containment: the child's effective tool set must sit within the parent's. A child `allowed_tools` entry the parent's effective policy denies is a violation (an unrestricted child under a restricted parent is itself a violation). The child may drop a parent `excluded_tools` entry only when its own allowlist already makes that tool unreachable. Effective access mirrors the runtime split between tool families: ordinary tools (built-ins, HTTP skills, WASM plugins) are exact-matched by `SecurityPolicy::is_tool_allowed`, while MCP tools carry an implicit grant, since a nonempty allowlist auto-admits `<server>__<tool>` names (see `ToolAccessPolicy::is_tool_allowed`). A tool counts as MCP only when its `<server>` prefix is one of the agent's configured MCP servers, which `ensure_no_escalation_beyond` receives so a same-shaped skill or plugin name (such as `weather_skill__get_weather`) is not mistaken for an auto-admitted MCP tool. Parent exclusions of genuine MCP tools must always be retained. Rejections chain a precise `EscalationViolation` so diagnostics name the offending field.
