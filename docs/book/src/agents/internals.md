...
- **SQLite with memory enrichment**: the same shared SQLite store remains canonical. One shared enrichment wrapper owns local-first fallback, scoping, merging, cooldown, and cleanup dispatch. Lucid and Shodh contain only protocol translation and capability declarations. Shodh reaches its independently supervised server through authenticated local IPC and uses each stable UUID as an isolated `user_id`; Lucid cannot express scoped external recall and therefore stays local-only when an agent allowlist is required.
...
5. Lucid protocol extensions for cross-agent scoping.
