...
- **Disabled by default.** The plugin system does not load anything unless
  ...
  no plugin code.
- **Deny by default.** A plugin reaches a host capability (HTTP, WebSocket, or
  TCP/TLS/STARTTLS egress, config, durable state, memory) only by declaring the
  matching permission in its manifest. An undeclared capability is unreachable,
  not merely unused.
...
- **Permissions** are what host services the plugin's code may reach at runtime:
  HTTP, WebSocket, or TCP/TLS/STARTTLS egress, configuration, durable state,
  memory. A permission the manifest does not declare is a host function the
  plugin cannot reach.
...
connection budget are enforced by the shared egress service. `socket_client`
uses that same policy for typed TCP, direct TLS, and STARTTLS resources. This
