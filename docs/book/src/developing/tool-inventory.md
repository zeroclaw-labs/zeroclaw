...
| `sessions_current`, `sessions_list`, `sessions_history`, `sessions_send`, `session_prompt_list`, `session_prompt_set`, `session_prompt_delete` | Session visibility, persistent prompt management, and message sending must share the daemon/gateway session backend and agent ownership boundaries. |
