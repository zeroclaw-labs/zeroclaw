...
| Android-native tools (`android_screenshot`, `android_ui_read`, `android_action`, `android_launch`, `android_device`) | Config-gated and compiled only for Android production targets; the APK and Rust client communicate over an app-private Unix-domain socket. | Keep first-party while device control stays coupled to approval policy and autonomy gating. `android_action` injects real input, so its approval behavior must remain visible to the runtime rather than delegated to a plugin. |
