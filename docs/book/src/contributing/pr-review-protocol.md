...
- A presentation-sensitive visual change includes actual-interface evidence on an identifiable revision and privacy-safe screenshots at representative terminal or viewport dimensions. A message-only change in a rendered interface still needs actual-interface evidence on an identifiable revision, but dimensions are not required when no presentation-sensitive property is claimed or at risk. A noninteractive, unstyled, deterministic plain-text CLI, stdout, stderr, or log change may instead use exact output from the supported interface when it makes no presentation-sensitive claim and no reviewer has identified a concrete presentation concern. The evidence records relevant output-shaping context such as locale and terminal width. Browser, TUI, prompt, and styled-output changes remain on the screenshot path. String assertions, component-only snapshots, helper-level renderer tests, or a statement that no interface smoke was performed do not satisfy the applicable requirement. Interaction and transition claims also include the action and observed result.
...
| The PR's central intended result is a message-only rendered-interface change, but actual-interface screenshot or action-and-result evidence is missing | `--request-changes` |
| A non-central message-only rendered-interface change lacks actual-interface screenshot or action-and-result evidence | `--comment` and withhold approval until the evidence is supplied |
...
| A reviewer identifies a concrete presentation concern in an otherwise qualifying plain-text change | Treat it as a visual presentation change and use the central or non-central screenshot-evidence row above |
