...
pane is the preferred path for changing settings, the Code pane drives agents,
and the connection works the same whether the daemon is local or on a remote
host. A legacy Chat pane can be enabled for memory/MCP chat workflows.
