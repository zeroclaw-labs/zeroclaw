...
SSE error events carry an explicit event name and vary the `type` field:

| Scenario | SSE `event` | `type` |
|----------|------------|--------|
| Cancellation / server error | `error` | `internal_error` |
...
| `X-RateLimit-Remaining` | Requests still allowed in the current sliding window (real count, decreases as you spend the window) | On success & 429 |
| `X-RateLimit-Reset` | Unix timestamp when the window frees a slot (derived from the oldest in-window request) | On success & 429 |
| `Retry-After` | Seconds until the window frees a slot on 429 (real value, not a fixed window) | Only on 429 responses |

\* Not present on 413 Payload Too Large responses. The 413 is generated by body-size middleware before the handler has access to client headers or a session context. The error body still follows the standard ChatError schema.
...
| **408** | `timeout` | Request timeout | Request exceeded the configured timeout (default 600s) |
...
**Important notes**:
```
📌 x-session-key is returned on all handler-processed responses (including errors). 413 responses from the body-size guard are generated before the handler sees the request and cannot include session headers.
...
| **408** | `timeout` | Request timeout | The request took longer than the configured timeout. Reduce message complexity or increase the timeout in `[gateway] long_running_request_timeout_secs`. |
...
| **413** | `payload_too_large` | Request body too large | Keep the request body under 64 KB. Large message histories should use `x-session-key` for multi-turn context instead. This response cannot include `X-Request-ID` or `x-session-key` headers (see Response Headers footnote). |
...
| `X-RateLimit-Remaining` | Requests still allowed in the current sliding window (real count) | On success & 429 |
| `X-RateLimit-Reset` | Unix timestamp when the window frees a slot (from the oldest in-window request) | On success & 429 |
| `Retry-After` | Seconds until the window frees a slot on 429 (real value) | Only on 429 |

\* Not present on 413 Payload Too Large responses. The 413 is generated by body-size middleware before the handler has access to client headers or a session context.
...
Exceeding the limit returns 429 with a `Retry-After` header indicating the real wait time from the sliding-window limiter (not a fixed window).
...
(the `SessionBackend` trait interface). If you implement a custom
...
Sessions created before cross-agent session isolation was introduced may
have message history but no recorded owner. The handler rejects these
sessions with HTTP 400 to prevent cross-agent context contamination.

Use `zeroclaw migrate session-ownership` to backfill ownership metadata:

```bash
# List all unowned non-empty sessions
zeroclaw migrate session-ownership --list

# Claim a specific session
zeroclaw migrate session-ownership --claim gw_<key> --agent-alias <alias> --yes

# Interactive mode; prompts for the agent alias and bulk-claims all
zeroclaw migrate session-ownership
```

This command works with both the JSONL backend (writes `.meta.json`
sidecar) and the SQLite backend (updates `session_metadata`).
For custom backends, see the `SessionBackend` trait migration guidance.

The command validates both the target agent alias and session existence
before any write; providing an unknown alias or nonexistent session key
bails with a clear error and no side effects.
