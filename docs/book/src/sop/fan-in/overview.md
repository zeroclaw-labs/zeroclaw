...
| Cron trigger never fires | daemon or gateway not running, `sops_dir` unset, or `maintenance_interval_secs = 0` | Run the daemon or gateway with `sop.sops_dir` set and `sop.maintenance_interval_secs` non-zero (default `60`) |
