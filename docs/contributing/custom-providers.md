...
QuantClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export QUANTCLAW_API_KEY="your-api-key"
quantclaw agent
```

## llama.cpp Server (Recommended Local Setup)

QuantClaw includes a first-class local provider for `llama-server`:
...
```bash
quantclaw models refresh --provider llamacpp
quantclaw agent -m "hello"
```

You do not need to export `QUANTCLAW_API_KEY=dummy` for this flow.

## SGLang Server

QuantClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
quantclaw models refresh --provider sglang
quantclaw agent -m "hello"
```
...
QuantClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
quantclaw models refresh --provider vllm
quantclaw agent -m "hello"
```
