...
DaemonClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export DAEMONCLAW_API_KEY="your-api-key"
daemonclaw agent
```

## llama.cpp Server (Recommended Local Setup)

DaemonClaw includes a first-class local provider for `llama-server`:
...
```bash
daemonclaw models refresh --provider llamacpp
daemonclaw agent -m "hello"
```

You do not need to export `DAEMONCLAW_API_KEY=dummy` for this flow.

## SGLang Server

DaemonClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
daemonclaw models refresh --provider sglang
daemonclaw agent -m "hello"
```
...
DaemonClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
daemonclaw models refresh --provider vllm
daemonclaw agent -m "hello"
```
...
# Single message test
daemonclaw agent -m "test message"
```
