...
```bash
# Add a board (updates ~/.quantclaw/config.toml)
quantclaw peripheral add nucleo-f401re /dev/ttyACM0
quantclaw peripheral add arduino-uno /dev/cu.usbmodem12345
quantclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
2. **Add to config** — `quantclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `quantclaw peripheral list` | List configured boards |
| `quantclaw peripheral add <board> <path>` | Add board (writes config) |
| `quantclaw peripheral flash` | Flash Arduino firmware |
| `quantclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `quantclaw hardware discover` | List USB devices |
| `quantclaw hardware info` | Chip info via probe-rs |
