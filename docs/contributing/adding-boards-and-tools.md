# Adding Boards and Tools — DaemonClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to DaemonClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.daemonclaw/config.toml)
daemonclaw peripheral add nucleo-f401re /dev/ttyACM0
daemonclaw peripheral add arduino-uno /dev/cu.usbmodem12345
daemonclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, DaemonClaw can index PDF files:
...
2. **Add to config** — `daemonclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `daemonclaw peripheral list` | List configured boards |
| `daemonclaw peripheral add <board> <path>` | Add board (writes config) |
| `daemonclaw peripheral flash` | Flash Arduino firmware |
| `daemonclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `daemonclaw hardware discover` | List USB devices |
| `daemonclaw hardware info` | Chip info via probe-rs |
