//! Performance benchmarks for DaemonClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/DeliveryBoyTech/daemonclaw/issues/618 (item 7)

use criterion::{Criterion, criterion_group, criterion_main};
use std::hint::black_box;
use std::sync::{Arc, Mutex};

use daemonclaw::agent::agent::Agent;
use daemonclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use daemonclaw::config::MemoryConfig;
use daemonclaw::memory;
use daemonclaw::memory::{Memory, MemoryCategory};
use daemonclaw::observability::{NoopObserver, Observer};
use daemonclaw::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
...
<tool_call>
{"name": "search", "arguments": {"query": "daemonclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "daemonclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about daemonclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("daemonclaw agent"), 10, None, None, None)
                    .await
                    .unwrap()
            })
        });
