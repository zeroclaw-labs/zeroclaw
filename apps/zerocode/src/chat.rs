...
impl Chat {
    ...
    async fn confirm_model_picker_selection(rpc: &Arc<RpcClient>, state: &mut ChatState) {
        ...
        let request = match &state.model_picker {
            ModelPickerOverlay::Model(p) => p
                .selected()
                .map(|model| SessionOverride::Model(model.to_string())),
    ...
    /// Apply one session override through `session/configure`, reporting the
    /// outcome on the info bar. On a model_provider switch the daemon rebuilds
    /// the provider box live. A failure quotes the daemon's own message, so a
    /// rejected thinking value shows the accepted list it was checked against.
...
mod tests {
    ...
    async fn transcript_failures_retain_resumes_and_promote_next_healthy_background() {
        ...
        for (session_id, transcript_ok) in [
            ...
            let request = next_rpc_request(&mut rx, "attaching reads thinking options").await;
    ...
    async fn prompt_transport_closure_preserves_interrupted_resume_recovery() {
        ...
        let options = next_rpc_request(&mut reconnect_rx, "reconnect reads thinking options").await;
        assert_eq!(options["method"], method::SESSION_THINKING_OPTIONS);
        respond_ok(
            &reconnect_rpc,
            &options,
            serde_json::json!({
                "session_id": options["params"]["session_id"].clone(),
                "overrides": {},
                "thinking_options": {}
            }),
        );
    ...
    async fn exhausted_resync_survives_reconnect_and_retries_without_losing_queue() {
        ...
        let options = next_rpc_request(&mut writer_rx, "reconnect reads thinking options").await;
        assert_eq!(options["method"], method::SESSION_THINKING_OPTIONS);
        respond_ok(
            &outbound,
            &options,
    ...
    async fn sidebar_add_creates_same_agent_sibling_while_turn_runs() {
        for pane in [PaneKind::Chat, PaneKind::Acp] {
            ...
            assert_eq!(request["method"], method::CONFIG_LIST);
            respond_ok(&rpc, &request, serde_json::json!([]));
            let request = next_rpc_request(&mut rx, "new session reads thinking options").await;
            assert_eq!(request["method"], method::SESSION_THINKING_OPTIONS);
            respond_ok(
                &rpc,
                &request,
