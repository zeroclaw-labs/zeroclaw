...
pub fn tooltip_for_state(connected: bool, status: AgentStatus) -> &'static str {
    if !connected {
        return "DaemonClaw — Disconnected";
    }
    match status {
        AgentStatus::Idle => "DaemonClaw — Idle",
        AgentStatus::Working => "DaemonClaw — Working",
        AgentStatus::Error => "DaemonClaw — Error",
    }
}
