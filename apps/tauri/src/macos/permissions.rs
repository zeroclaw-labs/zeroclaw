...
/// Check Input Monitoring (global keyboard/mouse listening) status.
pub fn check_input_monitoring() -> &'static str {
    // Safety: read-only IOKit query, no side effects.
...
/// Request Input Monitoring permission. Triggers a system dialog on first call.
pub fn request_input_monitoring() -> &'static str {
    // Safety: IOHIDRequestAccess is the supported entry point for prompting.
...
/// Check Full Disk Access by probing TCC.db read access.
pub fn check_full_disk_access() -> &'static str {
    let home = match std::env::var("HOME") {
        Ok(h) => h,
        Err(_) => return "not_determined",
    };
    let path = format!("{home}/Library/Application Support/com.apple.TCC/TCC.db");
...
/// Check Local Network status. Currently always returns not_determined.
