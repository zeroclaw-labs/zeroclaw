...
| 子系统 | Trait | 内置实现 | 扩展方式 |
|--------|-------|----------|----------|
| **AI 模型** | `Provider` | 通过 `zeroclaw providers` 查看（当前 28 个内置 + 别名，以及自定义端点） | `azure-openai`（Azure OpenAI）或 `custom:https://your-api.com`（OpenAI 兼容），`anthropic-custom:https://your-api.com` |
