...
|-------------|-------|----------|----------|
| **AI モデル** | `Provider` | `zeroclaw providers` で確認（現在 28 個の組み込み + エイリアス、カスタムエンドポイント対応） | `azure-openai`（Azure OpenAI）または `custom:https://your-api.com`（OpenAI 互換）、`anthropic-custom:https://your-api.com` |
