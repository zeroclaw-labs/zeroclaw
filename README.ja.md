...
|-------------|-------|----------|----------|
| **AI モデル** | `Provider` | `zeroclaw providers` で確認（組み込みプロバイダー + エイリアス、カスタムエンドポイント対応） | `azure-openai`（Azure OpenAI）または `custom:https://your-api.com`（OpenAI 互換）、`anthropic-custom:https://your-api.com` |
