...
**Documented native bridge exception — OpenAI STT.** `TRANSCRIPTION_API_KEY` and `OPENAI_API_KEY` are mapped into typed config at load time via `apply_native_stt_bridge()` as a last-resort credential fallback. Precedence: explicit config (`[transcription.openai].api_key` or `[providers.transcription.openai.<alias>].api_key`) > `ZEROCLAW_*` schema-mirror > `TRANSCRIPTION_API_KEY` > `OPENAI_API_KEY`. The bridge injects the credential only when no explicit STT provider intent exists (no legacy `[transcription.*]` table and no typed `[providers.transcription.*]` entry), so an ambient `OPENAI_API_KEY` (e.g. for model integration) does not perturb explicit Groq/Deepgram/etc. selection. The resolved value is tracked in `env_overridden_paths`, masked during saves, and never persisted to disk. See `docs/book/src/reference/env-vars.md` § Native bridge exceptions.
