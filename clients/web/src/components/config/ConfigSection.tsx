...
export default function ConfigSection({
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800">
      ...
      {!collapsed && (
        <div id={sectionPanelId} className="border-t border-gray-800 px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
          {fields.map((field) => {
            ...
            return (
              <div key={field.key} className={`flex flex-col${spanFull ? ' sm:col-span-2' : ''}`}>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-300 mb-1.5">
                  ...
                  {field.sensitive && (
                    <span className="text-[10px] text-yellow-400 bg-yellow-900/30 border border-yellow-800/50 px-1.5 py-0.5 rounded">sensitive</span>
                  )}
                  {masked && (
                    <span className="text-[10px] text-blue-400 bg-blue-900/30 border border-blue-800/50 px-1.5 py-0.5 rounded">masked</span>
                  )}
                </label>
                ...
                <div className="mt-auto">
                  {renderField(field, value, (v) => setFieldValue(section.path, field.key, v), masked)}
                </div>
              </div>
            );
          })}
