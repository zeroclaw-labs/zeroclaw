...
import { getMemory, storeMemory, deleteMemory } from '@/lib/gateway-api';
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {entries.map((entry) => (
                ...
                >
                  <td className="px-4 py-3 text-white font-medium font-mono text-xs">{entry.key}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    {confirmDelete === entry.key ? (
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => handleDelete(entry.key)} className="text-red-400 hover:text-red-300 text-xs font-medium">Yes</button>
                        <button onClick={() => setConfirmDelete(null)} className="text-gray-400 hover:text-white text-xs font-medium">No</button>
                      </div>
                    ) : (
                      <button onClick={() => setConfirmDelete(entry.key)} className="text-gray-400 hover:text-red-400 transition-colors">
