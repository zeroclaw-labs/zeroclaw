...
function statusBadge(status: Integration['status']) {
  switch (status) {
    case 'Active':
      return { icon: Check, label: 'Active', classes: 'bg-green-900/40 text-green-400 border-green-700/50' };
    case 'Available':
      return { icon: Zap, label: 'Available', classes: 'bg-blue-900/40 text-blue-400 border-blue-700/50' };
    case 'ComingSoon':
      return { icon: Clock, label: 'Coming Soon', classes: 'bg-gray-800 text-gray-400 border-gray-700' };
  }
}
...
export default function IntegrationsPage() {
  ...
  const [settingsByName, setSettingsByName] = useState<Record<string, IntegrationSettingsEntry>>({});
  const [settingsRevision, setSettingsRevision] = useState('');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeCategory, setActiveCategory] = useState<string>('all');
  const [activeEditor, setActiveEditor] = useState<IntegrationSettingsEntry | null>(null);
  ...
  const loadData = async (showLoadingState = true): Promise<Record<string, IntegrationSettingsEntry> | null> => {
    ...
    try {
      ...
      const nextSettingsByName = settings.integrations.reduce<Record<string, IntegrationSettingsEntry>>((acc, item) => {
        acc[item.name] = item;
        return acc;
      }, {});
    ...
    for (const field of activeEditor.fields) {
      ...
      if (isSelectField) {
        if (value === SELECT_KEEP) {
          if (field.required && !field.has_value) { setSaveError(`${field.label} is required.`); return; }
          if (isDirty) continue;
      ...
      if (isDirty) {
        if (isSelectField && value === SELECT_KEEP) continue;
        payload[field.key] = resolvedValue;
      }
    }

    if (Object.keys(payload).length === 0 && !activeEditor.activates_default_provider) {
      setSaveError('No changes to save.');
      return;
    }

    if (activeEditor.activates_default_provider && activeAiIntegrationId && activeEditor.id !== activeAiIntegrationId) {
      const currentProvider = activeAiIntegration?.name ?? 'current provider';
      const confirmed = window.confirm(`Switch default AI provider from ${currentProvider} to ${activeEditor.name}?`);
      if (!confirmed) return;
    }

    setSaving(true);
    try {
      await putIntegrationCredentials(activeEditor.id, { revision: settingsRevision, fields: payload });
      ...
      if (message.includes('API 409')) {
        ...
        setSaveError('Configuration changed elsewhere. Refreshed latest settings; re-enter values and save again.');
      } else {
  ...
  const saveQuickModel = async (
    ...
    if (!trimmedTarget || trimmedTarget === currentModel) return;

    if (activeAiIntegrationId && !isActiveDefaultProvider && integration.id !== activeAiIntegrationId) {
      const currentProvider = activeAiIntegration?.name ?? 'current provider';
      const confirmed = window.confirm(`Switch default AI provider from ${currentProvider} to ${integration.name} and set model to ${trimmedTarget}?`);
      if (!confirmed) return;
    }
    ...
    setSaveError(null);
    try {
      await putIntegrationCredentials(integration.id, { revision: settingsRevision, fields: { default_model: trimmedTarget } });
      await loadData(false);
      setSaveSuccess(`Model updated to ${trimmedTarget} for ${integration.name}.`);
      setQuickModelDrafts((prev) => { const next = { ...prev }; delete next[integration.id]; return next; });
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Failed to update model';
      if (message.includes('API 409')) {
        await loadData(false);
        setQuickModelError('Configuration changed elsewhere. Refreshed latest settings; choose the model again.');
      } else {
  ...
  if (error) {
    return (
      <div className="p-6">
        <div className="rounded-lg bg-red-900/30 border border-red-700 p-4 text-red-300">Failed to load integrations: {error}</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-semibold text-white">Integrations ({integrations.length})</h2>
      </div>

      {saveSuccess && <div className="rounded-lg bg-green-900/30 border border-green-700 p-3 text-sm text-green-300">{saveSuccess}</div>}
      {quickModelError && <div className="rounded-lg bg-red-900/30 border border-red-700 p-3 text-sm text-red-300">{quickModelError}</div>}
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((cat) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors capitalize ${activeCategory === cat ? 'bg-blue-600 text-white' : 'bg-gray-900 text-gray-400 border border-gray-700 hover:bg-gray-800 hover:text-white'}`}
          >
      ...
      {Object.keys(grouped).length === 0 ? (
        ...
        Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([category, items]) => (
          <div key={category}>
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3 capitalize">{formatCategory(category)}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {items.map((integration) => {
                ...
                const isActiveDefaultProvider = !!editable && isAiIntegration && editable.id === activeAiIntegrationId;
                const modelField = editable ? modelFieldFor(editable) : undefined;
                const modelOptions = editable && modelField ? modelOptionsForField(editable.id, modelField) : [];
                const currentModel = editable && isAiIntegration ? modelValueFor(editable, isActiveDefaultProvider) : null;
                const fallbackModel = editable && isAiIntegration ? fallbackModelFor(editable) : null;
                const modelSummary = currentModel ? currentModel : fallbackModel ? `default: ${fallbackModel}` : 'default';
                const modelBaseline = currentModel ?? fallbackModel ?? '';
                const quickDraft = editable ? quickModelDrafts[editable.id] ?? modelBaseline : '';
                const quickOptions = [
                  ...(currentModel && !modelOptions.includes(currentModel) ? [currentModel] : []),
                  ...modelOptions,
                ];
                const showQuickModelControls = !!editable && editable.configured && isAiIntegration && quickOptions.length > 0;

                return (
                  <div key={integration.name} className={`bg-gray-900 rounded-xl border p-5 transition-colors ${isActiveDefaultProvider ? 'border-green-700/70 bg-gradient-to-b from-green-950/20 to-gray-900' : 'border-gray-800 hover:border-gray-700'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h4 className="text-sm font-semibold text-white truncate">{integration.name}</h4>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap justify-end">
                        {isAiIntegration && editable?.configured && (
                          <span className={`flex-shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${isActiveDefaultProvider ? 'bg-emerald-900/40 text-emerald-300 border-emerald-700/60' : 'bg-gray-800 text-gray-300 border-gray-700'}`}>
                            {isActiveDefaultProvider ? 'Default' : 'Configured'}
                          </span>
                        )}
                        <span className={`flex-shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${badge.classes}`}>
                    ...
                    {editable && isAiIntegration && editable.configured && (
                      <div className="mt-3 rounded-lg border border-gray-800 bg-gray-950/50 p-3 space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-[11px] uppercase tracking-wider text-gray-500">Current model</span>
                          <span className="text-xs text-gray-200 truncate" title={modelSummary}>{modelSummary}</span>
                        </div>
                        {showQuickModelControls && editable && (
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <select
                                value={quickDraft}
                                onChange={(e) => setQuickModelDrafts((prev) => ({ ...prev, [editable.id]: e.target.value }))}
                              ...
                              <button
                                onClick={() => editable && void saveQuickModel(editable, quickDraft, modelBaseline, isActiveDefaultProvider)}
                                disabled={quickModelSavingId === editable.id || !quickDraft || quickDraft === modelBaseline}
                            ...
                            <p className="text-[11px] text-gray-500">For custom model IDs, use Edit Keys.</p>
                          </div>
                        )}
                      </div>
                    )}
                        ...
                        <div className="text-xs text-gray-400">
                          {editable.configured
                            ? editable.activates_default_provider
                              ? isActiveDefaultProvider ? 'Default provider configured' : 'Provider configured'
        ...
        >
          <div className="w-full max-w-lg bg-gray-900 border border-gray-800 rounded-xl shadow-xl">
            <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold text-white">Configure {activeEditor.name}</h3>
                <p className="text-xs text-gray-400 mt-0.5">
                  {activeEditor.configured ? 'Enter only fields you want to update.' : 'Enter required fields to configure this integration.'}
                </p>
              </div>
              <button onClick={closeEditor} disabled={saving} className="text-gray-400 hover:text-white transition-colors disabled:opacity-50" aria-label="Close">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              {activeEditor.activates_default_provider && (
                <div className="rounded-lg border border-blue-800 bg-blue-950/30 p-3 text-xs text-blue-200">
                  ...
                  <Link href="/workspace/config" className="underline underline-offset-2 hover:text-blue-100">Configuration</Link>.
                </div>
              )}
              ...
              {activeEditor.fields.map((field) => {
                ...
                const maskedSecretValue = isSecretField ? (field.masked_value || (field.has_value ? '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022' : undefined)) : undefined;
                const activeEditorIsDefaultProvider = activeEditor.activates_default_provider && activeEditor.id === activeAiIntegrationId;
                const currentModelValue = field.current_value?.trim() || (activeEditorIsDefaultProvider ? runtimeStatus?.model?.trim() || '' : '');
                const keepCurrentLabel = currentModelValue ? `Keep current model (${currentModelValue})` : 'Keep current model';
                ...
                return (
                  <div key={field.key}>
                    <label className="flex items-center gap-2 text-sm font-medium text-gray-300 mb-1.5">
                      ...
                      {field.has_value && (
                        <span className="text-[11px] text-green-400 bg-green-900/30 border border-green-800 px-1.5 py-0.5 rounded">Configured</span>
                      )}
                    </label>
                    {isSelectField ? (
                      <div className="space-y-2">
                        ...
                        <p className="text-[11px] text-gray-500">Pick a recommended model or choose Custom model. {customModelFormatHint(activeEditor.id)}.</p>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {maskedSecretValue && (
                          <p className="text-[11px] text-gray-500">Current value: <span className="font-mono text-gray-300">{maskedSecretValue}</span></p>
                        )}
                        <input
                          type={isSecretField ? 'password' : 'text'}
                          value={fieldValues[field.key] ?? ''}
                          onChange={(e) => updateField(field.key, e.target.value)}
                          placeholder={field.required ? (field.has_value ? 'Enter a new value to replace current' : 'Enter value') : (field.has_value ? 'Type new value, or leave empty to keep current' : 'Optional')}
                          className="w-full px-3 py-2 rounded-lg bg-gray-950 border border-gray-700 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        />
                      </div>
                    )}
              ...
              {saveError && <div className="rounded-lg bg-red-900/30 border border-red-700 p-3 text-sm text-red-300">{saveError}</div>}
            </div>

            <div className="px-5 py-4 border-t border-gray-800 flex items-center justify-end gap-2">
              <button onClick={closeEditor} disabled={saving} className="px-4 py-2 rounded-lg text-sm border border-gray-700 text-gray-300 hover:bg-gray-800 transition-colors disabled:opacity-50">Cancel</button>
              <button onClick={saveCredentials} disabled={saving} className="px-4 py-2 rounded-lg text-sm font-medium bg-blue-600 hover:bg-blue-700 text-white transition-colors disabled:opacity-50">
                {saving ? 'Saving...' : activeEditor.activates_default_provider ? 'Save & Activate' : 'Save Keys'}
              </button>
            </div>
          </div>
        </div>
