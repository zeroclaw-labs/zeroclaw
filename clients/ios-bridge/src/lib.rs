...
//! ZeroClaw iOS Bridge
//!
//! This crate provides UniFFI bindings for ZeroClaw to be used from Swift/iOS.
...
/// Currently unused until gateway integration is connected.
#[allow(dead_code)]
...
/// Agent status enum exposed to Swift
...
/// Main ZeroClaw controller exposed to iOS
...
mod tests {
    ...
    #[test]
    fn controller_creation_returns_stopped() {
    ...
    #[test]
    fn start_stop_transitions_status() {
        let controller = ZeroClawController::with_defaults("/tmp/zeroclaw".to_string());
    ...
    #[test]
    fn send_message_creates_user_and_echo() {
        let controller = ZeroClawController::with_defaults("/tmp/zeroclaw".to_string());
        let result = controller.send_message("Hello".to_string());
        assert!(result.success);

        let messages = controller.get_messages();
        assert_eq!(messages.len(), 2); // User + assistant echo
        assert_eq!(messages[0].role, "user");
        assert_eq!(messages[1].role, "assistant");
    }

    #[test]
    fn clear_messages_empties_history() {
        let controller = ZeroClawController::with_defaults("/tmp/zeroclaw".to_string());
        controller.send_message("Hello".to_string());
        assert!(!controller.get_messages().is_empty());
    ...
    #[test]
    fn is_configured_checks_api_key() {
        let controller = ZeroClawController::with_defaults("/tmp/zeroclaw".to_string());
        assert!(!controller.is_configured());

        let config = ZeroClawConfig {
            data_dir: "/tmp/zeroclaw".to_string(),
            provider: "anthropic".to_string(),
            model: "claude-sonnet-4-5".to_string(),
            api_key: "test_key".to_string(),
            system_prompt: None,
        };
        controller.update_config(config).unwrap();
        assert!(controller.is_configured());
    }
}
