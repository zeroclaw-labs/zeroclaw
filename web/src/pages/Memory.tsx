...
export default function Memory() {
  ...
  const handleDelete = async (key: string) => {
    ...
    catch (err: unknown) { setError(err instanceof Error ? err.message : t('memory.delete_error')); }
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      {error && (
        <div className="rounded-xl border p-3 text-sm animate-fade-in" style={{ background: 'rgba(239, 68, 68, 0.08)', borderColor: 'rgba(239, 68, 68, 0.2)', color: '#f87171' }}>{error}</div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 modal-backdrop flex items-center justify-center z-50">
          <div className="surface-panel p-6 w-full max-w-md mx-4 animate-fade-in-scale">
            ...
            {formError && (
              <div className="mb-4 rounded-xl border p-3 text-sm animate-fade-in" style={{ background: 'rgba(239, 68, 68, 0.08)', borderColor: 'rgba(239, 68, 68, 0.2)', color: '#f87171' }}>{formError}</div>
            )}
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--pc-text-secondary)' }}>{t('memory.key_required')} <span style={{ color: 'var(--color-status-error)' }}>*</span></label>
              ...
              <div>
                <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--pc-text-secondary)' }}>{t('memory.content_required')} <span style={{ color: 'var(--color-status-error)' }}>*</span></label>
              ...
              <div>
                <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--pc-text-secondary)' }}>{t('memory.category_optional')}</label>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => { setShowForm(false); setFormError(null); }} className="btn-secondary px-4 py-2 text-sm font-medium">{t('memory.cancel')}</button>
              <button onClick={handleAdd} disabled={submitting} className="btn-electric px-4 py-2 text-sm font-medium">{submitting ? t('memory.saving') : t('common.save')}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="card overflow-x-auto rounded-2xl">
          <table className="table-electric">
            ...
            <tbody>
              {entries.map((entry) => (
                <tr key={entry.id}>
                  <td className="font-mono text-xs" style={{ color: 'var(--pc-text-primary)' }}>{entry.key}</td>
                  <td className="max-w-[300px] text-sm" style={{ color: 'var(--pc-text-secondary)' }}><span title={entry.content}>{truncate(entry.content, 80)}</span></td>
                  <td>
                  ...
                  <td className="text-xs whitespace-nowrap" style={{ color: 'var(--pc-text-muted)' }}>{formatDate(entry.timestamp)}</td>
                  <td className="text-right">
                    {confirmDelete === entry.key ? (
                      <div className="flex items-center justify-end gap-2 animate-fade-in">
                        <span className="text-xs" style={{ color: 'var(--color-status-error)' }}>{t('memory.delete_confirm')}</span>
                        <button onClick={() => handleDelete(entry.key)} className="text-xs font-medium" style={{ color: 'var(--color-status-error)' }}>{t('memory.yes')}</button>
                        <button onClick={() => setConfirmDelete(null)} className="text-xs font-medium" style={{ color: 'var(--pc-text-muted)' }}>{t('memory.no')}</button>
                      </div>
                    ) : (
                      <button onClick={() => setConfirmDelete(entry.key)} className="btn-icon"><Trash2 className="h-4 w-4" /></button>
                    )}
                  </td>
                </tr>
              ))}
