      ...
      <div className="px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            size="sm"
            onClick={fetchRuns}
            aria-label={t("cron.refresh_runs")}
          >
...
export default function Cron() {
  ...
  const [runHistoryRefresh, setRunHistoryRefresh] = useState<
  ...
  const [formSessionTarget, setFormSessionTarget] = useState<
    "isolated" | "main"
  >("isolated");
  const [formAllowedTools, setFormAllowedTools] = useState("");
  const [formAgent, setFormAgent] = useState("");
  const [formDeliveryMode, setFormDeliveryMode] = useState<"none" | "announce">(
    "none",
  );
  ...
  const openEditModal = (job: CronJob) => {
    ...
    setFormAgent(
      (job as CronJob & { agent_alias?: string }).agent_alias ?? "default",
    );
    const delivery = job.delivery;
    if (
      delivery &&
      (delivery.mode === "announce" || delivery.mode === "none")
    ) {
    ...
    if (jobType === "agent") {
      ...
      setFormSessionTarget(job.session_target === "main" ? "main" : "isolated");
      setFormAllowedTools(
  ...
  const fetchSettings = () => {
    getCronSettings()
      ...
      .catch((err) => {
        console.warn(
          "[ZeroClaw] Failed to load cron settings:",
          describeCronSettingsError(err),
        );
      });
  };
  ...
  const toggleCatchUp = async () => {
    ...
    try {
      ...
      console.warn(
        "[ZeroClaw] Failed to update cron settings:",
        describeCronSettingsError(err),
      );
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        ...
        const updated = await patchCronJob((modalJob as CronJob).id, patch);
  ...
  const handleTrigger = async (id: string) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        setTriggerError(
          detail
            ? `${t("cron.trigger_error")}: ${detail}`
            : t("cron.trigger_error"),
        );
      }
    } catch (err: unknown) {
      setTriggerError(
        err instanceof Error ? err.message : t("cron.trigger_error"),
      );
    } finally {
      setTriggering(null);
    }
  ...
  return (
    <div className="flex flex-col h-full p-6 gap-6 overflow-hidden">
          ...
          >
            <span
              className={`inline-block h-4 w-4 rounded-full bg-white transition-transform duration-200 ${
                settings.catch_up_on_startup ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </button>
        </Card>
          ...
          <div className="bg-pc-surface border border-pc-border rounded-[var(--radius-lg)] shadow-[var(--pc-shadow-md)] p-6 w-full max-w-md mt-15 max-h-9/10 overflow-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-pc-text">
                {isEditing
                  ? t("cron.edit_modal_title")
                  : t("cron.add_modal_title")}
              </h3>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="space-y-4">
                ...
                {isEditing ? (
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 px-3 py-2.5 rounded-[var(--radius-md)] text-sm font-medium border transition-colors cursor-pointer ${
                        formJobType === "shell"
              ...
              <div>
                <label className="block text-[11px] font-medium mb-1.5 uppercase tracking-wider text-pc-text-faint">
                  {t("cron.agent_label")}{" "}
                  {!isEditing && <span className="text-status-error">*</span>}
                </label>
                {isEditing ? (
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t("cron.timezone_placeholder")}
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`flex-1 px-3 py-2 rounded-[var(--radius-md)] text-xs font-medium border transition-colors cursor-pointer ${
                ...
                <div className="border-t border-pc-border pt-4">
                  ...
                  {formDeliveryMode === "announce" ? (
                    <div className="space-y-1.5 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="text-pc-text-faint text-xs uppercase tracking-wider">
                          {t("cron.delivery_mode")}
                        </span>
                        <span className="text-pc-text-secondary font-medium">
                          {t("cron.delivery_announce")}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-pc-text-faint text-xs uppercase tracking-wider">
                          {t("cron.delivery_channel")}
                        </span>
                        <span className="text-pc-text-secondary font-mono break-all">
                          {formDeliveryChannel || "-"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-pc-text-faint text-xs uppercase tracking-wider">
                          {t("cron.delivery_best_effort")}
                        </span>
                        <span className="text-pc-text-secondary">
                          {formDeliveryBestEffort
                            ? t("cron.yes")
                            : t("cron.no")}
                        </span>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <label className="flex items-center gap-2 text-xs text-pc-text-muted">
                        <input
                          ...
                          onChange={(e) =>
                            setFormDeliveryBestEffort(e.target.checked)
                          }
                      ...
                      <p className="text-xs text-pc-text-faint">
                        {t("cron.delivery_channels_from")}{" "}
                        <code className="font-mono text-pc-text-secondary">
                          agents.{formAgent || "<agent>"}.channels
                        </code>
      ...
      {triggerError && (
        <div
          role="alert"
          className="rounded-[var(--radius-md)] border border-status-error/25 bg-status-error/10 p-3 text-sm text-status-error flex items-start justify-between gap-3"
        >
                ...
                <React.Fragment key={job.id}>
                  <tr className="border-b border-pc-border/60 last:border-0">
                    ...
                    <td className="px-4 py-2.5 font-mono text-xs text-center text-pc-text-muted">
                      {scheduleTimezone(job) ??
                        t("cron.runtime_local_timezone")}
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <Badge tone={job.enabled ? "ok" : "neutral"}>
                        {job.enabled
                          ? t("cron.enabled_status")
                          : t("cron.disabled_status")}
                      </Badge>
                    </td>
                    <td className="px-4 py-2.5 text-center">
                  ...
                  {expandedJob === job.id && (
                    <tr>
                      <td
                        colSpan={9}
                        className="bg-pc-elevated border-b border-pc-border"
                      >
                        <RunHistoryPanel
                          jobId={job.id}
                          refreshKey={runHistoryRefresh[job.id] ?? 0}
                        />
                      </td>
                    </tr>
                  )}
