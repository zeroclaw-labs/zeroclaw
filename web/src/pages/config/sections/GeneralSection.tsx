...
const MODELS_BY_PROVIDER: Record<string, { value: string; label: string }[]> = {
  ...
  together: [
    {
      value: "meta-llama/Llama-4-Maverick-400B",
      label: "Llama 4 Maverick 400B",
    },
    { value: "meta-llama/Llama-4-70B", label: "Llama 4 70B" },
    {
  ...
  fireworks: [
    {
      value: "accounts/fireworks/models/llama-4-maverick-400b",
      label: "Llama 4 Maverick 400B",
    },
    {
...
export default function GeneralSection({ config, onUpdate }: Props) {
  const provider = normalizeProvider(
    (config.default_provider as string) ?? "openrouter",
  );
  ...
  const handleProviderChange = (v: string) => {
    const normalized = normalizeProvider(v);
    onUpdate("default_provider", normalized);
    const models = MODELS_BY_PROVIDER[normalized];
    if (models && models.length > 0) {
      onUpdate(
        "default_model",
        normalized === "copilot" ? "gpt-5.4-mini" : models[0]!.value,
      );
    }
  };
  ...
  return (
    ...
    >
      <FieldRow
        label={t("config.field.default_provider")}
        description={t("config.field.default_provider.desc")}
      >
        <Select
          value={provider}
      ...
      <FieldRow
        label={t("config.field.default_model")}
        description={t("config.field.default_model.desc")}
      >
        {modelOptions ? (
          <Select
            value={
              modelOptions.some((o) => o.value === currentModel)
                ? currentModel
                : ""
            }
      ...
      <FieldRow
        label={t("config.field.default_temperature")}
        description={t("config.field.default_temperature.desc")}
      >
        <Slider
      ...
      <FieldRow
        label={t("config.field.provider_timeout_secs")}
        description={t("config.field.provider_timeout_secs.desc")}
      >
        <NumberInput
      ...
      <FieldRow
        label={t("config.field.locale")}
        description={t("config.field.locale.desc")}
      >
        <Select
