...
export default function AcpConsole() {
  ...
  const [initResult, setInitResult] = useState<AcpInitializeResult | null>(
    null,
  );
  const [agents, setAgents] = useState<AgentPickerSummary[]>([]);
  const [agentsLoading, setAgentsLoading] = useState(true);
  const [selectedAgentAlias, setSelectedAgentAlias] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    loadAgentPickerSummaries()
      .then((loadedAgents) => {
        ...
        setSelectedAgentAlias((current) => {
          if (
            current &&
            loadedAgents.some(
              (agent) => agent.alias === current && agent.enabled,
            )
          ) {
            return current;
          }
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        setError(
          err instanceof Error ? err.message : t("acp.error.load_agents"),
        );
      })
      .finally(() => {
  ...
  const handleSessionUpdate = useCallback(
    (params: unknown) => {
      ...
      addMessage(setMessages, {
        ...
        content:
          typeof updateKind === "string" ? updateKind : t("acp.unknown_update"),
        detail: stringifyDetail(update),
      });
    },
    ...
    (request: AcpRequest) => {
      ...
      const permission: PermissionRequest = {
        ...
        detail: stringifyDetail(
          toolRecord.rawInput ?? toolRecord.content ?? toolRecord,
        ),
        options,
      };
  ...
  const isCurrentConnection = useCallback(
    (client: AcpWebSocketClient, connectionSeq: number) =>
  ...
  const initializeSession = useCallback(
    async (
      ...
      if (!client.connected || !isCurrentConnection(client, connectionSeq))
        return;
      ...
      try {
        ...
        const session = await client.request<AcpSessionNewResult>(
          "session/new",
          sessionParams,
        );
        if (!isCurrentConnection(client, connectionSeq)) return;
        ...
        if (isCurrentConnection(client, connectionSeq)) {
          setError(
            err instanceof Error ? err.message : t("acp.error.init_failed"),
          );
        }
      } finally {
        if (isCurrentConnection(client, connectionSeq)) {
  ...
  const sendPrompt = async () => {
    ...
    try {
      const result = await client.request<AcpSessionPromptResult>(
        "session/prompt",
        {
          sessionId,
          prompt: outgoing,
      ...
      if (isCurrentConnection(client, connectionSeq)) {
        setError(
          err instanceof Error ? err.message : t("acp.error.prompt_failed"),
        );
      }
    } finally {
      if (isCurrentConnection(client, connectionSeq)) {
  ...
  const answerPermission = (
    ...
    pushEvent(
      optionId ? `permission selected: ${optionId}` : "permission cancelled",
    );
  };

  const statusTone: BadgeTone =
    status === "connected" ? "ok" : status === "connecting" ? "warn" : "error";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto h-full flex flex-col gap-4">
        ...
        actions={
          ...
          <div className="flex flex-wrap items-center gap-2 min-w-0">
            <select
              ...
              onChange={(event) =>
                setSelectedAgentAlias(event.target.value || null)
              }
              ...
              {agents.length === 0 || !agents.some((agent) => agent.enabled) ? (
                ...
                <>
                  ...
                  {agents.map((agent) => (
                    ...
                    >
                      {agent.alias}
                      {agent.enabled ? "" : ` (${t("acp.agent.disabled")})`}
                    </option>
                  ))}
                </>
              )}
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <StatusTile
          ...
          value={
            sessionId ??
            (initializing
              ? t("acp.status.creating")
              : t("acp.status.not_ready"))
          }
        />
        <StatusTile
          label={t("acp.status.workspace")}
          value={workspaceDir ?? t("acp.status.gateway_default")}
        />
      </section>
      ...
      <main className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <Card
          padded={false}
          className="overflow-hidden flex min-h-[560px] flex-col shadow-[var(--pc-shadow-sm)]"
        >
        ...
        <aside className="flex min-h-0 flex-col gap-4">
          <Card
            padded={false}
            className="overflow-hidden shadow-[var(--pc-shadow-sm)]"
          >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {permission.options.map((option) => (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            answerPermission(permission, option.optionId)
                          }
                        >
          ...
          <Card
            padded={false}
            className="overflow-hidden flex min-h-0 flex-1 flex-col shadow-[var(--pc-shadow-sm)]"
          >
...
function StatusTile({ label, value }: { label: string; value: string }) {
  return (
    <Card>
      ...
      <div
        className="mt-1 truncate text-sm font-medium text-pc-text"
        title={value}
      >
        {value}
      </div>
...
const TRANSCRIPT_TONE: Record<
  ConsoleMessageKind,
  { labelKey: string; className: string }
> = {
  user: {
    labelKey: "acp.role_you",
    className: "bg-pc-accent/[0.06] border-pc-accent/25",
  },
  assistant: {
    labelKey: "acp.role_agent",
    className: "bg-pc-elevated border-pc-border",
  },
  thought: {
    labelKey: "acp.role_thought",
    className: "bg-pc-surface border-pc-border",
  },
  tool: { labelKey: "acp.role_tool", className: "bg-pc-code border-pc-border" },
  system: {
    labelKey: "acp.role_system",
    className: "bg-pc-surface border-pc-border",
  },
};
...
function TranscriptMessage({ message }: { message: ConsoleMessage }) {
  ...
  return (
    <article
      className={`rounded-[var(--radius-md)] border p-3 ${tone.className}`}
    >
