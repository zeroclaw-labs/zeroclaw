...
const themeOptions: {
...
}[] = [
  {
    ...
    previewBg: "linear-gradient(135deg, #1e1e24 50%, #f4f4f5 50%)",
    previewFg: "#d4d4d8",
  },
  {
    value: "dark",
...
const chipActive = "border-pc-accent-dim bg-pc-accent/10 text-pc-accent-light";

function chip(active: boolean, extra = "") {
  return [chipBase, active ? chipActive : chipInactive, extra]
    .filter(Boolean)
    .join(" ");
}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: bg,
          border: `1px solid ${theme.scheme === "dark" ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)"}`,
        }}
      >
        {/* Title bar dots */}
        ...
        <div className="px-2 pb-2 flex flex-col gap-[3px]">
          <div className="flex gap-1 items-center">
            <span
              ...
              style={{ background: c1, width: "30%" }}
            />
            <span
              className="h-[3px] rounded-full"
              style={{ background: text, width: "20%", opacity: 0.4 }}
            />
          </div>
          <div className="flex gap-1 items-center">
            <span
              className="h-[3px] rounded-full"
              style={{ background: text, width: "15%", opacity: 0.3 }}
            />
            <span
              className="h-[3px] rounded-full"
              style={{ background: c2, width: "25%" }}
            />
            <span
              className="h-[3px] rounded-full"
              style={{ background: c3, width: "18%" }}
            />
          </div>
          <div className="flex gap-1 items-center">
            <span
              className="h-[3px] rounded-full"
              style={{ background: c3, width: "22%" }}
            />
            <span
              className="h-[3px] rounded-full"
              style={{ background: text, width: "28%", opacity: 0.3 }}
            />
          </div>
        </div>
      </div>
...
export function SettingsModal({ open, onClose }: Props) {
  ...
  const darkThemes = useMemo(
    () => colorThemes.filter((ct) => ct.scheme === "dark"),
    [],
  );
  const lightThemes = useMemo(
    () => colorThemes.filter((ct) => ct.scheme === "light"),
    [],
  );
        ...
        <div className="flex-1 min-h-0 px-6 py-4 overflow-y-auto">
          ...
          {tab === "appearance" && (
            <>
              ...
              {/* Theme Mode */}
              <div className="mb-3">
                <div className="text-xs mb-2 text-pc-text-secondary">
                  {t("theme.mode")}
                </div>
              ...
              <div className="rounded-[var(--radius-lg)] border border-pc-border bg-pc-surface p-3 mt-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-pc-text">
                    {colorThemes.find((ct) => ct.id === colorTheme)?.name ??
                      t("settings.default_dark")}
                  </span>
                  ...
                  {uiFontOptions.map((opt) => (
                    ...
                    >
                      <span
                        style={{
                          fontSize: "14px",
                          fontFamily: uiFontStacks[opt.value],
                        }}
                      >
                        {opt.sample}
                      </span>
                      <span
                        className="text-pc-text-faint"
                        style={{ fontSize: "11px" }}
                      >
                        {opt.label}
                      </span>
                  ...
                  {monoFontOptions.map((opt) => (
                    ...
                    >
                      <span
                        style={{
                          fontSize: "14px",
                          fontFamily: monoFontStacks[opt.value],
                        }}
                      >
                        {opt.sample}
                      </span>
