...
import {
  loadConfigSearchItems,
  type ConfigSearchItem,
} from "@/lib/configSearch";
...
export default function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  const {
    ...
    const scored = allItems
      ...
      .filter(
        (s): s is { item: PaletteItem; idx: number; score: number } =>
          s.score !== null,
      );
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2.5 border-b border-pc-border px-3.5">
          <Search
            className="h-4 w-4 shrink-0 text-pc-text-muted"
            aria-hidden="true"
          />
          <input
            ref={inputRef}
        ...
        >
            ...
            rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  {isSel && (
                    <CornerDownLeft
                      className="h-3.5 w-3.5 shrink-0 text-pc-text-muted"
                      aria-hidden="true"
                    />
                  )}
                </button>
          ...
          {extraCount > 0 && (
            <div className="px-3 py-2 text-center text-[11px] text-pc-text-faint">
              {t("nav.cmdk.more_prefix")}
              {extraCount} {t("nav.cmdk.more_suffix")}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 border-t border-pc-border px-3.5 py-2 text-[11px] text-pc-text-faint">
          <span className="flex items-center gap-1">
            <kbd className="rounded-[var(--radius-sm)] border border-pc-border bg-pc-elevated px-1.5 py-0.5 font-mono">
              ↑
            </kbd>
            <kbd className="rounded-[var(--radius-sm)] border border-pc-border bg-pc-elevated px-1.5 py-0.5 font-mono">
              ↓
