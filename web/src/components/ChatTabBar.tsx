    ...
    <div className="relative z-20 flex items-stretch border-b border-pc-border bg-pc-surface">
        ...
        {pickerOpen && (
          ...
          >
            ...
            {!pickerLoading && !pickerError && availableToOpen.length === 0 && (
              <div className="px-3 py-2 text-xs text-pc-text-muted">
                {t("workspace.no_more_agents")}
              </div>
            )}
            {!pickerLoading &&
              !pickerError &&
              availableToOpen.map((alias) => (
                <button
                  key={alias}
                  type="button"
                  role="menuitem"
                ...
                >
                  <Bot
                    className="h-3.5 w-3.5 text-pc-accent shrink-0"
                    aria-hidden
                  />
      ...
      <div className="flex items-center px-2 border-l border-pc-border">
        <Button
          ...
          aria-label={
            layout === "split"
              ? t("workspace.layout_tabs")
              : t("workspace.layout_split")
          }
          title={
            layout === "split"
              ? t("workspace.layout_tabs")
...
function StatusDot({
  ...
  if (unread) {
    return (
      <span
        className="inline-flex h-2 w-2 shrink-0 rounded-full bg-pc-accent"
        aria-hidden
      />
    );
  }
  return (
    <span
      className={[
        ...
        active ? "bg-pc-accent/40" : "bg-pc-text-faint/40",
      ].join(" ")}
      aria-hidden
    />
  );
