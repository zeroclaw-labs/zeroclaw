...
function loadStored(): StoredTheme {
  try {
    ...
    if (raw) {
      ...
      const uiFont: UiFont = uiFontStacks[parsed.uiFont as UiFont]
        ? (parsed.uiFont as UiFont)
        : DEFAULTS.uiFont;
      const monoFont: MonoFont = monoFontStacks[parsed.monoFont as MonoFont]
        ? (parsed.monoFont as MonoFont)
        : DEFAULTS.monoFont;
      const uiFontSize = Number.isFinite(parsed.uiFontSize)
        ? Math.min(20, Math.max(12, Number(parsed.uiFontSize)))
        : DEFAULTS.uiFontSize;
      const monoFontSize = Number.isFinite(parsed.monoFontSize)
        ? Math.min(20, Math.max(12, Number(parsed.monoFontSize)))
        : DEFAULTS.monoFontSize;

      let colorTheme: ColorThemeId = DEFAULTS.colorTheme;
      if (
        parsed.colorTheme &&
        colorThemeMap[parsed.colorTheme as ColorThemeId]
      ) {
...
function resolveColorTheme(
  ...
  if (mode === "system") {
    ...
    if (
      ct &&
      ((preferLight && ct.scheme === "light") ||
        (!preferLight && ct.scheme === "dark"))
    ) {
      return colorTheme;
    }
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [monoFontSize, setMonoFontSizeState] = useState<number>(
    stored.monoFontSize,
  );
  ...
  const setTheme = useCallback(
    (t: ThemeMode) => {
      ...
      const newColorTheme: ColorThemeId =
        ...
        targetScheme === null ||
        (currentCt && currentCt.scheme === targetScheme)
          ? colorTheme
          : t === "oled"
            ? "oled-black"
            : t === "light"
  ...
  const setMonoFontSize = useCallback(
    (size: number) => {
      const clamped = Math.min(20, Math.max(12, size));
