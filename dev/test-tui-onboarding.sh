...
# ── Step 1: Build ────────────────────────────────────────────────────
echo -e "${BOLD}[1/3] Building daemonclaw (${PROFILE})...${RESET}"
if [[ "$PROFILE" == "release" ]]; then
  cargo build --release 2>&1
  BIN="$REPO_ROOT/target/release/daemonclaw"
else
  cargo build 2>&1
  BIN="$REPO_ROOT/target/debug/daemonclaw"
fi
...
echo "  [ ] Welcome screen renders with DAEMONCLAW banner"
