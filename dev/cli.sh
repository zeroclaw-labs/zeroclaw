...
function ensure_config {
    CONFIG_DIR="$HOST_TARGET_DIR/.jhedaiclaw"
    CONFIG_FILE="$CONFIG_DIR/config.toml"
    WORKSPACE_DIR="$CONFIG_DIR/workspace"

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚙️  Config file missing in target/.jhedaiclaw. Creating default dev config from template...${NC}"
...
function print_help {
    echo -e "${YELLOW}JhedaiClaw Development Environment Manager${NC}"
    ...
    echo -e "  ${GREEN}agent${NC}   Enter Agent (JhedaiClaw CLI)"
...
case "$1" in
    up)
        ...
        echo -e "   - Config: target/.jhedaiclaw/config.toml (Edit locally to apply changes)"
        ;;
    ...
    shell)
        ...
        docker exec -it jhedaiclaw-sandbox /bin/bash
        ;;

    agent)
        echo -e "${GREEN}🤖 Entering Agent Container (JhedaiClaw)... (Type 'exit' to leave)${NC}"
        docker exec -it jhedaiclaw-dev /bin/bash
        ;;
    ...
    clean)
        echo -e "${RED}⚠️  WARNING: This will delete 'target/.jhedaiclaw' data and Docker volumes.${NC}"
        read -p "Are you sure? (y/N) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker compose -f "$COMPOSE_FILE" down -v
            rm -rf "$HOST_TARGET_DIR/.jhedaiclaw"
