...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    local build_args=(
      ...
      -t jhedaiclaw-local-smoke:latest
      .
    )
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t jhedaiclaw-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
JhedaiClaw Local CI in Docker
...
case "$1" in
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm jhedaiclaw-local-smoke:latest --version
    ;;
