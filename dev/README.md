# DaemonClaw Development Environment

A fully containerized development sandbox for DaemonClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`daemonclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `daemonclaw` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/daemonclaw-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/daemonclaw-data/workspace`
...
The agent configuration lives in `target/.daemonclaw` (mounted to `/daemonclaw-data/.daemonclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.daemonclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `daemonclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=daemonclaw-target`) to speed repeat local image builds.
