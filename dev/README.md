# QuantClaw Development Environment

A fully containerized development sandbox for QuantClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`quantclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `quantclaw` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/quantclaw-data/workspace`
...
The agent configuration lives in `target/.quantclaw` (mounted to `/quantclaw-data/.quantclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.quantclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `quantclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=quantclaw-target`) to speed repeat local image builds.
