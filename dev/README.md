...
- The CI image keeps Rust toolchain defaults from `rust:1.97-slim` and installs pinned toolchain `1.97.1` (no custom `CARGO_HOME`/`RUSTUP_HOME` overrides), preventing repeated toolchain bootstrapping on each run.
