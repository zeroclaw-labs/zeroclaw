...
TAG="v${VERSION}"
ASSET_NAME="daemonclaw-${TARGET}.tar.gz"
ASSET_URL="https://github.com/DeliveryBoyTech/daemonclaw/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t daemonclaw-termux-test-XXXXXX)"
...
# --- Test 5: Contains daemonclaw binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^daemonclaw$"; then
  pass "Archive contains 'daemonclaw' binary"
else
  fail "Archive does not contain 'daemonclaw' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/DeliveryBoyTech/daemonclaw/releases/download/${TAG}/SHA256SUMS"
