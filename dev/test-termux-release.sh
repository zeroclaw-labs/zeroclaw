...
TAG="v${VERSION}"
ASSET_NAME="quantclaw-${TARGET}.tar.gz"
ASSET_URL="https://github.com/quant-speed/quantclaw/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t quantclaw-termux-test-XXXXXX)"
...
# --- Test 5: Contains quantclaw binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^quantclaw$"; then
  pass "Archive contains 'quantclaw' binary"
else
  fail "Archive does not contain 'quantclaw' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/quant-speed/quantclaw/releases/download/${TAG}/SHA256SUMS"
