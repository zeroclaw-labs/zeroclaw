...
impl Tool for ClaudeCodeTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let work_dir = if let Some(wd) = args.get("working_directory").and_then(|v| v.as_str()) {
            ...
            let canonical_wd = wd_path.canonicalize().unwrap_or(wd_path.clone());
            let canonical_ws = workspace.canonicalize().unwrap_or(workspace.clone());
...
mod tests {
    ...
    async fn claude_code_blocks_rate_limited() {
        ...
        let tool = ClaudeCodeTool::new(security, test_runtime(), test_config());
        let result = tool
