...
impl HttpRequestTool {
    ...
    fn validate_url(&self, raw_url: &str) -> anyhow::Result<String> {
        ...
        let normalized = normalize_allowed_domains(self.config.allowed_domains.clone());
        if !host_matches_allowlist(&host, &normalized) {
...
mod tests {
    ...
    fn test_tool(allowed_domains: Vec<&str>) -> HttpRequestTool {
        ...
        HttpRequestTool::new(security, test_config(allowed_domains))
    }
    ...
    async fn execute_blocks_readonly_mode() {
        ...
        let tool = HttpRequestTool::new(security, test_config(vec!["example.com"]));
        let result = tool
