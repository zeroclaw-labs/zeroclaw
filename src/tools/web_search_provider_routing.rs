...
pub fn resolve_web_search_provider(raw_provider: &str) -> WebSearchProviderResolution {
    ...
    match normalized.as_str() {
        ...
        "gemini" | "google" | "gemini-search" | "google-search" | "google_search" => {
            WebSearchProviderResolution {
                route: WebSearchProviderRoute::Gemini,
                canonical_provider: GEMINI_PROVIDER,
                used_fallback: false,
            }
        }
...
mod tests {
    ...
    #[test]
    fn resolve_aliases_to_gemini() {
        let gemini_aliases = ["gemini", "google", "gemini-search", "google-search", "google_search"];
        for alias in gemini_aliases {
            let resolved = resolve_web_search_provider(alias);
            assert_eq!(resolved.route, WebSearchProviderRoute::Gemini);
            assert_eq!(resolved.canonical_provider, GEMINI_PROVIDER);
            assert!(!resolved.used_fallback);
        }
    }
