...
impl Tool for ComposioTool {
    ...
    fn description(&self) -> &str {
        "Execute actions on 1000+ apps via Composio (Gmail, Notion, GitHub, Slack, etc.). \
         ...
         action='list_accounts' (or 'connected_accounts') to list connected accounts and IDs, \
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        match action {
            ...
            _ => Ok(ToolResult {
                ...
                output: String::new(),
                error: Some(format!(
                    "Unknown action '{action}'. Use 'list', 'list_accounts', 'connected_accounts', 'execute', or 'connect'."
                )),
            }),
        }
    }
