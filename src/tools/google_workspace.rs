...
impl Tool for GoogleWorkspaceTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(sub_resource) = args.get("sub_resource").and_then(|v| v.as_str()) {
            if !sub_resource
                .chars()
                .all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-')
            {
        ...
        if let Some(page_limit) = args.get("page_limit").and_then(|v| v.as_u64()) {
            cmd_args.push("--page-limit".into());
            cmd_args.push(page_limit.to_string());
        }
...
mod tests {
    ...
    async fn rejects_shell_injection_in_service() {
        // Use a custom allowlist that includes the injected name prefix to test
        // the character validation layer specifically (not the allowlist).
