...
pub trait Channel: Send + Sync {
    ...
    /// Used for platforms (WhatsApp, Signal) that do not support message editing.
    fn supports_stream_segments(&self) -> bool {
        false
    }

    /// Minimum character count per streamed segment.  Only meaningful when
    /// `supports_stream_segments()` returns `true`.
