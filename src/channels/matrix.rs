...
impl MatrixChannel {
    ...
    async fn set_typing_state(&self, typing: bool) -> anyhow::Result<()> {
...
impl Channel for MatrixChannel {
        ...
        client.add_event_handler(move |event: OriginalSyncRoomMessageEvent, room: Room| {
            ...
            async move {
                ...
                if let Err(error) = channel
                    .send_read_markers(room.room_id().as_str(), &event_id)
                    .await
                {
                    tracing::debug!("Failed to send Matrix read markers: {error}");
                }
    ...
    async fn start_typing(&self, _recipient: &str) -> anyhow::Result<()> {
        self.set_typing_state(true).await
    }

    async fn stop_typing(&self, _recipient: &str) -> anyhow::Result<()> {
        self.set_typing_state(false).await
    }
}
...
mod tests {
    ...
    async fn send_read_markers_posts_read_and_fully_read() {
        ...
        Mock::given(method("POST"))
            .and(path("/_matrix/client/v3/rooms/%21room%3Amatrix.org/read_markers"))
