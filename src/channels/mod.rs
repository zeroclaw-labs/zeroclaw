...
fn sanitize_channel_response(
    ...
    let detector = match leak_sensitivity {
        Some(s) => crate::security::LeakDetector::with_sensitivity(s),
        None => crate::security::LeakDetector::new(),
    };
    ...
    match llm_result {
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            let sanitized_response = sanitize_channel_response(
                &outbound_response,
                ctx.tools_registry.as_ref(),
                ctx.prompt_config.security.leak_detector_sensitivity,
            );
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn sanitize_channel_response_strips_used_tools_with_leading_whitespace() {
        ...
        let result = sanitize_channel_response(input, &tools, None);
...
This is an example JSON object for profile settings."#;
    ...
    fn sanitize_channel_response_redacts_detected_credentials() {
        ...
        let result = sanitize_channel_response(leaked, &tools, None);
    ...
    fn sanitize_channel_response_passes_clean_text() {
        ...
        let result = sanitize_channel_response(clean_text, &tools, None);
