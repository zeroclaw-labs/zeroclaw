...
fn conversation_history_key(msg: &traits::ChannelMessage) -> String {
    ...
    // gets an independent session.  Include reply_target for per-channel
    // isolation (e.g. distinct Discord/Slack channels) and thread_ts for
    // per-topic isolation in forum groups.
    match &msg.thread_ts {
        Some(tid) => format!(
            "{}_{}_{}_{}",
...
/// Find the best split position in `text` at or after `min_chars` characters.
/// Uses *character* (not byte) positions to stay UTF-8 safe.  Prefers
...
fn find_segment_boundary(text: &str, min_chars: usize) -> Option<usize> {
    ...
    let search = &text[byte_start..];

    // Prefer sentence boundary: `. ` / `! ` / `? ` / `\n`
    let sentence_end = search
        .find(". ")
        .or_else(|| search.find("! "))
        .or_else(|| search.find("? "))
        .or_else(|| search.find('\n'));
    if let Some(offset) = sentence_end {
        ...
        return Some(boundary.min(text.len()));
    }
...
async fn process_channel_message(
    ...
    // Use refreshed system prompt for new sessions (/new support),
    ...
    let use_segment_streaming = target_channel
        .as_ref()
        .is_some_and(|ch| ch.supports_stream_segments());
    let use_streaming = use_draft_streaming || use_segment_streaming;
    ...
    tracing::debug!(
        ...
        use_streaming,
        use_draft_streaming,
        use_segment_streaming,
        "Streaming decision"
    );

    let (delta_tx, mut delta_rx) = if use_streaming {
    ...
    // Draft updater: for channels that support editing messages (Telegram).
    ...
    let segment_updater = if use_segment_streaming {
        if let (Some(rx), Some(channel_ref)) = (delta_rx.take(), target_channel.as_ref()) {
            ...
            let chunk_size = channel.stream_segment_size().max(20);
            Some(tokio::spawn(async move {
                let mut rx = rx;
                    ...
                    while buffer.chars().count() >= chunk_size {
                        if let Some(split) = find_segment_boundary(&buffer, chunk_size) {
                            ...
                            if !segment.is_empty() {
                                let _ = channel
                                    .send(
                                        &SendMessage::new(&segment, &reply_target)
                ...
                if !remaining.is_empty() {
                    let _ = channel
                        .send(
                            &SendMessage::new(&remaining, &reply_target)
                                .in_thread(thread_ts.clone()),
                        )
                        .await;
                }
    ...
    if let Some(ref sig) = config.channels_config.signal {
        channels.push(ConfiguredChannel {
            ...
            channel: Arc::new(
                ...
                .with_streaming(sig.streaming, sig.streaming_chunk_size),
            ),
        });
    }
                ...
                if wa.is_web_config() {
                    channels.push(ConfiguredChannel {
                        ...
                        channel: Arc::new(
                            ...
                            .with_transcription(config.transcription.clone())
                            .with_tts(config.tts.clone())
                            .with_streaming(wa.streaming, wa.streaming_chunk_size),
                        ),
                    });
                } else {
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        verbose_mode: config.agent.verbose_mode,
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            verbose_mode: crate::config::VerboseMode::Off,
            session_store: None,
