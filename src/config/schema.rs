...
/// Top-level JhedaiClaw configuration, loaded from `config.toml`.
///
/// Resolution order: `JHEDAICLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.jhedaiclaw/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `JHEDAICLAW_API_KEY` or `API_KEY` env vars.
    ...
    ///
    /// Can also be set via `JHEDAICLAW_EXTRA_HEADERS` environment variable using
...
fn default_workspaces_dir() -> String {
    "~/.jhedaiclaw/workspaces".to_string()
}
...
fn default_project_intel_report_dir() -> String {
    "~/.jhedaiclaw/project-reports".into()
}
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all JhedaiClaw-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "jhedaiclaw" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "jhedaiclaw_memories".
...
pub struct MemoryConfig {
    ...
    // ── Graph backend options (CozoDB knowledge graph) ───────
    /// Configuration for the CozoDB graph memory backend.
    /// Only used when `backend = "graph"`.
    #[serde(default)]
    pub graph: Option<crate::memory::graph::GraphConfig>,
}
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "jhedaiclaw".
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `JHEDAICLAW_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `JHEDAICLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
...
/// How JhedaiClaw receives events from Feishu / Lark.
///
...
fn default_estop_state_file() -> String {
    "~/.jhedaiclaw/estop-state.json".to_string()
}
...
///
/// When `enabled` is true, JhedaiClaw validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to JhedaiClaw permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to JhedaiClaw tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to jhedaiclaw dir)
...
fn default_report_output_dir() -> String {
    "~/.jhedaiclaw/security-reports".into()
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            workspace_dir: jhedaiclaw_dir.join("workspace"),
            config_path: jhedaiclaw_dir.join("config.toml"),
            api_key: None,
            api_url: None,
            api_path: None,
...
pub(crate) fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".jhedaiclaw"));
...
/// `JHEDAICLAW_CONFIG_DIR` > `JHEDAICLAW_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_jhedaiclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_jhedaiclaw_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "JHEDAICLAW_CONFIG_DIR",
            Self::EnvWorkspace => "JHEDAICLAW_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_jhedaiclaw_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("JHEDAICLAW_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let jhedaiclaw_dir = PathBuf::from(shellexpand::tilde(custom_config_dir).as_ref());
            return Ok((
                jhedaiclaw_dir.clone(),
                jhedaiclaw_dir.join("workspace"),
    ...
    if let Ok(custom_workspace) = std::env::var("JHEDAICLAW_WORKSPACE") {
        if !custom_workspace.is_empty() {
            ...
            let (jhedaiclaw_dir, workspace_dir) =
    ...
    if let Some((jhedaiclaw_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_jhedaiclaw_dir).await?
    {
        return Ok((
            jhedaiclaw_dir,
            workspace_dir,
    ...
    Ok((
        default_jhedaiclaw_dir.to_path_buf(),
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'jhedaiclaw'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "JHEDAICLAW_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `JHEDAICLAW_EXTRA_HEADERS` environment variable value.
///
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in JHEDAICLAW_EXTRA_HEADERS");
                continue;
            }
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (jhedaiclaw_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_jhedaiclaw_dir, &default_workspace_dir).await?;

        let config_path = jhedaiclaw_dir.join("config.toml");

        fs::create_dir_all(&jhedaiclaw_dir)
            .await
            .with_context(|| config_dir_creation_error(&jhedaiclaw_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&jhedaiclaw_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: JHEDAICLAW_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("JHEDAICLAW_API_KEY").or_else(|_| std::env::var("API_KEY")) {
            if !key.is_empty() {
        ...
        // 1) JHEDAICLAW_PROVIDER always wins when set.
        // 2) JHEDAICLAW_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("JHEDAICLAW_PROVIDER") {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        } else if let Ok(provider) =
            std::env::var("JHEDAICLAW_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        // Model: JHEDAICLAW_MODEL or MODEL
        if let Ok(model) = std::env::var("JHEDAICLAW_MODEL").or_else(|_| std::env::var("MODEL")) {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Provider HTTP timeout: JHEDAICLAW_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("JHEDAICLAW_PROVIDER_TIMEOUT_SECS") {
        ...
        // Extra provider headers: JHEDAICLAW_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("JHEDAICLAW_EXTRA_HEADERS") {
        ...
        // Workspace directory: JHEDAICLAW_WORKSPACE
        if let Ok(workspace) = std::env::var("JHEDAICLAW_WORKSPACE") {
        ...
        // Open-skills opt-in flag: JHEDAICLAW_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("JHEDAICLAW_OPEN_SKILLS_ENABLED") {
        ...
        // Open-skills directory override: JHEDAICLAW_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("JHEDAICLAW_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Skills prompt mode override: JHEDAICLAW_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("JHEDAICLAW_SKILLS_PROMPT_MODE") {
            if !mode.trim().is_empty() {
                if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                    self.skills.prompt_injection_mode = parsed;
                } else {
                    tracing::warn!(
                        "Ignoring invalid JHEDAICLAW_SKILLS_PROMPT_MODE (valid: full|compact)"
                    );
                }
            }
        }

        // Gateway port: JHEDAICLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("JHEDAICLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: JHEDAICLAW_GATEWAY_HOST or HOST
        if let Ok(host) =
            std::env::var("JHEDAICLAW_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
        {
            if !host.is_empty() {
        ...
        // Allow public bind: JHEDAICLAW_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("JHEDAICLAW_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: JHEDAICLAW_TEMPERATURE
        if let Ok(temp_str) = std::env::var("JHEDAICLAW_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring JHEDAICLAW_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!(
                        "Ignoring JHEDAICLAW_TEMPERATURE={temp_str:?}: not a valid number"
                    );
                }
            }
        }

        // Reasoning override: JHEDAICLAW_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("JHEDAICLAW_REASONING_ENABLED")
        ...
        // Web search enabled: JHEDAICLAW_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("JHEDAICLAW_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: JHEDAICLAW_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("JHEDAICLAW_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: JHEDAICLAW_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("JHEDAICLAW_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Web search max results: JHEDAICLAW_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("JHEDAICLAW_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: JHEDAICLAW_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("JHEDAICLAW_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): JHEDAICLAW_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("JHEDAICLAW_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): JHEDAICLAW_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("JHEDAICLAW_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: JHEDAICLAW_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("JHEDAICLAW_STORAGE_CONNECT_TIMEOUT_SECS") {
            if let Ok(timeout_secs) = timeout_secs.parse::<u64>() {
                if timeout_secs > 0 {
        ...
        // Proxy enabled flag: JHEDAICLAW_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("JHEDAICLAW_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: JHEDAICLAW_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("JHEDAICLAW_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("JHEDAICLAW_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("JHEDAICLAW_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("JHEDAICLAW_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("JHEDAICLAW_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid JHEDAICLAW_PROXY_SCOPE (valid: environment|jhedaiclaw|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("JHEDAICLAW_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_jhedaiclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (jhedaiclaw_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_jhedaiclaw_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = jhedaiclaw_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let jhedaiclaw_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::security::SecretStore::new(jhedaiclaw_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/jhedaiclaw"));
        assert!(msg.contains("/etc/jhedaiclaw"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("jhedaiclaw"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:jhedaiclaw");
        ...
        assert_eq!(
            headers[1],
            ("X-Title".to_string(), "jhedaiclaw".to_string())
        );
    }

    #[test]
    async fn parse_extra_headers_env_with_url_value() {
        let headers = parse_extra_headers_env("HTTP-Referer:https://github.com/jhedai/jhedaiclaw");
        assert_eq!(headers.len(), 1);
        // Only splits on first colon, preserving URL colons in value
        assert_eq!(headers[0].0, "HTTP-Referer");
        assert_eq!(headers[0].1, "https://github.com/jhedai/jhedaiclaw");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : jhedaiclaw , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("X-Title".to_string(), "jhedaiclaw".to_string())
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:jhedaiclaw,");
        assert_eq!(headers.len(), 1);
        assert_eq!(
            headers[0],
            ("X-Title".to_string(), "jhedaiclaw".to_string())
...
"#;
        ...
        assert_eq!(parsed.extra_headers.get("X-Title").unwrap(), "jhedaiclaw");
    }
...
provider = "postgres"
dbURL = "postgres://postgres:postgres@localhost:5432/jhedaiclaw"
schema = "public"
table = "memories"
...
"#;
        ...
        assert_eq!(
            ...
            Some("postgres://postgres:postgres@localhost:5432/jhedaiclaw")
        );
...
"#;
    ...
    async fn sync_directory_handles_existing_directory() {
        let dir = std::env::temp_dir().join(format!(
            "jhedaiclaw_test_sync_directory_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("jhedaiclaw_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "jhedaiclaw_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("jhedaiclaw_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.jhedaiclaw/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
        };
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "JHEDAICLAW_PROXY_ENABLED",
            "JHEDAICLAW_HTTP_PROXY",
            "JHEDAICLAW_HTTPS_PROXY",
            "JHEDAICLAW_ALL_PROXY",
            "JHEDAICLAW_NO_PROXY",
            "JHEDAICLAW_PROXY_SCOPE",
            "JHEDAICLAW_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_api_key() {
        ...
        std::env::set_var("JHEDAICLAW_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        std::env::remove_var("JHEDAICLAW_API_KEY");
    }
    ...
    async fn env_override_provider() {
        ...
        std::env::set_var("JHEDAICLAW_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        std::env::remove_var("JHEDAICLAW_PROVIDER");
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        std::env::remove_var("JHEDAICLAW_PROVIDER");
        std::env::set_var("JHEDAICLAW_MODEL_PROVIDER", "openai-codex");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        std::env::remove_var("JHEDAICLAW_MODEL_PROVIDER");
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        std::env::set_var("JHEDAICLAW_OPEN_SKILLS_ENABLED", "true");
        std::env::set_var("JHEDAICLAW_OPEN_SKILLS_DIR", "/tmp/open-skills");
        std::env::set_var("JHEDAICLAW_SKILLS_PROMPT_MODE", "compact");
        config.apply_env_overrides();
        ...
        std::env::remove_var("JHEDAICLAW_OPEN_SKILLS_ENABLED");
        std::env::remove_var("JHEDAICLAW_OPEN_SKILLS_DIR");
        std::env::remove_var("JHEDAICLAW_SKILLS_PROMPT_MODE");
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        std::env::set_var("JHEDAICLAW_OPEN_SKILLS_ENABLED", "maybe");
        std::env::set_var("JHEDAICLAW_SKILLS_PROMPT_MODE", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        std::env::set_var("JHEDAICLAW_PROVIDER", "openrouter");
    ...
    async fn env_override_model() {
        ...
        std::env::set_var("JHEDAICLAW_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        std::env::remove_var("JHEDAICLAW_MODEL");
    }
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("jhedaiclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".jhedaiclaw").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        std::env::set_var("HOME", &temp_home);
        std::env::set_var("JHEDAICLAW_WORKSPACE", &workspace_dir);
        ...
        std::env::remove_var("JHEDAICLAW_WORKSPACE");
    ...
    async fn env_override_workspace() {
        ...
        std::env::set_var("JHEDAICLAW_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        std::env::set_var("JHEDAICLAW_CONFIG_DIR", &explicit_config_dir);
        std::env::remove_var("JHEDAICLAW_WORKSPACE");
        ...
        std::env::remove_var("JHEDAICLAW_CONFIG_DIR");
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("jhedaiclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".jhedaiclaw").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".jhedaiclaw");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("jhedaiclaw_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".jhedaiclaw");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        persist_active_workspace_config_dir(&marker_config_dir)
            .await
            .unwrap();
        std::env::set_var("JHEDAICLAW_WORKSPACE", &env_workspace_dir);
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("jhedaiclaw_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".jhedaiclaw");
    ...
    async fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("JHEDAICLAW_PROVIDER", "");
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        std::env::set_var("JHEDAICLAW_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        std::env::remove_var("JHEDAICLAW_GATEWAY_PORT");
    }
    ...
    async fn env_override_gateway_host() {
        ...
        std::env::set_var("JHEDAICLAW_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        std::env::remove_var("JHEDAICLAW_GATEWAY_HOST");
    }
    ...
    async fn env_override_temperature() {
        ...
        std::env::set_var("JHEDAICLAW_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        std::env::remove_var("JHEDAICLAW_TEMPERATURE");
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        std::env::set_var("JHEDAICLAW_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        std::env::set_var("JHEDAICLAW_REASONING_ENABLED", "false");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        std::env::set_var("JHEDAICLAW_REASONING_ENABLED", "true");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        std::env::remove_var("JHEDAICLAW_REASONING_ENABLED");
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        std::env::set_var("JHEDAICLAW_REASONING_ENABLED", "maybe");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_storage_provider_config() {
        ...
        std::env::set_var("JHEDAICLAW_STORAGE_PROVIDER", "postgres");
        std::env::set_var("JHEDAICLAW_STORAGE_DB_URL", "postgres://example/db");
        std::env::set_var("JHEDAICLAW_STORAGE_CONNECT_TIMEOUT_SECS", "15");
        ...
        std::env::remove_var("JHEDAICLAW_STORAGE_PROVIDER");
        std::env::remove_var("JHEDAICLAW_STORAGE_DB_URL");
        std::env::remove_var("JHEDAICLAW_STORAGE_CONNECT_TIMEOUT_SECS");
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        std::env::set_var("JHEDAICLAW_PROXY_ENABLED", "true");
        std::env::set_var("JHEDAICLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var(
            "JHEDAICLAW_PROXY_SERVICES",
            "provider.openai, tool.http_request",
        );
        std::env::set_var("JHEDAICLAW_PROXY_SCOPE", "services");
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        std::env::set_var("JHEDAICLAW_PROXY_ENABLED", "true");
        std::env::set_var("JHEDAICLAW_PROXY_SCOPE", "environment");
        std::env::set_var("JHEDAICLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var("JHEDAICLAW_HTTPS_PROXY", "http://127.0.0.1:7891");
        std::env::set_var("JHEDAICLAW_NO_PROXY", "localhost,127.0.0.1");
...
[security.estop]
enabled = true
state_file = "~/.jhedaiclaw/estop-state.json"
require_otp_to_resume = true
"#,
        )
        .unwrap();
