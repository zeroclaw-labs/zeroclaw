...
pub struct WebSearchConfig {
    ...
    #[serde(default)]
    pub enabled: bool,
    /// Search provider: "duckduckgo" (free), "brave" (requires API key), or "gemini" (requires API key, uses Google Search grounding)
    #[serde(default = "default_web_search_provider")]
    pub provider: String,
    /// Brave Search API key (required if provider is "brave")
    #[serde(default)]
    pub brave_api_key: Option<String>,
    /// Google AI Studio API key (required if provider is "gemini")
    #[serde(default)]
    pub gemini_api_key: Option<String>,
    /// Gemini model for search grounding (default: "gemini-2.5-flash")
    #[serde(default = "default_gemini_search_model")]
...
fn default_gemini_search_model() -> String {
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            brave_api_key: None,
            gemini_api_key: None,
            gemini_model: default_gemini_search_model(),
...
pub struct SignalConfig {
    ...
    /// Stream LLM responses as sequential message segments instead of waiting
    /// for the full response.  Each segment is sent as a new message once the
    /// buffer exceeds `streaming_chunk_size` characters.
    #[serde(default)]
    pub streaming: bool,
    /// Minimum character count per streamed segment (default 200).
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            decrypt_optional_secret(
                &store,
                &mut config.web_search.gemini_api_key,
                "config.web_search.gemini_api_key",
            )?;
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        // Gemini API key: ZEROCLAW_GEMINI_API_KEY or GEMINI_API_KEY
        if let Ok(api_key) =
            std::env::var("ZEROCLAW_GEMINI_API_KEY").or_else(|_| std::env::var("GEMINI_API_KEY"))
        {
            let api_key = api_key.trim();
            if !api_key.is_empty() {
                self.web_search.gemini_api_key = Some(api_key.to_string());
            }
        }

        // Gemini search model: ZEROCLAW_GEMINI_SEARCH_MODEL or GEMINI_SEARCH_MODEL
        if let Ok(model) = std::env::var("ZEROCLAW_GEMINI_SEARCH_MODEL")
            .or_else(|_| std::env::var("GEMINI_SEARCH_MODEL"))
        {
            let model = model.trim();
            if !model.is_empty() {
                self.web_search.gemini_model = model.to_string();
            }
        }
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        encrypt_optional_secret(
            &store,
            &mut config_to_save.web_search.gemini_api_key,
            "config.web_search.gemini_api_key",
        )?;
