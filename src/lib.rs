...
Examples:
  jhedaiclaw gateway start              # use config defaults
  jhedaiclaw gateway start -p 8080      # listen on port 8080
  jhedaiclaw gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  jhedaiclaw gateway start -p 0         # random available port")]
    Start {
...
Examples:
  jhedaiclaw gateway restart            # restart with config defaults
  jhedaiclaw gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  jhedaiclaw gateway get-paircode       # show current pairing code
  jhedaiclaw gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  jhedaiclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  jhedaiclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  jhedaiclaw channel bind-telegram jhedaiclaw_user
...
Examples:
  jhedaiclaw channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  jhedaiclaw channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `JhedaiClaw` workspace
    Openclaw {
...
Examples:
  jhedaiclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York --agent
  jhedaiclaw cron add '*/30 * * * *' 'Check system health' --agent
  jhedaiclaw cron add '*/5 * * * *' 'echo ok'")]
    Add {
        /// Cron expression
        expression: String,
...
Examples:
  jhedaiclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  jhedaiclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  jhedaiclaw cron add-every 60000 'Ping heartbeat'     # every minute
  jhedaiclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  jhedaiclaw cron once 30m 'Run backup in 30 minutes'
  jhedaiclaw cron once 2h 'Follow up on deployment'
  jhedaiclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  jhedaiclaw cron update <task-id> --expression '0 8 * * *'
  jhedaiclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  jhedaiclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  jhedaiclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  jhedaiclaw hardware info
  jhedaiclaw hardware info --chip STM32F401RETx")]
    Info {
...
Examples:
  ...
  jhedaiclaw peripheral add rpi-gpio native
  jhedaiclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash JhedaiClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash JhedaiClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  jhedaiclaw peripheral flash
  jhedaiclaw peripheral flash --port /dev/cu.usbmodem12345
  jhedaiclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash JhedaiClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
