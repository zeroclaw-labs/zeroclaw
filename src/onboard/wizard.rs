use crate::config::{
    ...
    HeartbeatConfig, HttpRequestConfig, MemoryConfig,
    ObservabilityConfig, RuntimeConfig, SecretsConfig, StorageConfig, StreamMode, TelegramConfig,
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    print_step(6, 9, "Web & Internet Tools");
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        let key_url = if canonical_provider_name(provider_name) == "kimi-code" {
...
mod tests {
    ...
    #[test]
    fn channel_menu_choices_include_telegram_discord_webhook() {
        assert!(channel_menu_choices().contains(&ChannelMenuChoice::Telegram));
        assert!(channel_menu_choices().contains(&ChannelMenuChoice::Discord));
        assert!(channel_menu_choices().contains(&ChannelMenuChoice::Webhook));
    }
}
