...
fn print_no_command_help() -> Result<()> {
    ...
    println!("Try `jhedaiclaw onboard` to initialize your workspace.");
    println!();
...
/// `JhedaiClaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "jhedaiclaw")]
#[command(author = "Edison Vasquez")]
#[command(version)]
...
Examples:
  jhedaiclaw agent                              # interactive session
  jhedaiclaw agent -m \"Summarize today's logs\"  # single message
  jhedaiclaw agent -p anthropic --model claude-sonnet-4-20250514
...
Examples:
  jhedaiclaw gateway start              # start gateway
  jhedaiclaw gateway restart            # restart gateway
  jhedaiclaw gateway get-paircode       # show pairing code")]
    Gateway {
        #[command(subcommand)]
        gateway_command: Option<jhedaiclaw::GatewayCommands>,
    },
...
Launches the full JhedaiClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run JhedaiClaw in \
production or as an always-on assistant.

Use 'jhedaiclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  jhedaiclaw daemon                   # use config defaults
  jhedaiclaw daemon -p 9090           # gateway on port 9090
  jhedaiclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
    ...
    /// - `jhedaiclaw estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `jhedaiclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  ...
  jhedaiclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder' --agent
  jhedaiclaw cron add-every 60000 'Ping heartbeat'
  jhedaiclaw cron once 30m 'Run backup in 30 minutes' --agent
  jhedaiclaw cron pause <task-id>
  jhedaiclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, send, and health-check channels that connect JhedaiClaw \
...
Examples:
  ...
  jhedaiclaw channel remove my-bot
  jhedaiclaw channel bind-telegram jhedaiclaw_user
  jhedaiclaw channel send 'Alert!' --channel-id telegram --recipient 123456789")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  jhedaiclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: jhedaiclaw::HardwareCommands,
    },
...
Examples:
  ...
  jhedaiclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: jhedaiclaw::PeripheralCommands,
    },
...
Examples:
  jhedaiclaw memory stats
  jhedaiclaw memory list
  jhedaiclaw memory list --category core --limit 10
  jhedaiclaw memory get <key>
  jhedaiclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Manage JhedaiClaw configuration.
...
Examples:
  jhedaiclaw config schema              # print JSON Schema to stdout
  jhedaiclaw config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `jhedaiclaw`.
...
Examples:
  ...
  jhedaiclaw completions fish > ~/.config/fish/completions/jhedaiclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("JHEDAICLAW_CONFIG_DIR", config_dir);
    }
    ...
    // `jhedaiclaw onboard --api-key …` both take the fast path, while a bare
    // `jhedaiclaw onboard` in a terminal launches the wizard.
    ...
    } = &cli.command
    {
        ...
        if reinit {
            let (jhedaiclaw_dir, _) =
                crate::config::schema::resolve_runtime_dirs_for_onboarding().await?;

            if jhedaiclaw_dir.exists() {
                let timestamp = chrono::Local::now().format("%Y%m%d%H%M%S");
                let backup_dir = format!("{}.backup.{}", jhedaiclaw_dir.display(), timestamp);

                println!("⚠️  Reinitializing JhedaiClaw configuration...");
                println!("   Current config directory: {}", jhedaiclaw_dir.display());
                println!(
                ...
                tokio::fs::rename(&jhedaiclaw_dir, &backup_dir)
                    .await
                    .with_context(|| {
        ...
        if config.gateway.require_pairing {
            ...
            if let Some(code) = pairing.pairing_code() {
                ...
                println!("  \x1b[2mDocs: https://www.jhedaiclaw.ai/docs\x1b[0m");
                println!();
            }
        }

        // Auto-start channels if user said yes during wizard
        if std::env::var("JHEDAICLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for JhedaiClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(jhedaiclaw::GatewayCommands::Restart { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    let addr = format!("{host}:{port}");
                    info!("🔄 Restarting JhedaiClaw Gateway on {addr}");
                ...
                Some(jhedaiclaw::GatewayCommands::GetPaircode { new }) => {
                    ...
                    match fetch_paircode(host, port, new).await {
                        ...
                        Err(e) => {
                            ...
                            println!("     jhedaiclaw gateway start");
                        }
                    }
                    Ok(())
                }
                Some(jhedaiclaw::GatewayCommands::Start { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    log_gateway_start(&host, port);
                    gateway::run_gateway(&host, port, config).await
                }
        ...
        Commands::Daemon { port, host } => {
            ...
            if port == 0 {
                info!("🧠 Starting JhedaiClaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting JhedaiClaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 JhedaiClaw Status");
            println!();
...
fn log_gateway_start(host: &str, port: u16) {
    if port == 0 {
        info!("🚀 Starting JhedaiClaw Gateway on {host} (random port)");
    } else {
        info!("🚀 Starting JhedaiClaw Gateway on {host}:{port}");
    }
}
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `jhedaiclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `jhedaiclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `jhedaiclaw auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `jhedaiclaw auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `jhedaiclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `jhedaiclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["jhedaiclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["jhedaiclaw", "onboard", "--force"])
    ...
    fn onboard_cli_rejects_removed_interactive_flag() {
        ...
        assert!(Cli::try_parse_from(["jhedaiclaw", "onboard", "--interactive"]).is_err());
    }

    #[test]
    fn onboard_cli_bare_parses() {
        let cli =
            Cli::try_parse_from(["jhedaiclaw", "onboard"]).expect("bare onboard should parse");
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["jhedaiclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["jhedaiclaw", "estop", "resume", "--domain", "*.chase.com"])
    ...
    fn agent_command_parses_with_temperature() {
        let cli = Cli::try_parse_from(["jhedaiclaw", "agent", "--temperature", "0.5"])
    ...
    fn agent_command_parses_without_temperature() {
        let cli = Cli::try_parse_from(["jhedaiclaw", "agent", "--message", "hello"])
