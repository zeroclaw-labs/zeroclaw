...
/// Slash-command definitions for interactive-mode tab completion.
/// Each entry: (trigger keywords, display label, description, replacement).
/// Aliases are grouped with their primary command in display.
const SLASH_COMMANDS: &[(&[&str], &str, &str)] = &[
    (&["/help"], "/help", "Show this help message"),
    (&["/clear", "/new"], "/clear /new", "Clear conversation history"),
...
impl Completer for SlashCommandCompleter {
    ...
    fn complete(
        ...
        let matches: Vec<Pair> = SLASH_COMMANDS
            .iter()
            .filter(|(triggers, _, _)| triggers.iter().any(|t| t.starts_with(prefix)))
            .map(|(triggers, label, desc)| {
                // Pick the trigger that matches the prefix as replacement
...
impl Hinter for SlashCommandCompleter {
    ...
    fn hint(&self, line: &str, pos: usize, _ctx: &Context<'_>) -> Option<String> {
        ...
        SLASH_COMMANDS
            ...
            .find(|t| t.starts_with(prefix) && **t != prefix)
