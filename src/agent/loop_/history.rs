...
mod tests {
    ...
    async fn auto_compact_history_does_not_split_tool_run_boundary() {
        ...
        let compacted =
            auto_compact_history(&mut history, &StaticSummaryProvider, "test-model", 21, None, None)
                .await
    ...
    async fn auto_compact_with_memory_stores_durable_facts() {
        ...
        impl Memory for FactCapture {
            ...
            async fn recall(
                &self, _q: &str, _l: usize, _s: Option<&str>,
            ) -> anyhow::Result<Vec<MemoryEntry>> { Ok(vec![]) }
