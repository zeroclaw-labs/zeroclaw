...
impl PerSenderTracker {
    ...
    /// Returns true when count has reached or exceeded max.
...
mod tests {
    ...
    #[test]
    fn per_sender_tracker_zero_max_always_exhausted() {
        let t = PerSenderTracker::new();
        // A max of 0 means no actions are allowed
        assert!(t.is_exhausted("any_key", 0));
    }

    #[test]
    fn per_sender_tracker_clone_is_independent() {
        let t = PerSenderTracker::new();
        t.record_within("chat_a", 10);
        t.record_within("chat_a", 10);
        let cloned = t.clone();
        // Clone has same state
        assert!(cloned.is_exhausted("chat_a", 2));
        // Recording on original doesn't affect clone
        t.record_within("chat_a", 10);
        assert!(!cloned.is_exhausted("chat_a", 3));
        assert!(t.is_exhausted("chat_a", 3));
    }
}
