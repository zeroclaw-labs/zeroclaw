...
// in `~/.config/augusta/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.config/augusta/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(augusta_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: augusta_dir.join(".secret_key"),
            enabled,
        }
    }
