...
// in `~/.jhedaiclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.jhedaiclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(jhedaiclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: jhedaiclaw_dir.join(".secret_key"),
            enabled,
        }
    }
