...
"#;
    ...
    fn detects_generic_api_key() {
        let detector = LeakDetector::new();
        let content = "api_key=abcdefghijklmnopqrstu";
        let result = detector.scan(content);
        match result {
            LeakResult::Detected { patterns, redacted } => {
                assert!(patterns.iter().any(|p| p.contains("Generic API key")));
    ...
    fn detects_aws_secret_access_key() {
        ...
        match result {
            LeakResult::Detected { patterns, .. } => {
                assert!(patterns.iter().any(|p| p.contains("AWS Secret Access Key")));
            }
    ...
    fn high_sensitivity_detects_password() {
        let detector = LeakDetector::new();
        let content = "password=correct_horse_battery";
        let result = detector.scan(content);
        match result {
            LeakResult::Detected { patterns, redacted } => {
