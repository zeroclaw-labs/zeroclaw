...
impl PaymentManager {
    ...
    pub fn new(
        ...
        let conn = if enabled {
            ...
            conn.execute_batch(
                ...
                -- Spec (2026-04-22): unused credits expire 30 days after grant.
                -- `credit_grants` is the per-grant audit + expiration ledger; the
                -- single-row `credit_balances.balance` remains the authoritative
                -- spendable number (O(1) reads). `processed_at` is NULL while the
                -- grant is still live; `sweep_expired_grants` flips it to now()
                -- and subtracts the full `amount` from the balance (clamped to
                -- zero so a user who already spent the grant is not double-hit).
                -- `source` values: 'signup' | 'subscription' | 'topup' | 'promo'.
                ...
                --   auto_recharge_package_id  — 'topup_10' | 'topup_25' | 'topup_50'
    ...
    // ── Credit-grant ledger (spec, 2026-04-22) ───────────────────────
    //
    // Every grant (signup, subscription, one-off top-up, promo) is logged
    // here alongside the balance write so the monthly sweep has a
    // per-grant expiry to walk. `grant_id` is generated by the caller;
    // passing an empty string will auto-generate a UUID so ad-hoc callers
    // don't have to pull `uuid::Uuid::new_v4` in.

    /// Insert a grant row for an existing credit addition. Does NOT touch
    /// the balance — callers are expected to have already added the
    /// credits via `add_bonus_credits`, `complete_payment`, etc.
    pub fn record_grant(
        &self,
        grant_id: &str,
        user_id: &str,
        ...
        conn.execute(
            "INSERT INTO credit_grants
                (grant_id, user_id, amount, source, granted_at, expires_at, processed_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, NULL)",
            params![grant_id, user_id, amount, source, now, expires_at],
        )?;
        Ok(())
    }

    /// Walk the `credit_grants` table, expire every grant whose TTL has
    /// elapsed, and subtract the corresponding amount from the user's
    /// balance (clamped to zero). Returns the number of grants expired
    /// and the aggregate credits clawed back.
    ///
    /// Intentionally simple: we subtract the grant's full amount rather
    /// than tracking "remaining" per grant. A user who already burned
    /// the whole grant is not double-hit because the balance is clamped
    /// at zero inside the SQL UPDATE.
    pub fn sweep_expired_grants(&self) -> anyhow::Result<(u32, u64)> {
        if !self.enabled {
            return Ok((0, 0));
        }
        let Some(ref conn) = self.conn else {
        ...
        let mut stmt = conn.prepare(
            "SELECT grant_id, user_id, amount FROM credit_grants
             WHERE processed_at IS NULL AND expires_at < ?1",
        )?;
        ...
        let mut total_clawed: u64 = 0;
        for (grant_id, user_id, amount) in &rows {
            // Clamp the balance at zero so a user who already spent the
            // grant is not driven negative.
            conn.execute(
                "UPDATE credit_balances
                 SET balance = CASE WHEN balance >= ?2 THEN balance - ?2 ELSE 0 END,
                     updated_at = ?3
                 WHERE user_id = ?1",
                params![user_id, amount, now],
            )?;
            conn.execute(
                "UPDATE credit_grants SET processed_at = ?2 WHERE grant_id = ?1",
                params![grant_id, now],
            )?;
            total_clawed = total_clawed.saturating_add(*amount as u64);
        }
        Ok((rows.len() as u32, total_clawed))
    }

    // ── Billing preferences (spec, 2026-04-22) ───────────────────────

    /// Fetch the user's billing preferences, inserting default rows on
    /// the fly when the user has never customised them. Default values
    /// are keyed to the free-tier experience: 5,000-credit low-balance
    /// threshold and auto-recharge disabled.
    pub fn get_billing_preferences(&self, user_id: &str) -> anyhow::Result<BillingPreferences> {
        let Some(ref conn) = self.conn else {
            anyhow::bail!("Payment database not initialized");
        };
        let now = now_epoch();
        conn.execute(
            "INSERT OR IGNORE INTO billing_preferences (user_id, updated_at) VALUES (?1, ?2)",
            params![user_id, now],
        )?;
        let row = conn.query_row(
            ...
            params![user_id],
            |r| {
                Ok(BillingPreferences {
                    user_id: user_id.to_string(),
                    low_balance_threshold: r.get::<_, u32>(0)?,
                    auto_recharge_enabled: r.get::<_, i64>(1)? != 0,
                    auto_recharge_package_id: r.get::<_, Option<String>>(2)?,
    ...
    /// Upsert the user's billing preferences. Validates `low_balance_threshold`
    /// and `auto_recharge_threshold` against the spec-allowed pair (3_000 or
    /// 5_000) and `auto_recharge_package_id` against the whitelist.
    pub fn set_billing_preferences(&self, prefs: &BillingPreferences) -> anyhow::Result<()> {
        const ALLOWED_THRESHOLDS: [u32; 2] = [3_000, 5_000];
        if !ALLOWED_THRESHOLDS.contains(&prefs.low_balance_threshold) {
            anyhow::bail!("low_balance_threshold must be 3000 or 5000");
        }
        if !ALLOWED_THRESHOLDS.contains(&prefs.auto_recharge_threshold) {
            anyhow::bail!("auto_recharge_threshold must be 3000 or 5000");
        }
        if let Some(ref id) = prefs.auto_recharge_package_id {
            if !crate::billing::AUTO_RECHARGE_PACKAGE_IDS.contains(&id.as_str()) {
                anyhow::bail!(
                    "auto_recharge_package_id must be one of topup_10 / topup_25 / topup_50"
                );
            }
        }
        let Some(ref conn) = self.conn else {
        ...
        conn.execute(
            "INSERT INTO billing_preferences
             ...
             ON CONFLICT(user_id) DO UPDATE SET
                 ...
                 auto_recharge_package_id = excluded.auto_recharge_package_id,
            ...
            params![
                ...
                if prefs.auto_recharge_enabled { 1 } else { 0 },
                prefs.auto_recharge_package_id,
...
/// `billing_preferences` schema). Serialized verbatim by the
/// `GET/PUT /api/billing/preferences` handlers.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingPreferences {
    pub user_id: String,
    pub low_balance_threshold: u32,
    pub auto_recharge_enabled: bool,
    pub auto_recharge_package_id: Option<String>,
...
/// grant on each billing cycle (see `billing::subscription`).
pub const GRANT_TTL_SECS_30D: i64 = 30 * 24 * 60 * 60;
