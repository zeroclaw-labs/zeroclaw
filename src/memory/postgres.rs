...
impl Memory for PostgresMemory {
    ...
    async fn recall(
        ...
        tokio::task::spawn_blocking(move || -> Result<Vec<MemoryEntry>> {
            ...
            // Build optional time filter. created_at is TIMESTAMPTZ, ISO strings work.
            let (time_filter, param_count): (String, i32) = match (since_ref, until_ref) {
                (Some(_), Some(_)) => (
                    " AND created_at >= $4::TIMESTAMPTZ AND created_at <= $5::TIMESTAMPTZ".into(),
                    5,
                ),
                (Some(_), None) => (" AND created_at >= $4::TIMESTAMPTZ".into(), 4),
                (None, Some(_)) => (" AND created_at <= $4::TIMESTAMPTZ".into(), 4),
                (None, None) => (String::new(), 3),
            };
