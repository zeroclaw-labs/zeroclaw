...
pub struct AppState {
    ...
    /// Shared security policy (used for runtime `allowed_roots` mutations)
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            "/api/config/tool-api-key",
            put(api::handle_api_config_tool_api_key_put),
        )
        .route("/api/tools", get(api::handle_api_tools))
        .route(
            "/api/workspace",
            get(api::handle_api_workspace_get).put(api::handle_api_workspace_put),
        )
        .route(
            "/api/workspace/folder",
            put(api::handle_api_workspace_folder_put),
        )
        .route(
            "/api/workspace/folders",
            get(api::handle_api_workspace_folders_get),
        )
