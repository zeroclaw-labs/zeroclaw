...
pub async fn handle_api_cron_add(
    ...
    let result = match job_type.as_deref().unwrap_or("shell") {
        "shell" => {
            let Some(command) = command.as_deref().filter(|value| !value.trim().is_empty()) else {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({"error": "Missing command for shell cron job"})),
                )
                    .into_response();
            };
        ...
        "agent" => {
            let Some(prompt) = prompt.as_deref().filter(|value| !value.trim().is_empty()) else {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({"error": "Missing prompt for agent cron job"})),
                )
                    .into_response();
            };
        ...
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": format!("Invalid job_type: {other}")})),
            )
                .into_response();
        }
    };
