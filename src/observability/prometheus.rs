...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let agent_starts = IntCounterVec::new(
            prometheus::Opts::new("jhedaiclaw_agent_starts_total", "Total agent invocations"),
            &["provider", "model"],
        )
        .expect("valid metric");
        ...
        let tokens_output_total = IntCounterVec::new(
            prometheus::Opts::new(
                "jhedaiclaw_tokens_output_total",
        ...
        let tool_calls = IntCounterVec::new(
            prometheus::Opts::new("jhedaiclaw_tool_calls_total", "Total tool calls"),
            &["tool", "success"],
        )
        .expect("valid metric");
        ...
        let channel_messages = IntCounterVec::new(
            prometheus::Opts::new(
                "jhedaiclaw_channel_messages_total",
        ...
        let heartbeat_ticks = prometheus::IntCounter::new(
            "jhedaiclaw_heartbeat_ticks_total",
            "Total heartbeat ticks",
        )
        .expect("valid metric");

        let errors = IntCounterVec::new(
            prometheus::Opts::new("jhedaiclaw_errors_total", "Total errors by component"),
            &["component"],
        )
        .expect("valid metric");
        ...
        let agent_duration = HistogramVec::new(
            HistogramOpts::new(
                "jhedaiclaw_agent_duration_seconds",
        ...
        let tool_duration = HistogramVec::new(
            HistogramOpts::new(
                "jhedaiclaw_tool_duration_seconds",
        ...
        let request_latency = Histogram::with_opts(
            HistogramOpts::new(
                "jhedaiclaw_request_latency_seconds",
        ...
        let tokens_used = prometheus::IntGauge::new(
            "jhedaiclaw_tokens_used_last",
            "Tokens used in the last request",
        )
        .expect("valid metric");

        let active_sessions = GaugeVec::new(
            prometheus::Opts::new("jhedaiclaw_active_sessions", "Number of active sessions"),
            &[],
        )
        .expect("valid metric");

        let queue_depth = GaugeVec::new(
            prometheus::Opts::new("jhedaiclaw_queue_depth", "Message queue depth"),
            &[],
        )
        .expect("valid metric");
...
mod tests {
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("jhedaiclaw_agent_starts_total"));
        assert!(output.contains("jhedaiclaw_tool_calls_total"));
        assert!(output.contains("jhedaiclaw_heartbeat_ticks_total"));
        assert!(output.contains("jhedaiclaw_request_latency_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("jhedaiclaw_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"jhedaiclaw_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"jhedaiclaw_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"jhedaiclaw_errors_total{component="provider"} 2"#));
        assert!(output.contains(r#"jhedaiclaw_errors_total{component="channels"} 1"#));
    }
    ...
    fn gauge_reflects_latest_value() {
        ...
        let output = obs.encode();
        assert!(output.contains("jhedaiclaw_tokens_used_last 200"));
    }
    ...
    fn llm_response_tracks_request_count_and_tokens() {
        ...
        assert!(output.contains(
            r#"jhedaiclaw_llm_requests_total{model="claude-sonnet",provider="openrouter",success="true"} 2"#
        ));
        assert!(output.contains(
            r#"jhedaiclaw_tokens_input_total{model="claude-sonnet",provider="openrouter"} 300"#
        ));
        assert!(output.contains(
            r#"jhedaiclaw_tokens_output_total{model="claude-sonnet",provider="openrouter"} 130"#
        ));
    }
    ...
    fn llm_response_without_tokens_increments_request_only() {
        ...
        let output = obs.encode();
        assert!(output.contains(
            r#"jhedaiclaw_llm_requests_total{model="llama3",provider="ollama",success="false"} 1"#
        ));
        // Token counters should not appear (no data recorded)
        assert!(!output.contains("jhedaiclaw_tokens_input_total{"));
        assert!(!output.contains("jhedaiclaw_tokens_output_total{"));
    }
}
