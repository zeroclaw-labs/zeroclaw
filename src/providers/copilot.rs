...
/// Tokens are cached to `~/.config/jhedaiclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "jhedaiclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("jhedaiclaw-copilot-{user}"))
            });
