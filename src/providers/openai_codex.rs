...
fn resolve_reasoning_effort(configured_effort: Option<&str>, model_id: &str) -> String {
    let raw = std::env::var("ZEROCLAW_CODEX_REASONING_EFFORT")
        .ok()
        .and_then(|value| first_nonempty(Some(&value)))
        .or_else(|| first_nonempty(configured_effort))
...
mod tests {
    ...
    #[test]
    fn resolve_reasoning_effort_uses_runtime_option_when_env_missing() {
        let _guard = EnvGuard::set("ZEROCLAW_CODEX_REASONING_EFFORT", None);
        assert_eq!(
            resolve_reasoning_effort(Some("low"), "gpt-5-codex"),
            "low".to_string()
        );
    }

    #[test]
    fn resolve_reasoning_effort_env_override_wins_over_runtime_option() {
        let _guard = EnvGuard::set("ZEROCLAW_CODEX_REASONING_EFFORT", Some("medium"));
        assert_eq!(
            resolve_reasoning_effort(Some("low"), "gpt-5-codex"),
            "medium".to_string()
        );
