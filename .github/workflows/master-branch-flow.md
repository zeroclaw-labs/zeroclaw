...
| Event | What runs |
|---|---|
| PR opened or updated against `master` | `ci.yml` (full required lint + test + build plus advisory Windows scope measurement), `pr-path-labeler.yml`, `pr-size-labeler.yml`, and report-only `pr-risk-labeler.yml`; `platform-tests.yml` only when that workflow changes |
