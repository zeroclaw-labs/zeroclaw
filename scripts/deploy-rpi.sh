...
echo ""
echo "==> Stopping quantclaw service (if running)"
...
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/quantclaw.service ${SERVICE_DEST} && \
   ...
   sudo systemctl status quantclaw --no-pager || true"
...
# ── 8. Reset ACT LED trigger so QuantClaw can control it ──────────────────────
echo ""
...
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u quantclaw -f'"
