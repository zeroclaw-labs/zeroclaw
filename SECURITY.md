...
Aria implements defense-in-depth security:
...
Aria Docker images follow CIS Docker Benchmark best practices:
...
docker build -t afw .
docker inspect --format='{{.Config.User}}' afw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace afw gateway
```
