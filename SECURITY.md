...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/DeliveryBoyTech/daemonclaw/security/advisories/new)
...
DaemonClaw implements defense-in-depth security:
...
DaemonClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t daemonclaw .
docker inspect --format='{{.Config.User}}' daemonclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace daemonclaw gateway
```
