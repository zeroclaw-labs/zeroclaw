...
RUN --mount=type=cache,id=quantclaw-cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,id=quantclaw-cargo-git,target=/usr/local/cargo/git,sharing=locked \
    --mount=type=cache,id=quantclaw-target,target=/app/target,sharing=locked \
    if [ -n "$QUANTCLAW_CARGO_FEATURES" ]; then \
      cargo build --release --locked --features "$QUANTCLAW_CARGO_FEATURES"; \
    else \
...
RUN --mount=type=cache,id=quantclaw-cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,id=quantclaw-cargo-git,target=/usr/local/cargo/git,sharing=locked \
    --mount=type=cache,id=quantclaw-target,target=/app/target,sharing=locked \
    rm -rf target/release/.fingerprint/quantspeed-* \
           target/release/deps/quantspeed-* \
           target/release/incremental/quantspeed-* && \
    if [ -n "$QUANTCLAW_CARGO_FEATURES" ]; then \
      cargo build --release --locked --features "$QUANTCLAW_CARGO_FEATURES"; \
    else \
      cargo build --release --locked; \
...
RUN mkdir -p /quantclaw-data/.quantclaw /quantclaw-data/workspace && \
    printf '%s\n' \
        ...
        'web_dist_dir = "/quantclaw-data/web/dist"' \
        '' \
        '[autonomy]' \
        'level = "supervised"' \
...
COPY --from=builder /quantclaw-data /quantclaw-data
COPY --from=builder /app/quantclaw /usr/local/bin/quantclaw
COPY --from=web-builder /web/dist /quantclaw-data/web/dist
