...
const DOCS_BASE: &str = "https://www.quantspeed.ai/docs";
...
impl App {
    fn new() -> Self {
        ...
        let port = std::env::var("QUANTCLAW_GATEWAY_PORT")
        ...
        let host =
            std::env::var("QUANTCLAW_GATEWAY_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
    ...
    /// Run `quantclaw gateway get-paircode --new` locally to generate a code.
    async fn generate_code_via_cli() -> Option<String> {
        let output = tokio::process::Command::new("quantclaw")
    ...
    /// Run `docker exec <container> quantclaw gateway get-paircode --new`.
    async fn generate_code_via_docker() -> Option<String> {
        // Find quantclaw container
        let ps = tokio::process::Command::new("docker")
            .args([
                "ps",
                "--filter",
                "ancestor=ghcr.io/quant-speed/quantclaw",
                "--format",
                "{{.Names}}",
            ])
            .output()
        ...
        if container.is_empty() {
            ...
            let ps2 = tokio::process::Command::new("docker")
                .args(["ps", "--filter", "name=quantclaw", "--format", "{{.Names}}"])
                .output()
                .await
                .ok()?;
    ...
    if app.screen == Screen::Complete {
        ...
        match save_tui_config(&app).await {
            Ok(()) => {
                ...
                println!();
                println!("  \u{1f980} QuantClaw {VERSION} configured successfully!");
                println!(
                    "     Provider:   {} ({})",
                ...
                if channel != "Skip for now" {
                    ...
                    println!("        quantclaw config edit");
                    println!();
                }
                println!("  Run `quantclaw daemon` to start your agent.");
                println!();
            }
            Err(e) => {
                eprintln!();
                eprintln!("  \u{2717} Failed to save configuration: {e}");
                eprintln!("  You can re-run: quantclaw onboard --tui");
                eprintln!();
            }
        }
    }
...
fn apply_tui_selections_to_config(app: &App, config: &mut Config) {
    ...
    // real tokens via `quantclaw config edit` or the dashboard.
    let channel = app.selected_channel();
    match channel {
        "Telegram" => {
        ...
        "IRC" => {
            if config.channels_config.irc.is_none() {
                config.channels_config.irc = Some(IrcConfig {
                    ...
                    port: 6697,
                    nickname: String::from("quantclaw-bot"),
                    username: None,
...
/// If a QuantClaw Docker container is running, reconfigure it via `docker exec`.
async fn push_config_to_docker(app: &App) {
    ...
    // Use `quantclaw onboard --quick` inside the container to reconfigure
    let mut args = vec![
        "exec".to_string(),
        container,
        "quantclaw".to_string(),
...
fn render(frame: &mut Frame, app: &App) {
    ...
    let version_line = Line::from(vec![
        ...
        Span::styled(format!("QuantClaw {VERSION}"), theme::heading_style()),
        Span::styled(
...
fn setup_title() -> Paragraph<'static> {
    Paragraph::new(Line::from(vec![
        ...
        Span::styled("QuantClaw setup", theme::heading_style()),
    ]))
}
...
fn render_welcome(frame: &mut Frame, area: Rect) {
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Welcome to QuantClaw \u{2014} the fastest, smallest AI assistant.",
                theme::body_style(),
            ),
        ]),
        Line::from(vec![
...
fn render_security(frame: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "QuantClaw is optimized for single-operator deployments.",
            theme::body_style(),
        )),
        Line::from(Span::styled(
            "By default, QuantClaw is a personal agent: one trusted operator",
            theme::body_style(),
        )),
        ...
        Line::from(""),
        Line::from(Span::styled(
            "QuantClaw is not a hostile multi-tenant boundary by default.",
            theme::body_style(),
        )),
        Line::from(Span::styled(
...
fn render_how_channels_work(frame: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "  Approve with: quantclaw pairing approve <channel> <code>",
            theme::dim_style(),
        )),
        Line::from(Span::styled(
        ...
        Line::from(Span::styled(
            "  Multi-user DMs: run: quantclaw config set session.dmScope",
            theme::body_style(),
        )),
        Line::from(Span::styled(
    ...
    frame.render_widget(
        InfoPanel {
            ...
            lines: vec![
                Line::from(""),
                Line::from(Span::styled(
                    "  What now: https://quantspeed.ai/showcase",
                    theme::body_style(),
                )),
                Line::from(Span::styled(
...
fn render_complete(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let mut summary_lines = vec![
        Line::from(""),
        Line::from(Span::styled(
            "  \u{1f980} QuantClaw configured successfully!",
    ...
    summary_lines.extend([
        Line::from(""),
        Line::from(Span::styled(
            "  Run `quantclaw daemon` to start your agent.",
            theme::body_style(),
        )),
        Line::from(Span::styled(
            "  Run `quantclaw doctor` to validate your setup.",
            theme::body_style(),
        )),
        Line::from(""),
    ]);
...
mod tests {
    ...
    fn save_channel_irc() {
        ...
        assert_eq!(irc.nickname, "quantclaw-bot");
    }
