...
mod tests {
    ...
    async fn recall_decodes_category_from_tags() {
        ...
        // so dedup::daily_candidates can actually see Daily candidates instead
        // of every row reading back as Core.
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/v1/default/banks/zeroclaw-test/memories/recall"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
        ...
        // The whole point: the Daily gate can now find the Daily candidate.
