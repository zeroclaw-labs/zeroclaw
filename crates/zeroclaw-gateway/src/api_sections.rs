...
use zeroclaw_runtime::rpc::types::{
    ...
    ConfigSectionsResult, ConfigStatusResult,
};

use super::AppState;
use super::api::require_auth;
use super::api_config::persist_and_swap;
pub use zeroclaw_runtime::config_ops::sections::{
    PickerDispatch, agents_picker, any_usable_model_provider, apply_first_run_agent_defaults,
    apply_section_select, mark_section_completed, memory_picker, model_provider_alias_usable,
    one_tier_alias_map_picker, picker_items_for, provider_type_badge, providers_picker,
    schema_walk_picker, section_picker, select_alias, storage_picker, tunnel_provider_picker,
};
...
pub use zeroclaw_runtime::config_ops::agent_options::{AgentOptionsResponse, build_agent_options};
...
pub async fn handle_section_picker(
    ...
    let cfg = state.config.read().clone();
    match section_picker(&section, &cfg) {
        Ok(picker) => axum::Json(picker).into_response(),
...
pub async fn handle_section_select(
    ...
    let alias = select_alias(body.as_ref().and_then(|b| b.0.alias.as_deref()));
    ...
    let outcome = match apply_section_select(&mut working, &section, &key, &alias).await {
        Ok(outcome) => outcome,
        Err(error) => return error_response(error),
    };
    if outcome.needs_persist
        && let Err(e) = persist_and_swap(&state, working, &_cfg_guard).await
    {
        return error_response(e);
    }
    axum::Json(outcome.response).into_response()
}
