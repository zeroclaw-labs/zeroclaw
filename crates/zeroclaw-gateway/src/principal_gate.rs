...
impl GatewayInboundAuth {
    ...
    async fn authenticate_scoped(
        ...
        if let Err(error) = self.inner.refresh_from_config(&config) {
            // The live config's auth sections are invalid: nothing was
            // installed, so authenticate against the last valid policy
            ...
            ::zeroclaw_log::record!(
                ...
                "live authorization config is invalid; authenticating against the last valid policy"
            );
        }
        self.inner
            .authenticate(
