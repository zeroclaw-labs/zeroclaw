...
impl PluginHost {
    ...
    /// Build the per-instance network config for `name`. `service_key`
    /// defaults to `plugin.<name>` (matching the existing `"channel.<alias>"`
    /// convention native channels use with `ProxyConfig`'s `scope = services`
    ...
    /// can be instantiated. `service_key`/`proxy_url` resolve the per-instance
    /// `PluginNetworkConfig`.
    #[cfg(feature = "plugins-wasmtime")]
    pub async fn instantiate_channel_plugin(
        ...
        let (bytes, permissions) = self
            ...
            .await?;
        let network_config = self.network_config_for(name, service_key, proxy_url);
    ...
    pub async fn instantiate_tool_plugin(&self, name: &str) -> Result<Arc<dyn Tool>, PluginError> {
        let (bytes, permissions) = self
            ...
            .await?;
        let network_config = self.network_config_for(name, None, None);
