...
/// Callback invoked after each plugin tool execution for audit logging.
///
/// Arguments: plugin_name, plugin_version, tool_name, export_name, success,
///            duration_ms, error message (if any), redacted input JSON, raw args, result
pub type AuditCallback = Box<
    dyn Fn(
            /* plugin_name */ &str,
            /* plugin_version */ &str,
            /* tool_name */ &str,
...
/// Callback invoked before execution to check security policy.
/// Returns `Ok(())` if allowed, or `Err(reason)` if blocked.
pub type SecurityCheckCallback =
    Box<dyn Fn(/* tool_name */ &str) -> Result<(), String> + Send + Sync>;
...
pub struct WasmTool {
    ...
    audit_callback: Option<Arc<AuditCallback>>,
    security_check: Option<Arc<SecurityCheckCallback>>,
}
...
impl WasmTool {
    ...
    /// Attach an audit callback invoked after every plugin execution.
    pub fn with_audit_callback(mut self, cb: Arc<AuditCallback>) -> Self {
        self.audit_callback = Some(cb);
        self
    }

    /// Attach a security policy check invoked before execution.
    pub fn with_security_check(mut self, cb: Arc<SecurityCheckCallback>) -> Self {
        self.security_check = Some(cb);
        self
    }
}
...
impl Tool for WasmTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        // Enforce security policy if a check is attached.
        if let Some(ref check) = self.security_check
            && let Err(error) = check(&self.name)
        {
        ...
        if let (Some(cb), Ok(tool_result)) = (&self.audit_callback, &result) {
