...
fn new_channel_store(
    ...
    crate::component::new_store(PluginStoreSpec::new(scope, limits).with_inbound(inbound))
}
...
impl WasmChannel {
    pub async fn from_wasm(
        ...
        // Channel outbound HTTP is intentionally unavailable here. Granting it
        // would attach the ungoverned `default_hooks()` egress surface (the
        // egress SSRF hole, issue 9395); `new_channel_store` withholds it, so
        // `http` is always false and the guest never links `wasi:http`. The
        // `new_channel_store` doc carries the full rationale and the issue 9582
        // follow-up.
        let mut store = new_channel_store(endpoint.scope().clone(), limits, inbound.clone());
