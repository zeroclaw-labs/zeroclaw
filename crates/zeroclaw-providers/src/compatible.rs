...
mod tests {
    ...
    fn convert_tool_specs_returns_none_for_empty_slice() {
        ...
        let result = OpenAiCompatibleModelProvider::convert_tool_specs(Some(empty));
        assert!(
            result.is_none(),
    ...
    fn convert_tool_specs_returns_none_for_none_input() {
        let result = OpenAiCompatibleModelProvider::convert_tool_specs(None);
    ...
    fn request_omits_tool_choice_when_tools_empty() {
        ...
        let tools = OpenAiCompatibleModelProvider::convert_tool_specs(Some(&[]));
        let req = ApiChatRequest {
            model: "test-model".to_string(),
            messages: vec![Message {
                role: "user".to_string(),
