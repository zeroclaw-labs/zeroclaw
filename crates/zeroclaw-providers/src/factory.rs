...
pub trait CompatFamilySpec {
    ...
    /// [`crate::catalog`]).
    ///
    /// Opt in only with evidence, and only alongside a
    /// [`crate::factory::get_default_url`] entry — a probeable family with no
    /// resolvable default endpoint would silently answer `None` for every
    /// operator who did not set `uri` by hand. `every_probeable_family_resolves_a_default_catalog_url`
...
/// model catalog, or `None` when that family is not probeable — per
/// [`FamilyProviderFactory::MODEL_CONTEXT_CATALOG_AUTH`].
///
...
mod tests {
    ...
    const PROBEABLE_FAMILIES: [&str; 8] = [
        "together",
        "groq",
        "fireworks",
        "deepinfra",
    ...
    fn every_probeable_family_resolves_a_default_catalog_url() {
        ...
        for family in zeroclaw_config::providers::ModelProviders::slot_names() {
            ...
            assert!(
                get_default_url(family).is_some(),
