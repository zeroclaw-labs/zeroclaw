...
impl WebFetchTool {
    ...
    async fn deliver(&self, url: &str, content: FetchedContent) -> ToolResult {
        ...
        // Inline fallback. A body the decoder stopped early keeps its
        // truncation marker even when the converted text lands under the
        // display cap, so the reader is never told a short body was complete.
        let mut output = self.truncate_response(&content.text);
        if content.cap_hit && !output.ends_with(TRUNCATION_MARKER) {
    ...
    /// much of it there was. A hop the redirect policy denied is never retried
    /// through Firecrawl (see `build_redirect_guarded_client`).
...
impl Tool for WebFetchTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let mut standard_attempt = self.standard_fetch(&client, &url).await;
        if let Some(variable) = ignored_environment_proxy
            && let FetchAttempt::Failure(failure) = &mut standard_attempt
        {
            let note =
        ...
        let attempt = if self.should_fallback_to_firecrawl(
            &standard_attempt,
...
mod tests {
    ...
    async fn standard_fetch_with_zero_limit_returns_full_body_and_skips_firecrawl_fallback() {
        ...
        assert!(
            !tool.should_fallback_to_firecrawl(&standard_attempt, false),
    ...
    async fn fetch_encoded(encoding: &str, body: Vec<u8>, max_response_size: usize) -> ToolResult {
        ...
        // past the SSRF guard, then run the same delivery step execute() runs
        // so these assertions still read the finished tool result. The client
        // does no decoding; `web_fetch` decodes the body in `http_decode` from
    ...
    async fn standard_fetch_rejects_repeated_content_encoding_lines() {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .expect("reqwest client");
        let result = match tool.standard_fetch(&client, &url).await {
            FetchAttempt::Content(content) => tool.deliver(&url, content).await,
            FetchAttempt::Failure(result) => result,
        };
    ...
    async fn cross_host_redirect_is_denied_and_never_falls_back_to_firecrawl() {
        ...
        let attempt = tool.standard_fetch(&guarded.client, &target.url).await;
        ...
        assert_eq!(
            server_b.hits(),
            0,
            "cross-host redirect was followed to the off-host target; attempt={attempt:?}"
        );
        assert!(
            matches!(attempt, FetchAttempt::Failure(_)),
        ...
        assert!(
            !tool.should_fallback_to_firecrawl(&attempt, rejected),
    ...
    async fn same_host_redirect_is_followed_without_tripping_the_policy_flag() {
        ...
        assert!(
            matches!(attempt, FetchAttempt::Content(_)),
            "same-host redirect must still be followed; attempt={attempt:?}"
        );
        assert_eq!(
            attempt_text(&attempt),
            body,
            "must return the redirected body"
        );
        assert!(
            !rejected,
    ...
    fn fallback_disabled_when_firecrawl_not_enabled() {
        ...
        assert!(!tool.should_fallback_to_firecrawl(&attempt, false));
    }
    ...
    fn redirect_policy_rejection_never_falls_back_to_firecrawl() {
        ...
        let attempt = FetchAttempt::failure("Blocked redirect target".into());

        assert!(!tool.should_fallback_to_firecrawl(&attempt, true));
    }
    ...
    fn fallback_triggers_on_empty_body() {
        ...
        assert!(tool.should_fallback_to_firecrawl(&text_attempt(""), false));
    }
    ...
    fn fallback_triggers_on_short_body() {
        ...
        assert!(tool.should_fallback_to_firecrawl(&text_attempt("Loading..."), false));
    }
    ...
    fn fallback_skipped_on_good_response() {
        ...
        assert!(!tool.should_fallback_to_firecrawl(&text_attempt(&"A".repeat(200)), false));
    }
    ...
    fn fallback_triggers_at_exactly_99_chars() {
        ...
        assert!(
            tool.should_fallback_to_firecrawl(&text_attempt(&"A".repeat(99)), false),
    ...
    fn fallback_skipped_at_exactly_100_chars() {
        ...
        assert!(
            !tool.should_fallback_to_firecrawl(&text_attempt(&"A".repeat(100)), false),
    ...
    async fn execute_double_failure_returns_original_result() {
        ...
        assert!(tool.should_fallback_to_firecrawl(&standard_attempt, false));
    ...
    async fn spill_message_flags_stream_cap_truncation() {
        ...
        // The stream cap still bounds what was written: the decoded read is
        // truncated to exactly `max_response_size`, so the file holds 60_000
        // bytes of response (plus the line breaks hard-wrapping inserted),
        // not 80_000.
    ...
    async fn spilling_cannot_trigger_a_firecrawl_fallback() {
        ...
        assert!(
            !tool.should_fallback_to_firecrawl(&attempt, false),
