...
/// Bocha, AnySearch and Keenable keys have no boot-time snapshot at all — they
...
/// takes effect without a restart. The one value AnySearch and Keenable carry
/// is the schema-mirror env override (e.g.
/// `ZEROCLAW_web_search__keenable_api_key`), which the config loader applies
/// in memory only and which never appears on disk.
