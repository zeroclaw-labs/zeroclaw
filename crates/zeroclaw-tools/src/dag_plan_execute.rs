...
impl DagPlanExecuteTool {
    ...
    fn validate(
        ...
        if request.max_parallel == 0 {
            return Err(DagPlanExecuteError::InvalidMaxParallel);
        }
    ...
    async fn execute_task_with_snapshot(
        ...
        // All child execution goes through the runtime-owned dispatch
        // callback.  There is no raw tool.execute() fallback so every
        // child call respects the dispatch boundary (exclusion filtering,
        // future approval/cancellation/observer integration).
        let dispatch = self
            .child_dispatch
            .as_ref()
            .ok_or_else(|| anyhow::Error::msg("dag_plan_execute: no child dispatch configured"))?;
        let tool_result = dispatch(task.tool.clone(), interpolated_args).await?;
