...
fn select_interface(
    ...
    if let Some(card) = card {
        ...
        // shape) is valid only for a genuine standard single-agent card whose
        ...
        // defect (the route is then cached under the caller-supplied id). A
        // card that declares ANY non-JSON-RPC interface is treated as a
        // catalog/aggregate and requires an agent match; a card of only
        // JSON-RPC interfaces keeps the permissive opaque-agent behavior.
        let declares_aggregate_catalog = card
            .supported_interfaces
            .iter()
            .any(|i| !i.protocol_binding.eq_ignore_ascii_case("jsonrpc"));
        if v1_candidates.len() == 1 && !declares_aggregate_catalog {
