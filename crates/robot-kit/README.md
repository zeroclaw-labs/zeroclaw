...
A complete toolkit for building AI-powered robots with QuantClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build QuantClaw with robot tools

```bash
# Clone and build
git clone https://github.com/quant-speed/quantclaw
cd quantclaw
cargo build -p quantclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.quantclaw
cp crates/robot-kit/robot.toml ~/.quantclaw/
cp crates/robot-kit/SOUL.md ~/.quantclaw/workspace/
...
# Test in mock mode
./target/release/quantclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/quantclaw agent -m "Move forward 1 meter"
```
...
```rust
use quantclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `quantclaw`, add a thin adapter that maps this
...
mkdir quantclaw-robot-kit
cp -r target/release/quantclaw quantclaw-robot-kit/
cp -r examples/robot_kit quantclaw-robot-kit/
cp -r ~/.quantclaw quantclaw-robot-kit/dot-quantclaw

# Include models
mkdir -p quantclaw-robot-kit/models
cp ~/.quantclaw/models/ggml-base.bin quantclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf quantclaw-robot-kit.tar.gz quantclaw-robot-kit/

# Copy to USB
cp quantclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
