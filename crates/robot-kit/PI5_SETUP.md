...
Complete guide to setting up a DaemonClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.daemonclaw/models
cp models/ggml-base.bin ~/.daemonclaw/models/
```
...
mkdir -p ~/.daemonclaw/models/piper
cd ~/.daemonclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.daemonclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
# Build robot kit
cargo build --release -p daemonclaw-robot-kit

# Build main daemonclaw (optional, if using as agent)
cargo build --release
```
...
```toml
# ~/.daemonclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.daemonclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/daemonclaw_sensors.fifo"
...
### Start DaemonClaw Agent

```bash
# Configure DaemonClaw to use robot tools
cat > ~/.daemonclaw/config.toml << 'EOF'
...
cp ~/daemonclaw/crates/robot-kit/SOUL.md ~/.daemonclaw/workspace/

# Start agent
./target/release/daemonclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/daemonclaw_sensors.fifo ]; then
    mkfifo /tmp/daemonclaw_sensors.fifo
fi
...
# Start daemonclaw
cd ~/daemonclaw
./target/release/daemonclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/daemonclaw-robot.service
sudo tee /etc/systemd/system/daemonclaw-robot.service << 'EOF'
[Unit]
Description=DaemonClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/daemonclaw
...
sudo systemctl enable daemonclaw-robot
sudo systemctl start daemonclaw-robot

# Check status
sudo systemctl status daemonclaw-robot
journalctl -u daemonclaw-robot -f  # View logs
```
