...
| Manual `sops.run` in `src/rpc/dispatch.rs`, limited to #10513/#10522: route new manual `ExecuteStep` runs through `RpcContext.sop_driver_handles` and `spawn_and_register_sop_driver`; fail closed without an open generation, settle refused runs, and test admission plus per-step scope enforcement. No new driver, policy, run source, or RPC method. | RPC/daemon socket owner in the planned `zeroclaw-kernel` IPC extraction | Core Team approval pending; record the approving reviewer and permalink before merge | Review at `zeroclaw-kernel` IPC extraction or before widening to other RPC methods or run sources |
