...
cli-channel-history-cleared = 对话历史已清空。重新开始。
cli-channel-stop-signal-sent = 停止信号已发送。
cli-channel-no-in-flight-task = 该发送者范围内没有正在进行的任务。
cli-channel-current-provider = 当前 provider：`{$provider}`
cli-channel-current-model = 当前 model：`{$model}`
cli-channel-available-providers = 可用 providers：
cli-channel-model-switched = 已切换到 model `{$model}`（provider：`{$provider}`）。上下文已保留。
