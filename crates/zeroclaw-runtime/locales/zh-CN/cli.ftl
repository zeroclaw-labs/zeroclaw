...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Quickstart 未写入。请重新运行 Quickstart，并选择 `setup_token` 以完成 {$alias} 的 Anthropic 配置。
...
cli-config-patch-warning-legacy-colon-alias = 警告：{$path} 是仍然无效的旧版提供商别名，必须单独修复。
cli-config-patch-warning-pre-existing-validation = 警告：{$path} 保留了无关的验证错误，必须单独修复：{$warning}
