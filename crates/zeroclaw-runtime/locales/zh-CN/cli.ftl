...
channel-runtime-session-prompt-load-failed = ⚠️ 无法加载持久会话上下文，因此此请求未发送给模型。
...
channel-approval-reply-instruction-yesno-once = 回复：“{ $yes_command }” 或 “{ $no_command }”
channel-approval-reply-instruction-approve-deny-once = 回复 `{ $approve_command }` / `{ $deny_command }`。
...
session-prompt-approval-heading = 批准修改持久会话提示
