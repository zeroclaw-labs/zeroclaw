...
tool-git-operations = 执行结构化的 Git 操作（status、diff、log、branch、commit、add、checkout、stash、worktree）。提供解析后的 JSON 输出，并与安全策略集成以实现自主控制。
...
tool-git-operations-error-repository-outside-authorized-roots = 在 '{ $path }' 的允许根目录内没有可访问的 Git 仓库。请选择位于适用允许根目录内的仓库路径，或在运行 git_operations 前初始化仓库。
tool-git-operations-error-repository-not-authorized = '{ $path }' 的 Git 仓库元数据未获授权执行此操作。请选择位于适用允许根目录内的仓库。
...
tool-session-prompt-list = 列出当前运行此 agent 的聊天会话的持久提示附件。
tool-session-prompt-set = 为当前聊天会话创建或替换一个持久提示附件。
tool-session-prompt-delete = 从当前聊天会话删除一个持久提示附件。
tool-session-prompt-param-id = 小写符号 ID：[a-z][a-z0-9_.-]{"{"}0,63{"}"}。
tool-session-prompt-param-content = 提示文本，最多 2048 个 UTF-8 字节。
tool-session-prompt-error-no-context = 没有活动的聊天会话上下文。此工具不适用于辅助调用。
tool-session-prompt-error-no-backend = 没有活动的聊天会话后端。此工具不适用于辅助调用。
tool-session-prompt-error-missing-id = 缺少“id”参数。
tool-session-prompt-error-missing-content = 缺少“content”参数。
tool-session-prompt-error-list = 列出会话提示失败：{ $err }
tool-session-prompt-error-set = 设置会话提示失败：{ $err }
tool-session-prompt-error-delete = 删除会话提示失败：{ $err }
tool-session-prompt-set-created = 已创建会话提示“{ $id }”；它将在下一个顶级轮次生效。
tool-session-prompt-set-updated = 已更新会话提示“{ $id }”；它将在下一个顶级轮次生效。
tool-session-prompt-delete-success = 已删除会话提示“{ $id }”；更改将在下一个顶级轮次生效。
tool-session-prompt-error-missing = 会话提示“{ $id }”不存在。
