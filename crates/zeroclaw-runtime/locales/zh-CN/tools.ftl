...
tool-channel-room = 通过活动 channel 创建房间并邀请用户。提供 channel key（如 'matrix.default'）、action（'create_room' 或 'invite_user'）以及对应 action 所需的房间字段。
tool-channel-room-param-action = 要执行的房间管理 action。
tool-channel-room-param-channel = 活动 channel key，如 'matrix.default'。
tool-channel-room-param-name = create_room 时使用的可选房间名称。
tool-channel-room-param-topic = create_room 时使用的可选房间主题。
tool-channel-room-param-invites = 创建房间时可选邀请的用户 ID 列表。
tool-channel-room-param-visibility = create_room 时使用的可选房间可见性。
tool-channel-room-param-encryption = 是否在 create_room 时请求房间加密。
tool-channel-room-param-room-id = invite_user 时使用的现有房间 ID。
tool-channel-room-param-user-id = invite_user 时要邀请的用户 ID。
tool-channel-room-error-security = 操作被阻止：{ $err }
tool-channel-room-error-invalid-action = 无效的 action '{ $action }'：必须是 'create_room' 或 'invite_user'。
tool-channel-room-error-not-initialized = 暂无可用 channel（channel 尚未初始化）。
tool-channel-room-error-channel-not-found = 未找到 channel '{ $channel }'。可用 channel：{ $available }
...
tool-channel-room-error-invites-item = 'invites' 必须是非空字符串的数组。
tool-channel-room-error-invalid-visibility = 无效的房间可见性：{ $err }
tool-channel-room-error-missing-param = 缺少参数 '{ $param }'。
