...
tool-homeassistant = 通过 REST API 控制和查询 Home Assistant 智能家居实例。操作：'list_entities'（所有实体 ID 及状态，可按域如 'light' 或 'sensor' 过滤）、'get_state'（单个实体的完整状态和属性）、'call_service'（调用服务，如带 service_data 的 light.turn_on）。list_entities 和 get_state 为只读；call_service 会更改设备状态，并受操作者 allowed_domains 配置的限制。
tool-homeassistant-param-action = 要执行的 Home Assistant 操作。
tool-homeassistant-param-entity-id = get_state 的实体 ID（例如 'light.kitchen'）。
tool-homeassistant-param-domain = list_entities 的域过滤器，或 call_service 的服务域（例如 'light'）。
tool-homeassistant-param-service = call_service 的服务名称（例如 'turn_on'）。
tool-homeassistant-param-service-data = call_service 的可选 JSON 正文（例如 {"entity_id": "light.kitchen"}）。
tool-homeassistant-error-missing-action = 缺少必需参数：action
tool-homeassistant-error-unknown-action = 未知操作：{ $action }。有效操作：list_entities、get_state、call_service
tool-homeassistant-error-missing-entity-id = get_state 需要 entity_id 参数
tool-homeassistant-error-missing-domain = call_service 需要 domain 参数
tool-homeassistant-error-missing-service = call_service 需要 service 参数
tool-homeassistant-error-not-configured = 未配置 Home Assistant：请设置 homeassistant.url 和 homeassistant.token（或 HASS_URL / HASS_TOKEN 环境变量）
tool-homeassistant-error-invalid-slug = 无效的{ $kind } '{ $value }'。应为匹配 [a-z0-9_]+ 的小写标识符（例如 light、turn_on）
tool-homeassistant-error-invalid-entity-id = 无效的 entity_id '{ $entity_id }'。期望格式：<domain>.<object_id>，两部分均需匹配 [a-z0-9_]+（例如 light.kitchen）
tool-homeassistant-error-domain-not-allowed = 服务域 '{ $domain }' 不在 homeassistant.allowed_domains 中。允许的域：{ $allowed }
tool-homeassistant-allowed-domains-empty = （无 — homeassistant.allowed_domains 为空）
tool-homeassistant-error-list-entities-failed = Home Assistant 的 list_entities 失败（{ $status }）：{ $body }
tool-homeassistant-error-get-state-failed = Home Assistant 的 get_state 失败（{ $status }）：{ $body }
tool-homeassistant-error-call-service-failed = Home Assistant 的 call_service 失败（{ $status }）：{ $body }
tool-homeassistant-error-call-service-malformed-body = Home Assistant 的 call_service 返回了 2xx 状态，但正文无法解码为 JSON：{ $err }
tool-homeassistant-error-decode-failed = Home Assistant 的响应无法解码为 JSON：{ $err }
