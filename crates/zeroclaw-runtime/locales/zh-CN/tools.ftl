...
tool-shared-memory-store = 将事实存储到所有代理都能读取的共享（家庭）记忆中。仅当用户明确要求为所有人／整个家庭记住某事时使用。不用于私人备注（那些请使用 memory_store）。
tool-system-memory-store = 将事实存储到所有代理都能读取的系统记忆中。仅限管理员。仅当被明确要求记录系统级操作事实时使用。不用于私人或家庭备注。
tool-shared-memory-store-param-key = 此记忆的唯一键（例如 'house_wifi'、'trash_day'）
tool-shared-memory-store-param-content = 要记住的信息
tool-shared-memory-store-param-category = 记忆类别：'core'（永久）、'daily'（会话）、'conversation'（聊天）或自定义类别名称。默认为 'core'。
tool-shared-memory-store-error-missing-key = 缺少 'key' 参数
