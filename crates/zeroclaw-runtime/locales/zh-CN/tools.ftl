...
tool-file-download = 从配置的远程端点下载文件并将其写入 agent 的工作区。提供要获取的文档标识符和相对于工作区的目标路径；端点 URL 由主机配置固定，不受模型控制。字节流式传输到磁盘，不会加载到模型上下文中。返回 HTTP 状态、写入的字节数和目标路径。
...
tool-file-download-error-read-only = 操作被阻止：autonomy 是只读的
tool-file-download-error-rate-limited-hour = 超出速率限制：过去一小时内操作过多
tool-file-download-error-rate-limited-budget = 超出速率限制：已达到操作预算限制
...
tool-file-download-error-too-large-reported = 下载过大：端点报告 { $len } 字节（限制：{ $limit } 字节）
tool-file-download-error-too-large-stream = 下载过大：超过 { $limit } 字节的限制
tool-file-download-error-temp-create = 创建临时下载文件失败：{ $err }
tool-file-download-error-read-body = 读取响应体时失败：{ $err }
tool-file-download-error-write-body = 写入下载字节时失败：{ $err }
