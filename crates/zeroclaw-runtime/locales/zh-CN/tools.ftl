...
tool-file-download = 从配置的远程端点下载文件并写入 agent 的工作区。提供要获取的文档标识符和工作区相对的目标路径；端点 URL 由主机配置固定，不受模型控制。字节直接流式传输到磁盘，不会加载到模型上下文中。返回 HTTP 状态码、写入的字节数和目标路径。
tool-file-download-param-document-id = 要从配置的端点获取的文档的标识符。
tool-file-download-param-dest-path = 写入文件的工作区相对路径。父目录必须已存在。
tool-file-download-error-disabled = file_download 已禁用：[file_download].url 未配置
tool-file-download-error-read-only = 操作被阻止：自主模式为只读
tool-file-download-error-rate-limited-hour = 超出速率限制：过去一小时内操作过于频繁
tool-file-download-error-rate-limited-budget = 超出速率限制：已达到操作预算上限
tool-file-download-error-missing-document-id = 缺少 'document_id' 参数
tool-file-download-error-missing-dest-path = 缺少 'dest_path' 参数
tool-file-download-error-invalid-file-name = 无效的 dest_path '{ $dest_path }'：必须以具体文件名结尾
tool-file-download-error-no-parent = 无效的 dest_path '{ $dest_path }'：没有父目录
tool-file-download-error-resolve-dir = 无法解析目标目录 '{ $dest_path }'：{ $err }
tool-file-download-error-client-build = 构建下载客户端失败：{ $err }
tool-file-download-error-request = 下载请求失败：{ $err }
tool-file-download-error-status = 下载端点返回状态码 { $status }
tool-file-download-error-too-large-reported = 下载文件过大：端点报告大小为 { $len } 字节（限制：{ $limit } 字节）
tool-file-download-error-too-large-stream = 下载文件过大：超过限制 { $limit } 字节
tool-file-download-error-temp-create = 创建临时下载文件失败：{ $err }
tool-file-download-error-read-body = 读取响应体时出错：{ $err }
tool-file-download-error-write-body = 写入下载字节时出错：{ $err }
tool-file-download-error-flush = 刷新下载文件失败：{ $err }
tool-file-download-error-move = 将下载文件移动到目标位置失败：{ $err }
tool-file-download-success = 已下载 { $written } 字节到 { $dest_path }（状态码 { $status }）
