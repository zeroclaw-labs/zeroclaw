...
tool-file-download = 从配置的远程端点下载文件并写入 agent 工作区。提供要获取的文档标识符和工作区相对目标路径；端点 URL 由主机配置固定，不受模型控制。字节直接流式写入磁盘，不会加载到模型上下文中。返回 HTTP 状态码、写入的字节数和目标路径。
tool-file-download-param-document-id = 要从配置端点获取的文档标识符。
...
tool-file-download-error-rate-limited-budget = 超出速率限制：操作配额已用尽
...
tool-file-download-error-too-large-stream = 下载文件过大：超出 { $limit } 字节限制
tool-file-download-error-temp-create = 创建临时下载文件失败：{ $err }
tool-file-download-error-read-body = 读取响应体失败：{ $err }
tool-file-download-error-write-body = 写入下载字节失败：{ $err }
tool-file-download-error-flush = 刷新下载文件失败：{ $err }
tool-file-download-error-move = 移动下载文件到目标位置失败：{ $err }
