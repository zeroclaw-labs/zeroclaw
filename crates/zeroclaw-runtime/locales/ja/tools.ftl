...
tool-file-download = 設定されたリモートエンドポイントからファイルをダウンロードし、エージェントのワークスペースに書き込みます。取得するドキュメントの識別子とワークスペース相対の宛先パスを指定します。エンドポイントURLはホスト設定で固定され、モデルによって制御されることはありません。バイトはディスクに直接ストリーミングされ、モデルのコンテキストには読み込まれません。HTTPステータス、書き込まれたバイト数、宛先パスを返します。
tool-file-download-param-document-id = 設定されたエンドポイントから取得するドキュメントの識別子。
tool-file-download-param-dest-path = ファイルを書き込むワークスペース相対パス。親ディレクトリが既に存在している必要があります。
tool-file-download-error-disabled = file_download は無効です：[file_download].url が設定されていません
tool-file-download-error-read-only = 操作がブロックされました：自律性が読み取り専用です
tool-file-download-error-rate-limited-hour = レート制限を超過しました：過去1時間の操作が多すぎます
tool-file-download-error-rate-limited-budget = レート制限を超過しました：操作予算が枯渇しました
...
tool-file-download-error-too-large-reported = ダウンロードが大きすぎます：エンドポイントが { $len } バイトを報告しています（制限：{ $limit } バイト）
...
tool-file-download-error-read-body = レスポンスボディの読み取り中に失敗しました：{ $err }
tool-file-download-error-write-body = ダウンロードバイトの書き込み中に失敗しました：{ $err }
tool-file-download-error-flush = ダウンロードファイルのフラッシュに失敗しました：{ $err }
tool-file-download-error-move = ダウンロードファイルの移動に失敗しました：{ $err }
