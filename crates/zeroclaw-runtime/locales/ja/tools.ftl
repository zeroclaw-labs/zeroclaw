...
tool-session-prompt-list = このエージェントを現在実行しているチャットセッションの永続プロンプト添付を一覧表示します。
tool-session-prompt-set = 現在のチャットセッションの永続プロンプト添付を作成または置換します。
tool-session-prompt-delete = 現在のチャットセッションから永続プロンプト添付を削除します。
tool-session-prompt-param-id = 小文字の記号ID：[a-z][a-z0-9_.-]{"{"}0,63{"}"}。
tool-session-prompt-param-content = プロンプトテキスト。UTF-8で最大2048バイト。
tool-session-prompt-error-no-context = アクティブなチャットセッションコンテキストがありません。このツールは補助呼び出しでは利用できません。
tool-session-prompt-error-no-backend = アクティブなチャットセッションバックエンドがありません。このツールは補助呼び出しでは利用できません。
tool-session-prompt-error-missing-id = 「id」パラメーターがありません。
tool-session-prompt-error-missing-content = 「content」パラメーターがありません。
tool-session-prompt-error-list = セッションプロンプトの一覧取得に失敗しました：{ $err }
tool-session-prompt-error-set = セッションプロンプトの設定に失敗しました：{ $err }
tool-session-prompt-error-delete = セッションプロンプトの削除に失敗しました：{ $err }
tool-session-prompt-set-created = セッションプロンプト「{ $id }」を作成しました。次のトップレベルターンで適用されます。
tool-session-prompt-set-updated = セッションプロンプト「{ $id }」を更新しました。次のトップレベルターンで適用されます。
tool-session-prompt-delete-success = セッションプロンプト「{ $id }」を削除しました。変更は次のトップレベルターンで適用されます。
tool-session-prompt-error-missing = セッションプロンプト「{ $id }」は存在しません。
