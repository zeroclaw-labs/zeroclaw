...
tool-shared-memory-store = すべてのエージェントが読み取れる共有（世帯）メモリに事実を保存します。全員／世帯全体のために何かを覚えるようユーザーが明示的に求めた場合にのみ使用してください。プライベートなノートには使用しないでください（その場合は memory_store を使用）。
tool-system-memory-store = すべてのエージェントが読み取れるシステムメモリに事実を保存します。管理者専用。システム全体の運用上の事実を記録するよう明示的に求められた場合にのみ使用してください。プライベートや世帯のノートには使用しないでください。
tool-shared-memory-store-param-key = このメモリの一意のキー（例：'house_wifi'、'trash_day'）
tool-shared-memory-store-param-content = 記憶する情報
tool-shared-memory-store-param-category = メモリカテゴリ：'core'（永続）、'daily'（セッション）、'conversation'（チャット）、またはカスタムカテゴリ名。デフォルトは 'core'。
tool-shared-memory-store-error-missing-key = 'key' パラメータがありません
tool-shared-memory-store-error-missing-content = 'content' パラメータがありません
tool-shared-memory-store-error-not-available = { $tier } メモリはこのバックエンドでは利用できません
tool-shared-memory-store-error-no-bank = { $tier } バンクが設定されていません
tool-shared-memory-store-error-not-authorized = このエージェントは { $tier } メモリへの書き込みを許可されていません
tool-shared-memory-store-success = { $tier } メモリを保存しました：{ $key }
tool-shared-memory-store-error-failed = { $tier } メモリの保存に失敗しました：{ $error }
