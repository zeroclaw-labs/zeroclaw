...
tool-file-download = 設定されたリモートエンドポイントからファイルをダウンロードし、エージェントのワークスペースに書き込みます。取得するドキュメントの識別子と、ワークスペース相対の宛先パスを指定します。エンドポイント URL はホスト設定で固定されており、モデルから制御されることはありません。バイトはディスクにストリーミングされ、モデルコンテキストにはロードされません。HTTP ステータス、書き込まれたバイト数、宛先パスを返します。
tool-file-download-param-document-id = 設定されたエンドポイントから取得するドキュメントの識別子。
tool-file-download-param-dest-path = ファイルを書き込むワークスペース相対パス。親ディレクトリは既に存在している必要があります。
tool-file-download-error-disabled = file_download は無効です：[file_download].url が設定されていません
tool-file-download-error-read-only = アクションがブロックされました：autonomy は読み取り専用です
tool-file-download-error-rate-limited-hour = レート制限を超えました：過去 1 時間にアクションが多すぎます
tool-file-download-error-rate-limited-budget = レート制限を超えました：アクション予算に達しました
...
tool-file-download-error-request = ダウンロードリクエストに失敗しました：{ $err }
tool-file-download-error-status = ダウンロードエンドポイントがステータス { $status } を返しました
tool-file-download-error-too-large-reported = ダウンロードが大きすぎます：エンドポイントが { $len } バイトを報告しました（制限：{ $limit } バイト）
...
tool-file-download-error-read-body = レスポンスボディの読み込み中に失敗しました：{ $err }
tool-file-download-error-write-body = ダウンロードしたバイトの書き込み中に失敗しました：{ $err }
