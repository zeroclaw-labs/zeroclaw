...
tool-file-download = Descarga un archivo desde el endpoint remoto configurado y lo escribe en el espacio de trabajo del agente. Proporcione el identificador del documento a recuperar y una ruta de destino relativa al espacio de trabajo; la URL del endpoint está fijada por la configuración del host y nunca está controlada por el modelo. Los bytes se transmiten directamente al disco y no se cargan en el contexto del modelo. Devuelve el estado HTTP, la cantidad de bytes escritos y la ruta de destino.
tool-file-download-param-document-id = Identificador del documento a recuperar desde el endpoint configurado.
tool-file-download-param-dest-path = Ruta relativa al espacio de trabajo donde se escribirá el archivo. El directorio padre debe existir.
tool-file-download-error-disabled = file_download está desactivado: [file_download].url no está configurado
tool-file-download-error-read-only = Acción bloqueada: autonomía en modo de solo lectura
...
tool-file-download-error-invalid-file-name = dest_path inválido '{ $dest_path }': debe terminar con un nombre de archivo concreto
tool-file-download-error-no-parent = dest_path inválido '{ $dest_path }': no tiene directorio padre
...
tool-file-download-error-flush = Error al descargar el archivo descargado: { $err }
tool-file-download-error-move = Error al mover el archivo descargado a su ubicación final: { $err }
tool-file-download-success = Se descargaron { $written } bytes a { $dest_path } ({ $status })
