...
tool-shared-memory-store = Almacenar un hecho en la memoria COMPARTIDA del hogar que todos los agentes pueden leer. Use SOLO cuando el usuario pida explícitamente recordar algo para todos / todo el hogar. No para notas privadas (use memory_store para esas).
tool-system-memory-store = Almacenar un hecho en la memoria del SISTEMA que todos los agentes pueden leer. Solo administradores. Use SOLO cuando se le pida explícitamente registrar un hecho operativo de todo el sistema. No para notas privadas ni del hogar.
tool-shared-memory-store-param-key = Clave única para esta memoria (p. ej. 'house_wifi', 'trash_day')
tool-shared-memory-store-param-content = La información a recordar
tool-shared-memory-store-param-category = Categoría de memoria: 'core' (permanente), 'daily' (sesión), 'conversation' (chat), o un nombre de categoría personalizado. Predeterminado 'core'.
tool-shared-memory-store-error-missing-key = Falta el parámetro 'key'
