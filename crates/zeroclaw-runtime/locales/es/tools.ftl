...
tool-file-download = Descargar un archivo desde el endpoint remoto configurado y escribirlo en el espacio de trabajo del agente. Proporcione el identificador del documento a obtener y una ruta de destino relativa al espacio de trabajo; la URL del endpoint está fijada por la configuración del host y nunca es controlada por el modelo. Los bytes se transmiten por streaming directamente al disco y no se cargan en el contexto del modelo. Devuelve el estado HTTP, el número de bytes escritos y la ruta de destino.
...
tool-file-download-error-read-only = Acción bloqueada: autonomy es de solo lectura
tool-file-download-error-rate-limited-hour = Límite de tasa excedido: demasiadas acciones en la última hora
tool-file-download-error-rate-limited-budget = Límite de tasa excedido: se agotó el presupuesto de acciones
...
tool-file-download-error-invalid-file-name = dest_path inválido '{ $dest_path }': debe terminar en un nombre de archivo concreto
...
tool-file-download-error-request = Error en la solicitud de descarga: { $err }
...
tool-file-download-error-move = Error al mover el archivo descargado a su lugar: { $err }
