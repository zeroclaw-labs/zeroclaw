...
tool-homeassistant = Controlar y consultar una instancia de Home Assistant a través de su API REST. Acciones: 'list_entities' (todos los ids de entidad y su estado, opcionalmente filtrados por dominio como 'light' o 'sensor'), 'get_state' (estado completo y atributos de una entidad) y 'call_service' (invocar un servicio como light.turn_on con service_data). list_entities y get_state son de solo lectura; call_service cambia el estado del dispositivo y está restringido a la configuración allowed_domains del operador.
tool-homeassistant-param-action = La acción de Home Assistant a realizar.
tool-homeassistant-param-entity-id = Id de entidad para get_state (p. ej. 'light.kitchen').
tool-homeassistant-param-domain = Filtro de dominio para list_entities, o el dominio del servicio para call_service (p. ej. 'light').
tool-homeassistant-param-service = Nombre del servicio para call_service (p. ej. 'turn_on').
tool-homeassistant-param-service-data = Cuerpo JSON opcional para call_service (p. ej. {"entity_id": "light.kitchen"}).
tool-homeassistant-error-missing-action = Falta el parámetro requerido: action
tool-homeassistant-error-unknown-action = Acción desconocida: { $action }. Acciones válidas: list_entities, get_state, call_service
tool-homeassistant-error-missing-entity-id = get_state requiere el parámetro entity_id
tool-homeassistant-error-missing-domain = call_service requiere el parámetro domain
tool-homeassistant-error-missing-service = call_service requiere el parámetro service
tool-homeassistant-error-not-configured = Home Assistant no está configurado: establece homeassistant.url y homeassistant.token (o las variables de entorno HASS_URL / HASS_TOKEN)
tool-homeassistant-error-invalid-slug = { $kind } '{ $value }' no válido. Se esperaba un slug en minúsculas que coincida con [a-z0-9_]+ (p. ej. light, turn_on)
tool-homeassistant-error-invalid-entity-id = entity_id '{ $entity_id }' no válido. Formato esperado: <domain>.<object_id> con ambas partes coincidiendo con [a-z0-9_]+ (p. ej. light.kitchen)
tool-homeassistant-error-domain-not-allowed = El dominio de servicio '{ $domain }' no está en homeassistant.allowed_domains. Permitidos: { $allowed }
tool-homeassistant-allowed-domains-empty = (ninguno — homeassistant.allowed_domains está vacío)
tool-homeassistant-error-list-entities-failed = list_entities de Home Assistant falló ({ $status }): { $body }
tool-homeassistant-error-get-state-failed = get_state de Home Assistant falló ({ $status }): { $body }
tool-homeassistant-error-call-service-failed = call_service de Home Assistant falló ({ $status }): { $body }
tool-homeassistant-error-call-service-malformed-body = call_service de Home Assistant devolvió un estado 2xx con un cuerpo que no se pudo decodificar como JSON: { $err }
tool-homeassistant-error-decode-failed = La respuesta de Home Assistant no se pudo decodificar como JSON: { $err }
