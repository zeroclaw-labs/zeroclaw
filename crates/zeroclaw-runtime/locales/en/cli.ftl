...
cli-eval-calibrate-jsonl-invalid-data = Calibration JSONL {$path} line {$line} is inconsistent: {$reason}.
...
cli-eval-calibrate-calibration-wrong-prompt-hash = prompt_hash is '{$found}', expected '{$expected}'; recalibrate after a judge prompt or scoring-contract change.
cli-eval-calibrate-calibration-insufficient-records = labeled_records is {$found}, but at least {$minimum} are required.
cli-eval-calibrate-calibration-invalid-agreement = agreement is {$found}, expected a finite fraction between 0 and 1.
cli-eval-calibrate-calibration-low-agreement = agreement is {$found}%, below the {$minimum}% gating floor.
cli-eval-calibrate-calibration-invalid-evidence = calibration evidence is inconsistent: {$reason}.
...
cli-eval-calibrate-label-stale-prompt-records = Judge-run records do not match this binary's prompt/scoring contract. Expected {$expected}; found: {$found}. Re-run the eval before labeling.
cli-eval-calibrate-label-skipped-stale-records = Ignoring {$count} judge-run records from older prompt/scoring contracts.
cli-eval-calibrate-label-existing-labels-mismatch = Existing labels at {$path} belong to a different judge or prompt contract than {$judge_ref}; use a separate labels file.
...
cli-eval-calibrate-label-agent-transcript = Agent conversation transcript:
...
cli-eval-calibrate-finalize-multiple-prompt-hashes = Labels must share one judge prompt/scoring contract; found prompt hashes: {$hashes}
cli-eval-calibrate-finalize-rubric-hash-conflict = Rubric contract hash {$hash} maps to multiple names: {$names}
...
cli-eval-calibrate-finalize-empty-labeler = --labeler must not be empty.
...
cli-eval-calibrate-finalize-hint = Commit this calibration file, then set [eval].judge_gate = true. Only exact rubric contracts at or above 85% agreement will gate.
