//! Version check, restart classification and in-app upgrade, shared by the
//! dashboard's `/api/version/*` routes and the core's `system/*` methods
//! (RFC: In-app upgrade with optional supervised restart).
//!
...
fn detect_restart_uncached() -> RestartInfo {
    if crate::restart::is_desktop_supervised() {
        return RestartInfo {
...
async fn run_upgrade(
    ...
    match action {
        ...
        RestartAction::DesktopSupervised => {
            if let Err(error) = crate::restart::request_desktop_restart() {
                fail(
                    &progress,
...
fn trigger_graceful_shutdown() {
    ...
    #[cfg(not(unix))]
    {
        crate::restart::request_shutdown();
    }
}

// ── Operations the dashboard routes and `system/*` share ──────────────────

/// An upgrade request refused before any work started: the status the
/// dashboard route answers with and the message both surfaces carry.
...
impl UpgradeRefusal {
    fn new(http_status: u16, message: &str) -> Self {
        Self {
            http_status,
...
/// The version check, served from the hour-long cache unless `force`d or a
/// specific version is named. Never fails: a check that cannot complete
/// reports `is_newer: false` with the error, so a version badge degrades
/// gracefully.
pub async fn version_check(query: &CheckQuery) -> VersionCheckResponse {
    let use_cache = !query.force && query.version.is_none();
    if use_cache
        && let Some((ts, cached)) = lock_recover(&CHECK_CACHE).as_ref()
        && ts.elapsed() < CHECK_CACHE_TTL
    {
        return VersionCheckResponse::from(cached);
    }
    match run_cli_check(query.version.as_deref()).await {
        Ok(info) => {
            if use_cache {
                *lock_recover(&CHECK_CACHE) = Some((Instant::now(), info.clone()));
            }
            VersionCheckResponse::from(&info)
        }
        Err(e) => VersionCheckResponse {
...
/// Start an upgrade via `zeroclaw update` on a detached task and return the
/// id to poll [`upgrade_status`] with. Only one upgrade runs at a time.
///
/// `allow_self_upgrade` is `gateway.allow_self_upgrade`. `standalone_shutdown_tx`
/// is the gateway's own shutdown watch when it runs without a daemon; inside
/// the daemon it is `None` and a self-respawn goes through the daemon's
/// shutdown path.
pub fn start_upgrade(
    allow_self_upgrade: bool,
    req: UpgradeRequest,
    standalone_shutdown_tx: Option<tokio::sync::watch::Sender<bool>>,
) -> Result<UpgradeAcceptedResponse, UpgradeRefusal> {
    if !allow_self_upgrade {
        return Err(UpgradeRefusal::new(
            403,
    ...
    if let Some(existing) = slot.as_ref()
        && !lock_recover(existing).state.is_terminal()
    {
        return Err(UpgradeRefusal::new(
            409,
    ...
    *slot = Some(progress.clone());
    ...
    Ok(UpgradeAcceptedResponse { handoff_id })
}

/// The current or most recent upgrade's progress, `state: "idle"` when none
/// has run in this process. A `handoff_id` that names another upgrade is
/// refused.
pub fn upgrade_status(handoff_id: Option<&str>) -> Result<UpgradeStatusResponse, UpgradeRefusal> {
    let slot = lock_recover(&UPGRADE);
    let Some(progress) = slot.as_ref() else {
        return Ok(UpgradeStatusResponse {
            handoff_id: None,
    ...
    if let Some(id) = handoff_id
        && id != p.handoff_id
    {
        return Err(UpgradeRefusal::new(404, "unknown handoff_id"));
    }
