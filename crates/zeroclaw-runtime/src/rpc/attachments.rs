...
mod tests {
    ...
    async fn path_mode_image_marker_survives_original_removal() {
        ...
        // A real decodable 1x1 PNG, not a bare signature: preparation now
        // fully decodes image content, so bytes that only carry the magic
        // number are refused and `contains_images` would be false.
        let png: [u8; 67] = [
