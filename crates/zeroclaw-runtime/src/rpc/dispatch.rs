...
impl RpcDispatcher {
    ...
    async fn lock_session_publication(
    ...
    ) -> Result<crate::live_config_authority::ConfigCommit, JsonRpcError> {
        #[cfg(test)]
        {
            let pause = self
                .ctx
        ...
        let commit = self.ctx.begin_config_commit().await.map_err(|_| {
            rpc_err(
                INVALID_PARAMS,
    ...
    async fn insert_lifecycle_session(
        ...
        let _config_commit = self
            .lock_session_publication(agent_alias, construction_config)
            .await?;
    ...
    async fn handle_session_new(&self, params: &Value) -> RpcResult {
        ...
        let admitted_generation = self.ctx.sessions.get_generation(&session_id).await;
        if matches!(self.access_policy, RpcAccessPolicy::RemoteSessionOwner)
        ...
        let unpublished = if expected_generation.is_none() {
            let _config_commit = self
                .lock_session_publication(&req.agent_alias, &config)
                .await?;
            let lifecycle_lease = admission
                .take()
                .ok_or_else(|| rpc_err(INTERNAL_ERROR, "Missing session admission"))?
                .publish()
                .map_err(|_| {
                    rpc_err(INVALID_PARAMS, "Agent changed during session construction")
                })?;
            self.populate_local_session_channels(
                &mut *candidate_agent.lock().await,
                &req.agent_alias,
            );
            candidate = candidate.with_lifecycle_lease(lifecycle_lease);
            self.ctx
                .sessions
                ...
                crate::rpc::types::ChatMode::Chat => {
                    if let Some(ref backend) = self.ctx.session_backend {
                        ...
                        if !stored.is_empty() {
                            ...
                            history =
                                Some(stored.into_iter().map(ConversationMessage::Chat).collect());
                        }
                    }
                }
            }

            Ok::<_, JsonRpcError>((message_count, history, plan))
        }
        .await;
        let (message_count, history, plan) = match prepared {
        ...
        let publication = async {
            ...
            let seed_event = {
                ...
                history.and_then(|messages| agent.seed_conversation_history_with_event(messages))
            };
            ...
            Ok::<_, JsonRpcError>(seed_event)
        }
        .await;
        let seed_event = match publication {
            Ok(seed_event) => seed_event,
            Err(error) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn session_mode_replacement_publication_preserves_original_on_config_or_owner_change() {
    ...
    async fn session_construction_recovered_history_uses_checked_cap_after_config_aba() {
        ...
        *sessions.test_construction_publication_pause.lock().unwrap() =
        ...
        tokio::time::timeout(std::time::Duration::from_secs(5), entered.notified())
            ...
            .unwrap();
        {
            let commit = dispatcher.ctx.begin_config_commit().await.unwrap();
            ...
            commit.publish(revision, config).unwrap();
        }
        release.notify_one();
