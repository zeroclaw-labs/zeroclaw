...
impl RpcDispatcher {
                ...
                crate::rpc::types::ChatMode::Acp => {
                    ...
                    match loaded {
                        Ok(Ok(AcpSessionNewLoad::Restored(projected))) => {
                            ...
                            message_count = conversation_message_entries(&projected.data.messages).len();
    ...
    async fn handle_session_close(&self, params: &Value) -> RpcResult {
        ...
        self.ctx.sessions.signal_session_removal_for_generation(
            &req.session_id,
            requested_identity
                .as_ref()
    ...
    async fn rehydrate_reaped_session_under_guard(
        ...
        match cancel_generation {
            Some(cancel_generation) => {
                self.ctx
                    .sessions
                    .insert_for_prompt(sid.to_string(), session, cancel_generation)
                    .await
                    .ok()?;
            }
            None => {
    ...
    async fn handle_session_prompt(&self, params: &Value) -> RpcResult {
        ...
        let agent = match self.ctx.sessions.get_agent(sid).await {
            Some(a) => a,
            None => match self
                .rehydrate_reaped_session_under_guard(sid, Some(cancel_registration.generation()))
                .await
            {
                Some(a) => a,
                None => {
...
fn filter_replay_safe_failed_turn_messages(
    ...
    // (first unmatched match wins). IDs may repeat across separate batches,
    // but duplicate call IDs within one batch are ambiguous and rejected.
    ...
    for (index, message) in messages.iter().enumerate() {
        match message {
            ...
            ConversationMessage::AssistantToolCalls {
                ...
                if !complete {
                    if let Some(text) = text.as_ref().filter(|text| !text.is_empty()) {
                        kept.push(ConversationMessage::Chat(ChatMessage::assistant(text)));
                    }
                    continue;
                }
...
mod tests {
    ...
    async fn rpc_acp_agents_receive_owned_session_tools_on_create_and_rehydrate() {
        ...
        let admission = sessions.session_queue.acquire(current).await.unwrap();
        let rehydrated = dispatcher
            .rehydrate_reaped_session_under_guard(current, None)
            .await
    ...
    async fn compaction_composition_covers_reload_restore_and_recompact() {
        ...
        // replace a live Chat predecessor through session/new; the prepared
        // ACP candidate must seed the exact committed projection before publication.
        dispatcher
            ...
            .await
            .expect("the Chat predecessor must be created");
        let predecessor = sessions.get_agent(sid).await.unwrap();
        ...
        assert_eq!(resumed["message_count"], 8, "report retained originals");
        assert_eq!(
            sessions.get_generation(sid).await,
            Some(predecessor_generation + 1)
        );
        assert!(!Arc::ptr_eq(
            &predecessor,
            &sessions.get_agent(sid).await.unwrap()
        ...
        assert_eq!(
            sessions.interaction_surface(sid).await,
            Some(Some(crate::agent::prompt::InteractionSurface::ZerocodeCode)),
        );
        {
            let agent = sessions.get_agent(sid).await.unwrap();
            let agent = agent.lock().await;
    ...
    #[tokio::test]
    async fn acp_persistence_preserves_empty_cancel_for_recovery_and_discards_messageless_failures()
    {
        let tmp = tempfile::TempDir::new().unwrap();
        let store =
            Arc::new(zeroclaw_infra::acp_session_store::AcpSessionStore::new(tmp.path()).unwrap());
    ...
    #[tokio::test]
    async fn acp_checkpoint_duplicate_call_ids_preserve_text_without_replaying_tools() {
        use zeroclaw_api::model_provider::{ToolCall, ToolResultMessage};
        use zeroclaw_infra::acp_session_store::{AcpSessionStore, FAILED_TURN_MARKER};
        ...
        for result_count in [1, 2] {
            ...
            let sid = "failed-duplicate-calls";
            store.create_session(sid, "agent", "/tmp").unwrap();
            let sessions = store_with_one_session(sid).await;
            let live_agent = sessions.get_agent(sid).await.unwrap();
            ...
            let raw = vec![
                ...
                ConversationMessage::AssistantToolCalls {
                    ...
                    tool_calls: vec![call("duplicate"), call("duplicate"), call("unique")],
                    reasoning_content: None,
                },
                ConversationMessage::ToolResults(results),
            ];
            ...
            let prefix_len = live_agent.lock().await.history().len() - raw.len();
            store
                .begin_turn_checkpoint(sid, "turn-duplicate", &raw[..1])
                .unwrap();
            store
                .append_turn_checkpoint(sid, "turn-duplicate", &raw[1..])
                .unwrap();
            assert_eq!(
                finish_acp_checkpoint(&store, sid, "turn-duplicate", &failed).await,
                Ok(AcpCheckpointFinish::Finalized)
            );

            // Failed batches remain all-or-nothing: even the unique peer is
            // omitted with its malformed envelope, but ordinary text survives.
            let expected_provider = vec![
                ConversationMessage::Chat(ChatMessage::user("question")),
                ConversationMessage::Chat(ChatMessage::assistant("partial text")),
            ];
            let mut expected_transcript = expected_provider.clone();
            expected_transcript.push(ConversationMessage::Chat(ChatMessage::system(
                FAILED_TURN_MARKER,
            )));
            let reloaded = store.load_session(sid).unwrap().unwrap();
            assert_eq!(
                ...
                "duplicate result count={result_count}"
            );
            let provider_history = AcpSessionStore::provider_safe_history(&reloaded.messages);
            assert_eq!(
                serde_json::to_value(&provider_history).unwrap(),
                serde_json::to_value(expected_provider).unwrap()
            );
            let mut agent = live_agent.lock().await;
            assert!(reconcile_acp_checkpoint_history(&mut agent, &raw, &failed));
            assert_eq!(
    ...
    async fn config_set_refresh_stale_gen_replaced_by_acp_rehydration() {
        ...
        let rehydrated = dispatcher
            .rehydrate_reaped_session_under_guard(&session_id, None)
            .await;
        assert!(
            rehydrated.is_some(),
