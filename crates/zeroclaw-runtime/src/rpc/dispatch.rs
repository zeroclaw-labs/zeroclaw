...
impl RpcDispatcher {
    ...
    /// for `sid`, covering the owner check through insertion. Rebuilds the
    /// reaped session from a restorable durable row so a fresh prompt recovers
    /// to a working session instead of hanging; returns `None` for missing,
    /// killed, or unreadable durable state.
