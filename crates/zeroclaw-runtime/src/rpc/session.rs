...
impl SessionStore {
    ...
    pub async fn insert_if_absent(
        ...
        self.publish_prepared(id, session, None, None).await
    }
...
mod tests {
    ...
    async fn session_mode_replacement_publication_is_capacity_neutral_and_fenced() {
        ...
        store
            .publish_prepared("s".into(), candidate(), Some(first_generation), None)
            .await
            .unwrap();
        ...
        assert_eq!(
            store
                .publish_prepared("s".into(), candidate(), Some(successor_generation), None)
                .await,
            Err("session changed during preparation")
        );
        assert_eq!(
