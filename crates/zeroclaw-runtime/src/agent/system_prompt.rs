...
/// Builds a system prompt section listing configured channel targets.
/// Returns `None` when no enabled channels have `default_target` set.
/// Only used in daemon/CLI contexts where there is no incoming message.
pub fn build_channel_targets_section(
    ...
    if let Some(c) = &channels.telegram
        && c.enabled
        && let Some(t) = c.default_target.as_deref().filter(|v| !v.trim().is_empty())
    {
        entries.push(("telegram", t));
    }
    if let Some(c) = &channels.discord
        && c.enabled
        && let Some(t) = c.default_target.as_deref().filter(|v| !v.trim().is_empty())
    {
        entries.push(("discord", t));
    }
    if let Some(c) = &channels.slack
        && c.enabled
        && let Some(t) = c.default_target.as_deref().filter(|v| !v.trim().is_empty())
    {
        entries.push(("slack", t));
    }
    if let Some(c) = &channels.mattermost
        && c.enabled
        && let Some(t) = c.default_target.as_deref().filter(|v| !v.trim().is_empty())
    {
        entries.push(("mattermost", t));
    }
    if let Some(c) = &channels.matrix
        && c.enabled
        && let Some(t) = c.default_target.as_deref().filter(|v| !v.trim().is_empty())
    {
        entries.push(("matrix", t));
    }
    ...
    let mut section =
        "## Configured Channel Targets\n\nUse these recipients when sending messages or scheduling deliveries:\n\n".to_string();
    for (channel, target) in entries {
        section.push_str(&format!("- {channel}: {target}\n"));
    }
    section.push('\n');
    Some(section)
}
...
#[cfg(test)]
mod tests {
    use super::build_channel_targets_section;
    ...
    fn returns_none_when_no_channels_configured() {
        assert!(build_channel_targets_section(&ChannelsConfig::default()).is_none());
    }
    ...
    #[test]
    fn returns_none_when_default_target_empty() {
        assert!(build_channel_targets_section(&channels_with_telegram("  ")).is_none());
    }

    #[test]
    fn contains_channel_and_target() {
        let section = build_channel_targets_section(&channels_with_telegram("987654321")).unwrap();
        assert!(section.contains("telegram"));
        assert!(section.contains("987654321"));
    }
    ...
    fn lists_multiple_channels() {
        ...
        assert!(section.contains("#alerts"));
    }
