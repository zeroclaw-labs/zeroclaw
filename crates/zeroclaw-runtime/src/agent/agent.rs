...
impl Agent {
    ...
    fn trim_history(&mut self) {
        ...
        if other_messages.len() > max {
            ...
            // Drop the oldest `drop_count` non-system messages. Any tool-call
            // pairing broken by the cut (a leading ToolResults whose call was
            // dropped, or a trailing unanswered AssistantToolCalls) is repaired
            // by the canonical `enforce_tool_pairing` pass at the top of every
            // turn, so trimming does not re-implement pairing here.
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        // transcript.
        //
        // Pairing is enforced here, the single chokepoint every provider and
        // resume path funnels through, so no per-provider converter repeats it.
        zeroclaw_api::model_provider::enforce_tool_pairing(&mut self.history);
    ...
    pub async fn turn_streamed_with_steering_state(
        ...
        //
        // Pairing is enforced here too, the same single invariant as the
        // non-streamed path, before the history is flattened for any provider.
        zeroclaw_api::model_provider::enforce_tool_pairing(&mut self.history);
