...
/// Returns a list of `(name, description)` pairs for all tools that are enabled
/// in the given configuration. Used by channels (e.g. Telegram `setMyCommands`)
/// to register tool commands without constructing actual tool instances.
///
...
pub fn tool_command_specs_from_config(config: &Config) -> Vec<(&'static str, &'static str)> {
    let mut specs: Vec<(&str, &str)> = vec![
        ...
        ("file_edit", "Edit a file with find-and-replace"),
        ...
        ("cron_run", "Run a scheduled task now"),
        ("cron_runs", "View cron execution history"),
        ...
        ("memory_purge", "Purge all stored memories"),
        ("schedule", "Schedule a one-time reminder"),
        ("model_routing_config", "Configure model routing rules"),
        ...
        ("canvas", "Create and manage canvas content"),
        ("llm_task", "Delegate a task to another LLM"),
        ("screenshot", "Take a screenshot"),
        ("image_info", "Analyze image content"),
        ("ask_user", "Ask the user a question"),
        ("escalate_to_human", "Escalate to a human operator"),
        ("poll", "Create a poll"),
        ("reaction", "Add or remove emoji reactions"),
    ];

    // ── Config-gated tools ───────────────────────────────────────────
    if config.browser.enabled {
        specs.push(("browser_open", "Open a URL in a browser"));
        specs.push(("browser", "Full browser automation"));
    }
    if config.browser_delegate.enabled {
        specs.push(("browser_delegate", "Delegate browser tasks"));
    }
    if config.http_request.enabled {
        specs.push(("http_request", "Make HTTP requests"));
    }
    if config.web_fetch.enabled {
        specs.push(("web_fetch", "Fetch web page content"));
    }
    if config.text_browser.enabled {
        specs.push(("text_browser", "Render web pages as text"));
    }
    if config.web_search.enabled {
        specs.push(("web_search_tool", "Search the web"));
    }
    if config.notion.enabled {
        specs.push(("notion", "Interact with Notion"));
    }
    if config.jira.enabled {
        specs.push(("jira", "Interact with Jira"));
    }
    if config.project_intel.enabled {
        specs.push(("project_intel", "Project delivery intelligence"));
        specs.push(("report_template", "Generate project reports"));
    }
    if config.security_ops.enabled {
        specs.push(("security_ops", "Security operations"));
    }
    if config.backup.enabled {
        specs.push(("backup", "Create workspace backups"));
    }
    if config.data_retention.enabled {
        specs.push(("data_management", "Manage data retention"));
    }
    if config.cloud_ops.enabled {
        specs.push(("cloud_ops", "Cloud operations advisory"));
        specs.push(("cloud_patterns", "Cloud architecture patterns"));
    }
    if config.google_workspace.enabled {
        specs.push(("google_workspace", "Google Workspace operations"));
    }
    if config.claude_code.enabled {
        specs.push(("claude_code", "Delegate to Claude Code"));
    }
    if config.claude_code_runner.enabled {
        specs.push(("claude_code_runner", "Run Claude Code tasks"));
    }
    if config.codex_cli.enabled {
        specs.push(("codex_cli", "Delegate to Codex CLI"));
    }
    if config.gemini_cli.enabled {
        specs.push(("gemini_cli", "Delegate to Gemini CLI"));
    }
    if config.opencode_cli.enabled {
        specs.push(("opencode_cli", "Delegate to OpenCode CLI"));
    }
    if config.linkedin.enabled {
        specs.push(("linkedin", "LinkedIn integration"));
    }
    if config.image_gen.enabled {
        specs.push(("image_gen", "Generate images"));
    }
    if config.sop.sops_dir.is_some() {
        specs.push(("sop_list", "List standard operating procedures"));
        specs.push(("sop_execute", "Execute an SOP"));
        specs.push(("sop_advance", "Advance an SOP step"));
        specs.push(("sop_approve", "Approve an SOP action"));
        specs.push(("sop_status", "Check SOP execution status"));
    }
    if config.microsoft365.enabled {
        specs.push(("microsoft365", "Microsoft 365 integration"));
    }
    if config.knowledge.enabled {
        specs.push(("knowledge", "Knowledge graph operations"));
    }
    if config.workspace.enabled {
        specs.push(("workspace", "Workspace management"));
    }
    if config.verifiable_intent.enabled {
        specs.push(("verifiable_intent", "Verifiable intent confirmation"));
    }
    if config.pipeline.enabled {
        specs.push(("execute_pipeline", "Multi-step tool pipeline"));
    }
    if !config.agents.is_empty() {
        specs.push(("delegate", "Delegate to a sub-agent"));
    }
    if !config.swarms.is_empty() {
        specs.push(("swarm", "Run an agent swarm"));
    }
