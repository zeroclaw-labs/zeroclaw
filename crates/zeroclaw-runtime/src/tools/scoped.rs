...
impl ScopedToolRegistry {
    /// Consume the assembled registry into the owned `Vec`. NOT a turn-path
    /// escape hatch: the engine's carriers only accept the sealed type, so this
    /// exists for non-turn consumers of assembly output - today the gateway's
    /// listing-only `/api/tools` registry (no turn ever runs against it) and
    /// tests that inspect the assembled set.
