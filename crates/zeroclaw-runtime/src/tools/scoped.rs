            ...
            match tools::McpRegistry::connect_all(&agent_mcp_servers).await {
                Ok(registry) => {
                    ...
                    if config.mcp.deferred_loading {
                        ...
                        // tool set inside `assemble`: the same `filtered_deferred`
                        // drives the prompt-side `build_deferred_tools_section_filtered`
                        // and the `ToolSearchTool` constructor, so a denied tool
