...
pub async fn pause_current_goal_for_human_gate(
    ...
    let admission = pause_running_goal_for_human_gate(
        cp.goal_store.as_ref(),
        resolved,
        config,
        GoalPauseState {
...
/// Stop a live goal at a synchronous human gate without overwriting a
/// concurrent operator or terminal transition. The task row and goal extension
/// are updated by one conditional store operation.
async fn pause_running_goal_for_human_gate(
    ...
    let budget = task_goal_budget_summary(&task_goal, config);
    if !goal_store
        .pause_running_goal_task(&task_id, pause)
        .await
        .with_context(|| msg("goal-command-error-pause-failed", &[("task_id", &task_id)]))?
    {
        bail!("goal task {task_id} changed before human-gate pause");
    }
    Ok(GoalAdmission {
        task_id: Some(task_id.clone()),
...
async fn resolve_goal_task(
    ...
    // A caller-provided target is authoritative for an operator command. For
    // trusted controller continuations, the task bound to the admission scope
    // is equally authoritative: never replace it with the latest active goal
    // from the same route/principal context.
    if let Some(task_id) = task_id.or_else(|| ctx.goal_task_id.clone()) {
        let task = store
            .get(&task_id)
            .await
...
/// Resolve the exact task already bound to this turn, or the latest visible
/// active task for an interactive controller command. The bound task wins so
...
mod tests {
    ...
    #[tokio::test]
    async fn bound_terminal_goal_never_falls_back_to_newer_active_goal() {
        let store = SqliteTaskStore::new_in_memory().unwrap();
        let ctx = GoalAdmissionContext::new("agent-a")
            .with_originator_route(Some("matrix:room-1".into()))
            .with_principal_id(Some("principal-1".into()));
        let first = start_goal(
            &store,
            "boot-a",
            ctx.clone(),
            "finish the first task".into(),
            None,
            None,
            None,
        )
        ...
        cancel_goal(&store, &store, &ctx, Some(first_id.clone()), None)
            .await
            .unwrap();

        let second = start_goal(
            &store,
            "boot-b",
            ctx.clone(),
            "finish the replacement task".into(),
            None,
            None,
            None,
        )
        ...
        let bound_first = ctx.with_goal_task_id(Some(first_id.clone()));
        let err = pause_goal_for_blocker(
            &store,
            &store,
            &bound_first,
            None,
            ...
            GoalPauseState {
                reason: GoalPauseReason::OperatorPaused,
                description: Some("stale continuation".into()),
                blockers: Vec::new(),
            },
        )
        .await
        ...
        assert_eq!(
            ...
            TaskStatus::Running,
            "a continuation bound to the terminal first goal must not pause its replacement"
        );
    }
