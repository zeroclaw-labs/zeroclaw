...
tokio::task_local! {
    ...
    static GOAL_TURN_EVALUATION_REQUESTED: Option<Arc<AtomicBool>>;
...
where
    F: std::future::Future,
{
    GOAL_TURN_EVALUATION_REQUESTED.scope(marker, future).await
}
...
pub fn mark_current_goal_turn_for_evaluation() {
    let _ = GOAL_TURN_EVALUATION_REQUESTED.try_with(|marker| {
...
pub fn current_goal_turn_evaluation_requested() -> bool {
    GOAL_TURN_EVALUATION_REQUESTED
        .try_with(|marker| {
            marker
                .as_ref()
