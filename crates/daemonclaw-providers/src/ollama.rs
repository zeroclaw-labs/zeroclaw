...
impl Provider for OllamaProvider {
    ...
    fn supports_native_tools(&self) -> bool {
        ...
        // See: https://github.com/DeliveryBoyTech/daemonclaw/issues/3999
        false
    }

    async fn chat(
        &self,
        request: daemonclaw_api::provider::ChatRequest<'_>,
        model: &str,
        temperature: f64,
    ) -> anyhow::Result<ChatResponse> {
            ...
            && !specs.is_empty()
        {
            let tools: Vec<serde_json::Value> = specs
                ...
                .map(|s| {
                    let params =
                        daemonclaw_api::schema::SchemaCleanr::clean_for_openai(s.parameters.clone());
                    serde_json::json!({
                        "type": "function",
                        "function": {
                            "name": s.name,
