...
use daemonclaw_config::schema::{MultimodalConfig, build_runtime_proxy_client_with_timeouts};
...
mod tests {
    ...
    fn parse_image_markers_collapses_line_wrapped_path() {
        ...
        let input = "from the logs whether the agent emits\n  [IMAGE:/home/daemonclaw_user/.daemonclaw/workspace/signal_i\n  nbound/attachment.jpg] (which the\n  channel resolves)";
        let (_, refs) = parse_image_markers(input);
        assert_eq!(refs.len(), 1);
        assert_eq!(
            refs[0],
            "/home/daemonclaw_user/.daemonclaw/workspace/signal_inbound/attachment.jpg"
        );
    }
