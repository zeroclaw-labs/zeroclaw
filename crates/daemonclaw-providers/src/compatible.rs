...
impl OpenAiCompatibleProvider {
    ...
    fn http_client(&self) -> Client {
        ...
        if has_user_agent || has_extra_headers {
            ...
            let builder = daemonclaw_config::schema::apply_runtime_proxy_to_builder(
                builder,
                "provider.compatible",
            );
        ...
        daemonclaw_config::schema::build_runtime_proxy_client_with_timeouts(
            "provider.compatible",
            timeout,
            10,
        )
    ...
    fn tool_specs_to_openai_format(
        tools: &[daemonclaw_api::tool::ToolSpec],
    ) -> Vec<serde_json::Value> {
        tools
            .iter()
            .map(|tool| {
                let params =
                    daemonclaw_api::schema::SchemaCleanr::clean_for_openai(tool.parameters.clone());
                serde_json::json!({
                    "type": "function",
                    "function": {
                        "name": tool.name,
...
impl OpenAiCompatibleProvider {
    ...
    fn convert_tool_specs(
        tools: Option<&[daemonclaw_api::tool::ToolSpec]>,
    ) -> Option<Vec<serde_json::Value>> {
        tools.map(|items| {
            items
                .iter()
                .map(|tool| {
                    let params = daemonclaw_api::schema::SchemaCleanr::clean_for_openai(
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions = daemonclaw_api::provider::build_tool_instructions_text(tools);
...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(&self) -> daemonclaw_api::provider::ProviderCapabilities {
        daemonclaw_api::provider::ProviderCapabilities {
...
mod tests {
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are DaemonClaw".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn prompt_guided_tool_fallback_injects_system_instruction() {
        ...
        let tools = vec![daemonclaw_api::tool::ToolSpec {
    ...
    fn user_agent_constructor_keeps_native_tool_calling_enabled() {
        ...
        assert!(caps.native_tool_calling);
        assert!(!caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("daemonclaw-test/1.0"));
    }
    ...
    fn user_agent_and_vision_constructor_preserves_capability_flags() {
        let p = OpenAiCompatibleProvider::new_with_user_agent_and_vision(
            ...
            Some("k"),
            AuthStyle::Bearer,
            "daemonclaw-test/vision",
            true,
        );
        let caps = <OpenAiCompatibleProvider as Provider>::capabilities(&p);
        assert!(caps.native_tool_calling);
        assert!(caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("daemonclaw-test/vision"));
    }
    ...
    fn tool_specs_convert_to_openai_format() {
        let specs = vec![daemonclaw_api::tool::ToolSpec {
    ...
    fn with_extra_headers_sets_headers() {
        ...
        headers.insert("X-Title".to_string(), "daemonclaw".to_string());
        headers.insert(
        ...
        assert_eq!(p.extra_headers.get("X-Title").unwrap(), "daemonclaw");
        assert_eq!(
