//! Wraps a discovered MCP tool as a daemonclaw [`Tool`] so it is dispatched
...
/// A daemonclaw [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // DaemonClaw's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // DaemonClaw's security model injects `approved: bool` into built-in tool args.
