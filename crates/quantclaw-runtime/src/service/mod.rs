...
const SERVICE_LABEL: &str = "com.quantclaw.daemon";
const WINDOWS_TASK_NAME: &str = "QuantClaw Daemon";
...
/// Returns whether the QuantClaw daemon service is currently running.
...
fn is_running_linux() -> bool {
    ...
    if run_capture(Command::new("systemctl").args(["--user", "is-active", "quantclaw.service"]))
        .map(|out| out.trim() == "active")
        .unwrap_or(false)
    {
        return true;
    }
    run_capture(Command::new("rc-service").args(["quantclaw", "status"]))
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "quantclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["quantclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "quantclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["quantclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "quantclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["quantclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["quantclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/quantclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn logs_linux(config: &Config, init_system: InitSystem, lines: usize, follow: bool) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            // OpenRC logs go to /var/log/quantclaw/error.log (as configured in the init script)
            let log_file = Path::new("/var/log/quantclaw/error.log");
            if !log_file.exists() {
                // Fall back to access log
                let access_log = Path::new("/var/log/quantclaw/access.log");
                if !access_log.exists() {
                    bail!("No log files found at /var/log/quantclaw/. Is the service installed?");
                }
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/quantclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "quantclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove quantclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/quantclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
/// corresponding `var/quantclaw` directory.
///
...
fn detect_homebrew_var_dir(exe: &Path) -> Option<PathBuf> {
    ...
    // Symlinked binary: <prefix>/bin/quantclaw
    // Cellar binary:    <prefix>/Cellar/quantclaw/<version>/bin/quantclaw
    let prefix = if path_str.contains("/Cellar/") {
        // Walk up from .../Cellar/quantclaw/<ver>/bin/quantclaw to the prefix
        let mut ancestor = exe.to_path_buf();
        while let Some(parent) = ancestor.parent() {
            ancestor = parent.to_path_buf();
            if ancestor.file_name().is_some_and(|n| n == "Cellar") {
                // prefix is one level above Cellar
                return ancestor.parent().map(|p| p.join("var").join("quantclaw"));
            }
        }
        return None;
    } else if let Some(bin_parent) = exe.parent() {
        // <prefix>/bin/quantclaw → check if <prefix>/Cellar exists (Homebrew marker)
    ...
    prefix.map(|p| p.join("var").join("quantclaw"))
}
...
fn install_macos(config: &Config) -> Result<()> {
    ...
    // data so that `brew services start quantclaw` works out of the box.
    ...
    // When running under Homebrew, inject QUANTCLAW_CONFIG_DIR and
...
"#,
    ...
    println!("   Start with: quantclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "quantclaw.service"]));
...
/// Check if the quantclaw user exists and has expected properties.
...
fn check_quantclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "quantclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser quantclaw && delgroup quantclaw",
            "addgroup -S quantclaw && adduser -S -s /sbin/nologin -H -D -G quantclaw quantclaw",
        )
    } else {
        ("userdel quantclaw", "useradd -r -s /sbin/nologin quantclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'quantclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'quantclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/quantclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: quantclaw user has home directory '{}' (expected /var/lib/quantclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_quantclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "quantclaw"]).output();
    if let Ok(output) = output
        && output.status.success()
    {
        return check_quantclaw_user();
    }

    let is_alpine = Path::new("/etc/alpine-release").exists();

    if is_alpine {
        let group_output = Command::new("getent").args(["group", "quantclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "quantclaw"])
                .output()
                .context("Failed to create quantclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create quantclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: quantclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create quantclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create quantclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "quantclaw"])
            .output()
            .context("Failed to create quantclaw user")?;
    ...
    println!("✅ Created system user: quantclaw");
    Ok(())
}

/// Change ownership of a path to quantclaw:quantclaw
#[cfg(unix)]
fn chown_to_quantclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["quantclaw:quantclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to quantclaw:quantclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_quantclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_quantclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "quantclaw:quantclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to quantclaw:quantclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_quantclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn resolve_invoking_user_config_dir() -> Option<PathBuf> {
        ...
        && output.status.success()
    {
        ...
        if fields.len() >= 6 {
            return Some(PathBuf::from(fields[5]).join(".quantclaw"));
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'quantclaw' cannot write {} ({details}). \
             Re-run `sudo quantclaw service install` and ensure ownership is quantclaw:quantclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/quantclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="quantclaw:quantclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/quantclaw/access.log"
error_log="/var/log/quantclaw/error.log"
...
start_pre() {{
    checkpath --directory --owner quantclaw:quantclaw --mode 0750 /var/lib/quantclaw
}}
"#,
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/quantclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo quantclaw service install"
        );
    }

    ensure_quantclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/quantclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/quantclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_quantclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by quantclaw:quantclaw)",
    ...
    chown_recursive_to_quantclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/quantclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "quantclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/quantclaw");
    println!("   Config path: /etc/quantclaw/config.toml");
    println!("   Start with: sudo quantclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("quantclaw-daemon.cmd");
    let stdout_log = logs_dir.join("daemon.stdout.log");
    let stderr_log = logs_dir.join("daemon.stderr.log");
...
#[cfg(all(test, quantclaw_root_crate))]
mod tests {
    use super::*;
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/quantclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "QuantClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/quantclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/quantclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"quantclaw\""));
        assert!(script.contains("description=\"QuantClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/quantclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/quantclaw daemon\""));
        assert!(!script.contains("env QUANTCLAW_CONFIG_DIR"));
        assert!(!script.contains("env QUANTCLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"quantclaw:quantclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/quantclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/quantclaw/error.log\""));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/quantclaw\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner quantclaw:quantclaw"),
            "start_pre must ensure /var/lib/quantclaw exists with correct ownership"
        );
    }
    ...
    fn systemd_unit_contains_home_and_pass_environment() {
        let unit = "[Unit]\n\
             ...
             [Service]\n\
             Type=simple\n\
             ExecStart=/usr/local/bin/quantclaw daemon\n\
             Restart=always\n\
             RestartSec=3\n\
             # Ensure HOME is set so headless browsers can create profile/cache dirs.\n\
             Environment=HOME=%h\n\
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/quantclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/quantclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/quantclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/quantclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/quantclaw'".to_string()
            ]
        );
    }
    ...
    fn detect_homebrew_var_dir_from_cellar_path() {
        let exe = PathBuf::from("/opt/homebrew/Cellar/quantclaw/1.2.3/bin/quantclaw");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/opt/homebrew/var/quantclaw")));
    }

    #[test]
    fn detect_homebrew_var_dir_intel_cellar_path() {
        let exe = PathBuf::from("/usr/local/Cellar/quantclaw/1.0.0/bin/quantclaw");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/usr/local/var/quantclaw")));
    }

    #[test]
    fn detect_homebrew_var_dir_non_homebrew_path() {
        let exe = PathBuf::from("/home/user/.cargo/bin/quantclaw");
        let var_dir = detect_homebrew_var_dir(&exe);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/quantclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/quantclaw/workspace'".to_string(),
                "quantclaw".to_string()
            ]
        );
    }
    ...
    fn tail_file_errors_on_missing_file() {
        let missing = Path::new("/tmp/quantclaw-test-nonexistent-log-file.log");
