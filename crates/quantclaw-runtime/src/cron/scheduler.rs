...
use quantclaw_config::schema::{CronJobDecl, CronScheduleDecl};
...
async fn run_agent_job(
    ...
    let memory_context = match quantclaw_memory::create_memory(
        &config.memory,
        &config.workspace_dir,
...
///
/// Uses `sh -c <command>` (non-login shell). On Windows, QuantClaw users
...
mod tests {
    ...
    async fn persist_job_result_delivery_stubbed_succeeds() {
        // Delivery is stubbed (moved to quantclaw-channels orchestrator).
    ...
    // scan_and_redact_output tests moved to quantclaw-channels orchestrator
