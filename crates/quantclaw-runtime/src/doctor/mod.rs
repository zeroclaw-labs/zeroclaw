...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 QuantClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `quantclaw doctor` again.");
    }
...
fn doctor_model_targets(provider_override: Option<&str>) -> Vec<String> {
    ...
    quantclaw_providers::list_providers()
        .into_iter()
...
pub async fn run_models(
    ...
    println!("🩺 QuantClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `quantclaw doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `quantclaw onboard` to set one up",
        ));
    }
...
fn provider_validation_error(name: &str) -> Option<String> {
    match quantclaw_providers::create_provider(name, None) {
        Ok(_) => None,
        Err(err) => Some(
            err.to_string()
                .lines()
...
mod tests {
    ...
    fn config_validation_warns_empty_model_route() {
        let config = Config {
            model_routes: vec![quantclaw_config::schema::ModelRouteConfig {
                hint: "fast".into(),
                provider: "groq".into(),
                model: String::new(),
                api_key: None,
    ...
    fn config_validation_warns_empty_embedding_route_model() {
        let config = Config {
            embedding_routes: vec![quantclaw_config::schema::EmbeddingRouteConfig {
                hint: "semantic".into(),
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(
            first
                ...
                .is_some_and(|name| name.starts_with(".quantclaw_doctor_probe_"))
        );
    }
    ...
    fn config_validation_reports_delegate_agents_in_sorted_order() {
        ...
        config.agents.insert(
            "zeta".into(),
            quantclaw_config::schema::DelegateAgentConfig {
