...
use quantclaw_memory::{self, Memory, MemoryCategory};
...
pub async fn migrate_openclaw_memory(
    ...
    if paths_equal(&source_workspace, &config.workspace_dir) {
        bail!("Source workspace matches current QuantClaw workspace; refusing self-migration");
    }
...
fn target_memory_backend(config: &Config) -> Result<Box<dyn Memory>> {
    quantclaw_memory::create_memory_for_migration(&config.memory.backend, &config.workspace_dir)
}
...
mod tests {
    ...
    use quantclaw_config::schema::{Config, MemoryConfig};
