...
use quantclaw_api::provider::{ChatMessage, Provider};
use quantclaw_memory::traits::Memory;

pub use quantclaw_config::scattered_types::ContextCompressionConfig;
        ...
        if let Some(ref memory) = self.memory {
            ...
            if let Err(e) = memory
                .store(
                    &facts_key,
                    &summary,
                    quantclaw_memory::traits::MemoryCategory::Daily,
                    None,
                )
                .await
            {
