...
pub struct DaemonSubsystems {
    ...
    pub mqtt_start: Option<
        Box<
            dyn Fn(
                    quantclaw_config::schema::MqttConfig,
                ) -> std::pin::Pin<Box<dyn Future<Output = Result<()>> + Send>>
                + Send
                + Sync,
        >,
...
pub async fn run(
    ...
    println!("🧠 QuantClaw daemon started");
...
async fn run_heartbeat_worker(config: Config) -> Result<()> {
    ...
    loop {
        ...
        let heartbeat_memory: Option<Box<dyn quantclaw_memory::Memory>> =
            quantclaw_memory::create_memory(
                &config.memory,
                    ...
                    Ok(entries) if !entries.is_empty() => {
                        let ctx: String = entries
                            ...
                            .filter(|e| {
                                !matches!(
                                    e.category,
                                    quantclaw_memory::traits::MemoryCategory::Conversation
                                )
                            })
                        ...
                        && let Some(ref mem) = heartbeat_memory
                    {
                        ...
                        let _ = mem
                            .store(
                                ...
                                quantclaw_memory::MemoryCategory::Daily,
                                None,
                            )
                            .await;
                    }
...
fn load_heartbeat_session_context(config: &Config) -> Option<String> {
    use quantclaw_providers::traits::ChatMessage;
...
fn load_jsonl_messages(path: &std::path::Path) -> Vec<quantclaw_providers::traits::ChatMessage> {
    ...
    let mut window: VecDeque<quantclaw_providers::traits::ChatMessage> =
    ...
    for line in reader.lines() {
        ...
        if let Ok(msg) = serde_json::from_str::<quantclaw_providers::traits::ChatMessage>(trimmed) {
...
// run_mqtt_sop_listener has been moved to quantclaw-channels::orchestrator::mqtt.
...
mod tests {
    ...
    fn detects_supervised_channels_present() {
        ...
        config.channels_config.telegram = Some(quantclaw_config::schema::TelegramConfig {
            enabled: true,
            bot_token: "token".into(),
            allowed_users: vec![],
            stream_mode: quantclaw_config::schema::StreamMode::default(),
    ...
    fn detects_dingtalk_as_supervised_channel() {
        let mut config = Config::default();
        config.channels_config.dingtalk = Some(quantclaw_config::schema::DingTalkConfig {
            enabled: true,
    ...
    fn detects_mattermost_as_supervised_channel() {
        let mut config = Config::default();
        config.channels_config.mattermost = Some(quantclaw_config::schema::MattermostConfig {
            enabled: true,
    ...
    fn detects_qq_as_supervised_channel() {
        let mut config = Config::default();
        config.channels_config.qq = Some(quantclaw_config::schema::QQConfig {
            enabled: true,
            app_id: "app-id".into(),
    ...
    fn detects_nextcloud_talk_as_supervised_channel() {
        ...
        config.channels_config.nextcloud_talk =
            Some(quantclaw_config::schema::NextcloudTalkConfig {
                enabled: true,
