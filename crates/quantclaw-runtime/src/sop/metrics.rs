...
use quantclaw_memory::traits::{Memory, MemoryCategory};
...
mod tests {
    ...
    async fn warm_start_roundtrip() {
        let mem_cfg = quantclaw_config::schema::MemoryConfig {
            backend: "sqlite".into(),
            ..quantclaw_config::schema::MemoryConfig::default()
        };
        let tmp = tempfile::tempdir().unwrap();
        let memory: std::sync::Arc<dyn Memory> = std::sync::Arc::from(
            quantclaw_memory::create_memory(&mem_cfg, tmp.path(), None).unwrap(),
        );
