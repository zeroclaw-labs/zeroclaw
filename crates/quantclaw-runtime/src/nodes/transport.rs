...
/// - `X-QuantClaw-Timestamp` — unix epoch seconds
/// - `X-QuantClaw-Nonce` — random UUID v4
/// - `X-QuantClaw-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-QuantClaw-Timestamp", timestamp.to_string())
            .header("X-QuantClaw-Nonce", &nonce)
            .header("X-QuantClaw-Signature", &signature)
