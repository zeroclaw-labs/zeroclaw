...
pub struct MatrixChannel {
    ...
    stream_mode: daemonclaw_config::schema::StreamMode,
...
impl MatrixChannel {
    ...
    fn sanitize_error_for_log(error: &impl std::fmt::Display) -> String {
        daemonclaw_providers::sanitize_api_error(&error.to_string())
    }
    ...
    pub fn new_with_session_hint_and_daemonclaw_dir(
        homeserver: String,
        access_token: String,
        room_id: String,
        allowed_users: Vec<String>,
    ...
    pub fn new_full(
        ...
        Self {
            ...
            stream_mode: daemonclaw_config::schema::StreamMode::Off,
    ...
    async fn load_or_generate_device_id(&self) -> anyhow::Result<String> {
        ...
        let device_id = format!("DAEMONCLAW_{}", &uuid::Uuid::new_v4().to_string()[..8]);
        tracing::info!(
                    ...
                    if let Some(hinted) = self.session_owner_hint.as_ref()
                        && hinted != &whoami.user_id {
                            tracing::warn!(
                                ...
                                daemonclaw_runtime::security::redact(hinted),
                                daemonclaw_runtime::security::redact(&whoami.user_id)
                            );
                        }
                        ...
                        if let Some(whoami_device_id) = whoami.device_id.as_ref() {
                            if whoami_device_id != hinted {
                                tracing::warn!(
                                    ...
                                    daemonclaw_runtime::security::redact(hinted),
                                    daemonclaw_runtime::security::redact(whoami_device_id)
                                );
                            }
...
impl Channel for MatrixChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        if self.voice_mode.load(Ordering::Relaxed) {
            ...
            let voice_work = std::path::PathBuf::from("/tmp/daemonclaw-voice");
            ...
            async move {
                ...
                let body = if let Some((source, filename)) = media_download {
                    let workspace = std::path::PathBuf::from(
                        shellexpand::tilde(
                            &std::env::var("DAEMONCLAW_WORKSPACE")
                                .unwrap_or_else(|_| "/tmp/daemonclaw-uploads".to_string()),
                        )
                        .as_ref(),
                    );
    ...
    fn supports_draft_updates(&self) -> bool {
        self.stream_mode != daemonclaw_config::schema::StreamMode::Off
    }
    ...
    async fn update_draft_progress(
        ...
        if self.stream_mode == daemonclaw_config::schema::StreamMode::Partial {
...
mod tests {
    ...
    #[test]
    fn matrix_store_dir_is_derived_from_daemonclaw_dir() {
        let ch = MatrixChannel::new_with_session_hint_and_daemonclaw_dir(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
            vec![],
            None,
            None,
            Some(PathBuf::from("/tmp/daemonclaw")),
        );

        assert_eq!(
            ch.matrix_store_dir(),
            Some(PathBuf::from("/tmp/daemonclaw/state/matrix"))
        );
    }

    #[test]
    fn matrix_store_dir_absent_without_daemonclaw_dir() {
        let ch = MatrixChannel::new_with_session_hint(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
