...
use tokio::fs;
use daemonclaw_api::channel::{Channel, ChannelMessage, SendMessage};
use daemonclaw_config::schema::{Config, StreamMode};
use daemonclaw_runtime::security::pairing::PairingGuard;
...
pub struct TelegramChannel {
    ...
    api_base: String,
    transcription: Option<daemonclaw_config::schema::TranscriptionConfig>,
    transcription_manager: Option<std::sync::Arc<super::transcription::TranscriptionManager>>,
    voice_transcriptions: Mutex<std::collections::HashMap<String, String>>,
    workspace_dir: Option<std::path::PathBuf>,
    ack_reactions: bool,
    tts_config: Option<daemonclaw_config::schema::TtsConfig>,
    ...
    pending_approvals: Arc<
        tokio::sync::Mutex<
            std::collections::HashMap<
                String,
                tokio::sync::oneshot::Sender<daemonclaw_api::channel::ChannelApprovalResponse>,
            >,
        >,
    >,
...
impl TelegramChannel {
    ...
    pub fn with_transcription(
        mut self,
        config: daemonclaw_config::schema::TranscriptionConfig,
    ) -> Self {
        if !config.enabled {
            return self;
        }
    ...
    pub fn with_tts(mut self, config: daemonclaw_config::schema::TtsConfig) -> Self {
        if config.enabled {
    ...
    fn http_client(&self) -> reqwest::Client {
        daemonclaw_config::schema::build_channel_proxy_client(
            "channel.telegram",
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = daemonclaw_dir.join("workspace");
        Ok(config)
    }
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        ...
        let Some(telegram) = config.channels.telegram.as_mut() else {
            anyhow::bail!(
                ...
                or run `daemonclaw onboard --channels-only` to configure interactively"
            );
        };
    ...
    async fn synthesize_and_send_voice(
        ...
        text: &str,
        tts_config: &daemonclaw_config::schema::TtsConfig,
    ) -> anyhow::Result<()> {
        ...
        let client = daemonclaw_config::schema::build_runtime_proxy_client("channel.telegram");
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to DaemonClaw now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Ensure only one `daemonclaw` process is using this bot token."
                    );
                    ...
                    if let Some(cb) = update.get("callback_query") {
                            ...
                            && let Some((approval_id, action)) = rest.rsplit_once(':')
                        {
                            let response = match action {
                                "approve" => {
                                    Some(daemonclaw_api::channel::ChannelApprovalResponse::Approve)
                                }
                                "always" => Some(
                                    daemonclaw_api::channel::ChannelApprovalResponse::AlwaysApprove,
                                ),
                                "deny" => {
                                    Some(daemonclaw_api::channel::ChannelApprovalResponse::Deny)
                                }
                                other => {
    ...
    async fn request_approval(
        &self,
        recipient: &str,
        request: &daemonclaw_api::channel::ChannelApprovalRequest,
    ) -> anyhow::Result<Option<daemonclaw_api::channel::ChannelApprovalResponse>> {
        use daemonclaw_api::channel::ChannelApprovalResponse;
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@daemonclaw_bot 654321"),
            Some("654321")
        );
    }
    ...
    fn with_transcription_sets_config_when_enabled() {
        let tc = daemonclaw_config::schema::TranscriptionConfig {
            enabled: true,
            api_key: Some("test_key".to_string()),
            ..daemonclaw_config::schema::TranscriptionConfig::default()
        };
    ...
    fn with_transcription_skips_when_disabled() {
        let tc = daemonclaw_config::schema::TranscriptionConfig::default(); // enabled = false
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false).with_transcription(tc);
    ...
    async fn e2e_live_voice_transcription_and_reply_cache() {
        ...
        let config = daemonclaw_config::schema::TranscriptionConfig {
            enabled: true,
            ..Default::default()
        };
    ...
    fn markdown_attachment_not_detected_by_multimodal_image_markers() {
        ...
        let messages = vec![daemonclaw_providers::ChatMessage::user(content)];
        assert_eq!(
            daemonclaw_providers::multimodal::count_image_markers(&messages),
            0,
    ...
    fn photo_image_marker_detected_by_multimodal() {
        ...
        let messages = vec![daemonclaw_providers::ChatMessage::user(
            photo_content.to_string(),
        )];
        let count = daemonclaw_providers::multimodal::count_image_markers(&messages);
        assert_eq!(
            count, 1,
    ...
    fn e2e_attachment_saves_file_and_formats_content() {
        ...
        let doc_msgs = vec![daemonclaw_providers::ChatMessage::user(doc_content)];
        assert_eq!(
            daemonclaw_providers::multimodal::count_image_markers(&doc_msgs),
            0,
        ...
        let photo_msgs = vec![daemonclaw_providers::ChatMessage::user(photo_content.clone())];
        assert_eq!(
            daemonclaw_providers::multimodal::count_image_markers(&photo_msgs),
            1,
        ...
        let cap_msgs = vec![daemonclaw_providers::ChatMessage::user(captioned.clone())];
        assert_eq!(
            daemonclaw_providers::multimodal::count_image_markers(&cap_msgs),
            1,
        ...
        let md_msgs = vec![daemonclaw_providers::ChatMessage::user(md_content)];
        assert_eq!(
            daemonclaw_providers::multimodal::count_image_markers(&md_msgs),
            0,
            "markdown file must not trigger image marker detection"
        );
    }
    ...
    fn groq_provider_rejects_photo_with_vision_error() {
        ...
        use daemonclaw_providers::compatible::{AuthStyle, OpenAiCompatibleProvider};
        ...
        let marker_count = daemonclaw_providers::multimodal::count_image_markers(&messages);
