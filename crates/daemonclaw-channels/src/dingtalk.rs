...
impl Channel for DingTalkChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        let title = message.subject.as_deref().unwrap_or("DaemonClaw");
        let body = serde_json::json!({
            "msgtype": "markdown",
            "markdown": {
                "title": title,
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        let (ws_stream, _) = daemonclaw_config::schema::ws_connect_with_proxy(
            &ws_url,
            "channel.dingtalk",
            self.proxy_url.as_deref(),
        )
...
allowed_users = ["user1", "*"]
"#;
        let config: daemonclaw_config::schema::DingTalkConfig = toml::from_str(toml_str).unwrap();
