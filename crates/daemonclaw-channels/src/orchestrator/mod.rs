...
//! DaemonClaw to external platforms. Each channel implements the [`Channel`] trait
...
//! `daemonclaw-channels/src/`, declare it in `lib.rs` behind the appropriate feature
...
pub use daemonclaw_api::channel::{Channel, ChannelMessage, SendMessage};
// Local channel types (in misc, not daemonclaw-channels)
...
pub use daemonclaw_infra::debounce::MessageDebouncer;
pub use daemonclaw_infra::session_backend::SessionBackend;
pub use daemonclaw_infra::session_sqlite::SqliteSessionBackend;
pub use daemonclaw_infra::stall_watchdog::StallWatchdog;
...
use daemonclaw_providers::reliable::{scope_provider_fallback, take_last_provider_fallback};
use daemonclaw_providers::{self, ChatMessage, Provider};
use daemonclaw_runtime::agent::loop_::{
...
use daemonclaw_runtime::approval::ApprovalManager;
use daemonclaw_runtime::observability::traits::{ObserverEvent, ObserverMetric};
use daemonclaw_runtime::observability::{self, Observer, runtime_trace};
use daemonclaw_runtime::platform;
use daemonclaw_runtime::security::{AutonomyLevel, SecurityPolicy};
use daemonclaw_runtime::tools::{self, Tool};
use daemonclaw_runtime::util::truncate_with_ellipsis;
...
// System prompt functions live in `daemonclaw_runtime::agent::system_prompt`.
#[allow(unused_imports)]
pub use daemonclaw_runtime::agent::system_prompt::{
...
struct ChannelRuntimeDefaults {
    ...
    reliability: daemonclaw_config::schema::ReliabilityConfig,
}
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "daemonclaw.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "daemonclaw.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["daemonclaw", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["daemonclaw", "restart"];
...
struct ChannelCostTrackingState {
    tracker: Arc<daemonclaw_runtime::cost::CostTracker>,
    prices: Arc<HashMap<String, daemonclaw_config::schema::ModelPricing>>,
}
...
struct ChannelRuntimeContext {
    ...
    prompt_config: Arc<daemonclaw_config::schema::Config>,
    ...
    api_key: Option<String>,
    api_url: Option<String>,
    reliability: Arc<daemonclaw_config::schema::ReliabilityConfig>,
    provider_runtime_options: daemonclaw_providers::ProviderRuntimeOptions,
    workspace_dir: Arc<PathBuf>,
    message_timeout_secs: u64,
    interrupt_on_new_message: InterruptOnNewMessageConfig,
    multimodal: daemonclaw_config::schema::MultimodalConfig,
    media_pipeline: daemonclaw_config::schema::MediaPipelineConfig,
    transcription_config: daemonclaw_config::schema::TranscriptionConfig,
    hooks: Option<Arc<daemonclaw_runtime::hooks::HookRunner>>,
    non_cli_excluded_tools: Arc<Vec<String>>,
    autonomy_level: AutonomyLevel,
    tool_call_dedup_exempt: Arc<Vec<String>>,
    model_routes: Arc<Vec<daemonclaw_config::schema::ModelRouteConfig>>,
    ...
    activated_tools:
        Option<std::sync::Arc<std::sync::Mutex<daemonclaw_runtime::tools::ActivatedToolSet>>>,
...
fn conversation_memory_key(msg: &daemonclaw_api::channel::ChannelMessage) -> String {
...
fn followup_thread_id(msg: &daemonclaw_api::channel::ChannelMessage) -> Option<String> {
    msg.thread_ts.clone().or_else(|| Some(msg.id.clone()))
}

fn interruption_scope_key(msg: &daemonclaw_api::channel::ChannelMessage) -> String {
...
fn decrypt_optional_secret_for_runtime_reload(
    store: &daemonclaw_runtime::security::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone()
        && daemonclaw_runtime::security::SecretStore::is_encrypted(&raw)
    {
        *value = Some(
            store
                .decrypt(&raw)
...
async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
    ...
    let next_default_provider = daemonclaw_providers::create_resilient_provider_with_options(
    ...
    if let Err(err) = next_default_provider.warmup().await {
        if daemonclaw_providers::reliable::is_non_retryable(&err) {
            tracing::warn!(
...
fn refreshed_new_session_system_prompt(ctx: &ChannelRuntimeContext) -> String {
    let refreshed_skills = daemonclaw_runtime::skills::skills_to_prompt_with_mode(
        &daemonclaw_runtime::skills::load_skills_with_config(
...
fn should_rollback_failed_user_turn(error: &anyhow::Error) -> bool {
    if error
        .downcast_ref::<daemonclaw_providers::ProviderCapabilityError>()
        .is_some_and(|capability| capability.capability.eq_ignore_ascii_case("vision"))
    {
        return true;
    }

    daemonclaw_providers::reliable::is_non_retryable(error)
}

fn should_skip_memory_context_entry(key: &str, content: &str) -> bool {
    if daemonclaw_memory::is_assistant_autosave_key(key) {
        return true;
    }
    ...
    if daemonclaw_memory::is_user_autosave_key(key) {
        return true;
    }

    if daemonclaw_memory::should_skip_autosave_content(content) {
        return true;
    }
...
async fn create_resilient_provider_nonblocking(
    ...
    tokio::task::spawn_blocking(move || {
        daemonclaw_providers::create_resilient_provider_with_options(
            &provider_name,
            api_key.as_deref(),
            api_url.as_deref(),
            &reliability,
...
fn build_models_help_response(
    ...
    workspace_dir: &Path,
    model_routes: &[daemonclaw_config::schema::ModelRouteConfig],
) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `daemonclaw models refresh --provider {}`.",
...
fn build_providers_help_response(current: &ChannelRouteSelection) -> String {
    ...
    for provider in daemonclaw_providers::list_providers() {
...
fn build_config_text_response(
    ...
    resp.push_str("\nAvailable providers:\n");
    for p in daemonclaw_providers::list_providers() {
...
fn build_config_block_kit(
    ...
    let provider_options: Vec<serde_json::Value> = daemonclaw_providers::list_providers()
        .iter()
        .map(|p| {
            serde_json::json!({
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "daemonclaw_config_provider",
    ...
    let mut model_select = serde_json::json!({
        "type": "static_select",
        "action_id": "daemonclaw_config_model",
...
async fn handle_runtime_command_if_needed(
    ctx: &ChannelRuntimeContext,
    msg: &daemonclaw_api::channel::ChannelMessage,
    ...
    let response = match command {
            ...
            match resolve_provider_alias(&raw_provider) {
                Some(provider_name) => {
                    match get_or_create_provider(ctx, &provider_name, None).await {
                        ...
                        Err(err) => {
                            let safe_err = daemonclaw_providers::sanitize_api_error(&err.to_string());
                            format!(
        ...
        ChannelRuntimeCommand::ShowConfig => {
            if msg.channel == "slack" {
                ...
                format!("__DAEMONCLAW_BLOCK_KIT__{blocks_json}")
            } else {
...
fn sanitize_channel_response(response: &str, tools: &[Box<dyn Tool>]) -> String {
    ...
    match daemonclaw_runtime::security::LeakDetector::new().scan(&sanitized) {
        daemonclaw_runtime::security::LeakResult::Clean => sanitized,
        daemonclaw_runtime::security::LeakResult::Detected { patterns, redacted } => {
            tracing::warn!(
                patterns = ?patterns,
...
fn spawn_supervised_listener(
    ch: Arc<dyn Channel>,
    tx: tokio::sync::mpsc::Sender<daemonclaw_api::channel::ChannelMessage>,
...
fn spawn_supervised_listener_with_health_interval(
    ...
    tokio::spawn(async move {
        ...
        loop {
            daemonclaw_runtime::health::mark_component_ok(&component);
            ...
            match result {
                Ok(()) => {
                    ...
                    daemonclaw_runtime::health::mark_component_error(
                        &component,
                ...
                Err(e) => {
                    ...
                    daemonclaw_runtime::health::mark_component_error(&component, e.to_string());
                }
            }

            daemonclaw_runtime::health::bump_component_restart(&component);
...
async fn process_channel_message(
    ...
    let mut msg = if let Some(hooks) = &ctx.hooks {
        match hooks.run_on_message_received(msg).await {
            daemonclaw_runtime::hooks::HookResult::Cancel(reason) => {
                tracing::info!(%reason, "incoming message dropped by hook");
                return;
            }
            daemonclaw_runtime::hooks::HookResult::Continue(modified) => modified,
        }
    } else {
        msg
    };
    ...
    if let Some(hint) =
        daemonclaw_runtime::agent::classifier::classify(&ctx.query_classification, &msg.content)
    ...
    if ctx.auto_save_memory
        ...
        && !daemonclaw_memory::should_skip_autosave_content(&msg.content)
    {
        ...
        let _ = ctx
            ...
            .store(
                &autosave_key,
                &msg.content,
                daemonclaw_memory::MemoryCategory::Conversation,
                Some(&history_key),
            )
            .await;
    }
    ...
    if !active_provider.supports_vision() && prior_turns.len() > 1 {
        ...
        for turn in &mut prior_turns[..last_idx] {
            if turn.content.contains("[IMAGE:") {
                let (cleaned, _refs) =
                    daemonclaw_providers::multimodal::parse_image_markers(&turn.content);
    ...
    let (delta_tx, delta_rx) = if use_draft_streaming {
        let (tx, rx) = tokio::sync::mpsc::channel::<daemonclaw_runtime::agent::loop_::DraftEvent>(64);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let draft_updater = if use_draft_streaming {
        ...
        if let (Some(mut rx), Some(draft_id_ref), Some(channel_ref)) = (
            ...
            Some(tokio::spawn(async move {
                use daemonclaw_runtime::agent::loop_::StreamDelta;
    ...
    let cost_tracking_context = ctx.cost_tracking.clone().map(|state| {
        daemonclaw_runtime::agent::loop_::ToolLoopCostTrackingContext::new(
            state.tracker,
            state.prices,
        )
    });
            ...
            let loop_result = tokio::select! {
                ...
                result = tokio::time::timeout(
                    ...
                    scope_thread_id(
                        ...
                        daemonclaw_runtime::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
    ...
    match llm_result {
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            if let Some(hooks) = &ctx.hooks {
                    ...
                    .await
                {
                    ...
                    daemonclaw_runtime::hooks::HookResult::Continue((
                        hook_channel,
                        hook_recipient,
            ...
            if ctx.auto_save_memory && msg.content.chars().count() >= AUTOSAVE_MIN_MESSAGE_CHARS {
                ...
                tokio::spawn(async move {
                    if let Err(e) = daemonclaw_memory::consolidation::consolidate_turn(
                        provider.as_ref(),
                        &model,
                        memory.as_ref(),
                        &user_msg,
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
            if daemonclaw_runtime::agent::loop_::is_tool_loop_cancelled(&e)
                || cancellation_token.is_cancelled()
            {
                ...
                if daemonclaw_providers::reliable::is_auth_error(&e) {
                ...
                let safe_error = daemonclaw_providers::sanitize_api_error(&e.to_string());
                runtime_trace::record_event(
                    "channel_message_error",
                    Some(msg.channel.as_str()),
                    Some(route.provider.as_str()),
...
async fn run_message_dispatch_loop(
    mut rx: tokio::sync::mpsc::Receiver<daemonclaw_api::channel::ChannelMessage>,
    ctx: Arc<ChannelRuntimeContext>,
    ...
    while let Some(msg) = rx.recv().await {
        ...
        let msg = if msg.channel != "cli" && ctx.debouncer.enabled() {
            ...
            match ctx.debouncer.debounce(&debounce_key, &msg.content).await {
                daemonclaw_infra::debounce::DebounceResult::Pending(rx) => {
                ...
                daemonclaw_infra::debounce::DebounceResult::Passthrough(content) => {
                    let mut m = msg;
                    m.content = content;
                    m
                }
...
pub async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `daemonclaw onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `daemonclaw daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `daemonclaw service stop && daemonclaw service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.daemonclaw.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.daemonclaw.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.daemonclaw.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.daemonclaw.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/daemonclaw");
...
fn build_channel_by_id(config: &Config, channel_id: &str) -> Result<Arc<dyn Channel>> {
    match channel_id {
        ...
        "discord_history" | "discord-history" => {
            ...
            let discord_mem =
                daemonclaw_memory::SqliteMemory::new_named(&config.workspace_dir, "discord")
...
fn collect_configured_channels(
    ...
    if let Some(ref dh) = config.channels.discord_history {
        if dh.enabled {
            match daemonclaw_memory::SqliteMemory::new_named(&config.workspace_dir, "discord") {
                Ok(discord_mem) => {
                    channels.push(ConfiguredChannel {
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `daemonclaw onboard` first.");
        return Ok(());
    }

    println!("🩺 DaemonClaw Channel Doctor");
    println!();
    ...
    if config.channels.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `daemonclaw gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let mut ch_activated_handle: Option<
        std::sync::Arc<std::sync::Mutex<daemonclaw_runtime::tools::ActivatedToolSet>>,
    > = None;
    if config.mcp.enabled && !config.mcp.servers.is_empty() {
        ...
        match daemonclaw_runtime::tools::McpRegistry::connect_all(&config.mcp.servers).await {
            Ok(registry) => {
                let registry = std::sync::Arc::new(registry);
                if config.mcp.deferred_loading {
                    let deferred_set = daemonclaw_runtime::tools::DeferredMcpToolSet::from_registry(
                    ...
                    deferred_section =
                        daemonclaw_runtime::tools::build_deferred_tools_section(&deferred_set);
                    let activated = std::sync::Arc::new(std::sync::Mutex::new(
                        daemonclaw_runtime::tools::ActivatedToolSet::new(),
                    ));
                    ch_activated_handle = Some(std::sync::Arc::clone(&activated));
                    built_tools.push(Box::new(daemonclaw_runtime::tools::ToolSearchTool::new(
                        deferred_set,
                        activated,
                    )));
                } else {
                    ...
                    for name in names {
                        if let Some(def) = registry.get_tool_def(&name).await {
                            let wrapper: std::sync::Arc<dyn Tool> =
                                std::sync::Arc::new(daemonclaw_runtime::tools::McpToolWrapper::new(
                                    name,
                                    def,
                                    std::sync::Arc::clone(&registry),
                                ));
                            ...
                            built_tools
                                .push(Box::new(daemonclaw_runtime::tools::ArcToolRef(wrapper)));
                            registered += 1;
                        }
                    }
                    tracing::info!(
    ...
    let skills = daemonclaw_runtime::skills::load_skills_with_config(&workspace, &config);
    ...
    daemonclaw_runtime::tools::register_skill_tools(&mut built_tools, &skills, security.clone());
    ...
    let i18n_search_dirs = daemonclaw_runtime::i18n::default_search_dirs(&workspace);
    let i18n_descs =
        daemonclaw_runtime::i18n::ToolDescriptions::load(&i18n_locale, &i18n_search_dirs);
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `daemonclaw onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 DaemonClaw Channel Server");
    println!("  🤖 Model:    {model}");
    let effective_backend = daemonclaw_memory::effective_memory_backend_name(
    ...
    daemonclaw_runtime::health::mark_component_ok("channels");
    ...
    let (tx, rx) = tokio::sync::mpsc::channel::<daemonclaw_api::channel::ChannelMessage>(100);
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        hooks: if config.hooks.enabled {
            let mut runner = daemonclaw_runtime::hooks::HookRunner::new();
            if config.hooks.builtin.command_logger {
                runner.register(Box::new(
                    daemonclaw_runtime::hooks::builtin::CommandLoggerHook::new(),
                ));
            }
            if config.hooks.builtin.webhook_audit.enabled {
                runner.register(Box::new(
                    daemonclaw_runtime::hooks::builtin::WebhookAuditHook::new(
        ...
        cost_tracking: daemonclaw_runtime::cost::CostTracker::get_or_init_global(
            config.cost.clone(),
            &config.workspace_dir,
        )
    ...
    if let Some(ref store) = runtime_ctx.session_store {
        ...
        for key in session_keys {
            ...
            daemonclaw_runtime::agent::history_pruner::remove_orphaned_tool_messages(&mut msgs);
            hydrated += 1;
...
pub async fn deliver_announcement(
    ...
    use daemonclaw_api::channel::SendMessage;

    // Scan for credential leaks before delivering
    let leak_detector = daemonclaw_runtime::security::LeakDetector::new();
    let safe_output = match leak_detector.scan(output) {
        daemonclaw_runtime::security::LeakResult::Detected { redacted, .. } => redacted,
        daemonclaw_runtime::security::LeakResult::Clean => output.to_string(),
    };
    ...
    match channel.to_ascii_lowercase().as_str() {
        ...
        "telegram" => {
            ...
            daemonclaw_api::channel::Channel::send(&ch, &SendMessage::new(&safe_output, target))
                .await?;
        }
        "discord" => {
            let dc = config
...
mod tests {
    ...
    use daemonclaw_providers::{ChatMessage, Provider};
    use daemonclaw_runtime::observability::NoopObserver;
    use daemonclaw_runtime::tools::{Tool, ToolResult};

    fn make_workspace() -> TempDir {
        let tmp = TempDir::new().unwrap();
        // Create minimal workspace files
        std::fs::write(tmp.path().join("SOUL.md"), "# Soul\nBe helpful.").unwrap();
        std::fs::write(tmp.path().join("IDENTITY.md"), "# Identity\nName: DaemonClaw").unwrap();
    ...
    fn pacing_config_defaults_preserve_existing_behavior() {
        let pacing = daemonclaw_config::schema::PacingConfig::default();
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    impl Channel for TelegramRecordingChannel {
        ...
        async fn listen(
            &self,
            _tx: tokio::sync::mpsc::Sender<daemonclaw_api::channel::ChannelMessage>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(daemonclaw_config::schema::ReliabilityConfig::default()),
            provider_runtime_options: daemonclaw_providers::ProviderRuntimeOptions {
                daemonclaw_dir: Some(temp.path().to_path_buf()),
    ...
    impl Memory for NoopMemory {
        ...
        async fn store(
            ...
            _key: &str,
            _content: &str,
            _category: daemonclaw_memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _since: Option<&str>,
            _until: Option<&str>,
        ) -> anyhow::Result<Vec<daemonclaw_memory::MemoryEntry>> {
            Ok(Vec::new())
        }

        async fn get(&self, _key: &str) -> anyhow::Result<Option<daemonclaw_memory::MemoryEntry>> {
            Ok(None)
        }

        async fn list(
            &self,
            _category: Option<&daemonclaw_memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<daemonclaw_memory::MemoryEntry>> {
            Ok(Vec::new())
        }
    ...
    impl Memory for RecallMemory {
        ...
        async fn recall(
            ...
            Ok(vec![daemonclaw_memory::MemoryEntry {
                id: "entry-1".to_string(),
                key: "memory_key_1".to_string(),
                content: "Age is 45".to_string(),
                category: daemonclaw_memory::MemoryCategory::Conversation,
    ...
    async fn message_dispatch_processes_messages_in_parallel() {
        ...
        tx.send(daemonclaw_api::channel::ChannelMessage {
            id: "1".to_string(),
            sender: "alice".to_string(),
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(
            prompt.contains("Name: DaemonClaw"),
    ...
    fn prompt_skills_include_instructions_and_tools() {
        let ws = make_workspace();
        let skills = vec![daemonclaw_runtime::skills::Skill {
            name: "code-review".into(),
            description: "Review code for bugs".into(),
            version: "1.0.0".into(),
            author: None,
            tags: vec![],
            tools: vec![daemonclaw_runtime::skills::SkillTool {
                name: "lint".into(),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from DaemonClaw 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";

        // Reproduces the production crash path where channel logs truncate at 80 chars.
        let result =
            std::panic::catch_unwind(|| daemonclaw_runtime::util::truncate_with_ellipsis(msg, 80));
        assert!(
            result.is_ok(),
    ...
    fn full_autonomy_prompt_executes_allowed_tools_without_extra_approval() {
        let ws = make_workspace();
        let config = daemonclaw_config::schema::AutonomyConfig {
            level: daemonclaw_runtime::security::AutonomyLevel::Full,
            ..daemonclaw_config::schema::AutonomyConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ...
            Some(&config),
            false,
            daemonclaw_config::schema::SkillsPromptInjectionMode::Full,
            false,
            0,
        );
    ...
    fn readonly_prompt_explains_policy_blocks_without_fake_approval() {
        let ws = make_workspace();
        let config = daemonclaw_config::schema::AutonomyConfig {
            level: daemonclaw_runtime::security::AutonomyLevel::ReadOnly,
            ..daemonclaw_config::schema::AutonomyConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ws.path(),
    ...
    fn channel_notify_observer_truncates_utf8_arguments_safely() {
        ...
        observer.record_event(
            &daemonclaw_runtime::observability::traits::ObserverEvent::ToolCallStart {
    ...
    fn conversation_memory_key_uses_message_id() {
        let msg = daemonclaw_api::channel::ChannelMessage {
            id: "msg_abc123".into(),
            sender: "U123".into(),
    ...
    fn conversation_memory_key_is_unique_per_message() {
        let msg1 = daemonclaw_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "first".into(),
        ...
        let msg2 = daemonclaw_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "second".into(),
    ...
    async fn process_channel_message_refreshes_available_skills_after_new_session() {
        ...
        let initial_skills =
            daemonclaw_runtime::skills::load_skills_with_config(workspace.path(), &config);
...
This is an example JSON object for profile settings."#;
    ...
    fn aieos_identity_from_file() {
        use tempfile::TempDir;
        use daemonclaw_config::schema::IdentityConfig;
    ...
    fn collect_configured_channels_skips_disabled_email() {
        let mut config = Config::default();
        config.channels.email = Some(daemonclaw_config::scattered_types::EmailConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    fn collect_configured_channels_skips_disabled_voice_call() {
        let mut config = Config::default();
        config.channels.voice_call = Some(daemonclaw_config::scattered_types::VoiceCallConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    async fn supervised_listener_marks_error_and_restarts_on_failures() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<daemonclaw_api::channel::ChannelMessage>(1);
        ...
        let snapshot = daemonclaw_runtime::health::snapshot_json();
    ...
    async fn supervised_listener_refreshes_health_while_running() {
        ...
        let first_last_ok = daemonclaw_runtime::health::snapshot_json()["components"]
        ...
        let second_last_ok = daemonclaw_runtime::health::snapshot_json()["components"]
            [&component_name]["last_ok"]
            .as_str()
            .unwrap_or("")
            .to_string();
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "daemonclaw.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "daemonclaw.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["daemonclaw", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["daemonclaw", "restart"]);
    }
    ...
    async fn e2e_photo_attachment_rejected_by_non_vision_provider() {
        ...
        process_channel_message(
            ...
            daemonclaw_api::channel::ChannelMessage {
                ...
                sender: "daemonclaw_user".to_string(),
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .peek("test-channel_chat-photo_daemonclaw_user")
    ...
    async fn e2e_failed_non_retryable_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .peek("test-channel_chat-format_daemonclaw_user")
            .expect("history should exist for sender");
        assert_eq!(turns.len(), 2);
        assert_eq!(turns[0].role, "user");
        assert_eq!(turns[0].content, "What is WAL?");
    ...
    async fn process_channel_message_applies_query_classification_route() {
        ...
        let classification_config = daemonclaw_config::schema::QueryClassificationConfig {
            enabled: true,
        ...
        let model_routes = vec![daemonclaw_config::schema::ModelRouteConfig {
            hint: "vision".into(),
    ...
    async fn process_channel_message_classification_priority_selects_highest() {
        ...
        let classification_config = daemonclaw_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![
                daemonclaw_config::schema::ClassificationRule {
                    hint: "fast".into(),
        ...
        let model_routes = vec![
            daemonclaw_config::schema::ModelRouteConfig {
                hint: "fast".into(),
    ...
    fn build_channel_by_id_configured_voice_call_succeeds() {
        ...
        config.channels.voice_call = Some(daemonclaw_config::scattered_types::VoiceCallConfig {
            enabled: true,
            provider: daemonclaw_config::scattered_types::VoiceProvider::Twilio,
    ...
    fn is_stop_command_matches_with_bot_suffix() {
        assert!(is_stop_command("/stop@daemonclaw_bot"));
    }
    ...
    fn default_keep_tool_context_turns_is_two() {
        let config = daemonclaw_config::schema::AgentConfig::default();
