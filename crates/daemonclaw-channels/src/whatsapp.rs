...
mod tests {
    ...
    fn whatsapp_parse_valid_text_message() {
        ...
        assert_eq!(msgs[0].content, "Hello DaemonClaw!");
    ...
    fn make_group_mention_channel() -> WhatsAppChannel {
        ...
        .with_group_mention_patterns(vec!["@?DaemonClaw".into()])
    }
    ...
    fn make_dm_mention_channel() -> WhatsAppChannel {
        ...
        .with_dm_mention_patterns(vec!["@?DaemonClaw".into()])
    }
    ...
    fn whatsapp_compile_skips_invalid_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into(), "[invalid".into()]);
    ...
    fn whatsapp_compile_skips_empty_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into(), "  ".into()]);
        assert_eq!(patterns.len(), 1);
    }
    ...
    fn whatsapp_text_matches_at_name() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
    ...
    fn whatsapp_strip_at_name() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@DaemonClaw what is the weather?"),
    ...
    fn whatsapp_strip_at_end() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "Help me @DaemonClaw"),
            Some("Help me".into())
        );
    }
    ...
    fn whatsapp_strip_mid_sentence() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "Hey @DaemonClaw how are you?"),
    ...
    fn whatsapp_strip_multiple_occurrences() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@DaemonClaw hello @DaemonClaw"),
            Some("hello".into())
        );
    }
    ...
    fn whatsapp_strip_returns_none_when_only_mention() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(WhatsAppChannel::strip_patterns(&pats, "@DaemonClaw"), None);
    }
    ...
    fn whatsapp_strip_returns_none_for_whitespace_only() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "  @DaemonClaw  "),
            None
        );
    }
    ...
    fn whatsapp_strip_collapses_whitespace() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DaemonClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@DaemonClaw   status   please"),
    ...
    fn whatsapp_group_mention_accepts_and_strips_in_group() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@DaemonClaw what is the weather?")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_strips_from_group_content() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "Hey @DaemonClaw tell me a joke")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_drops_mention_only_group_message() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@DaemonClaw")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_case_insensitive_group_match() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@daemonclaw status")]
                    }
                }]
            }]
        });
                ...
                "changes": [{
                    "value": {
                        "messages": [
                            ...
                            group_msg("222", "2", "@DaemonClaw help me"),
    ...
    fn whatsapp_group_mention_dm_not_stripped() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [dm_msg("111", "1", "@DaemonClaw what is the weather?")]
                    }
                }]
            }]
        });
        ...
        assert_eq!(msgs.len(), 1);
        assert_eq!(
            msgs[0].content, "@DaemonClaw what is the weather?",
