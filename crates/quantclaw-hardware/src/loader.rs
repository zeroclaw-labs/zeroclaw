//! Plugin manifest loader — scans `~/.quantclaw/tools/` at startup.
//!
...
/// Scan `~/.quantclaw/tools/` and return all valid plugins.
///
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.quantclaw/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_quantclaw_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".quantclaw").join("tools");
