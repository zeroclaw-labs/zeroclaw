...
/// Default baud rate for QuantClaw serial devices.
...
/// Serial transport for QuantClaw hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify QuantClaw firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "quantclaw"`.
    ///
    /// Returns `true` if a QuantClaw device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "quantclaw" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
