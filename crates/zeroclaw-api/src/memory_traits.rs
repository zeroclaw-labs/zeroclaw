...
pub trait Memory: Send + Sync + crate::attribution::Attributable {
    ...
    /// stop surfacing through the recall/list retrieval paths (`recall`/`list`)
    /// **without being deleted**. This is the non-destructive alternative to
    /// [`forget`](Self::forget): the data stays in the store (recoverable,
    /// auditable) but stops surfacing in normal retrieval. Returns `true` if at
    /// least one row was updated.
    ///
    /// `get` is intentionally the admin/recovery path and MUST still return a
    /// superseded row (carrying its supersede marker) so operators can inspect
    /// or restore it. Backends implementing this trait must not filter `get`.
    ///
