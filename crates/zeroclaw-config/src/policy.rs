...
fn strip_fd_merge_redirects(command: &str) -> String {
    ...
    let re = FD_MERGE_RE.get_or_init(|| regex::Regex::new(r"\d*[><]&[\d-]").expect("FD_MERGE_RE regex must compile"));
...
fn contains_unquoted_input_redirect(command: &str) -> bool {
    ...
    let re =
        SAFE_INPUT_RE.get_or_init(|| Regex::new(r"<[ ]?/dev/(null|zero)(\s|[;&|)]|$)").expect("SAFE_INPUT_RE regex must compile"));
