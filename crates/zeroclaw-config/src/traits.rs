...
pub trait HasPropKind {
    ...
    /// Variant-name lister for enum-kind types, populated by
    /// `#[derive(ConfigEnum)]`. `None` for every non-enum type so the
    /// registry can read it uniformly without schemars or specialization.
    const ENUM_VARIANTS: Option<fn() -> Vec<String>> = None;
...
/// Enumerates a config enum's serde wire variant names without schemars.
/// Generated by `#[derive(ConfigEnum)]` so every surface can offer the
/// variants as choices regardless of the `schema-export` feature.
pub trait HasEnumVariants {
