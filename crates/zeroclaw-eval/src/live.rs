...
pub async fn run_live_case(
    ...
    // `capabilities_for_model` is model-aware (a composite/routing provider
    // can report different capabilities per model), so it must be asked about
    // the same model the built agent will actually dispatch with: `model_name`
    // when the case provider supplied one, otherwise the `Agent::builder()`
