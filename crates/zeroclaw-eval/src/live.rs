...
pub async fn run_live_case(
    ...
    // can report different capabilities per model). The live path below never
    // sets a model on the builder, so the agent dispatches with
    // `Agent::builder()`'s default; probe capabilities for that same string.
