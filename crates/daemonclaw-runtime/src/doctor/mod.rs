...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 DaemonClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `daemonclaw doctor` again.");
    }
...
fn doctor_model_targets(provider_override: Option<&str>) -> Vec<String> {
    ...
    daemonclaw_providers::list_providers()
        .into_iter()
...
pub async fn run_models(
    ...
    println!("🩺 DaemonClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `daemonclaw doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `daemonclaw onboard` to set one up",
        ));
    }
...
fn provider_validation_error(name: &str) -> Option<String> {
    match daemonclaw_providers::create_provider(name, None) {
        Ok(_) => None,
        Err(err) => Some(
            err.to_string()
                .lines()
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(
            first
                ...
                .is_some_and(|name| name.starts_with(".daemonclaw_doctor_probe_"))
        );
    }
