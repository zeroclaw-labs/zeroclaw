...
use daemonclaw_memory::{self, Memory, MemoryCategory};
use daemonclaw_providers::{self, ChatMessage, ChatRequest, ConversationMessage, Provider};

// Re-export TurnEvent from daemonclaw-types for backwards compatibility.
pub use daemonclaw_api::agent::TurnEvent;
...
pub struct Agent {
    ...
    config: daemonclaw_config::schema::AgentConfig,
    model_name: String,
    temperature: f64,
    workspace_dir: std::path::PathBuf,
    identity_config: daemonclaw_config::schema::IdentityConfig,
    skills: Vec<crate::skills::Skill>,
    skills_prompt_mode: daemonclaw_config::schema::SkillsPromptInjectionMode,
    auto_save: bool,
    memory_session_id: Option<String>,
    history: Vec<ConversationMessage>,
    classification_config: daemonclaw_config::schema::QueryClassificationConfig,
    available_hints: Vec<String>,
    route_model_by_hint: HashMap<String, String>,
    #[allow(dead_code)] // WIP: stored for future runtime tool filtering
    allowed_tools: Option<Vec<String>>,
    response_cache: Option<Arc<daemonclaw_memory::response_cache::ResponseCache>>,
...
pub struct AgentBuilder {
    ...
    config: Option<daemonclaw_config::schema::AgentConfig>,
    model_name: Option<String>,
    temperature: Option<f64>,
    workspace_dir: Option<std::path::PathBuf>,
    identity_config: Option<daemonclaw_config::schema::IdentityConfig>,
    skills: Option<Vec<crate::skills::Skill>>,
    skills_prompt_mode: Option<daemonclaw_config::schema::SkillsPromptInjectionMode>,
    auto_save: Option<bool>,
    memory_session_id: Option<String>,
    classification_config: Option<daemonclaw_config::schema::QueryClassificationConfig>,
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: daemonclaw_config::schema::AgentConfig) -> Self {
    ...
    pub fn response_cache(
        mut self,
        cache: Option<Arc<daemonclaw_memory::response_cache::ResponseCache>>,
    ) -> Self {
...
impl Agent {
    ...
    pub async fn from_config(config: &Config) -> Result<Self> {
        ...
        let response_cache = if config.memory.response_cache_enabled {
            daemonclaw_memory::response_cache::ResponseCache::with_hot_cache(
                &config.workspace_dir,
            ...
            let cache_key = if self.temperature == 0.0 {
                self.response_cache.as_ref().map(|_| {
                    ...
                    let system = messages
                        ...
                        .map(|m| m.content.as_str());
                    daemonclaw_memory::response_cache::ResponseCache::cache_key(
                        &effective_model,
                        system,
                        last_user,
                    )
    ...
    pub async fn turn_streamed(
        ...
        for _ in 0usize.. {
            ...
            let stream_opts = daemonclaw_providers::traits::StreamOptions::new(true);
            let mut stream = self.provider.stream_chat(
                daemonclaw_providers::ChatRequest {
                    messages: &messages,
                    tools: if self.tool_dispatcher.should_send_tool_specs() {
                        Some(&self.tool_specs)
                    } else {
            ...
            let mut streamed_tool_calls: Vec<daemonclaw_providers::traits::ToolCall> = Vec::new();
            let mut got_stream = false;

            while let Some(item) = stream.next().await {
                match item {
                    Ok(event) => match event {
                        daemonclaw_providers::traits::StreamEvent::TextDelta(chunk) => {
                        ...
                        daemonclaw_providers::traits::StreamEvent::PreExecutedToolResult {
                            name,
                            output,
                        } => {
                            let _ = event_tx.send(TurnEvent::ToolResult { name, output }).await;
                        }
                        daemonclaw_providers::traits::StreamEvent::Final => break,
                    },
                    Err(_) => break,
                }
            }
            ...
            let response = if got_stream {
                ...
                daemonclaw_providers::ChatResponse {
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        println!("🦀 DaemonClaw Interactive Mode");
        ...
        let listen_handle = tokio::spawn(async move {
            let _ = daemonclaw_api::channel::Channel::listen(&*cli, tx).await;
        });
...
mod tests {
    ...
    struct MockProvider {
        responses: Mutex<Vec<daemonclaw_providers::ChatResponse>>,
    }
    ...
    impl Provider for MockProvider {
        ...
        async fn chat(
            ...
            if guard.is_empty() {
                return Ok(daemonclaw_providers::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
                    reasoning_content: None,
    ...
    async fn turn_routes_with_hint_when_query_classification_matches() {
        ...
        let mut agent = Agent::builder()
            ...
            .classification_config(daemonclaw_config::schema::QueryClassificationConfig {
                enabled: true,
                rules: vec![daemonclaw_config::schema::ClassificationRule {
                    hint: "fast".to_string(),
    ...
    async fn from_config_passes_extra_headers_to_custom_provider() {
        ...
        let mut config = daemonclaw_config::schema::Config {
            workspace_dir,
        ...
        config.providers.fallback = Some(format!("custom:http://{addr}"));
        {
            ...
            entry.extra_headers.insert(
                ...
                "daemonclaw-web-test/1.0".to_string(),
            );
            entry
                .extra_headers
    ...
    impl Provider for StreamToolCaptureProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                let tc = daemonclaw_providers::traits::StreamEvent::ToolCall(
                ...
                let chunk = daemonclaw_providers::traits::StreamEvent::TextDelta(
    ...
    fn trim_history_does_not_leave_orphan_tool_results() {
        use daemonclaw_providers::{ToolCall, ToolResultMessage};
        ...
        let agent_config = daemonclaw_config::schema::AgentConfig {
            max_history_messages: 4,
            ..daemonclaw_config::schema::AgentConfig::default()
        };
