...
// under `~/.daemonclaw/workspace/skills/<slug>/`.

use anyhow::{Context, Result};
use std::path::PathBuf;
use daemonclaw_config::schema::SkillCreationConfig;
use daemonclaw_memory::embeddings::EmbeddingProvider;
use daemonclaw_memory::vector::cosine_similarity;
...
impl SkillCreator {
    ...
    fn generate_skill_toml(slug: &str, description: &str, tool_calls: &[ToolCallRecord]) -> String {
        ...
        toml.push_str("author = \"daemonclaw-auto\"\n");
    ...
    async fn enforce_lru_limit(&self) -> Result<()> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            let content = tokio::fs::read_to_string(&toml_path).await?;
            if content.contains("\"daemonclaw-auto\"") || content.contains("\"auto-generated\"") {
...
pub fn extract_tool_calls_from_history(
    history: &[daemonclaw_providers::ChatMessage],
) -> Vec<ToolCallRecord> {
...
mod tests {
    ...
    use daemonclaw_memory::embeddings::{EmbeddingProvider, NoopEmbedding};
...
"#
    ...
    async fn create_from_execution_success() {
        ...
        assert!(toml_content.contains("daemonclaw-auto"));
    }
