...
const OPEN_SKILLS_SYNC_MARKER: &str = ".daemonclaw-open-skills-sync";
...
/// Skills live in `~/.daemonclaw/workspace/skills/<name>/SKILL.md`
    ...
    if scripts_blocked && !allow_scripts {
        ...
        eprintln!(
            "warning: skill '{}' was skipped because it contains script files. \
             Set `skills.allow_scripts = true` in your daemonclaw config to enable it.",
            path.file_name()
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("DAEMONCLAW_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("DAEMONCLAW_OPEN_SKILLS_DIR").ok();
...
pub fn skills_to_prompt_with_mode(
    skills: &[Skill],
    workspace_dir: &Path,
    mode: daemonclaw_config::schema::SkillsPromptInjectionMode,
) -> String {
    use std::fmt::Write;
    ...
    let mut prompt = match mode {
        daemonclaw_config::schema::SkillsPromptInjectionMode::Full => String::from(
            "## Available Skills\n\n\
             Skill instructions and tool metadata are preloaded below.\n\
             Follow these instructions directly; do not read skill files at runtime unless the user asks.\n\n\
             <available_skills>\n",
        ),
        daemonclaw_config::schema::SkillsPromptInjectionMode::Compact => String::from(
            "## Available Skills\n\n\
...
) -> Vec<Box<dyn daemonclaw_api::tool::Tool>> {
    let mut tools: Vec<Box<dyn daemonclaw_api::tool::Tool>> = Vec::new();
    for skill in skills {
