...
use daemonclaw_config::schema::{HardwareConfig, HardwareTransport};
#[cfg(feature = "channel-nostr")]
use daemonclaw_config::schema::{NostrConfig, default_nostr_relays};
use daemonclaw_memory::{
...
/// implementations live in downstream crates (daemonclaw-hardware, daemonclaw-channels).
///
/// NOTE: Transitional bridge — see RFC #5574 Phase 2 D4. This struct will be
/// replaced when `daemonclaw onboard` integrates with `PluginRegistry::install`.
#[derive(Default)]
...
pub async fn run_wizard(force: bool, callbacks: WizardCallbacks) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to DaemonClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to DaemonClaw)");
    ...
    let config = Config {
        ...
        backup: daemonclaw_config::schema::BackupConfig::default(),
        data_retention: daemonclaw_config::schema::DataRetentionConfig::default(),
        cloud_ops: daemonclaw_config::schema::CloudOpsConfig::default(),
        conversational_ai: daemonclaw_config::schema::ConversationalAiConfig::default(),
        security: daemonclaw_config::schema::SecurityConfig::default(),
        security_ops: daemonclaw_config::schema::SecurityOpsConfig::default(),
        runtime: RuntimeConfig::default(),
        reliability: daemonclaw_config::schema::ReliabilityConfig::default(),
        scheduler: daemonclaw_config::schema::SchedulerConfig::default(),
        agent: daemonclaw_config::schema::AgentConfig::default(),
        pacing: daemonclaw_config::schema::PacingConfig::default(),
        skills: daemonclaw_config::schema::SkillsConfig::default(),
        pipeline: daemonclaw_config::schema::PipelineConfig::default(),
        heartbeat: HeartbeatConfig::default(),
        cron: daemonclaw_config::schema::CronConfig::default(),
        channels,
        memory: memory_config, // User-selected memory backend
        storage: StorageConfig::default(),
        tunnel: tunnel_config,
        gateway: daemonclaw_config::schema::GatewayConfig::default(),
        composio: composio_config,
        microsoft365: daemonclaw_config::schema::Microsoft365Config::default(),
        secrets: secrets_config,
        browser: BrowserConfig::default(),
        browser_delegate: daemonclaw_tools::browser_delegate::BrowserDelegateConfig::default(),
        http_request: daemonclaw_config::schema::HttpRequestConfig::default(),
        ...
        web_fetch: daemonclaw_config::schema::WebFetchConfig::default(),
        link_enricher: daemonclaw_config::schema::LinkEnricherConfig::default(),
        text_browser: daemonclaw_config::schema::TextBrowserConfig::default(),
        ...
        identity_provider: daemonclaw_config::schema::IdentityProviderConfig::default(),
        ...
        nodes: daemonclaw_config::schema::NodesConfig::default(),
        workspace: daemonclaw_config::schema::WorkspaceConfig::default(),
        notion: daemonclaw_config::schema::NotionConfig::default(),
        jira: daemonclaw_config::schema::JiraConfig::default(),
        node_transport: daemonclaw_config::schema::NodeTransportConfig::default(),
        knowledge: daemonclaw_config::schema::KnowledgeConfig::default(),
        linkedin: daemonclaw_config::schema::LinkedInConfig::default(),
        image_gen: daemonclaw_config::schema::ImageGenConfig::default(),
        plugins: daemonclaw_config::schema::PluginsConfig::default(),
        locale: None,
        verifiable_intent: daemonclaw_config::schema::VerifiableIntentConfig::default(),
        claude_code: daemonclaw_config::schema::ClaudeCodeConfig::default(),
        claude_code_runner: daemonclaw_config::schema::ClaudeCodeRunnerConfig::default(),
        codex_cli: daemonclaw_config::schema::CodexCliConfig::default(),
        gemini_cli: daemonclaw_config::schema::GeminiCliConfig::default(),
        opencode_cli: daemonclaw_config::schema::OpenCodeCliConfig::default(),
            ...
            .is_some()
    {
        ...
        if launch {
            ...
            unsafe { std::env::set_var("DAEMONCLAW_AUTOSTART_CHANNELS", "1") };
        }
    }
...
pub async fn run_channels_repair_wizard(callbacks: WizardCallbacks) -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to DaemonClaw)");
...
/// Use `daemonclaw onboard` or `daemonclaw onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
...
fn memory_config_defaults_for_backend(backend: &str) -> MemoryConfig {
    ...
    MemoryConfig {
        ...
        vector_weight: 0.7,
        keyword_weight: 0.3,
        search_mode: daemonclaw_config::schema::SearchMode::default(),
        ...
        policy: daemonclaw_config::schema::MemoryPolicyConfig::default(),
        sqlite_open_timeout_secs: None,
        qdrant: daemonclaw_config::schema::QdrantConfig::default(),
    }
}
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    if let Ok(custom_workspace) = std::env::var("DAEMONCLAW_WORKSPACE") {
        ...
        if !trimmed.is_empty() {
            ...
            return daemonclaw_config::schema::resolve_config_dir_for_workspace(&PathBuf::from(
                expanded.as_ref(),
            ));
        }
    }
    ...
    // instead of ~/.daemonclaw so the Homebrew service finds the same config.
    if let Some(prefix) = std::env::current_exe()
        .ok()
        .as_deref()
        .and_then(homebrew_prefix_for_exe)
    {
        let config_dir = PathBuf::from(prefix).join("var").join("daemonclaw");
        return (config_dir.clone(), config_dir.join("workspace"));
    }

    let config_dir = home.join(".daemonclaw");
    (config_dir.clone(), config_dir.join("workspace"))
}

fn homebrew_prefix_for_exe(exe: &Path) -> Option<&'static str> {
    let exe = exe.to_string_lossy();
    if exe == "/opt/homebrew/bin/daemonclaw"
        || exe.starts_with("/opt/homebrew/Cellar/daemonclaw/")
        || exe.starts_with("/opt/homebrew/opt/daemonclaw/")
    {
        return Some("/opt/homebrew");
    }

    if exe == "/usr/local/bin/daemonclaw"
        || exe.starts_with("/usr/local/Cellar/daemonclaw/")
        || exe.starts_with("/usr/local/opt/daemonclaw/")
    {
...
fn quick_setup_homebrew_service_note(
    ...
    let service_root = Path::new(prefix).join("var").join("daemonclaw");
    ...
    Some(format!(
        "Homebrew service note: `brew services` uses {} (config {}) by default. Your onboarding just wrote {}. If you plan to run DaemonClaw as a service, copy or link this workspace first.",
...
async fn run_quick_setup_with_home(
    ...
    let (daemonclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    let config = Config {
        ...
        storage: StorageConfig::default(),
        tunnel: daemonclaw_config::schema::TunnelConfig::default(),
        gateway: daemonclaw_config::schema::GatewayConfig::default(),
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     daemonclaw agent -m \"Hello!\"");
            println!("    2. Gateway:  daemonclaw gateway");
            println!("    3. Status:   daemonclaw status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             daemonclaw auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              daemonclaw agent -m \"Hello!\"");
                println!("    3. Gateway:           daemonclaw gateway");
                println!("    4. Status:            daemonclaw status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.daemonclaw/config.toml");
            println!("    3. Chat:              daemonclaw agent -m \"Hello!\"");
            println!("    4. Gateway:           daemonclaw gateway");
        }
    } else {
        println!("    1. Chat:     daemonclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  daemonclaw gateway");
...
pub async fn run_models_refresh(
        ...
        .await?
    {
        ...
        println!();
        println!(
            "Tip: run `daemonclaw models refresh --force --provider {}` to fetch latest now.",
            provider_name
        );
        return Ok(());
    }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: daemonclaw models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
pub async fn run_models_refresh_all(config: &Config, force: bool) -> Result<()> {
    let mut targets: Vec<String> = daemonclaw_providers::list_providers()
        .into_iter()
...
async fn persist_workspace_selection(config_path: &Path) -> Result<()> {
    ...
    daemonclaw_config::schema::persist_active_workspace_config_dir(config_dir)
        .await
        .with_context(|| {
            format!(
...
async fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    ...
    let (config_dir, workspace_dir) = if use_default {
        ...
        daemonclaw_config::schema::resolve_config_dir_for_workspace(&PathBuf::from(expanded))
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("DaemonClaw works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if daemonclaw_providers::gemini::GeminiProvider::has_cli_credentials() {
            print_bullet(&format!(
                "{} Gemini CLI credentials detected! You can skip the API key.",
                style("✓").green().bold()
            ));
            print_bullet("DaemonClaw will reuse your existing Gemini CLI authentication.");
            println!();
        ...
        if std::env::var("GITHUB_TOKEN").is_ok() {
            ...
            print_bullet("DaemonClaw will use it for GitHub Copilot authentication.");
        } else {
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how DaemonClaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("DaemonClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("DaemonClaw can encrypt API keys stored in config.toml.");
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how DaemonClaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels(
    ...
    print_bullet("Channels let you talk to DaemonClaw from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to DaemonClaw from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to DaemonClaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to DaemonClaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("DaemonClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.daemonclaw/state/whatsapp-web/session.db")
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("daemonclaw-whatsapp-verify")
                    .interact_text()?;
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to DaemonClaw from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("DaemonClaw will listen for encrypted DMs on Nostr relays.");
                ...
                // Validate the key via callback (requires nostr-sdk in daemonclaw-channels)
...
#[allow(clippy::too_many_lines)]
fn setup_tunnel() -> Result<daemonclaw_config::schema::TunnelConfig> {
    use daemonclaw_config::schema::{
...
fn print_summary(config: &Config) {
        ...
        && !provider_supports_keyless_local_usage(provider)
    {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("daemonclaw auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: daemonclaw auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("daemonclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("daemonclaw agent -m \"Hello, DaemonClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("daemonclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("daemonclaw status").yellow());
...
mod tests {
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        tokio::fs::create_dir_all(&daemonclaw_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("daemonclaw-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".daemonclaw").join("config.toml");

        let _workspace_env = EnvVarGuard::set(
            "DAEMONCLAW_WORKSPACE",
        ...
        .await
        .expect("quick setup should honor DAEMONCLAW_WORKSPACE");
    ...
    fn homebrew_prefix_for_exe_detects_supported_layouts() {
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/opt/homebrew/bin/daemonclaw")),
            Some("/opt/homebrew")
        );
        assert_eq!(
            homebrew_prefix_for_exe(Path::new(
                "/opt/homebrew/Cellar/daemonclaw/0.5.0/bin/daemonclaw",
            )),
            Some("/opt/homebrew")
        );
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/usr/local/bin/daemonclaw")),
            Some("/usr/local")
        );
        assert_eq!(homebrew_prefix_for_exe(Path::new("/tmp/daemonclaw")), None);
    }

    #[test]
    fn quick_setup_homebrew_service_note_mentions_service_workspace() {
        let note = quick_setup_homebrew_service_note(
            Path::new("/Users/alix/.daemonclaw/config.toml"),
            Path::new("/Users/alix/.daemonclaw/workspace"),
            Path::new("/opt/homebrew/bin/daemonclaw"),
        )
        .expect("homebrew installs should emit a service workspace note");

        assert!(note.contains("/opt/homebrew/var/daemonclaw/workspace"));
        assert!(note.contains("/opt/homebrew/var/daemonclaw/config.toml"));
        assert!(note.contains("/Users/alix/.daemonclaw/config.toml"));
    }

    #[test]
    fn quick_setup_homebrew_service_note_skips_matching_service_layout() {
        let service_config = Path::new("/opt/homebrew/var/daemonclaw/config.toml");
        let service_workspace = Path::new("/opt/homebrew/var/daemonclaw/workspace");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** DaemonClaw"),
            "should default agent name to DaemonClaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("DaemonClaw-v2"));
    }
    ...
    fn launchable_channels_include_signal_mattermost_qq_nextcloud_and_feishu() {
        ...
        channels.signal = Some(daemonclaw_config::schema::SignalConfig {
            enabled: true,
        ...
        channels.mattermost = Some(daemonclaw_config::schema::MattermostConfig {
            enabled: true,
        ...
        channels.qq = Some(daemonclaw_config::schema::QQConfig {
            enabled: true,
            app_id: "app-id".into(),
        ...
        channels.qq = None;
        channels.nextcloud_talk = Some(daemonclaw_config::schema::NextcloudTalkConfig {
            enabled: true,
        ...
        channels.feishu = Some(daemonclaw_config::schema::FeishuConfig {
            enabled: true,
            app_id: "cli_123".into(),
            ...
            allowed_users: vec!["*".into()],
            mention_only: false,
            receive_mode: daemonclaw_config::schema::LarkReceiveMode::Websocket,
            port: None,
            proxy_url: None,
        });
        assert!(has_launchable_channels(&channels));
    ...
    fn webhook_only_config_is_launchable() {
        let channels = ChannelsConfig {
            webhook: Some(daemonclaw_config::schema::WebhookConfig {
                enabled: true,
                port: 8080,
                listen_path: None,
                send_url: None,
    ...
    fn channels_repair_preserves_unmodified_channels() {
        use daemonclaw_config::schema::{DiscordConfig, MatrixConfig, StreamMode};
    ...
    fn matrix_reconfigure_preserves_non_prompted_fields() {
        use daemonclaw_config::schema::{MatrixConfig, StreamMode};
        ...
        let existing = ChannelsConfig {
            matrix: Some(MatrixConfig {
                ...
                user_id: None,
                device_id: Some("DAEMONCLAW".into()),
    ...
    fn matrix_fresh_install_uses_defaults_for_non_prompted_fields() {
        use daemonclaw_config::schema::StreamMode;

        let existing_mx: Option<&daemonclaw_config::schema::MatrixConfig> = None;
