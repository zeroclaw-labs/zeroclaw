...
pub fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `daemonclaw onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: daemonclaw onboard --channels-only");
            println!("    4. Start: daemonclaw channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: daemonclaw onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: daemonclaw onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    DaemonClaw can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.daemonclaw/workspace/cron/");
            println!("    Run: daemonclaw cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        "Webhooks" => {
            ...
            println!("    Run: daemonclaw gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/DeliveryBoyTech/daemonclaw");
            }
        }
    }
