...
/// - `X-DaemonClaw-Timestamp` — unix epoch seconds
/// - `X-DaemonClaw-Nonce` — random UUID v4
/// - `X-DaemonClaw-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-DaemonClaw-Timestamp", timestamp.to_string())
            .header("X-DaemonClaw-Nonce", &nonce)
            .header("X-DaemonClaw-Signature", &signature)
