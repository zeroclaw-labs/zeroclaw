...
pub async fn migrate_openclaw_memory(
    ...
    if paths_equal(&source_workspace, &config.workspace_dir) {
        bail!("Source workspace matches current DaemonClaw workspace; refusing self-migration");
    }
...
fn target_memory_backend(config: &Config) -> Result<Box<dyn Memory>> {
    daemonclaw_memory::create_memory_for_migration(&config.memory.backend, &config.workspace_dir)
}
...
mod tests {
    ...
    use daemonclaw_config::schema::{Config, MemoryConfig};
    use daemonclaw_memory::SqliteMemory;
