...
impl TelnyxChannel {
    ...
    pub fn new(
        ...
        let verifying_key = ed25519_dalek::VerifyingKey::from_bytes(&bytes).map_err(|e| {
            anyhow::Error::msg(format!(
                "Telnyx public_key is not a valid Ed25519 verifying key: {e}"
            ))
        })?;
        Ok(Self {
            api_key,
...
impl ::zeroclaw_api::attribution::Attributable for TelnyxChannel {
    fn role(&self) -> ::zeroclaw_api::attribution::Role {
        ::zeroclaw_api::attribution::Role::Channel(::zeroclaw_api::attribution::ChannelKind::Telnyx)
    }
    fn alias(&self) -> &str {
        ""
    }
...
impl Channel for TelnyxChannel {
    ...
    async fn listen(&self, _tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        // Configure Telnyx's webhook URL to POST to the gateway's
        // TELNYX_WEBHOOK_PATH endpoint.
        loop {
