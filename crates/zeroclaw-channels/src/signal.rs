...
impl SignalChannel {
    ...
    /// `"[choice-index]N"` when only the index is available).
    /// Calciforge's dispatcher recognises the sentinel and resolves it
    /// back to the originating `ChoiceOption.callback_data` / `command`.
    /// Consumers that don't care about poll votes get the raw text
    /// path; the sentinel format is documented so they can grep for
    /// and ignore it as needed.
    ///
    ...
    fn process_envelope(&self, envelope: &Envelope) -> Vec<ChannelMessage> {
        ...
        contents
            .into_iter()
            .enumerate()
            .map(|(idx, content)| ChannelMessage {
                // Unique id per emitted message so multi-select votes
                // don't dedupe downstream.
                id: format!("sig_{timestamp}_{idx}"),
                sender: sender.clone(),
