...
impl LemmyChannel {
    ...
    /// Return the alias under `[channels.lemmy.<alias>]` that this channel
    /// handle is bound to.
    ...
    async fn ensure_auth(&self) -> Result<AuthState> {
        ...
        // getSite returning no my_user, or failing, leaves self-suppression
        // disabled until the next login resolves the bot's user id.
        if let Ok(Some(id)) = self.fetch_my_user_id(&jwt).await {
            state.bot_user_id = Some(id);
        }
    ...
    async fn login(&self) -> Result<String> {
        ...
        payload
            .jwt
            .filter(|j| !j.is_empty())
            .ok_or_else(|| anyhow::Error::msg("Lemmy login response missing jwt"))
    }
...
impl ::zeroclaw_api::attribution::Attributable for LemmyChannel {
    fn role(&self) -> ::zeroclaw_api::attribution::Role {
        ::zeroclaw_api::attribution::Role::Channel(::zeroclaw_api::attribution::ChannelKind::Lemmy)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
        ...
        loop {
            ...
            for view in &pms {
                ...
                if let Some(channel_msg) = self.parse_private_message(view, auth.bot_user_id) {
                    ...
                    // mark_as_read is best-effort; a failure means the PM may
                    // reappear next poll, which is acceptable.
