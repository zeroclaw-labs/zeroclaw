...
fn markdown_to_plain_text(text: &str) -> String {
    ...
    let table_separator_re = regex::Regex::new(r"^\|[\s:|-]+\|$").expect("table_separator_re must compile");
...
fn build_headers(token: Option<&str>) -> reqwest::header::HeaderMap {
    ...
    headers.insert("X-WECHAT-UIN", random_wechat_uin().parse().expect("X-WECHAT-UIN header value must be valid"));
    if let Some(t) = token
        && !t.is_empty()
