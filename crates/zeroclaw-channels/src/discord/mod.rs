...
mod tests {
    ...
    /// A real, decodable 1x1 baseline JPEG (same fixture shape as the
    /// telegram tests). Multimodal preparation validates the decoded bytes of
    /// data-URI image markers, so a stub header no longer survives as an
    /// inline image.
    fn tiny_jpeg() -> Vec<u8> {
        vec![
