...
///
/// `request_approval` registers a token in [`PENDING_APPROVALS`] before it does
/// anything else, and the token is a bearer credential for a tool-call decision:
/// whoever presents it decides that call. An entry left behind is therefore a
/// decision that stays resolvable for the lifetime of the process, and the map is
/// process-global, so it is not bounded by a channel instance either.
///
/// Two exits used to leave one behind, and neither is exotic.
///
...
///
/// The other is cancellation. Every explicit cleanup lives inside the async body,
/// so none of it runs if the future is dropped, which a caller-side timeout or a
/// shutdown both do. `Drop` is the only hook that survives that, which is why the
/// entry's lifetime is tied to a guard rather than to any code path.
///
/// The guard is disarmed once a decision has been taken through a normal path, so
/// the existing removals still own their own cleanup and nothing is removed twice.
...
impl Channel for WhatsAppChannel {
    ...
    async fn request_approval(
        ...
        // The `?` here is what used to orphan a live token: it returns with the
        // entry already registered. The guard now cleans up on that path.
        ...
        let response = match tokio::time::timeout(timeout, rx_approval).await {
            ...
            _ => {
                // Timed out or the sender was dropped. This arm already owned its
                // own removal before the guard existed, so keep it and disarm,
                // which also keeps the removal synchronous with the deny rather
                // than deferring it to a spawned task.
                guard.disarm();
                let mut map = PENDING_APPROVALS.lock().await;
                map.remove(&token);
...
mod tests {
    ...
    /// Park a token the way `request_approval` does, and hand back the guard so
    /// a test can decide when the requesting future goes away.
    ...
    /// failure used to return with the entry already in the map. That path needs
    /// no cancellation to reach, which is what made it the cheapest to exploit.
    #[tokio::test]
    ...
    /// The control for the two above. A guard that removed unconditionally would
    /// pass both of them while breaking the normal path, where the reply handler
    /// has already taken the sender out of the map and a second removal would be
    /// removing whatever registered next.
    #[tokio::test]
