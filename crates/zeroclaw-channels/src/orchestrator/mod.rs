...
This is an example JSON object for profile settings."#;
    ...
    fn make_acp_test_config(cwd: &std::path::Path) -> zeroclaw_config::schema::Config {
    ...
    #[cfg(feature = "channel-acp-server")]
    #[tokio::test]
    async fn acp_session_close_releases_memory_but_keeps_store_record() {
        let cwd = tempfile::TempDir::new().unwrap();
        let store = Arc::new(
            zeroclaw_infra::acp_session_store::AcpSessionStore::new(cwd.path()).unwrap(),
        );
        let server: Arc<acp_server::AcpServer> = Arc::new(acp_server::AcpServer::new_with_store(
            make_acp_test_config(cwd.path()),
    ...
    #[cfg(feature = "channel-acp-server")]
    #[tokio::test]
    async fn acp_session_close_returns_not_found_for_unknown_session() {
        let cwd = tempfile::TempDir::new().unwrap();
        let server: acp_server::AcpServer = acp_server::AcpServer::new(
            make_acp_test_config(cwd.path()),
            acp_server::AcpServerConfig::default(),
        );
        ...
        assert_eq!(err.code, acp_server::SESSION_NOT_FOUND);
    }
}
