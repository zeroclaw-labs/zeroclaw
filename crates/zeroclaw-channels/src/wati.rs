    ...
    fn extract_sender(&self, payload: &serde_json::Value) -> Option<String> {
        ...
        if !self.is_number_allowed(&normalized_phone) {
            ::zeroclaw_log::record!(
                ...
                "ignoring message from unauthorized sender: . Add to channels.wati.allowed_numbers in config.toml, or run `zeroclaw config set channels.wati.allowed_numbers='[\"<msisdn>\"]'`."
            );
            return None;
        }
