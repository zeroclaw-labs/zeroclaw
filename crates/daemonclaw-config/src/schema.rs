...
/// Top-level DaemonClaw configuration, loaded from `config.toml`.
///
/// Resolution order: `DAEMONCLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.daemonclaw/config.toml`.
...
pub struct Config {
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `DAEMONCLAW_LOCALE`,
...
fn default_workspaces_dir() -> String {
    "~/.daemonclaw/workspaces".to_string()
}
...
fn default_project_intel_report_dir() -> String {
    "~/.daemonclaw/project-reports".into()
}
...
/// execution events back to DaemonClaw's gateway, updating a Slack message
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all DaemonClaw-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "daemonclaw" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "daemonclaw_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "daemonclaw".
    #[serde(default)]
...
/// Controls whether the heartbeat automatically picks up tasks from `tasks.db`.
///
/// - `none`: heartbeat does not claim tasks (legacy HEARTBEAT.md path only).
/// - `assisted`: heartbeat finds open tasks and reports them to the delivery
///   channel for operator approval before claiming.
/// - `full`: heartbeat autonomously claims and executes open tasks.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[serde(rename_all = "lowercase")]
pub enum AutonomousPickup {
    #[default]
    None,
    Assisted,
    Full,
...
pub struct MatrixConfig {
    ...
    /// When set, DaemonClaw recovers room keys and cross-signing secrets on startup.
    #[secret]
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `DAEMONCLAW_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    #[secret]
    ...
    /// Example: `["@?DaemonClaw", "\\+?15555550123"]`
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `DAEMONCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    #[secret]
    ...
    #[serde(default)]
    pub proxy_url: Option<String>,
    /// Display name of the bot in Nextcloud Talk (e.g. "daemonclaw").
...
/// How DaemonClaw receives events from Feishu / Lark.
///
...
pub enum LineDmPolicy {
    ...
    /// DaemonClaw prints the bind code on startup; send it once to unlock access.
    #[default]
    Pairing,
...
pub struct WebAuthnConfig {
    ...
    /// Relying Party display name. Default: "DaemonClaw".
...
fn default_estop_state_file() -> String {
    "~/.daemonclaw/estop-state.json".to_string()
}
...
///
/// When `enabled` is true, DaemonClaw validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to DaemonClaw permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to DaemonClaw tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[serde(deny_unknown_fields)]
pub struct NevisRoleMappingConfig {
    ...
    #[serde(default)]
    pub daemonclaw_permissions: Vec<String>,
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to daemonclaw dir)
...
#[cfg(feature = "voice-wake")]
#[derive(Debug, Clone, Serialize, Deserialize, daemonclaw_macros::Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[prefix = "voice-wake"]
pub struct VoiceWakeConfig {
    /// Wake word phrase to listen for (case-insensitive substring match).
    /// Default: `"hey daemonclaw"`.
...
fn default_report_output_dir() -> String {
    "~/.daemonclaw/security-reports".into()
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        let daemonclaw_dir = home.join(".daemonclaw");
...
fn default_config_dir() -> Result<PathBuf> {
    if let Ok(home) = std::env::var("HOME")
        && !home.is_empty()
    {
        return Ok(PathBuf::from(home).join(".daemonclaw"));
    }

    let home = UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    Ok(home.join(".daemonclaw"))
}
...
pub fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".daemonclaw"));
...
/// `DAEMONCLAW_CONFIG_DIR` > `DAEMONCLAW_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_daemonclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_daemonclaw_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "DAEMONCLAW_CONFIG_DIR",
            Self::EnvWorkspace => "DAEMONCLAW_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_daemonclaw_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("DAEMONCLAW_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let daemonclaw_dir = expand_tilde_path(custom_config_dir);
            return Ok((
                daemonclaw_dir.clone(),
                daemonclaw_dir.join("workspace"),
                ConfigResolutionSource::EnvConfigDir,
            ));
        }
    }

    if let Ok(custom_workspace) = std::env::var("DAEMONCLAW_WORKSPACE")
        && !custom_workspace.is_empty()
    {
        let expanded = expand_tilde_path(&custom_workspace);
        let (daemonclaw_dir, workspace_dir) = resolve_config_dir_for_workspace(&expanded);
        return Ok((
            daemonclaw_dir,
            workspace_dir,
    ...
    if let Some((daemonclaw_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_daemonclaw_dir).await?
    {
        return Ok((
            daemonclaw_dir,
            workspace_dir,
    ...
    Ok((
        default_daemonclaw_dir.to_path_buf(),
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'daemonclaw'.",
        path.display()
    )
}
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in DAEMONCLAW_EXTRA_HEADERS");
                continue;
            }
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am DaemonClaw, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are DaemonClaw, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    ...
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (daemonclaw_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_daemonclaw_dir, &default_workspace_dir).await?;

        let config_path = daemonclaw_dir.join("config.toml");

        fs::create_dir_all(&daemonclaw_dir)
            .await
            .with_context(|| config_dir_creation_error(&daemonclaw_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::secrets::SecretStore::new(&daemonclaw_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        // Open-skills opt-in flag: DAEMONCLAW_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("DAEMONCLAW_OPEN_SKILLS_ENABLED")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid DAEMONCLAW_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Open-skills directory override: DAEMONCLAW_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("DAEMONCLAW_OPEN_SKILLS_DIR") {
        ...
        // Skills script-file audit override: DAEMONCLAW_SKILLS_ALLOW_SCRIPTS
        if let Ok(flag) = std::env::var("DAEMONCLAW_SKILLS_ALLOW_SCRIPTS")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid DAEMONCLAW_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Skills prompt mode override: DAEMONCLAW_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("DAEMONCLAW_SKILLS_PROMPT_MODE")
            && !mode.trim().is_empty()
        {
            if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                ...
                tracing::warn!(
                    "Ignoring invalid DAEMONCLAW_SKILLS_PROMPT_MODE (valid: full|compact)"
                );
            }
        }

        // Gateway port: DAEMONCLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("DAEMONCLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
            && let Ok(port) = port_str.parse::<u16>()
        {
            self.gateway.port = port;
        }

        // Gateway host: DAEMONCLAW_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("DAEMONCLAW_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
            && !host.is_empty()
        {
            self.gateway.host = host;
        }

        // Allow public bind: DAEMONCLAW_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("DAEMONCLAW_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Require pairing: DAEMONCLAW_REQUIRE_PAIRING
        if let Ok(val) = std::env::var("DAEMONCLAW_REQUIRE_PAIRING") {
            self.gateway.require_pairing = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Web dist dir: DAEMONCLAW_WEB_DIST_DIR
        if let Ok(path) = std::env::var("DAEMONCLAW_WEB_DIST_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Reasoning override: DAEMONCLAW_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("DAEMONCLAW_REASONING_ENABLED")
        ...
        if let Ok(raw) = std::env::var("DAEMONCLAW_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("DAEMONCLAW_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: DAEMONCLAW_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("DAEMONCLAW_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: DAEMONCLAW_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("DAEMONCLAW_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: DAEMONCLAW_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("DAEMONCLAW_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // SearXNG instance URL: DAEMONCLAW_SEARXNG_INSTANCE_URL or SEARXNG_INSTANCE_URL
        if let Ok(instance_url) = std::env::var("DAEMONCLAW_SEARXNG_INSTANCE_URL")
        ...
        // Web search max results: DAEMONCLAW_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("DAEMONCLAW_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: DAEMONCLAW_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("DAEMONCLAW_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): DAEMONCLAW_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("DAEMONCLAW_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): DAEMONCLAW_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("DAEMONCLAW_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: DAEMONCLAW_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("DAEMONCLAW_STORAGE_CONNECT_TIMEOUT_SECS")
            && let Ok(timeout_secs) = timeout_secs.parse::<u64>()
            && timeout_secs > 0
        {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_daemonclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (daemonclaw_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_daemonclaw_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = daemonclaw_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let daemonclaw_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::secrets::SecretStore::new(daemonclaw_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.daemonclaw");
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/daemonclaw"));
        assert!(msg.contains("/etc/daemonclaw"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("daemonclaw"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:daemonclaw");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("User-Agent".to_string(), "MyApp/1.0".to_string())
        );
        assert_eq!(headers[1], ("X-Title".to_string(), "daemonclaw".to_string()));
    }

    #[test]
    async fn parse_extra_headers_env_with_url_value() {
        let headers =
            parse_extra_headers_env("HTTP-Referer:https://github.com/DeliveryBoyTech/daemonclaw");
        assert_eq!(headers.len(), 1);
        // Only splits on first colon, preserving URL colons in value
        assert_eq!(headers[0].0, "HTTP-Referer");
        assert_eq!(headers[0].1, "https://github.com/DeliveryBoyTech/daemonclaw");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : daemonclaw , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:daemonclaw,");
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0], ("X-Title".to_string(), "daemonclaw".to_string()));
    }
...
"#;
    ...
    async fn sync_directory_handles_existing_directory() {
        let dir = std::env::temp_dir().join(format!(
            "daemonclaw_test_sync_directory_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("daemonclaw_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "daemonclaw_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("daemonclaw_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.daemonclaw/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
            mention_only: false,
    ...
    async fn checklist_gateway_serde_roundtrip() {
        let g = GatewayConfig {
            ...
            path_prefix: Some("/daemonclaw".into()),
        ...
        assert_eq!(parsed.path_prefix.as_deref(), Some("/daemonclaw"));
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "DAEMONCLAW_PROXY_ENABLED",
            "DAEMONCLAW_HTTP_PROXY",
            "DAEMONCLAW_HTTPS_PROXY",
            "DAEMONCLAW_ALL_PROXY",
            "DAEMONCLAW_NO_PROXY",
            "DAEMONCLAW_PROXY_SCOPE",
            "DAEMONCLAW_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_OPEN_SKILLS_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_OPEN_SKILLS_DIR", "/tmp/open-skills") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_SKILLS_ALLOW_SCRIPTS", "yes") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_SKILLS_PROMPT_MODE", "compact") };
        config.apply_env_overrides();
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_OPEN_SKILLS_ENABLED") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_OPEN_SKILLS_DIR") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_SKILLS_ALLOW_SCRIPTS") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_SKILLS_PROMPT_MODE") };
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_OPEN_SKILLS_ENABLED", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_SKILLS_ALLOW_SCRIPTS", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_SKILLS_PROMPT_MODE", "invalid") };
        config.apply_env_overrides();
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("daemonclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".daemonclaw").join("config.toml");
    ...
    async fn env_override_workspace() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_WORKSPACE", "/custom/workspace") };
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_CONFIG_DIR", &explicit_config_dir) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_WORKSPACE") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("daemonclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".daemonclaw").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".daemonclaw");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("daemonclaw_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".daemonclaw");
    ...
    async fn load_or_init_uses_persisted_active_workspace_marker() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("daemonclaw_test_home_{}", uuid::Uuid::new_v4()));
        let temp_default_dir = temp_home.join(".daemonclaw");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_WORKSPACE", &env_workspace_dir) };
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        let temp_home =
            std::env::temp_dir().join(format!("daemonclaw_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".daemonclaw");
...
"#,
    ...
    async fn env_override_gateway_port() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_GATEWAY_PORT", "8080") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_GATEWAY_PORT") };
    }
    ...
    async fn env_override_gateway_host() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_GATEWAY_HOST", "0.0.0.0") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_GATEWAY_HOST") };
    }
    ...
    async fn env_override_require_pairing() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_REQUIRE_PAIRING", "false") };
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_REQUIRE_PAIRING", "true") };
        config.apply_env_overrides();
        assert!(config.gateway.require_pairing);
    ...
    async fn env_override_reasoning_enabled() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_REASONING_ENABLED", "false") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_REASONING_ENABLED", "true") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_REASONING_ENABLED") };
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_REASONING_ENABLED", "maybe") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_REASONING_EFFORT", "HIGH") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_REASONING_EFFORT") };
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_CODEX_REASONING_EFFORT", "minimal") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_CODEX_REASONING_EFFORT") };
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_STORAGE_PROVIDER", "qdrant") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_STORAGE_DB_URL", "http://localhost:6333") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_STORAGE_CONNECT_TIMEOUT_SECS", "15") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_STORAGE_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_STORAGE_DB_URL") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DAEMONCLAW_STORAGE_CONNECT_TIMEOUT_SECS") };
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe {
            std::env::set_var(
                "DAEMONCLAW_PROXY_SERVICES",
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_PROXY_SCOPE", "services") };
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        unsafe { std::env::set_var("DAEMONCLAW_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_PROXY_SCOPE", "environment") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_HTTPS_PROXY", "http://127.0.0.1:7891") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DAEMONCLAW_NO_PROXY", "localhost,127.0.0.1") };
...
[security.estop]
enabled = true
state_file = "~/.daemonclaw/estop-state.json"
require_otp_to_resume = true
"#,
        );
...
workspace_dir = "/daemonclaw-data/workspace"
config_path = "/daemonclaw-data/.daemonclaw/config.toml"
api_key = ""
default_provider = "openrouter"
