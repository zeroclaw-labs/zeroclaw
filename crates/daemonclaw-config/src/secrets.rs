...
// in `~/.daemonclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.daemonclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(daemonclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: daemonclaw_dir.join(".secret_key"),
            enabled,
        }
    }
            ...
            .map_err(|_| {
                tracing::error!(
                    ...
                    "enc2: decryption failed. `.secret_key` is missing or does not match the key used to encrypt this value. \
                     ...
                     Restore the original `.secret_key` from backup, or re-encrypt the affected secrets via `daemonclaw onboard`."
                );
