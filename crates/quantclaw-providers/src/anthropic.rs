...
use quantclaw_api::tool::ToolSpec;
...
impl AnthropicProvider {
    ...
    fn http_client(&self) -> Client {
        quantclaw_config::schema::build_runtime_proxy_client_with_timeouts(
            "provider.anthropic",
            120,
            10,
        )
...
impl Provider for AnthropicProvider {
    ...
    async fn chat(
        ...
        let tool_choice_override = quantclaw_api::TOOL_CHOICE_OVERRIDE
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = AnthropicProvider::new(None);
        let result = p
            .chat_with_system(Some("You are QuantClaw"), "hello", "claude-3-opus", 0.7)
            .await;
        assert!(result.is_err());
    }
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are QuantClaw".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are QuantClaw\""));
    }
