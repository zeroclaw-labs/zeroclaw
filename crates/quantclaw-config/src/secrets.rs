...
// in `~/.quantclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.quantclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(quantclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: quantclaw_dir.join(".secret_key"),
            enabled,
        }
    }
