...
/// Top-level QuantClaw configuration, loaded from `config.toml`.
///
/// Resolution order: `QUANTCLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.quantclaw/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `QUANTCLAW_API_KEY` or `API_KEY` env vars.
    #[secret]
    ...
    ///
    /// Can also be set via `QUANTCLAW_EXTRA_HEADERS` environment variable using
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `QUANTCLAW_LOCALE`,
...
fn default_workspaces_dir() -> String {
    "~/.quantclaw/workspaces".to_string()
}
...
fn default_project_intel_report_dir() -> String {
    "~/.quantclaw/project-reports".into()
}
...
/// execution events back to QuantClaw's gateway, updating a Slack message
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all QuantClaw-managed HTTP traffic (default).
    #[default]
    Quantclaw,
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "quantclaw_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "quantclaw".
    #[serde(default)]
...
pub struct MatrixConfig {
    ...
    /// When set, QuantClaw recovers room keys and cross-signing secrets on startup.
    #[secret]
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `QUANTCLAW_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    #[secret]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `QUANTCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    #[secret]
    ...
    #[serde(default)]
    pub proxy_url: Option<String>,
    /// Display name of the bot in Nextcloud Talk (e.g. "quantclaw").
...
/// How QuantClaw receives events from Feishu / Lark.
///
...
pub enum LineDmPolicy {
    ...
    /// QuantClaw prints the bind code on startup; send it once to unlock access.
    #[default]
    Pairing,
...
pub struct WebAuthnConfig {
    ...
    /// Relying Party display name. Default: "QuantClaw".
...
fn default_estop_state_file() -> String {
    "~/.quantclaw/estop-state.json".to_string()
}
...
///
/// When `enabled` is true, QuantClaw validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to QuantClaw permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to QuantClaw tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[serde(deny_unknown_fields)]
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to quantclaw dir)
...
fn default_report_output_dir() -> String {
    "~/.quantclaw/security-reports".into()
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            workspace_dir: quantclaw_dir.join("workspace"),
            config_path: quantclaw_dir.join("config.toml"),
            api_key: None,
            api_url: None,
            api_path: None,
...
fn default_config_dir() -> Result<PathBuf> {
    if let Ok(home) = std::env::var("HOME")
        && !home.is_empty()
    {
        return Ok(PathBuf::from(home).join(".quantclaw"));
    }

    let home = UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    Ok(home.join(".quantclaw"))
}
...
pub fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".quantclaw"));
...
/// `QUANTCLAW_CONFIG_DIR` > `QUANTCLAW_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_quantclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_quantclaw_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "QUANTCLAW_CONFIG_DIR",
            Self::EnvWorkspace => "QUANTCLAW_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_quantclaw_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("QUANTCLAW_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let quantclaw_dir = expand_tilde_path(custom_config_dir);
            return Ok((
                quantclaw_dir.clone(),
                quantclaw_dir.join("workspace"),
        ...
        && !custom_workspace.is_empty()
    {
        ...
        let (quantclaw_dir, workspace_dir) = resolve_config_dir_for_workspace(&expanded);
        return Ok((
            quantclaw_dir,
            workspace_dir,
    ...
    if let Some((quantclaw_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_quantclaw_dir).await?
    {
        return Ok((
            quantclaw_dir,
            workspace_dir,
    ...
    Ok((
        default_quantclaw_dir.to_path_buf(),
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'quantclaw'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "QUANTCLAW_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `QUANTCLAW_EXTRA_HEADERS` environment variable value.
///
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in QUANTCLAW_EXTRA_HEADERS");
                continue;
            }
...
///
/// When the workspace is created outside of `quantclaw onboard` (e.g., non-tty
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am QuantClaw, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are QuantClaw, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    ...
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (quantclaw_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_quantclaw_dir, &default_workspace_dir).await?;

        let config_path = quantclaw_dir.join("config.toml");

        fs::create_dir_all(&quantclaw_dir)
            .await
            .with_context(|| config_dir_creation_error(&quantclaw_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::secrets::SecretStore::new(&quantclaw_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: QUANTCLAW_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("QUANTCLAW_API_KEY").or_else(|_| std::env::var("API_KEY"))
            && !key.is_empty()
        {
        ...
        // 1) QUANTCLAW_PROVIDER always wins when set.
        // 2) QUANTCLAW_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("QUANTCLAW_PROVIDER") {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        } else if let Ok(provider) =
            std::env::var("QUANTCLAW_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        // Model: QUANTCLAW_MODEL or MODEL
        if let Ok(model) = std::env::var("QUANTCLAW_MODEL").or_else(|_| std::env::var("MODEL"))
            && !model.is_empty()
        {
            self.default_model = Some(model);
        }

        // Provider HTTP timeout: QUANTCLAW_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("QUANTCLAW_PROVIDER_TIMEOUT_SECS")
        ...
        // Extra provider headers: QUANTCLAW_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("QUANTCLAW_EXTRA_HEADERS") {
        ...
        // Workspace directory: QUANTCLAW_WORKSPACE
        if let Ok(workspace) = std::env::var("QUANTCLAW_WORKSPACE")
            && !workspace.is_empty()
        {
        ...
        // Open-skills opt-in flag: QUANTCLAW_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("QUANTCLAW_OPEN_SKILLS_ENABLED")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid QUANTCLAW_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Open-skills directory override: QUANTCLAW_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("QUANTCLAW_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Skills script-file audit override: QUANTCLAW_SKILLS_ALLOW_SCRIPTS
        if let Ok(flag) = std::env::var("QUANTCLAW_SKILLS_ALLOW_SCRIPTS")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid QUANTCLAW_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Skills prompt mode override: QUANTCLAW_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("QUANTCLAW_SKILLS_PROMPT_MODE")
            && !mode.trim().is_empty()
        {
            if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                ...
                tracing::warn!(
                    "Ignoring invalid QUANTCLAW_SKILLS_PROMPT_MODE (valid: full|compact)"
                );
            }
        }

        // Gateway port: QUANTCLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("QUANTCLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
            && let Ok(port) = port_str.parse::<u16>()
        {
            self.gateway.port = port;
        }

        // Gateway host: QUANTCLAW_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("QUANTCLAW_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
            && !host.is_empty()
        {
            self.gateway.host = host;
        }

        // Allow public bind: QUANTCLAW_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("QUANTCLAW_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Require pairing: QUANTCLAW_REQUIRE_PAIRING
        if let Ok(val) = std::env::var("QUANTCLAW_REQUIRE_PAIRING") {
            self.gateway.require_pairing = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Web dist dir: QUANTCLAW_WEB_DIST_DIR
        if let Ok(path) = std::env::var("QUANTCLAW_WEB_DIST_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Temperature: QUANTCLAW_TEMPERATURE
        if let Ok(temp_str) = std::env::var("QUANTCLAW_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring QUANTCLAW_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!(
                        "Ignoring QUANTCLAW_TEMPERATURE={temp_str:?}: not a valid number"
                    );
                }
            }
        }

        // Reasoning override: QUANTCLAW_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("QUANTCLAW_REASONING_ENABLED")
        ...
        if let Ok(raw) = std::env::var("QUANTCLAW_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("QUANTCLAW_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: QUANTCLAW_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("QUANTCLAW_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: QUANTCLAW_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("QUANTCLAW_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: QUANTCLAW_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("QUANTCLAW_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // SearXNG instance URL: QUANTCLAW_SEARXNG_INSTANCE_URL or SEARXNG_INSTANCE_URL
        if let Ok(instance_url) = std::env::var("QUANTCLAW_SEARXNG_INSTANCE_URL")
        ...
        // Web search max results: QUANTCLAW_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("QUANTCLAW_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: QUANTCLAW_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("QUANTCLAW_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): QUANTCLAW_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("QUANTCLAW_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): QUANTCLAW_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("QUANTCLAW_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: QUANTCLAW_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("QUANTCLAW_STORAGE_CONNECT_TIMEOUT_SECS")
            && let Ok(timeout_secs) = timeout_secs.parse::<u64>()
            && timeout_secs > 0
        {
            self.storage.provider.config.connect_timeout_secs = Some(timeout_secs);
        }
        // Proxy enabled flag: QUANTCLAW_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("QUANTCLAW_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: QUANTCLAW_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("QUANTCLAW_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("QUANTCLAW_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("QUANTCLAW_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("QUANTCLAW_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("QUANTCLAW_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid QUANTCLAW_PROXY_SCOPE (valid: environment|quantclaw|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("QUANTCLAW_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_quantclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (quantclaw_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_quantclaw_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = quantclaw_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let quantclaw_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::secrets::SecretStore::new(quantclaw_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.quantclaw");
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/quantclaw"));
        assert!(msg.contains("/etc/quantclaw"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("quantclaw"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:quantclaw");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("User-Agent".to_string(), "MyApp/1.0".to_string())
        );
        assert_eq!(headers[1], ("X-Title".to_string(), "quantclaw".to_string()));
    }

    #[test]
    async fn parse_extra_headers_env_with_url_value() {
        let headers =
            parse_extra_headers_env("HTTP-Referer:https://github.com/quant-speed/quantclaw");
        assert_eq!(headers.len(), 1);
        // Only splits on first colon, preserving URL colons in value
        assert_eq!(headers[0].0, "HTTP-Referer");
        assert_eq!(headers[0].1, "https://github.com/quant-speed/quantclaw");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : quantclaw , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
        assert_eq!(headers[0], ("X-Title".to_string(), "quantclaw".to_string()));
        assert_eq!(
            headers[1],
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:quantclaw,");
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0], ("X-Title".to_string(), "quantclaw".to_string()));
    }
...
"#;
        ...
        assert_eq!(parsed.extra_headers.get("X-Title").unwrap(), "quantclaw");
    }
...
"#;
    ...
    async fn sync_directory_handles_existing_directory() {
        let dir = std::env::temp_dir().join(format!(
            "quantclaw_test_sync_directory_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("quantclaw_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "quantclaw_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("quantclaw_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.quantclaw/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
            mention_only: false,
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "QUANTCLAW_PROXY_ENABLED",
            "QUANTCLAW_HTTP_PROXY",
            "QUANTCLAW_HTTPS_PROXY",
            "QUANTCLAW_ALL_PROXY",
            "QUANTCLAW_NO_PROXY",
            "QUANTCLAW_PROXY_SCOPE",
            "QUANTCLAW_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_api_key() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_API_KEY", "sk-test-env-key") };
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_API_KEY") };
    }
    ...
    async fn env_override_provider() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_PROVIDER", "anthropic") };
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_PROVIDER") };
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        unsafe { std::env::remove_var("QUANTCLAW_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_MODEL_PROVIDER", "openai-codex") };
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_MODEL_PROVIDER") };
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_OPEN_SKILLS_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_OPEN_SKILLS_DIR", "/tmp/open-skills") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_SKILLS_ALLOW_SCRIPTS", "yes") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_SKILLS_PROMPT_MODE", "compact") };
        config.apply_env_overrides();
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_OPEN_SKILLS_ENABLED") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_OPEN_SKILLS_DIR") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_SKILLS_ALLOW_SCRIPTS") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_SKILLS_PROMPT_MODE") };
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_OPEN_SKILLS_ENABLED", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_SKILLS_ALLOW_SCRIPTS", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_SKILLS_PROMPT_MODE", "invalid") };
        config.apply_env_overrides();
    ...
    #[test]
    async fn env_override_quant_claw_provider_overrides_non_default_provider() {
        let _env_guard = env_override_lock().await;
        let mut config = Config {
            default_provider: Some("custom:https://proxy.example.com/v1".to_string()),
            ..Config::default()
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_PROVIDER", "openrouter") };
        // SAFETY: test-only, single-threaded test runner.
    ...
    async fn env_override_model() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_MODEL", "gpt-4o") };
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_MODEL") };
    }
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("quantclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".quantclaw").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_WORKSPACE", &workspace_dir) };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_WORKSPACE") };
    ...
    async fn env_override_workspace() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_WORKSPACE", "/custom/workspace") };
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_CONFIG_DIR", &explicit_config_dir) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_WORKSPACE") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("quantclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".quantclaw").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".quantclaw");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("quantclaw_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".quantclaw");
    ...
    async fn load_or_init_uses_persisted_active_workspace_marker() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("quantclaw_test_home_{}", uuid::Uuid::new_v4()));
        let temp_default_dir = temp_home.join(".quantclaw");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_WORKSPACE", &env_workspace_dir) };
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        let temp_home =
            std::env::temp_dir().join(format!("quantclaw_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".quantclaw");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_PROVIDER", "") };
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_GATEWAY_PORT", "8080") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_GATEWAY_PORT") };
    }
    ...
    async fn env_override_gateway_host() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_GATEWAY_HOST", "0.0.0.0") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_GATEWAY_HOST") };
    }
    ...
    async fn env_override_require_pairing() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_REQUIRE_PAIRING", "false") };
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_REQUIRE_PAIRING", "true") };
        config.apply_env_overrides();
        assert!(config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_REQUIRE_PAIRING") };
    }
    ...
    async fn env_override_temperature() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_TEMPERATURE", "0.5") };
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_TEMPERATURE") };
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_TEMPERATURE", "3.0") };
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_REASONING_ENABLED", "false") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_REASONING_ENABLED", "true") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_REASONING_ENABLED") };
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_REASONING_ENABLED", "maybe") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_REASONING_EFFORT", "HIGH") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_REASONING_EFFORT") };
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_CODEX_REASONING_EFFORT", "minimal") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_CODEX_REASONING_EFFORT") };
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_STORAGE_PROVIDER", "qdrant") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_STORAGE_DB_URL", "http://localhost:6333") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_STORAGE_CONNECT_TIMEOUT_SECS", "15") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_STORAGE_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_STORAGE_DB_URL") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("QUANTCLAW_STORAGE_CONNECT_TIMEOUT_SECS") };
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe {
            std::env::set_var(
                "QUANTCLAW_PROXY_SERVICES",
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_PROXY_SCOPE", "services") };
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        unsafe { std::env::set_var("QUANTCLAW_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_PROXY_SCOPE", "environment") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_HTTPS_PROXY", "http://127.0.0.1:7891") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("QUANTCLAW_NO_PROXY", "localhost,127.0.0.1") };
...
[security.estop]
enabled = true
state_file = "~/.quantclaw/estop-state.json"
require_otp_to_resume = true
"#,
        );
...
workspace_dir = "/quantclaw-data/workspace"
config_path = "/quantclaw-data/.quantclaw/config.toml"
api_key = ""
default_provider = "openrouter"
