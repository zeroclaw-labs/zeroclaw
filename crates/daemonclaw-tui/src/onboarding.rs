...
const DOCS_BASE: &str = "https://www.daemonclawlabs.ai/docs";
...
impl App {
    fn new() -> Self {
        ...
        let port = std::env::var("DAEMONCLAW_GATEWAY_PORT")
        ...
        let host =
            std::env::var("DAEMONCLAW_GATEWAY_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
    ...
    /// Run `daemonclaw gateway get-paircode --new` locally to generate a code.
    async fn generate_code_via_cli() -> Option<String> {
        let output = tokio::process::Command::new("daemonclaw")
    ...
    /// Run `docker/podman exec <container> daemonclaw gateway get-paircode --new`.
    async fn generate_code_via_docker() -> Option<String> {
        let cli = container_cli();
        // Find daemonclaw container
        let ps = tokio::process::Command::new(cli)
            .args([
                "ps",
                "--filter",
                "ancestor=ghcr.io/DeliveryBoyTech/daemonclaw",
                "--format",
                "{{.Names}}",
            ])
            .output()
        ...
        if container.is_empty() {
            ...
            let ps2 = tokio::process::Command::new(cli)
                .args(["ps", "--filter", "name=daemonclaw", "--format", "{{.Names}}"])
                .output()
                .await
                .ok()?;
    ...
    if app.screen == Screen::Complete {
        ...
        match save_tui_config(&app).await {
            Ok(()) => {
                ...
                println!();
                println!("  \u{1f980} DaemonClaw {VERSION} configured successfully!");
                println!(
                    "     Provider:   {} ({})",
                ...
                if channel != "Skip for now" {
                    ...
                    println!("        daemonclaw config edit");
                    println!();
                }
                println!("  Run `daemonclaw daemon` to start your agent.");
                println!();
            }
            Err(e) => {
                eprintln!();
                eprintln!("  \u{2717} Failed to save configuration: {e}");
                eprintln!("  You can re-run: daemonclaw onboard --tui");
                eprintln!();
            }
        }
    }
...
fn apply_tui_selections_to_config(app: &App, config: &mut Config) {
    ...
    // real tokens via `daemonclaw config edit` or the dashboard.
    let channel = app.selected_channel();
    match channel {
        "Telegram" => {
        ...
        "IRC" => {
            if config.channels.irc.is_none() {
                config.channels.irc = Some(IrcConfig {
                    ...
                    port: 6697,
                    nickname: String::from("daemonclaw-bot"),
                    username: None,
...
/// If a DaemonClaw Docker container is running, reconfigure it via `docker exec`.
async fn push_config_to_docker(app: &App) {
    ...
    // Use `daemonclaw onboard --quick` inside the container to reconfigure
    let mut args = vec![
        "exec".to_string(),
        container,
...
fn render(frame: &mut Frame, app: &App) {
    ...
    let version_line = Line::from(vec![
        ...
        Span::styled(format!("DaemonClaw {VERSION}"), theme::heading_style()),
        Span::styled(
...
fn setup_title() -> Paragraph<'static> {
    Paragraph::new(Line::from(vec![
        ...
        Span::styled("DaemonClaw setup", theme::heading_style()),
    ]))
}
...
fn render_welcome(frame: &mut Frame, area: Rect) {
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Welcome to DaemonClaw \u{2014} the fastest, smallest AI assistant.",
                theme::body_style(),
            ),
        ]),
        Line::from(vec![
...
fn render_security(frame: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "DaemonClaw is optimized for single-operator deployments.",
            theme::body_style(),
        )),
        Line::from(Span::styled(
            "By default, DaemonClaw is a personal agent: one trusted operator",
            theme::body_style(),
        )),
        ...
        Line::from(""),
        Line::from(Span::styled(
            "DaemonClaw is not a hostile multi-tenant boundary by default.",
            theme::body_style(),
        )),
        Line::from(Span::styled(
        ...
        Line::from(Span::styled(
            "  daemonclaw security audit --deep",
            theme::dim_style(),
        )),
        Line::from(Span::styled(
            "  daemonclaw security audit --fix",
            theme::dim_style(),
        )),
        Line::from(""),
        Line::from(Span::styled(
...
fn render_how_channels_work(frame: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "  Approve with: daemonclaw pairing approve <channel> <code>",
            theme::dim_style(),
        )),
        Line::from(Span::styled(
        ...
        Line::from(Span::styled(
            "  Multi-user DMs: run: daemonclaw config set session.dmScope",
            theme::body_style(),
        )),
        Line::from(Span::styled(
    ...
    frame.render_widget(
        InfoPanel {
            ...
            lines: vec![
                Line::from(""),
                Line::from(Span::styled(
                    "  What now: https://daemonclawlabs.ai/showcase",
                    theme::body_style(),
                )),
                Line::from(Span::styled(
...
fn render_complete(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let title = Line::from(vec![
        Span::styled("\u{2514}  ", theme::border_style()),
        Span::styled(
            "Onboarding complete. Use the dashboard link above to control DaemonClaw.",
            theme::heading_style(),
        ),
    ]);
    ...
    let mut summary_lines = vec![
        Line::from(""),
        Line::from(Span::styled(
            "  \u{1f980} DaemonClaw configured successfully!",
    ...
    summary_lines.extend([
        Line::from(""),
        Line::from(Span::styled(
            "  Run `daemonclaw daemon` to start your agent.",
            theme::body_style(),
        )),
        Line::from(Span::styled(
            "  Run `daemonclaw doctor` to validate your setup.",
            theme::body_style(),
        )),
        Line::from(""),
    ]);
...
mod tests {
    ...
    fn save_channel_irc() {
        ...
        assert_eq!(irc.nickname, "daemonclaw-bot");
    }
    ...
    fn config_serializes_to_valid_toml() {
        ...
        let _: Config = toml::from_str::<daemonclaw_config::migration::V1Compat>(&toml_str)
