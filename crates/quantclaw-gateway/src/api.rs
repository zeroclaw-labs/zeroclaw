...
pub struct CronAddBody {
    ...
    pub delivery: Option<quantclaw_runtime::cron::DeliveryConfig>,
...
pub async fn handle_api_status(
    ...
    let locale = config
        ...
        .filter(|s| !s.is_empty())
        .map(String::from)
        .unwrap_or_else(quantclaw_runtime::i18n::detect_locale);
...
pub async fn handle_api_config_put(
    ...
    let incoming: quantclaw_config::schema::Config = match toml::from_str(&body) {
        Ok(c) => c,
        Err(e) => {
            return (
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.lock().clone();
    match quantclaw_runtime::cron::list_jobs(&config) {
...
pub async fn handle_api_cron_add(
    ...
    let config = state.config.lock().clone();
    let schedule = quantclaw_runtime::cron::Schedule::Cron {
        expr: schedule,
        tz: None,
    };
    if let Err(e) = quantclaw_runtime::cron::validate_delivery_config(delivery.as_ref()) {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let result = if is_agent {
        ...
        let session_target = session_target
            .as_deref()
            .map(quantclaw_runtime::cron::SessionTarget::parse)
            .unwrap_or_default();

        let default_delete = matches!(schedule, quantclaw_runtime::cron::Schedule::At { .. });
        let delete_after_run = delete_after_run.unwrap_or(default_delete);

        quantclaw_runtime::cron::add_agent_job(
            &config,
            name,
            schedule,
            prompt,
        ...
        quantclaw_runtime::cron::add_shell_job_with_approval(
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = quantclaw_runtime::cron::get_job(&config, &id) {
        return (
    ...
    match quantclaw_runtime::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_patch(
    ...
    let schedule = match body.schedule {
        Some(expr) if !expr.trim().is_empty() => Some(quantclaw_runtime::cron::Schedule::Cron {
    ...
    let existing = match quantclaw_runtime::cron::get_job(&config, &id) {
        Ok(j) => j,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let is_agent = matches!(existing.job_type, quantclaw_runtime::cron::JobType::Agent);
    ...
    let patch = quantclaw_runtime::cron::CronJobPatch {
        name: body.name,
        schedule,
        command: patch_command,
        prompt: patch_prompt,
        ..quantclaw_runtime::cron::CronJobPatch::default()
    };

    match quantclaw_runtime::cron::update_shell_job_with_approval(&config, &id, patch, false) {
        Ok(job) => Json(serde_json::json!({"status": "ok", "job": job})).into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.lock().clone();
    match quantclaw_runtime::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.lock().clone();
    let entries = quantclaw_runtime::integrations::registry::all_integrations();
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        ...
        let enabled = matches!(
            status,
            quantclaw_runtime::integrations::IntegrationStatus::Active
        );
        settings.insert(
...
pub async fn handle_api_doctor(
    ...
    let config = state.config.lock().clone();
    let results = quantclaw_runtime::doctor::diagnose(&config);

    let ok_count = results
        .iter()
        .filter(|r| r.severity == quantclaw_runtime::doctor::Severity::Ok)
        .count();
    let warn_count = results
        .iter()
        .filter(|r| r.severity == quantclaw_runtime::doctor::Severity::Warn)
        .count();
    let error_count = results
        .iter()
        .filter(|r| r.severity == quantclaw_runtime::doctor::Severity::Error)
        .count();
...
pub async fn handle_api_memory_list(
    ...
    if params.query.is_some() || params.since.is_some() || params.until.is_some() {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => quantclaw_memory::MemoryCategory::Core,
            "daily" => quantclaw_memory::MemoryCategory::Daily,
            "conversation" => quantclaw_memory::MemoryCategory::Conversation,
            other => quantclaw_memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(quantclaw_memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = quantclaw_tools::cli_discovery::discover_cli_tools(&[], &[]);
...
pub async fn handle_api_health(
    ...
    let snapshot = quantclaw_runtime::health::snapshot();
...
fn model_route_identity_matches(
    incoming: &quantclaw_config::schema::ModelRouteConfig,
    current: &quantclaw_config::schema::ModelRouteConfig,
) -> bool {
...
fn embedding_route_identity_matches(
    incoming: &quantclaw_config::schema::EmbeddingRouteConfig,
    current: &quantclaw_config::schema::EmbeddingRouteConfig,
) -> bool {
    normalize_route_field(&incoming.hint) == normalize_route_field(&current.hint)
        && normalize_route_field(&incoming.provider) == normalize_route_field(&current.provider)
        && normalize_route_field(&incoming.model) == normalize_route_field(&current.model)
...
fn restore_model_route_api_keys(
    incoming: &mut [quantclaw_config::schema::ModelRouteConfig],
    current: &[quantclaw_config::schema::ModelRouteConfig],
) {
...
fn restore_embedding_route_api_keys(
    incoming: &mut [quantclaw_config::schema::EmbeddingRouteConfig],
    current: &[quantclaw_config::schema::EmbeddingRouteConfig],
) {
    let mut used_current = vec![false; current.len()];
    for incoming_route in incoming {
...
fn mask_sensitive_fields(
    config: &quantclaw_config::schema::Config,
) -> quantclaw_config::schema::Config {
...
fn restore_masked_sensitive_fields(
    incoming: &mut quantclaw_config::schema::Config,
    current: &quantclaw_config::schema::Config,
) {
...
fn hydrate_config_for_save(
    mut incoming: quantclaw_config::schema::Config,
    current: &quantclaw_config::schema::Config,
) -> quantclaw_config::schema::Config {
...
pub async fn handle_claude_code_hook(
    State(state): State<AppState>,
    Json(payload): Json<quantclaw_tools::claude_code_runner::ClaudeCodeHookEvent>,
) -> impl IntoResponse {
...
mod tests {
    ...
    fn test_state(config: quantclaw_config::schema::Config) -> AppState {
        AppState {
            ...
            wati: None,
            gmail_push: None,
            observer: Arc::new(quantclaw_runtime::observability::NoopObserver),
            ...
            web_dist_dir: None,
            canvas_store: quantclaw_runtime::tools::CanvasStore::new(),
    ...
    fn masking_keeps_toml_valid_and_preserves_api_keys_type() {
        let mut cfg = quantclaw_config::schema::Config {
        ...
        cfg.tunnel.cloudflare = Some(quantclaw_config::schema::CloudflareTunnelConfig {
            token: "cf-token".to_string(),
        });
        cfg.memory.qdrant.api_key = Some("qdrant-key".to_string());
        cfg.channels_config.wati = Some(quantclaw_config::schema::WatiConfig {
            enabled: true,
        ...
        cfg.channels_config.feishu = Some(quantclaw_config::schema::FeishuConfig {
            enabled: true,
        ...
        cfg.channels_config.email = Some(quantclaw_config::scattered_types::EmailConfig {
            enabled: true,
            ...
            default_subject: "QuantClaw Message".to_string(),
            max_attachment_bytes: 25 * 1024 * 1024,
        });
        cfg.model_routes = vec![quantclaw_config::schema::ModelRouteConfig {
            hint: "reasoning".to_string(),
            provider: "openrouter".to_string(),
            model: "anthropic/claude-sonnet-4.6".to_string(),
            api_key: Some("route-model-key".to_string()),
        }];
        cfg.embedding_routes = vec![quantclaw_config::schema::EmbeddingRouteConfig {
        ...
        let parsed: quantclaw_config::schema::Config =
    ...
    fn hydrate_config_for_save_restores_masked_secrets_and_paths() {
        let mut current = quantclaw_config::schema::Config {
        ...
        current.tunnel.cloudflare = Some(quantclaw_config::schema::CloudflareTunnelConfig {
            token: "cf-token-real".to_string(),
        });
        current.tunnel.ngrok = Some(quantclaw_config::schema::NgrokTunnelConfig {
            auth_token: "ngrok-token-real".to_string(),
            domain: None,
        });
        current.memory.qdrant.api_key = Some("qdrant-real".to_string());
        current.channels_config.wati = Some(quantclaw_config::schema::WatiConfig {
            enabled: true,
        ...
        current.channels_config.feishu = Some(quantclaw_config::schema::FeishuConfig {
            enabled: true,
        ...
        current.channels_config.email = Some(quantclaw_config::scattered_types::EmailConfig {
            enabled: true,
            imap_host: "imap.example.com".to_string(),
            imap_port: 993,
            imap_folder: "INBOX".to_string(),
        ...
        current.model_routes = vec![
            quantclaw_config::schema::ModelRouteConfig {
                hint: "reasoning".to_string(),
                provider: "openrouter".to_string(),
                model: "anthropic/claude-sonnet-4.6".to_string(),
        ...
        current.embedding_routes = vec![
            quantclaw_config::schema::EmbeddingRouteConfig {
                hint: "semantic".to_string(),
                provider: "openai".to_string(),
                model: "text-embedding-3-small".to_string(),
                dimensions: Some(1536),
    ...
    fn hydrate_config_for_save_restores_route_keys_by_identity_and_clears_unmatched_masks() {
        ...
        incoming
            .model_routes
            .push(quantclaw_config::schema::ModelRouteConfig {
                hint: "new".to_string(),
                provider: "openai".to_string(),
        ...
        incoming
            .embedding_routes
            .push(quantclaw_config::schema::EmbeddingRouteConfig {
    ...
    async fn cron_api_shell_roundtrip_includes_delivery() {
        ...
        let config = quantclaw_config::schema::Config {
            workspace_dir: tmp.path().join("workspace"),
            config_path: tmp.path().join("config.toml"),
            ..quantclaw_config::schema::Config::default()
        };
    ...
    async fn cron_api_accepts_agent_jobs() {
        ...
        let config = state.config.lock().clone();
        let jobs = quantclaw_runtime::cron::list_jobs(&config).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].job_type, quantclaw_runtime::cron::JobType::Agent);
    ...
    async fn cron_api_rejects_announce_delivery_without_target() {
        ...
        let config = state.config.lock().clone();
        assert!(
            quantclaw_runtime::cron::list_jobs(&config)
                .unwrap()
                .is_empty()
        );
    }
