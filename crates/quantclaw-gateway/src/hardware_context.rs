...
//!
//! QuantClaw's agent loop calls [`quantclaw_hardware::boot`] on **every** request,
//! which re-reads `~/.quantclaw/hardware/` from disk.  Writing to those files
...
/// Return `~/.quantclaw/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".quantclaw").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.quantclaw/hardware/devices/<device>.md`:
/// ```text
...
///
/// Since [`quantclaw_hardware::boot`] re-reads from disk on every agent invocation,
...
pub async fn handle_hardware_reload(
    ...
    let context = quantclaw_hardware::load_hardware_context_prompt(&[]);
