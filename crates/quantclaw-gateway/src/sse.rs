...
pub struct BroadcastObserver {
    inner: Box<dyn quantclaw_runtime::observability::Observer>,
...
impl BroadcastObserver {
    ...
    pub fn inner(&self) -> &dyn quantclaw_runtime::observability::Observer {
        self.inner.as_ref()
    }
}

impl quantclaw_runtime::observability::Observer for BroadcastObserver {
    fn record_event(&self, event: &quantclaw_runtime::observability::ObserverEvent) {
        // Forward to inner observer
        self.inner.record_event(event);

        // Broadcast to SSE subscribers
        let json = match event {
            quantclaw_runtime::observability::ObserverEvent::LlmRequest {
                provider, model, ..
            } => serde_json::json!({
                "type": "llm_request",
                "provider": provider,
            ...
            quantclaw_runtime::observability::ObserverEvent::ToolCall {
                tool,
                duration,
                success,
            } => serde_json::json!({
            ...
            quantclaw_runtime::observability::ObserverEvent::ToolCallStart { tool, .. } => {
                serde_json::json!({
            ...
            quantclaw_runtime::observability::ObserverEvent::Error { component, message } => {
                serde_json::json!({
                    "type": "error",
                    "component": component,
                    "message": message,
            ...
            quantclaw_runtime::observability::ObserverEvent::AgentStart { provider, model } => {
                serde_json::json!({
                    "type": "agent_start",
                    "provider": provider,
                    "model": model,
            ...
            quantclaw_runtime::observability::ObserverEvent::AgentEnd {
                provider,
                model,
                duration,
                tokens_used,
    ...
    fn record_metric(&self, metric: &quantclaw_runtime::observability::traits::ObserverMetric) {
