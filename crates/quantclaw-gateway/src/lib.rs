...
use quantclaw_infra::session_sqlite::SqliteSessionBackend;
use quantclaw_memory::{self, Memory, MemoryCategory};
use quantclaw_providers::{self, ChatMessage, Provider};
use quantclaw_runtime::cost::CostTracker;
use quantclaw_runtime::platform;
use quantclaw_runtime::security::pairing::{PairingGuard, constant_time_eq, is_public_bind};
use quantclaw_runtime::tools;
use quantclaw_runtime::tools::CanvasStore;
...
/// Read gateway request timeout from `QUANTCLAW_GATEWAY_TIMEOUT_SECS` env var
...
pub fn gateway_request_timeout_secs() -> u64 {
    std::env::var("QUANTCLAW_GATEWAY_TIMEOUT_SECS")
        .ok()
...
fn whatsapp_memory_key(msg: &quantclaw_api::channel::ChannelMessage) -> String {
    format!("whatsapp_{}_{}", msg.sender, msg.id)
}

fn linq_memory_key(msg: &quantclaw_api::channel::ChannelMessage) -> String {
    format!("linq_{}_{}", msg.sender, msg.id)
}

fn wati_memory_key(msg: &quantclaw_api::channel::ChannelMessage) -> String {
    format!("wati_{}_{}", msg.sender, msg.id)
}

fn nextcloud_talk_memory_key(msg: &quantclaw_api::channel::ChannelMessage) -> String {
    format!("nextcloud_talk_{}_{}", msg.sender, msg.id)
}

fn sender_session_id(channel: &str, msg: &quantclaw_api::channel::ChannelMessage) -> String {
    match &msg.thread_ts {
...
pub struct AppState {
    ...
    pub observer: Arc<dyn quantclaw_runtime::observability::Observer>,
...
pub async fn run_gateway(
    ...
    // ── Hooks ──────────────────────────────────────────────────────
    let hooks: Option<std::sync::Arc<quantclaw_runtime::hooks::HookRunner>> = if config.hooks.enabled
    {
        Some(std::sync::Arc::new(
            quantclaw_runtime::hooks::HookRunner::new(),
        ))
    } else {
        None
    };
    ...
    let provider: Arc<dyn Provider> =
        Arc::from(quantclaw_providers::create_resilient_provider_with_options(
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("QUANTCLAW_WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    // Priority: environment variable > config file
    let linq_signing_secret: Option<Arc<str>> = std::env::var("QUANTCLAW_LINQ_SIGNING_SECRET")
        .ok()
        .and_then(|secret| {
            let secret = secret.trim();
            (!secret.is_empty()).then(|| secret.to_owned())
    ...
    let nextcloud_talk_webhook_secret: Option<Arc<str>> =
        std::env::var("QUANTCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET")
            .ok()
            .and_then(|secret| {
                let secret = secret.trim();
                (!secret.is_empty()).then(|| secret.to_owned())
    ...
    // ── Tunnel ────────────────────────────────────────────────
    let tunnel = quantclaw_runtime::tunnel::create_tunnel(&config.tunnel)?;
    ...
    let web_dist_dir: Option<std::path::PathBuf> = config
        ...
        .or_else(|| {
            ...
            if let Some(data_dir) = dirs_data_local() {
                candidates.push(data_dir.join("quantclaw/web/dist"));
            }
            candidates
                .into_iter()
    ...
    if let Some(ref dir) = web_dist_dir {
        ...
        tracing::info!(
            "Web dashboard: not available (set gateway.web_dist_dir or QUANTCLAW_WEB_DIST_DIR)"
        );
    }

    let pfx = path_prefix.unwrap_or("");
    println!("🦀 QuantClaw Gateway listening on http://{display_addr}{pfx}");
    ...
    if let Some(code) = pairing.pairing_code() {
        ...
        println!("     To pair a new device: quantclaw gateway get-paircode --new");
        println!();
    } else {
    ...
    quantclaw_runtime::health::mark_component_ok("gateway");
    ...
    let broadcast_observer: Arc<dyn quantclaw_runtime::observability::Observer> =
        Arc::new(sse::BroadcastObserver::new(
            quantclaw_runtime::observability::create_observer(&config.observability),
            event_tx.clone(),
            event_buffer.clone(),
        ));
    ...
    let state = AppState {
        ...
        webauthn: if config.security.webauthn.enabled {
            let secret_store = Arc::new(quantclaw_runtime::security::SecretStore::new(
                &config.workspace_dir,
                true,
            ));
            let wa_config = quantclaw_runtime::security::webauthn::WebAuthnConfig {
                enabled: true,
            ...
            Some(Arc::new(api_webauthn::WebAuthnState {
                manager: quantclaw_runtime::security::webauthn::WebAuthnManager::new(
                    wa_config,
                    secret_store,
                    &config.workspace_dir,
                ),
        ...
        loop {
            tokio::select! {
                ...
                _ = shutdown_signal.changed() => {
                    tracing::info!("🦀 QuantClaw Gateway shutting down...");
                    break;
                }
            }
        }
...
async fn handle_health(State(state): State<AppState>) -> impl IntoResponse {
    let body = serde_json::json!({
        ...
        "runtime": quantclaw_runtime::health::snapshot_json(),
    });
    Json(body)
}
...
fn prometheus_observer_from_state(
    observer: &dyn quantclaw_runtime::observability::Observer,
) -> Option<&quantclaw_runtime::observability::PrometheusObserver> {
    observer
        .as_any()
        .downcast_ref::<quantclaw_runtime::observability::PrometheusObserver>()
        .or_else(|| {
            observer
                .as_any()
...
async fn run_gateway_chat_simple(state: &AppState, message: &str) -> anyhow::Result<String> {
    ...
    let system_prompt = {
        ...
        quantclaw_runtime::agent::system_prompt::build_system_prompt(
    ...
    let prepared = quantclaw_providers::multimodal::prepare_messages_for_provider(
        &messages,
        &multimodal_config,
    )
    .await?;
...
async fn run_gateway_chat_with_tools(
    ...
    Box::pin(quantclaw_runtime::agent::process_message(
...
async fn handle_webhook(
    ...
    if state.auto_save && !quantclaw_memory::should_skip_autosave_content(message) {
    ...
    state.observer.record_event(
        &quantclaw_runtime::observability::ObserverEvent::AgentStart {
    ...
    match run_gateway_chat_simple(&state, message).await {
        Ok(response) => {
            ...
            state.observer.record_event(
                &quantclaw_runtime::observability::ObserverEvent::LlmResponse {
                    provider: provider_label.clone(),
                    model: model_label.clone(),
                    duration,
                    success: true,
            ...
            state.observer.record_metric(
                &quantclaw_runtime::observability::traits::ObserverMetric::RequestLatency(duration),
            );
            state.observer.record_event(
        ...
        Err(e) => {
            ...
            state
                .observer
                .record_event(&quantclaw_runtime::observability::ObserverEvent::Error {
...
async fn handle_whatsapp_message(
    ...
    for msg in &messages {
        ...
        // Auto-save to memory
        if state.auto_save && !quantclaw_memory::should_skip_autosave_content(&msg.content) {
...
async fn handle_linq_webhook(
    ...
    if let Some(ref signing_secret) = state.linq_signing_secret {
        ...
        if !quantclaw_channels::linq::verify_linq_signature(
            signing_secret,
            &body_str,
            timestamp,
            signature,
...
async fn handle_nextcloud_talk_webhook(
    ...
    if let Some(ref webhook_secret) = state.nextcloud_talk_webhook_secret {
        ...
        if !quantclaw_channels::nextcloud_talk::verify_nextcloud_talk_signature(
            webhook_secret,
            random,
            &body_str,
            signature,
...
async fn handle_gmail_push_webhook(
    ...
    let body_str = String::from_utf8_lossy(&body);
    let envelope: quantclaw_channels::gmail_push::PubSubEnvelope =
...
mod tests {
    ...
    fn gateway_timeout_falls_back_to_default() {
        ...
        unsafe { std::env::remove_var("QUANTCLAW_GATEWAY_TIMEOUT_SECS") };
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let wrapped = sse::BroadcastObserver::new(
            Box::new(quantclaw_runtime::observability::PrometheusObserver::new()),
            event_tx.clone(),
            event_buffer,
        );
        quantclaw_runtime::observability::Observer::record_event(
            &wrapped,
            &quantclaw_runtime::observability::ObserverEvent::HeartbeatTick,
        );

        let observer: Arc<dyn quantclaw_runtime::observability::Observer> = Arc::new(wrapped);
        let state = AppState {
            config: Arc::new(Mutex::new(Config::default())),
            provider: Arc::new(MockProvider::default()),
            model: "test-model".into(),
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("quantclaw_heartbeat_ticks_total 1"));
    }
    ...
    async fn persist_pairing_tokens_writes_config_tokens() {
        ...
        assert!(
            quantclaw_runtime::security::SecretStore::is_encrypted(on_disk),
