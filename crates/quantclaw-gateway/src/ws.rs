...
const WS_PROTOCOL: &str = "quantclaw.v1";
...
async fn handle_socket(
    ...
    let mut agent = match quantclaw_runtime::agent::Agent::from_config(&config).await {
        Ok(a) => a,
        Err(e) => {
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(text) {
            if parsed["type"].as_str() == Some("message") {
                ...
                if !content.is_empty() {
                    ...
                    if let Some(ref backend) = state.session_backend {
                        let user_msg = quantclaw_providers::ChatMessage::user(&content);
...
async fn process_chat_message(
    state: &AppState,
    agent: &mut quantclaw_runtime::agent::Agent,
    sender: &mut futures_util::stream::SplitSink<WebSocket, Message>,
    content: &str,
    session_key: &str,
) {
    use quantclaw_runtime::agent::TurnEvent;
    ...
    match result {
        Ok(response) => {
            ...
            if let Some(ref backend) = state.session_backend {
                let assistant_msg = quantclaw_providers::ChatMessage::assistant(&response);
            ...
            if state.auto_save {
                ...
                tokio::spawn(async move {
                    if let Err(e) = quantclaw_memory::consolidation::consolidate_turn(
                        provider.as_ref(),
                        &model,
                        mem.as_ref(),
                        &user_msg,
        ...
        Err(e) => {
            ...
            let sanitized = quantclaw_providers::sanitize_api_error(&e.to_string());
...
mod tests {
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        headers.insert(
            ...
            "quantclaw.v1, bearer.zc_sub456".parse().unwrap(),
        );
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        headers.insert(
            "sec-websocket-protocol",
            "quantclaw.v1, bearer.zc_tok, other".parse().unwrap(),
        );
