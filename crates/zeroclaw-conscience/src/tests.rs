...
fn cosmic_none_signals_preserve_original_behavior() {
    ...
    // A bare low-harm tool scores 0.74. After the default-threshold
    // rebalance (#23 moved allow_above 0.80 → 0.70 precisely so low-harm
    // tools cross into Allow), 0.74 ≥ 0.70 ⇒ Allow. This assertion read
    // `Ask` against the pre-#23 0.80 threshold and was left stale by that
    // PR; it is corrected here to match the shipped behavior.
    ...
    assert_eq!(tool_verdict, GateVerdict::Allow);
}
