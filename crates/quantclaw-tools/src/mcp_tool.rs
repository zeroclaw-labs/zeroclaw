//! Wraps a discovered MCP tool as a quantclaw [`Tool`] so it is dispatched
...
/// A quantclaw [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // QuantClaw's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // QuantClaw's security model injects `approved: bool` into built-in tool args.
