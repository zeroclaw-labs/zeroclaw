...
impl WebSearchTool {
    ...
    fn resolve_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if let Some(ref key) = self.boot_brave_api_key
            && !key.is_empty()
            && !quantclaw_config::secrets::SecretStore::is_encrypted(key)
        {
            return Ok(key.clone());
        }
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        let config: quantclaw_config::schema::Config = toml::from_str(&contents).map_err(|e| {
            anyhow::anyhow!(
        ...
        if quantclaw_config::secrets::SecretStore::is_encrypted(&raw_key) {
            let quantclaw_dir = self.config_path.parent().unwrap_or_else(|| Path::new("."));
            let store =
                quantclaw_config::secrets::SecretStore::new(quantclaw_dir, self.secrets_encrypt);
    ...
    async fn search_duckduckgo(&self, query: &str) -> anyhow::Result<String> {
        ...
        let builder =
            quantclaw_config::schema::apply_runtime_proxy_to_builder(builder, "tool.web_search");
...
mod tests {
    ...
    fn test_resolve_brave_api_key_decrypts_encrypted_key() {
        let tmp = tempfile::TempDir::new().unwrap();
        let store = quantclaw_config::secrets::SecretStore::new(tmp.path(), true);
