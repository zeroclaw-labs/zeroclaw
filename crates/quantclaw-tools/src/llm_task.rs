...
use serde_json::json;
use std::sync::Arc;
use quantclaw_api::provider::Provider;
use quantclaw_api::tool::{Tool, ToolResult};
use quantclaw_config::policy::SecurityPolicy;
use quantclaw_config::policy::ToolOperation;
...
pub struct LlmTaskTool {
    ...
    provider_runtime_options: quantclaw_providers::ProviderRuntimeOptions,
}
...
impl Tool for LlmTaskTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let provider: Box<dyn Provider> = match quantclaw_providers::create_provider_with_options(
...
mod tests {
    ...
    fn tool_metadata() {
        let tool = LlmTaskTool::new(
            ...
            0.7,
            None,
            quantclaw_providers::ProviderRuntimeOptions::default(),
        );
