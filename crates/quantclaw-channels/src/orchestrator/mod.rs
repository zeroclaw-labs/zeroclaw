...
//! QuantClaw to external platforms. Each channel implements the [`Channel`] trait
...
//! `quantclaw-channels/src/`, declare it in `lib.rs` behind the appropriate feature
...
pub use quantclaw_api::channel::{Channel, ChannelMessage, SendMessage};
// Local channel types (in misc, not quantclaw-channels)
...
pub use quantclaw_infra::stall_watchdog::StallWatchdog;
...
use quantclaw_memory::{self, Memory};
use quantclaw_providers::reliable::{scope_provider_fallback, take_last_provider_fallback};
use quantclaw_providers::{self, ChatMessage, Provider};
use quantclaw_runtime::agent::loop_::{
...
use quantclaw_runtime::approval::ApprovalManager;
use quantclaw_runtime::observability::traits::{ObserverEvent, ObserverMetric};
use quantclaw_runtime::observability::{self, Observer, runtime_trace};
use quantclaw_runtime::platform;
use quantclaw_runtime::security::{AutonomyLevel, SecurityPolicy};
use quantclaw_runtime::tools::{self, Tool};
use quantclaw_runtime::util::truncate_with_ellipsis;
...
// System prompt functions live in `quantclaw_runtime::agent::system_prompt`.
#[allow(unused_imports)]
pub use quantclaw_runtime::agent::system_prompt::{
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "quantclaw.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "quantclaw.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["quantclaw", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["quantclaw", "restart"];
...
struct ChannelCostTrackingState {
    tracker: Arc<quantclaw_runtime::cost::CostTracker>,
    prices: Arc<HashMap<String, quantclaw_config::schema::ModelPricing>>,
}
...
struct ChannelRuntimeContext {
    ...
    prompt_config: Arc<quantclaw_config::schema::Config>,
    ...
    api_key: Option<String>,
    api_url: Option<String>,
    reliability: Arc<quantclaw_config::schema::ReliabilityConfig>,
    provider_runtime_options: quantclaw_providers::ProviderRuntimeOptions,
    workspace_dir: Arc<PathBuf>,
    message_timeout_secs: u64,
    interrupt_on_new_message: InterruptOnNewMessageConfig,
    multimodal: quantclaw_config::schema::MultimodalConfig,
    media_pipeline: quantclaw_config::schema::MediaPipelineConfig,
    transcription_config: quantclaw_config::schema::TranscriptionConfig,
    hooks: Option<Arc<quantclaw_runtime::hooks::HookRunner>>,
    non_cli_excluded_tools: Arc<Vec<String>>,
    autonomy_level: AutonomyLevel,
    tool_call_dedup_exempt: Arc<Vec<String>>,
    model_routes: Arc<Vec<quantclaw_config::schema::ModelRouteConfig>>,
    query_classification: quantclaw_config::schema::QueryClassificationConfig,
    ack_reactions: bool,
    show_tool_calls: bool,
    session_store: Option<Arc<quantclaw_infra::session_store::SessionStore>>,
    ...
    activated_tools:
        Option<std::sync::Arc<std::sync::Mutex<quantclaw_runtime::tools::ActivatedToolSet>>>,
    cost_tracking: Option<ChannelCostTrackingState>,
    pacing: quantclaw_config::schema::PacingConfig,
    max_tool_result_chars: usize,
    context_token_budget: usize,
    debouncer: Arc<quantclaw_infra::debounce::MessageDebouncer>,
}
...
fn conversation_memory_key(msg: &quantclaw_api::channel::ChannelMessage) -> String {
...
fn followup_thread_id(msg: &quantclaw_api::channel::ChannelMessage) -> Option<String> {
    msg.thread_ts.clone().or_else(|| Some(msg.id.clone()))
}

fn interruption_scope_key(msg: &quantclaw_api::channel::ChannelMessage) -> String {
...
fn decrypt_optional_secret_for_runtime_reload(
    store: &quantclaw_runtime::security::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone()
        && quantclaw_runtime::security::SecretStore::is_encrypted(&raw)
    {
        *value = Some(
            store
                .decrypt(&raw)
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(quantclaw_dir) = path.parent() {
        let store =
            quantclaw_runtime::security::SecretStore::new(quantclaw_dir, parsed.secrets.encrypt);
...
async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
    ...
    let next_default_provider = quantclaw_providers::create_resilient_provider_with_options(
    ...
    if let Err(err) = next_default_provider.warmup().await {
        if quantclaw_providers::reliable::is_non_retryable(&err) {
            tracing::warn!(
...
fn refreshed_new_session_system_prompt(ctx: &ChannelRuntimeContext) -> String {
    let refreshed_skills = quantclaw_runtime::skills::skills_to_prompt_with_mode(
        &quantclaw_runtime::skills::load_skills_with_config(
...
fn should_rollback_failed_user_turn(error: &anyhow::Error) -> bool {
    if error
        .downcast_ref::<quantclaw_providers::ProviderCapabilityError>()
        .is_some_and(|capability| capability.capability.eq_ignore_ascii_case("vision"))
    {
        return true;
    }

    quantclaw_providers::reliable::is_non_retryable(error)
}

fn should_skip_memory_context_entry(key: &str, content: &str) -> bool {
    if quantclaw_memory::is_assistant_autosave_key(key) {
        return true;
    }

    if quantclaw_memory::should_skip_autosave_content(content) {
        return true;
    }
...
async fn create_resilient_provider_nonblocking(
    ...
    tokio::task::spawn_blocking(move || {
        quantclaw_providers::create_resilient_provider_with_options(
            &provider_name,
            api_key.as_deref(),
            api_url.as_deref(),
            &reliability,
...
fn build_models_help_response(
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `quantclaw models refresh --provider {}`.",
...
fn build_providers_help_response(current: &ChannelRouteSelection) -> String {
    ...
    for provider in quantclaw_providers::list_providers() {
...
fn build_config_text_response(
    ...
    resp.push_str("\nAvailable providers:\n");
    for p in quantclaw_providers::list_providers() {
...
fn build_config_block_kit(
    ...
    let provider_options: Vec<serde_json::Value> = quantclaw_providers::list_providers()
        .iter()
        .map(|p| {
            serde_json::json!({
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "quantclaw_config_provider",
    ...
    let mut model_select = serde_json::json!({
        "type": "static_select",
        "action_id": "quantclaw_config_model",
...
async fn handle_runtime_command_if_needed(
    ctx: &ChannelRuntimeContext,
    msg: &quantclaw_api::channel::ChannelMessage,
    ...
    let response = match command {
            ...
            match resolve_provider_alias(&raw_provider) {
                Some(provider_name) => {
                    match get_or_create_provider(ctx, &provider_name, None).await {
                        ...
                        Err(err) => {
                            let safe_err = quantclaw_providers::sanitize_api_error(&err.to_string());
                            format!(
        ...
        ChannelRuntimeCommand::ShowConfig => {
            if msg.channel == "slack" {
                ...
                format!("__QUANTCLAW_BLOCK_KIT__{blocks_json}")
            } else {
...
fn sanitize_channel_response(response: &str, tools: &[Box<dyn Tool>]) -> String {
    ...
    match quantclaw_runtime::security::LeakDetector::new().scan(&sanitized) {
        quantclaw_runtime::security::LeakResult::Clean => sanitized,
        quantclaw_runtime::security::LeakResult::Detected { patterns, redacted } => {
            tracing::warn!(
                patterns = ?patterns,
...
fn spawn_supervised_listener(
    ch: Arc<dyn Channel>,
    tx: tokio::sync::mpsc::Sender<quantclaw_api::channel::ChannelMessage>,
...
fn spawn_supervised_listener_with_health_interval(
    ...
    tokio::spawn(async move {
        ...
        loop {
            quantclaw_runtime::health::mark_component_ok(&component);
            ...
            match result {
                Ok(()) => {
                    ...
                    quantclaw_runtime::health::mark_component_error(
                        &component,
                ...
                Err(e) => {
                    ...
                    quantclaw_runtime::health::mark_component_error(&component, e.to_string());
                }
            }

            quantclaw_runtime::health::bump_component_restart(&component);
...
async fn process_channel_message(
    ...
    let mut msg = if let Some(hooks) = &ctx.hooks {
        match hooks.run_on_message_received(msg).await {
            quantclaw_runtime::hooks::HookResult::Cancel(reason) => {
                tracing::info!(%reason, "incoming message dropped by hook");
                return;
            }
            quantclaw_runtime::hooks::HookResult::Continue(modified) => modified,
        }
    } else {
        msg
    };
    ...
    if let Some(hint) =
        quantclaw_runtime::agent::classifier::classify(&ctx.query_classification, &msg.content)
    ...
    if ctx.auto_save_memory
        ...
        && !quantclaw_memory::should_skip_autosave_content(&msg.content)
    {
        ...
        let _ = ctx
            ...
            .store(
                &autosave_key,
                &msg.content,
                quantclaw_memory::MemoryCategory::Conversation,
                Some(&history_key),
            )
            .await;
    }
    ...
    if !active_provider.supports_vision() && prior_turns.len() > 1 {
        ...
        for turn in &mut prior_turns[..last_idx] {
            if turn.content.contains("[IMAGE:") {
                let (cleaned, _refs) =
                    quantclaw_providers::multimodal::parse_image_markers(&turn.content);
    ...
    // and preserving key decisions through LLM-driven summarization.
    {
        ...
        let compressor = quantclaw_runtime::agent::context_compressor::ContextCompressor::new(
            cc_config,
    ...
    let (delta_tx, delta_rx) = if use_draft_streaming {
        let (tx, rx) = tokio::sync::mpsc::channel::<quantclaw_runtime::agent::loop_::DraftEvent>(64);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let draft_updater = if use_draft_streaming {
        ...
        if let (Some(mut rx), Some(draft_id_ref), Some(channel_ref)) = (
            ...
            Some(tokio::spawn(async move {
                use quantclaw_runtime::agent::loop_::DraftEvent;
    ...
    let cost_tracking_context = ctx.cost_tracking.clone().map(|state| {
        quantclaw_runtime::agent::loop_::ToolLoopCostTrackingContext::new(
            state.tracker,
            state.prices,
        )
    });
            ...
            let loop_result = tokio::select! {
                ...
                result = tokio::time::timeout(
                    ...
                    scope_thread_id(
                        ...
                        quantclaw_runtime::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
    ...
    match llm_result {
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            if let Some(hooks) = &ctx.hooks {
                    ...
                    .await
                {
                    ...
                    quantclaw_runtime::hooks::HookResult::Continue((
                        hook_channel,
                        hook_recipient,
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
            if quantclaw_runtime::agent::loop_::is_tool_loop_cancelled(&e)
                || cancellation_token.is_cancelled()
            {
                ...
                if quantclaw_providers::reliable::is_auth_error(&e) {
...
async fn run_message_dispatch_loop(
    mut rx: tokio::sync::mpsc::Receiver<quantclaw_api::channel::ChannelMessage>,
    ctx: Arc<ChannelRuntimeContext>,
    ...
    while let Some(msg) = rx.recv().await {
        ...
        let msg = if msg.channel != "cli" && ctx.debouncer.enabled() {
            ...
            match ctx.debouncer.debounce(&debounce_key, &msg.content).await {
                quantclaw_infra::debounce::DebounceResult::Pending(rx) => {
                ...
                quantclaw_infra::debounce::DebounceResult::Passthrough(content) => {
                    let mut m = msg;
                    m.content = content;
                    m
                }
...
pub async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `quantclaw onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `quantclaw daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `quantclaw service stop && quantclaw service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.quantclaw.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.quantclaw.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.quantclaw.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.quantclaw.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/quantclaw");
...
fn build_channel_by_id(config: &Config, channel_id: &str) -> Result<Arc<dyn Channel>> {
    match channel_id {
        ...
        "discord_history" | "discord-history" => {
            ...
            let discord_mem =
                quantclaw_memory::SqliteMemory::new_named(&config.workspace_dir, "discord")
...
fn collect_configured_channels(
    ...
    if let Some(ref dh) = config.channels_config.discord_history {
        if dh.enabled {
            match quantclaw_memory::SqliteMemory::new_named(&config.workspace_dir, "discord") {
                Ok(discord_mem) => {
                    channels.push(ConfiguredChannel {
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `quantclaw onboard` first.");
        return Ok(());
    }

    println!("🩺 QuantClaw Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `quantclaw gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let mut ch_activated_handle: Option<
        std::sync::Arc<std::sync::Mutex<quantclaw_runtime::tools::ActivatedToolSet>>,
    > = None;
    if config.mcp.enabled && !config.mcp.servers.is_empty() {
        ...
        match quantclaw_runtime::tools::McpRegistry::connect_all(&config.mcp.servers).await {
            Ok(registry) => {
                let registry = std::sync::Arc::new(registry);
                if config.mcp.deferred_loading {
                    let deferred_set = quantclaw_runtime::tools::DeferredMcpToolSet::from_registry(
                    ...
                    deferred_section =
                        quantclaw_runtime::tools::build_deferred_tools_section(&deferred_set);
                    let activated = std::sync::Arc::new(std::sync::Mutex::new(
                        quantclaw_runtime::tools::ActivatedToolSet::new(),
                    ));
                    ch_activated_handle = Some(std::sync::Arc::clone(&activated));
                    built_tools.push(Box::new(quantclaw_runtime::tools::ToolSearchTool::new(
                        deferred_set,
                        activated,
                    )));
                } else {
                    ...
                    for name in names {
                        if let Some(def) = registry.get_tool_def(&name).await {
                            let wrapper: std::sync::Arc<dyn Tool> =
                                std::sync::Arc::new(quantclaw_runtime::tools::McpToolWrapper::new(
                                    name,
                                    def,
                                    std::sync::Arc::clone(&registry),
                                ));
                            ...
                            built_tools
                                .push(Box::new(quantclaw_runtime::tools::ArcToolRef(wrapper)));
                            registered += 1;
                        }
                    }
                    tracing::info!(
    ...
    let skills = quantclaw_runtime::skills::load_skills_with_config(&workspace, &config);
    ...
    let i18n_search_dirs = quantclaw_runtime::i18n::default_search_dirs(&workspace);
    let i18n_descs =
        quantclaw_runtime::i18n::ToolDescriptions::load(&i18n_locale, &i18n_search_dirs);
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `quantclaw onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 QuantClaw Channel Server");
    println!("  🤖 Model:    {model}");
    let effective_backend = quantclaw_memory::effective_memory_backend_name(
    ...
    quantclaw_runtime::health::mark_component_ok("channels");
    ...
    let (tx, rx) = tokio::sync::mpsc::channel::<quantclaw_api::channel::ChannelMessage>(100);
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        hooks: if config.hooks.enabled {
            let mut runner = quantclaw_runtime::hooks::HookRunner::new();
            if config.hooks.builtin.command_logger {
                runner.register(Box::new(
                    quantclaw_runtime::hooks::builtin::CommandLoggerHook::new(),
                ));
            }
            if config.hooks.builtin.webhook_audit.enabled {
                runner.register(Box::new(
                    quantclaw_runtime::hooks::builtin::WebhookAuditHook::new(
        ...
        session_store: if config.channels_config.session_persistence {
            match quantclaw_infra::session_store::SessionStore::new(&config.workspace_dir) {
                Ok(store) => {
        ...
        cost_tracking: quantclaw_runtime::cost::CostTracker::get_or_init_global(
            config.cost.clone(),
            &config.workspace_dir,
        )
        ...
        debouncer: Arc::new(quantclaw_infra::debounce::MessageDebouncer::new(
...
pub async fn deliver_announcement(
    ...
    let leak_detector = quantclaw_runtime::security::LeakDetector::new();
    let safe_output = match leak_detector.scan(output) {
        quantclaw_runtime::security::LeakResult::Detected { redacted, .. } => redacted,
        quantclaw_runtime::security::LeakResult::Clean => output.to_string(),
    };
    ...
    match channel.to_ascii_lowercase().as_str() {
        ...
        "telegram" => {
            ...
            quantclaw_api::channel::Channel::send(&ch, &SendMessage::new(&safe_output, target))
                .await?;
        }
        "discord" => {
            let dc = config
...
mod tests {
    ...
    use quantclaw_providers::{ChatMessage, Provider};
    use quantclaw_runtime::observability::NoopObserver;
    use quantclaw_runtime::tools::{Tool, ToolResult};

    fn make_workspace() -> TempDir {
        let tmp = TempDir::new().unwrap();
        // Create minimal workspace files
        std::fs::write(tmp.path().join("SOUL.md"), "# Soul\nBe helpful.").unwrap();
        std::fs::write(tmp.path().join("IDENTITY.md"), "# Identity\nName: QuantClaw").unwrap();
    ...
    fn pacing_config_defaults_preserve_existing_behavior() {
        let pacing = quantclaw_config::schema::PacingConfig::default();
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            api_key: None,
            api_url: None,
            reliability: Arc::new(quantclaw_config::schema::ReliabilityConfig::default()),
            interrupt_on_new_message: InterruptOnNewMessageConfig {
                telegram: false,
                slack: false,
                discord: false,
            ...
            transcription_config: quantclaw_config::schema::TranscriptionConfig::default(),
            hooks: None,
            provider_runtime_options: quantclaw_providers::ProviderRuntimeOptions::default(),
            workspace_dir: Arc::new(std::env::temp_dir()),
            prompt_config: Arc::new(quantclaw_config::schema::Config::default()),
    ...
    fn rollback_orphan_user_turn_also_removes_from_session_store() {
        ...
        let store = Arc::new(quantclaw_infra::session_store::SessionStore::new(tmp.path()).unwrap());
    ...
    impl Channel for TelegramRecordingChannel {
        ...
        async fn listen(
            &self,
            _tx: tokio::sync::mpsc::Sender<quantclaw_api::channel::ChannelMessage>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_executes_tool_calls_instead_of_sending_raw_json() {
        ...
        process_channel_message(
            runtime_ctx,
            quantclaw_api::channel::ChannelMessage {
                id: "msg-1".to_string(),
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(quantclaw_config::schema::ReliabilityConfig::default()),
            provider_runtime_options: quantclaw_providers::ProviderRuntimeOptions {
                quantclaw_dir: Some(temp.path().to_path_buf()),
    ...
    impl Memory for NoopMemory {
        ...
        async fn store(
            ...
            _key: &str,
            _content: &str,
            _category: quantclaw_memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _since: Option<&str>,
            _until: Option<&str>,
        ) -> anyhow::Result<Vec<quantclaw_memory::MemoryEntry>> {
            Ok(Vec::new())
        }

        async fn get(&self, _key: &str) -> anyhow::Result<Option<quantclaw_memory::MemoryEntry>> {
            Ok(None)
        }

        async fn list(
            &self,
            _category: Option<&quantclaw_memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<quantclaw_memory::MemoryEntry>> {
            Ok(Vec::new())
        }
    ...
    impl Memory for RecallMemory {
        ...
        async fn recall(
            ...
            Ok(vec![quantclaw_memory::MemoryEntry {
                id: "entry-1".to_string(),
                key: "memory_key_1".to_string(),
                content: "Age is 45".to_string(),
                category: quantclaw_memory::MemoryCategory::Conversation,
    ...
    async fn message_dispatch_processes_messages_in_parallel() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<quantclaw_api::channel::ChannelMessage>(4);
        tx.send(quantclaw_api::channel::ChannelMessage {
            id: "1".to_string(),
            sender: "alice".to_string(),
    ...
    async fn message_dispatch_interrupts_in_flight_telegram_request_and_preserves_context() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<quantclaw_api::channel::ChannelMessage>(8);
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(
            prompt.contains("Name: QuantClaw"),
    ...
    fn prompt_skills_include_instructions_and_tools() {
        let ws = make_workspace();
        let skills = vec![quantclaw_runtime::skills::Skill {
            name: "code-review".into(),
            description: "Review code for bugs".into(),
            version: "1.0.0".into(),
            author: None,
            tags: vec![],
            tools: vec![quantclaw_runtime::skills::SkillTool {
                name: "lint".into(),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from QuantClaw 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";

        // Reproduces the production crash path where channel logs truncate at 80 chars.
        let result =
            std::panic::catch_unwind(|| quantclaw_runtime::util::truncate_with_ellipsis(msg, 80));
        assert!(
            result.is_ok(),
    ...
    fn full_autonomy_prompt_executes_allowed_tools_without_extra_approval() {
        let ws = make_workspace();
        let config = quantclaw_config::schema::AutonomyConfig {
            level: quantclaw_runtime::security::AutonomyLevel::Full,
            ..quantclaw_config::schema::AutonomyConfig::default()
        };
    ...
    fn readonly_prompt_explains_policy_blocks_without_fake_approval() {
        let ws = make_workspace();
        let config = quantclaw_config::schema::AutonomyConfig {
            level: quantclaw_runtime::security::AutonomyLevel::ReadOnly,
            ..quantclaw_config::schema::AutonomyConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ws.path(),
    ...
    fn channel_notify_observer_truncates_utf8_arguments_safely() {
        ...
        observer.record_event(
            &quantclaw_runtime::observability::traits::ObserverEvent::ToolCallStart {
    ...
    fn conversation_memory_key_uses_message_id() {
        let msg = quantclaw_api::channel::ChannelMessage {
            id: "msg_abc123".into(),
            sender: "U123".into(),
    ...
    fn conversation_memory_key_is_unique_per_message() {
        let msg1 = quantclaw_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "first".into(),
        ...
        let msg2 = quantclaw_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "second".into(),
    ...
    async fn process_channel_message_refreshes_available_skills_after_new_session() {
        ...
        let initial_skills =
            quantclaw_runtime::skills::load_skills_with_config(workspace.path(), &config);
...
This is an example JSON object for profile settings."#;
    ...
    fn collect_configured_channels_skips_disabled_email() {
        let mut config = Config::default();
        config.channels_config.email = Some(quantclaw_config::scattered_types::EmailConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    fn collect_configured_channels_skips_disabled_voice_call() {
        ...
        config.channels_config.voice_call =
            Some(quantclaw_config::scattered_types::VoiceCallConfig {
                enabled: false,
                ..Default::default()
            });
    ...
    async fn supervised_listener_marks_error_and_restarts_on_failures() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<quantclaw_api::channel::ChannelMessage>(1);
        ...
        let snapshot = quantclaw_runtime::health::snapshot_json();
    ...
    async fn supervised_listener_refreshes_health_while_running() {
        ...
        let first_last_ok = quantclaw_runtime::health::snapshot_json()["components"]
        ...
        let second_last_ok = quantclaw_runtime::health::snapshot_json()["components"]
            [&component_name]["last_ok"]
            .as_str()
            .unwrap_or("")
            .to_string();
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "quantclaw.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "quantclaw.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["quantclaw", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["quantclaw", "restart"]);
    }
    ...
    async fn e2e_photo_attachment_rejected_by_non_vision_provider() {
        ...
        process_channel_message(
            ...
            quantclaw_api::channel::ChannelMessage {
                ...
                sender: "quantclaw_user".to_string(),
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .peek("test-channel_chat-photo_quantclaw_user")
    ...
    async fn e2e_failed_non_retryable_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .peek("test-channel_chat-format_quantclaw_user")
            .expect("history should exist for sender");
        assert_eq!(turns.len(), 2);
        assert_eq!(turns[0].role, "user");
        assert_eq!(turns[0].content, "What is WAL?");
    ...
    async fn process_channel_message_applies_query_classification_route() {
        ...
        let classification_config = quantclaw_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![quantclaw_config::schema::ClassificationRule {
                hint: "vision".into(),
        ...
        let model_routes = vec![quantclaw_config::schema::ModelRouteConfig {
            hint: "vision".into(),
    ...
    async fn process_channel_message_classification_priority_selects_highest() {
        ...
        let classification_config = quantclaw_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![
                quantclaw_config::schema::ClassificationRule {
                    hint: "fast".into(),
    ...
    fn build_channel_by_id_configured_voice_call_succeeds() {
        ...
        config.channels_config.voice_call =
            Some(quantclaw_config::scattered_types::VoiceCallConfig {
                enabled: true,
                provider: quantclaw_config::scattered_types::VoiceProvider::Twilio,
    ...
    fn is_stop_command_matches_with_bot_suffix() {
        assert!(is_stop_command("/stop@quantclaw_bot"));
    }
    ...
    fn default_keep_tool_context_turns_is_two() {
        let config = quantclaw_config::schema::AgentConfig::default();
