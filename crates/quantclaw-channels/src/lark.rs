...
pub struct LarkChannel {
    ...
    receive_mode: quantclaw_config::schema::LarkReceiveMode,
...
impl LarkChannel {
    ...
    fn new_with_platform(
        ...
        Self {
            ...
            mention_only,
            platform,
            receive_mode: quantclaw_config::schema::LarkReceiveMode::default(),
    ...
    pub fn from_config(config: &quantclaw_config::schema::LarkConfig) -> Self {
    ...
    pub fn from_lark_config(config: &quantclaw_config::schema::LarkConfig) -> Self {
        let mut ch = Self::new_with_platform(
            config.app_id.clone(),
            config.app_secret.clone(),
            config.verification_token.clone().unwrap_or_default(),
    ...
    pub fn from_feishu_config(config: &quantclaw_config::schema::FeishuConfig) -> Self {
        let mut ch = Self::new_with_platform(
            config.app_id.clone(),
            config.app_secret.clone(),
            config.verification_token.clone().unwrap_or_default(),
    ...
    async fn listen_ws(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        let (ws_stream, _) = quantclaw_config::schema::ws_connect_with_proxy(
            &wss_url,
            "channel.lark",
            self.proxy_url.as_deref(),
        )
...
impl Channel for LarkChannel {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        use quantclaw_config::schema::LarkReceiveMode;
...
mod tests {
    ...
    fn lark_config_serde() {
        use quantclaw_config::schema::{LarkConfig, LarkReceiveMode};
        let lc = LarkConfig {
            enabled: true,
    ...
    fn lark_from_feishu_config_sets_feishu_platform() {
        use quantclaw_config::schema::{FeishuConfig, LarkReceiveMode};
    ...
    fn lark_reaction_url_matches_region() {
        ...
        let feishu_cfg = quantclaw_config::schema::FeishuConfig {
            enabled: true,
            app_id: "cli_app123".into(),
            app_secret: "secret456".into(),
            encrypt_key: None,
            verification_token: Some("vtoken789".into()),
            allowed_users: vec!["*".into()],
            receive_mode: quantclaw_config::schema::LarkReceiveMode::Webhook,
            port: Some(9898),
            proxy_url: None,
        };
