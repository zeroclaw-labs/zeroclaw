...
mod tests {
    ...
    fn make_group_mention_channel() -> WhatsAppChannel {
        ...
        .with_group_mention_patterns(vec!["@?QuantClaw".into()])
    }
    ...
    fn make_dm_mention_channel() -> WhatsAppChannel {
        ...
        .with_dm_mention_patterns(vec!["@?QuantClaw".into()])
    }
    ...
    fn whatsapp_compile_skips_invalid_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into(), "[invalid".into()]);
    ...
    fn whatsapp_compile_skips_empty_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into(), "  ".into()]);
        assert_eq!(patterns.len(), 1);
    }
    ...
    fn whatsapp_text_matches_at_name() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
    ...
    fn whatsapp_strip_at_name() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@QuantClaw what is the weather?"),
    ...
    fn whatsapp_strip_at_end() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "Help me @QuantClaw"),
            Some("Help me".into())
        );
    }
    ...
    fn whatsapp_strip_mid_sentence() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "Hey @QuantClaw how are you?"),
    ...
    fn whatsapp_strip_multiple_occurrences() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@QuantClaw hello @QuantClaw"),
            Some("hello".into())
        );
    }
    ...
    fn whatsapp_strip_returns_none_when_only_mention() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(WhatsAppChannel::strip_patterns(&pats, "@QuantClaw"), None);
    }
    ...
    fn whatsapp_strip_returns_none_for_whitespace_only() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "  @QuantClaw  "),
            None
        );
    }
    ...
    fn whatsapp_strip_collapses_whitespace() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?QuantClaw".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@QuantClaw   status   please"),
    ...
    fn whatsapp_group_mention_accepts_and_strips_in_group() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@QuantClaw what is the weather?")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_strips_from_group_content() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "Hey @QuantClaw tell me a joke")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_drops_mention_only_group_message() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@QuantClaw")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_case_insensitive_group_match() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@quantclaw status")]
                    }
                }]
            }]
        });
                ...
                "changes": [{
                    "value": {
                        "messages": [
                            ...
                            group_msg("222", "2", "@QuantClaw help me"),
    ...
    fn whatsapp_group_mention_dm_not_stripped() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [dm_msg("111", "1", "@QuantClaw what is the weather?")]
                    }
                }]
            }]
        });
        ...
        assert_eq!(msgs.len(), 1);
        assert_eq!(
            msgs[0].content, "@QuantClaw what is the weather?",
