...
//!
//! DaemonClaw's agent loop calls [`daemonclaw_hardware::boot`] on **every** request,
//! which re-reads `~/.daemonclaw/hardware/` from disk.  Writing to those files
...
/// Return `~/.daemonclaw/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".daemonclaw").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.daemonclaw/hardware/devices/<device>.md`:
/// ```text
...
///
/// Since [`daemonclaw_hardware::boot`] re-reads from disk on every agent invocation,
...
pub async fn handle_hardware_reload(
    ...
    let context = daemonclaw_hardware::load_hardware_context_prompt(&[]);
