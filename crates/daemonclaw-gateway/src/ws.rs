...
const WS_PROTOCOL: &str = "daemonclaw.v1";
...
async fn handle_socket(
    ...
    let mut agent = match daemonclaw_runtime::agent::Agent::from_config(&config).await {
        Ok(a) => a,
        Err(e) => {
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(text) {
            if parsed["type"].as_str() == Some("message") {
                ...
                if !content.is_empty() {
                    ...
                    if let Some(ref backend) = state.session_backend {
                        let user_msg = daemonclaw_providers::ChatMessage::user(&content);
...
async fn process_chat_message(
    state: &AppState,
    agent: &mut daemonclaw_runtime::agent::Agent,
    ...
    match result {
        Ok(response) => {
            ...
            if let Some(ref backend) = state.session_backend {
                let assistant_msg = daemonclaw_providers::ChatMessage::assistant(&response);
...
mod tests {
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        headers.insert(
            ...
            "daemonclaw.v1, bearer.zc_sub456".parse().unwrap(),
        );
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        headers.insert(
            "sec-websocket-protocol",
            "daemonclaw.v1, bearer.zc_tok, other".parse().unwrap(),
        );
