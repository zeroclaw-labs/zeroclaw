...
use daemonclaw_runtime::cost::CostTracker;
use daemonclaw_runtime::platform;
use daemonclaw_runtime::security::pairing::{PairingGuard, constant_time_eq, is_public_bind};
use daemonclaw_runtime::tools;
use daemonclaw_runtime::tools::CanvasStore;
...
/// Read gateway request timeout from `DAEMONCLAW_GATEWAY_TIMEOUT_SECS` env var
...
pub fn gateway_request_timeout_secs() -> u64 {
    std::env::var("DAEMONCLAW_GATEWAY_TIMEOUT_SECS")
        .ok()
...
fn whatsapp_memory_key(msg: &daemonclaw_api::channel::ChannelMessage) -> String {
    format!("whatsapp_{}_{}", msg.sender, msg.id)
}

fn linq_memory_key(msg: &daemonclaw_api::channel::ChannelMessage) -> String {
    format!("linq_{}_{}", msg.sender, msg.id)
}

fn wati_memory_key(msg: &daemonclaw_api::channel::ChannelMessage) -> String {
    format!("wati_{}_{}", msg.sender, msg.id)
}

fn nextcloud_talk_memory_key(msg: &daemonclaw_api::channel::ChannelMessage) -> String {
    format!("nextcloud_talk_{}_{}", msg.sender, msg.id)
}

fn sender_session_id(channel: &str, msg: &daemonclaw_api::channel::ChannelMessage) -> String {
    match &msg.thread_ts {
...
pub struct AppState {
    ...
    pub observer: Arc<dyn daemonclaw_runtime::observability::Observer>,
...
pub async fn run_gateway(
    ...
    // ── Hooks ──────────────────────────────────────────────────────
    let hooks: Option<std::sync::Arc<daemonclaw_runtime::hooks::HookRunner>> = if config.hooks.enabled
    {
        Some(std::sync::Arc::new(
            daemonclaw_runtime::hooks::HookRunner::new(),
        ))
    } else {
        None
    };
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("DAEMONCLAW_WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    // Priority: environment variable > config file
    let linq_signing_secret: Option<Arc<str>> = std::env::var("DAEMONCLAW_LINQ_SIGNING_SECRET")
        .ok()
        .and_then(|secret| {
            let secret = secret.trim();
            (!secret.is_empty()).then(|| secret.to_owned())
    ...
    let nextcloud_talk_webhook_secret: Option<Arc<str>> =
        std::env::var("DAEMONCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET")
            .ok()
            .and_then(|secret| {
                let secret = secret.trim();
                (!secret.is_empty()).then(|| secret.to_owned())
    ...
    // ── Tunnel ────────────────────────────────────────────────
    let tunnel = daemonclaw_runtime::tunnel::create_tunnel(&config.tunnel)?;
    ...
    let web_dist_dir: Option<std::path::PathBuf> = config
        ...
        .or_else(|| {
            ...
            let mut candidates = vec![
                ...
                std::path::PathBuf::from("/daemonclaw-data/web/dist"),
                // AUR / system package
                std::path::PathBuf::from("/usr/share/daemonclawlabs/web/dist"),
            ];
            // XDG data home (prebuilt binary installer)
            if let Some(data_dir) = dirs_data_local() {
                candidates.push(data_dir.join("daemonclaw/web/dist"));
            }
            candidates
                .into_iter()
    ...
    if let Some(ref dir) = web_dist_dir {
        ...
        tracing::info!(
            "Web dashboard: not available (set gateway.web_dist_dir or DAEMONCLAW_WEB_DIST_DIR)"
        );
    }

    let pfx = path_prefix.unwrap_or("");
    println!("🦀 DaemonClaw Gateway listening on http://{display_addr}{pfx}");
    ...
    if let Some(code) = pairing.pairing_code() {
        ...
        println!("     To pair a new device: daemonclaw gateway get-paircode --new");
        println!();
    } else {
    ...
    daemonclaw_runtime::health::mark_component_ok("gateway");
    ...
    let state = AppState {
        ...
        webauthn: if config.security.webauthn.enabled {
            let secret_store = Arc::new(daemonclaw_runtime::security::SecretStore::new(
                &config.workspace_dir,
                true,
            ));
            let wa_config = daemonclaw_runtime::security::webauthn::WebAuthnConfig {
                enabled: true,
            ...
            Some(Arc::new(api_webauthn::WebAuthnState {
                manager: daemonclaw_runtime::security::webauthn::WebAuthnManager::new(
                    wa_config,
                    secret_store,
                    &config.workspace_dir,
                ),
        ...
        loop {
            tokio::select! {
                ...
                _ = shutdown_signal.changed() => {
                    tracing::info!("🦀 DaemonClaw Gateway shutting down...");
                    break;
                }
            }
        }
...
async fn handle_health(State(state): State<AppState>) -> impl IntoResponse {
    let body = serde_json::json!({
        ...
        "runtime": daemonclaw_runtime::health::snapshot_json(),
    });
    Json(body)
}
...
fn prometheus_observer_from_state(
    observer: &dyn daemonclaw_runtime::observability::Observer,
) -> Option<&daemonclaw_runtime::observability::PrometheusObserver> {
    observer
        .as_any()
        .downcast_ref::<daemonclaw_runtime::observability::PrometheusObserver>()
        .or_else(|| {
            observer
                .as_any()
...
async fn run_gateway_chat_simple(
    state: &AppState,
    message: &str,
) -> anyhow::Result<daemonclaw_api::provider::ChatResponse> {
    ...
    let system_prompt = {
        ...
        daemonclaw_runtime::agent::system_prompt::build_system_prompt(
    ...
    let prepared = daemonclaw_providers::multimodal::prepare_messages_for_provider(
        &messages,
        &multimodal_config,
    )
    .await?;
    ...
    state
        .provider
        .chat(
            daemonclaw_api::provider::ChatRequest {
...
async fn run_gateway_chat_with_tools(
    ...
    Box::pin(daemonclaw_runtime::agent::process_message(
...
async fn handle_webhook(
    ...
    if state.auto_save && !daemonclaw_memory::should_skip_autosave_content(message) {
    ...
    state.observer.record_event(
        &daemonclaw_runtime::observability::ObserverEvent::AgentStart {
            provider: provider_label.clone(),
            model: model_label.clone(),
        },
    );
    state.observer.record_event(
        &daemonclaw_runtime::observability::ObserverEvent::LlmRequest {
            provider: provider_label.clone(),
            model: model_label.clone(),
            messages_count: 1,
        },
    ...
    match run_gateway_chat_simple(&state, message).await {
        Ok(chat_response) => {
            ...
            let tokens_used = input_tokens
                ...
                .or(output_tokens);
            state.observer.record_event(
                &daemonclaw_runtime::observability::ObserverEvent::LlmResponse {
                    provider: provider_label.clone(),
                    model: model_label.clone(),
                    duration,
                    success: true,
            ...
            state.observer.record_metric(
                &daemonclaw_runtime::observability::traits::ObserverMetric::RequestLatency(duration),
            );
            state.observer.record_event(
                &daemonclaw_runtime::observability::ObserverEvent::AgentEnd {
        ...
        Err(e) => {
            let duration = started_at.elapsed();
            let sanitized = daemonclaw_providers::sanitize_api_error(&e.to_string());
            ...
            state
                .observer
                .record_event(&daemonclaw_runtime::observability::ObserverEvent::Error {
...
async fn handle_whatsapp_message(
    ...
    for msg in &messages {
        ...
        // Auto-save to memory
        if state.auto_save && !daemonclaw_memory::should_skip_autosave_content(&msg.content) {
...
async fn handle_linq_webhook(
    ...
    if let Some(ref signing_secret) = state.linq_signing_secret {
        ...
        if !daemonclaw_channels::linq::verify_linq_signature(
            signing_secret,
            &body_str,
            timestamp,
            signature,
...
async fn handle_nextcloud_talk_webhook(
    ...
    if let Some(ref webhook_secret) = state.nextcloud_talk_webhook_secret {
        ...
        if !daemonclaw_channels::nextcloud_talk::verify_nextcloud_talk_signature(
            webhook_secret,
            random,
            &body_str,
            signature,
...
async fn handle_gmail_push_webhook(
    ...
    let body_str = String::from_utf8_lossy(&body);
    let envelope: daemonclaw_channels::gmail_push::PubSubEnvelope =
...
mod tests {
    ...
    fn gateway_timeout_falls_back_to_default() {
        ...
        unsafe { std::env::remove_var("DAEMONCLAW_GATEWAY_TIMEOUT_SECS") };
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let wrapped = sse::BroadcastObserver::new(
            Box::new(daemonclaw_runtime::observability::PrometheusObserver::new()),
            event_tx.clone(),
            event_buffer,
        );
        daemonclaw_runtime::observability::Observer::record_event(
            &wrapped,
            &daemonclaw_runtime::observability::ObserverEvent::HeartbeatTick,
        );

        let observer: Arc<dyn daemonclaw_runtime::observability::Observer> = Arc::new(wrapped);
        let state = AppState {
            config: Arc::new(Mutex::new(Config::default())),
            provider: Arc::new(MockProvider::default()),
            model: "test-model".into(),
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("daemonclaw_heartbeat_ticks_total 1"));
    }
