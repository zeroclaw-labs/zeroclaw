//! Plugin manifest loader — scans `~/.daemonclaw/tools/` at startup.
//!
...
/// Scan `~/.daemonclaw/tools/` and return all valid plugins.
///
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.daemonclaw/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_daemonclaw_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".daemonclaw").join("tools");
