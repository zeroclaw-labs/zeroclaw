...
pub use daemonclaw_config::schema::{HardwareConfig, HardwareTransport};
...
pub fn merge_hardware_tools(
    tools: &mut Vec<Box<dyn daemonclaw_api::tool::Tool>>,
    ...
    if !hw_boot.tools.is_empty() {
        ...
        let new_hw_tools: Vec<Box<dyn daemonclaw_api::tool::Tool>> = hw_boot
            .tools
            .into_iter()
...
pub struct HardwareBootResult {
    ...
    pub tools: Vec<Box<dyn daemonclaw_api::tool::Tool>>,
    /// Human-readable device summary for the LLM system prompt.
    pub device_summary: String,
    /// Content of `~/.daemonclaw/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.daemonclaw/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.daemonclaw/hardware/HARDWARE.md`
/// 2. `~/.daemonclaw/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.daemonclaw/hardware/skills/*.md` files (sorted by name)
///
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(&home.join(".daemonclaw").join("hardware"), aliases)
}
...
///
/// Without the feature: loads plugin tools from `~/.daemonclaw/tools/` only,
...
pub async fn boot(
    peripherals: &daemonclaw_config::schema::PeripheralsConfig,
    ...
    if uf2::find_rpi_rp2_mount().is_some() {
        ...
        tracing::info!("Say \"flash my pico\" to install DaemonClaw firmware automatically");
    }
...
#[allow(unused_mut)] // tools and context_files_prompt are mutated on Linux+peripheral-rpi
pub async fn boot(
    _peripherals: &daemonclaw_config::schema::PeripheralsConfig,
) -> anyhow::Result<HardwareBootResult> {
...
pub fn run_info(chip: &str) -> Result<()> {
    ...
    #[cfg(not(feature = "probe"))]
    {
        ...
        println!();
        println!("Then run: daemonclaw hardware info --chip {}", chip);
        println!();
