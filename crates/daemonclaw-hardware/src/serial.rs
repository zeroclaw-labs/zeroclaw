...
/// Default baud rate for DaemonClaw serial devices.
...
/// Serial transport for DaemonClaw hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify DaemonClaw firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "daemonclaw"`.
    ///
    /// Returns `true` if a DaemonClaw device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "daemonclaw" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
