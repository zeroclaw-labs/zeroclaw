...
- **Hardware-capable** — GPIO / I2C / SPI / USB on Raspberry Pi, STM32, Arduino, and ESP32 via the `Peripheral` trait. A standalone companion crate, [`zeroclaw-robot-kit`](crates/robot-kit), packages higher-level robot control (drive, vision, speech, sensors, safety). See [Hardware](docs/book/src/hardware/index.md).
