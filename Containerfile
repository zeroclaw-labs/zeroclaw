...
RUN --mount=type=cache,target=/root/.cargo/registry,sharing=locked \
    ...
    ZEROCLAW_FEATURES="acp-bridge,agent-runtime,channel-acp-server,channel-discord,channel-email,channel-filesystem,channel-lark,channel-matrix,channel-telegram,channel-webhook,gateway,observability-prometheus,onboarding,schema-export,whatsapp-web"
